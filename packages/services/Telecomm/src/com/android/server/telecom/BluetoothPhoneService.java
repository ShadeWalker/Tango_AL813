...
public final class BluetoothPhoneService extends Service {
    ...
    /// M: DSDA
    private static final int MSG_RESTRICT_DUALTALK = 9;
    ...
    private final IBluetoothHeadsetPhone.Stub mBinder = new IBluetoothHeadsetPhone.Stub() {
        ...
        public boolean restrictDualTalk() throws RemoteException {
            ...
            if (SystemProperties.getInt("ro.mtk_dt_support", 0) == 1) {
                enforceModifyPermission();
                return sendSynchronousRequest(MSG_RESTRICT_DUALTALK);
            } else {
                return true;
            }
        }
            ...
            switch (msg.what) {
                ...
                case MSG_GET_SUBSCRIBER_NUMBER:
                    ...
                    try {
                        ...
                        /// M: to aviod passing null ptr to HeadsetStateMachine,
                ...
                /// M: DSDA @{
                case MSG_RESTRICT_DUALTALK:
                    try {
                        restrictDualTalk(false);
                    } finally {
                        if (request != null) {
                            request.setResult(true);
                        }
    ...
    private CallsManagerListener mCallsManagerListener = new CallsManagerListenerBase() {
        ...
        public void onBackgroundCallListChanged(List<Call> newList) {
            mBackgroundCallList = newList;
            Log.i(TAG, "onBackgroundCallListChanged:" + mBackgroundCallList.size());
        }

        @Override
        public void onInComingCallListChanged(List<Call> newList) {
            mIncomingCallList = newList;
            Log.i(TAG, "onInComingCallListChanged:" + mIncomingCallList.size());
        }
        /// @}
    ...
    /// M: DSDA @{
    List<Call> mBackgroundCallList = null;
    List<Call> mIncomingCallList = null;
    /// @}
    ...
    private boolean processChld(int chld) {
        ...
        if (chld == CHLD_TYPE_RELEASEHELD) {
            ...
            if (activeCall != null) {
                ...
                ///    telephony get into wrong state.
                if (ringingCall != null) {
                    callsManager.hangupActiveAndAnswerWaiting();
                } else {
    ...
    ///M : DSDA @{
    private void updateHeadsetWithCallStateForDsda(boolean force) {
        CallsManager callsManager = getCallsManager();
        Call activeCall = callsManager.getActiveCall();
        Call ringingCall = callsManager.getRingingCall();
        Call heldCall = callsManager.getHeldCall();
        ...
        Log.i(TAG, "updateHeadsetWithCallStateForDsda");

        if (isHfpConnected() && restrictDualTalk(true)) {
            Log.i(TAG, "updateHeadsetWithCallStateForDsda: return for restriction");
            return;
        }
    ...
    private void updateHeadsetWithCallState(boolean force) {
        ...
        /// M: DSDA @{
        if (SystemProperties.getInt("ro.mtk_dt_support", 0) == 1) {
            updateHeadsetWithCallStateForDsda(force);
            return;
        }
        /// @}
    ...
    /**
     * Outgoing call and incoming call co-exist is not reasonable to HFP.
     * So we will disconnect the outgoing call when found
     */
    private boolean restrictIOIfNeeded() {
        Log.i(TAG, "restrictIOIfNeeded");
        Call outGoingCall = getCallsManager().getDialingCall();

        if (outGoingCall == null) {
            return false;
        }

        Log.i(TAG, "(I,O) conflict existed :" + outGoingCall);

        getCallsManager().disconnectCall(outGoingCall);
     ...
     * if HFP is just connected, hangup the background incoming call
     * if HFP is already connected, hangup the new incoming call
     */
    private boolean restrictIIIfNeeded(boolean hfpConnected) {
        Log.i(TAG, "restrictIIIfNeeded");
        List<Call> ringCallList = getCallsManager().getCallsWithStates(CallState.RINGING);

        if (ringCallList.size() <= 1) {
            return false;
        }

        Call ringingCall = getCallsManager().getRingingCall();
        ...
        if (hfpConnected) {
            ...
            if (mIncomingCallList != null && mIncomingCallList.size() > 1) {
                callToReject = mIncomingCallList.get(mIncomingCallList.size() - 1);
                ringingCall = mIncomingCallList.get(mIncomingCallList.size() - 2);
            }
        }
        ...
        if (callToReject != null) {
            ...
            getCallsManager().rejectCall(callToReject, false, null);

            // update mRingingAddress and mRingingAddressType 
            // in case we update old value to remote
            if (ringingCall != null) {
    ...
    /**
     * More than one Hold calls is not reasonable to HFP.
     * if HFP is just connected, hangup the first hold Hold call
     * if HFP is already connected, Phone UI shall not let this happen
     */
    private boolean restrictHHIfNeeded(boolean hfpConnected) {
        Log.i(TAG, "restrictHHIfNeeded");
        List<Call> holdCallList = getCallsManager().getCallsWithStates(CallState.ON_HOLD);

        if (holdCallList.size() <= 1) {
            return false;
        }
        ...
        if (hfpConnected) {
            Log.d(TAG, "restrictHHIfNeeded : HH happened when HFP already connected");
            // return true for this must be a temp state, we shall ignore this (by return true)
            // instead of reporting to remote device.
            return true;
        } else {
            // the major held call will be put in the front of the list
            if (mBackgroundCallList != null && mBackgroundCallList.size() > 1) {
                callToReject = mBackgroundCallList.get(mBackgroundCallList.size() - 1);
            }
        }

        if (callToReject != null) {
            Log.i(TAG, "(H,H) conflict existed :" + callToReject);
            getCallsManager().disconnectCall(callToReject);
            return true;
        }
    ...
    /**
     * hang up call to make BT HFP work fine in DSDA case.
     */
    private boolean restrictDualTalk(boolean hfpConnected) {
        Log.i(TAG, "restrictDualTalk > hfpConneccted:" + hfpConnected);
        ...
        //(H,H) restriction
        if (restrictHHIfNeeded(hfpConnected)) {
            executed = true;
        }

        Log.i(TAG, "[restrictDualTalk] :" + executed);
