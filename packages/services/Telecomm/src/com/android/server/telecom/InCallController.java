...
public final class InCallController extends CallsManagerListenerBase {
    ...
    public void onCallRemoved(Call call) {
        ...
        if (CallsManager.getInstance().getCalls().isEmpty()
                /// M: ALPS01853640, should not unbind service if still have pending ecc. @{
                && !CallsManager.getInstance().hasPendingEcc()) {
                /// @}
     ...
     */
    /// M: for performance enhancement. bind InCallUI in advance. @{
    // original code:
            ...
            for (ResolveInfo entry : packageManager.queryIntentServices(serviceIntent, 0)) {
                ...
                if (serviceInfo != null) {
                    ...
                    if (!mServiceConnections.containsKey(componentName)) {
                        ...
                        /**
                         * M: migration
                         * if bind earlier, call might be null.
    ...
    private ParcelableCall toParcelableCall(Call call, boolean includeVideoProvider) {
        ...
        ///M: add to filter sip account for ip dial @{
        Uri handle = call.getHandle();
        if (handle != null && call.isIpCall()) {
            String num = handle.getSchemeSpecificPart();
            if (!TextUtils.isEmpty(num)) {
                num = num.trim().replaceAll("%20", "");
            }
        }
        handle = call.getHandlePresentation() == TelecomManager.PRESENTATION_ALLOWED ?
                handle : null;
        //Log.d(this, "call handle %s, is ip dial= %b", handle, call.isIpCall());
        /// @}
        ...
        properties = properties | (call.isConferenceInvite() ? CallProperties.CONFERENCE : 0);
        /// @}
    ...
    private static final int[] CONNECTION_TO_CALL_CAPABILITY = new int[] {
        ...
        /* M: CC part start */
        Connection.CAPABILITY_ANSWER,
        android.telecom.Call.Details.CAPABILITY_ANSWER,

        Connection.CAPABILITY_UNHOLD,
        android.telecom.Call.Details.CAPABILITY_UNHOLD,

        Connection.CAPABILITY_ADD_CALL,
        android.telecom.Call.Details.CAPABILITY_ADD_CALL,
