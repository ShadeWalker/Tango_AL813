...
public class MuteCommand extends DefaultCommand {
    ...
    protected int beforeExecute() {
        ...
        if (activeCall != null && activeCall.can(PhoneCapabilities.MUTE)) {
            retval = ICommand.RESULT_OK;
        } else {
            log("beforeExecute has no call or call not support mute");
        }
        mMuted= CallsManager.getInstance().getAudioState().isMuted;
        log("mMuted = " + mMuted);
        return retval;
    }
    ...
    protected int executeInner(String content) {
        ...
        boolean newMuteState = !mMuted;
        log("onMuteClick(): newMuteState = " + newMuteState);
        CallsManager.getInstance().mute(newMuteState);
    ...
    protected int afterExecute() {
        ...
        final boolean newMuted = CallsManager.getInstance().getAudioState().isMuted;
        if (mMuted == newMuted) {
            log("!!!afterExecute mute failed!!!");
            retval = ICommand.RESULT_FAIL;
        }
        log("PhoneUtils.getMute() = " + newMuted);
        return retval;
    }
