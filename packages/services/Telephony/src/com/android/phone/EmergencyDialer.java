...
// @}
	...
	//		hq_hxb modified by	for  HQ01308688 begin
	private static final int EMERGENCY_NUMBER_DIALOG_110 = 4;
	private static final int EMERGENCY_NUMBER_DIALOG_119 = 8;
	private static final int EMERGENCY_NUMBER_DIALOG_CLOSE_FLIGHT = 12;
	//		hq_hxb modified by	for  HQ01308688 end
    ...
    protected void onCreate(Bundle icicle) {
        ...
        // M: check dial button empty for plug in modify action. @{
        if (null != mDialButton && res.getBoolean(R.bool.config_show_onscreen_dial_button)) {
            mDialButton.setOnClickListener(this);
        } else if (null != mDialButton) {
    ...
    protected void onDestroy() {
        ...
        //add for plug in.@{
        ExtensionManager.getEmergencyDialerExt().onDestroy();
        //add for plug in.@}
    }
	...
	Log.d("hxb, placeCall mLastNumber:",  mLastNumber);
	...
	if(SystemProperties.get("ro.config.emergency_call_taiwan").equals("1")){
		...
		boolean ariplanestatus=getAirplaneMode();

		Log.d("hxb, sim1_State:",  sim1_State);
		Log.d("hxb, sim2_State:", sim2_State);
		Log.d("hxb, mMccMncOp:", mMccMncOp);
		Log.d("hxb, sim1_MccMnc:", sim1_MccMnc);
		Log.d("hxb, sim2_MccMnc:", sim2_MccMnc);
	if(ariplanestatus)
		Log.d("hxb, ariplanestatus:", "true");
	else
		Log.d("hxb, ariplanestatus:", "false");

		if(mLastNumber!=null&&(!mLastNumber.equals(""))&&("110".equals(mLastNumber)||"119".equals(mLastNumber))){
			//	 hq_hxb modified by  for  HQ01548397
			if(getAirplaneMode()&&(!sim1_State.equals("READY"))&&(!sim2_State.equals("READY"))) {
				mLastNumber="112";
			}else if(mMccMncOp.startsWith("466")){
				if(sim1_State.equals("ABSENT")&&sim2_State.equals("ABSENT")){
					mLastNumber="112";
				}else if((sim1_State.equals("READY")&&sim1_MccMnc.startsWith("466"))||(sim2_State.equals("READY")&&sim2_MccMnc.startsWith("466"))){
					mLastNumber="112";
				}else if("PIN_REQUIRED".equals(sim1_State)||"PIN_REQUIRED".equals(sim2_State)){
					mLastNumber="112";
				}else if("PUK_REQUIRED".equals(sim1_State)||"PUK_REQUIRED".equals(sim2_State)){
					mLastNumber="112";
				}
			}
		Log.d("hxb, placeCall mLastNumber 2:",  mLastNumber);		
		}
	}
	//	 hq_hxb modified by  for  HQ01308688 end
		...
		Log.d(LOG_TAG, "hxb number placeCall 11::"+mLastNumber);

			//	 hq_hxb modified by  for  HQ01308688
			/* do not remove now for other unexpected situation in future
			if(mLastNumber!=null&&(!mLastNumber.equals(""))&&SystemProperties.get("ro.config.emergency_call_taiwan").equals("1")){
							//just test

					if(mMccMncOp!=null&&(mMccMncOp.startsWith("466")||"999999".equals(mMccMncOp))
						&&(sim1_MccMnc.startsWith("466")||sim2_MccMnc.startsWith("466"))&&("110".equals(mLastNumber)||"119".equals(mLastNumber))){

							mDigits.getText().delete(0, mDigits.getText().length());
							if(hintAnotherNumWhenDialEmergencyNumber())
							{
									if("110".equals(mLastNumber)){
											showDialog(EMERGENCY_NUMBER_DIALOG_110);
									}else if("119".equals(mLastNumber)){
											showDialog(EMERGENCY_NUMBER_DIALOG_119);										
									}
							}else if(hintFlightWhenDialEmergencyNumber()){
													Log.d(LOG_TAG, "hxb number placeCall 44");
									showDialog(EMERGENCY_NUMBER_DIALOG_CLOSE_FLIGHT);
							}
					}else{
							mDigits.getText().delete(0, mDigits.getText().length());
				...
				Log.d(LOG_TAG, "hxb number placeCall 66");
    ...
    private CharSequence createErrorMessage(String number) {
        if (!TextUtils.isEmpty(number)) {
            //modify by lizhao for HQ01563336 at 20151223
...
//	 hq_hxb modified by  for  HQ01308688
/* do not remove now for other unexpected situation in future		
		else if(id == EMERGENCY_NUMBER_DIALOG_110){ //
                 CharSequence hint_110=getText(R.string.emergency_hint_TW_110); 
             dialog = new AlertDialog.Builder(this)
                     .setTitle(getText(R.string.emergency_enable_radio_dialog_title))
                     .setMessage(hint_110)
                     .setPositiveButton(R.string.ok, null)
                 ...
                 }else if(id == EMERGENCY_NUMBER_DIALOG_119){
                 CharSequence hint_119=getText(R.string.emergency_hint_TW_119); 
             dialog = new AlertDialog.Builder(this)
                     .setTitle(getText(R.string.emergency_enable_radio_dialog_title))
                     .setMessage(hint_119)
                     .setPositiveButton(R.string.ok, null)
                     .setCancelable(true).create();
                 }else if(id == EMERGENCY_NUMBER_DIALOG_CLOSE_FLIGHT){
                 CharSequence hint_closeflight=getText(R.string.emergency_hint_TW_closeflight); 
             dialog = new AlertDialog.Builder(this)
                     .setTitle(getText(R.string.emergency_enable_radio_dialog_title))
                     .setMessage(hint_closeflight)
                     .setPositiveButton(R.string.ok, null)
                     .setCancelable(true).create();
                 }
 */
...
/* do not remove now for other unexpected situation in future		
		...
		CharSequence hint_110=getText(R.string.emergency_hint_TW_110);
		AlertDialog alert = (AlertDialog) dialog;
		alert.setMessage(hint_110);

             }else if(id == EMERGENCY_NUMBER_DIALOG_119){
		CharSequence hint_119=getText(R.string.emergency_hint_TW_119);
		AlertDialog alert = (AlertDialog) dialog;
		alert.setMessage(hint_119);

             }else if(id == EMERGENCY_NUMBER_DIALOG_CLOSE_FLIGHT){
		CharSequence hint_closeflight=getText(R.string.emergency_hint_TW_closeflight);
		AlertDialog alert = (AlertDialog) dialog;
		alert.setMessage(hint_closeflight);
             }
*/
    }
...
// 	 hq_hxb modified by  for  HQ01308688 begin
      private boolean getAirplaneMode() {
          ...
          public boolean hintAnotherNumWhenDialEmergencyNumber(){
		  String sim1_State = PhoneNumberUtils.getSimStatusForSlot(0);
		  String sim2_State = PhoneNumberUtils.getSimStatusForSlot(1);
              ...
              Log.d("hxb, sim2_State:", sim2_State);
                  ...
                  if ("ABSENT".equals(sim1_State)&&"ABSENT".equals(sim2_State)&&getAirplaneMode()){
                          return true;                    
                  }else if ("PIN_REQUIRED".equals(sim1_State)||"PIN_REQUIRED".equals(sim2_State)) {
                          return true;            
                  }else if ("PUK_REQUIRED".equals(sim1_State)||"PUK_REQUIRED".equals(sim2_State)) {
                          return true;            
                  }else if ("PIN_REQUIRED".equals(sim1_State)||"PIN_REQUIRED".equals(sim2_State)&&getAirplaneMode()) {
                          return true;            
                  }else if("PUK_REQUIRED".equals(sim1_State)||"PUK_REQUIRED".equals(sim2_State)&&getAirplaneMode()){
                          return true;
                  }else if ("READY".equals(sim1_State)||"READY".equals(sim2_State)) {
                          return true;
                  }else {
                          return false;
                  }
          ...
          public boolean hintFlightWhenDialEmergencyNumber(){
			  String sim1_State = PhoneNumberUtils.getSimStatusForSlot(0);
			  String sim2_State = PhoneNumberUtils.getSimStatusForSlot(1);
                  
                  if ("ABSENT".equals(sim1_State)&&"ABSENT".equals(sim2_State)){
                          return true;
                  }else if ("READY".equals(sim1_State)||"READY".equals(sim2_State)&&getAirplaneMode()) {
                          return true;
                  }else {
                          return false;
                  }               
