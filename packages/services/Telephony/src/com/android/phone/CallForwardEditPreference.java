...
import java.util.Locale;//add by zhaizhanfeng for ATT test
...
public class CallForwardEditPreference extends EditPhoneNumberPreference {
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        if (this.mButtonClicked != DialogInterface.BUTTON_NEGATIVE) {
            ...
            if (DBG && Build.TYPE.equals("eng")) Log.d(LOG_TAG, "callForwardInfo=" + callForwardInfo);
            ...
            if (action == CommandsInterface.CF_ACTION_REGISTRATION
                ...
                // set to network
                if (DBG && Build.TYPE.equals("eng")) Log.d(LOG_TAG, "reason=" + reason + ", action=" + action
                        + ", number=" + number);
    ...
    void handleCallForwardResult(CallForwardInfo cf) {
        callForwardInfo = cf;
        if (DBG && Build.TYPE.equals("eng")) Log.d(LOG_TAG, "handleGetCFResponse done, callForwardInfo=" + callForwardInfo);
        setToggled(callForwardInfo.status == 1);
        //setPhoneNumber(callForwardInfo.number);
        if (Build.TYPE.equals("eng")) {
            Log.d("zhai","handleCallForwardResult callForwardInfo.number=" + callForwardInfo.number);
        }
		if (callForwardInfo.number == null) {
			setPhoneNumber("");
		} else {
            //add by zhaizhanfeng for HQ01404288 at 151002 start
            String langage = Locale.getDefault().getLanguage();
            if(langage.startsWith("ar")||langage.startsWith("fa")||langage.startsWith("iw")){
			    setPhoneNumber('\u202d' + callForwardInfo.number + '\u202c');
            }else{
                setPhoneNumber(callForwardInfo.number);
            }
		}
		//add by zhaizhanfeng for HQ01404288 at 151002 end
		}
            ...
            if (ar.exception != null) {
                ...
                if (cfInfoArray.length == 0) {
                    ...
                    for (int i = 0, length = cfInfoArray.length; i < length; i++) {
                        if (DBG && Build.TYPE.equals("eng")) Log.d(LOG_TAG, "handleGetCFResponse, cfInfoArray[" + i + "]="
                                + cfInfoArray[i]);
