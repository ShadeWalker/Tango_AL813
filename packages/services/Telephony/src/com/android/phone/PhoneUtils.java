    ...
    public static void displayMMIComplete(final Phone phone, Context context, final MmiCode mmiCode,
        ...
        switch (state) {
            case PENDING:
                ...
                log("- using text from PENDING MMI message: '" + text + "'");
                break;
            case CANCELLED:
                text = null;
                break;
            ...
            case FAILED:
                /*HQ_guomiao add for HQ01455131 begin*/
                if (SystemProperties.get("ro.hq.claro.call.forwarding").equals("1")){
                    log("- claro.call.forwarding using text from MMI message: '" + mmiCode.getMessage() + "'");
                    break;
                }
                /*HQ_guomiao add for HQ01455131 end*/
                text = mmiCode.getMessage();
                log("- using text from MMI message: '" + text + "'");
                break;
            default:
        ...
        if ((app.getPUKEntryActivity() != null) && (state == MmiCode.State.COMPLETE)) {
            ...
            if (state != MmiCode.State.PENDING) {
                ...
                log("Extended NW displayMMIInitiate (" + text + ")");
                ...
                if (sUssdDialog == null) {
                    ...
                    sUssdDialog = new AlertDialog.Builder(context, themeID)
                ...
                // now show the dialog!
                android.widget.Button posBtn = sNewDialog.getButton(DialogInterface.BUTTON_POSITIVE);
                android.widget.Button negBtn = sNewDialog.getButton(DialogInterface.BUTTON_NEGATIVE);
                log("positive button =" + posBtn +", negative button =" + negBtn);
                if (posBtn != null) {
                    posBtn.setTextColor(context.getResources().getColor(R.color.dialer_theme_color));
                }
                if (negBtn != null ) {
                    negBtn.setTextColor(context.getResources().getColor(R.color.dialer_theme_color));
                }
                sNewDialog.setMessage(text);
                sNewDialog.show();
                //add for plug in.@{
                //to show Ussd dialog.
                ExtensionManager.getMmiCodeExt().configBeforeMmiDialogShow(mmiCode);
    ...
    private static void playUSSDTone(final Context context) {
       ...
       if (ringerMode == AudioManager.RINGER_MODE_VIBRATE){
        ...
        if ( SystemProperties.get("ro.hq.russia.spussd").equals("1") ) {
			 Log.d(LOG_TAG, "Russia , do not play USSD tone...");
             return;
		}
        // / End
