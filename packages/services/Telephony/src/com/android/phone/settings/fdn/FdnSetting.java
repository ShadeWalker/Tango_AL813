...
public class FdnSetting extends PreferenceActivity implements
    ...
    private void toggleFDNEnable(boolean positiveResult) {
        ...
        updateEnableFDNDialog(DEFAULT_ENTER_PIN_TIP_ID);
        /// @}
        if (!positiveResult) {
            return;
        }
    ...
    private static final int DEFAULT_ENTER_PIN_TIP_ID = R.string.enter_pin2_text;
     ...
     */
    private void updateEnableFDNDialog(int resId) {
        if (DEFAULT_ENTER_PIN_TIP_ID != resId) {
            log("[updateEnableFDNDialog] Add user resId = " + resId);
            mButtonEnableFDN.setDialogMessage(getString(resId) + "\n"
                    + getString(DEFAULT_ENTER_PIN_TIP_ID) + "\n"
                    + TelephonyUtils.getPinPuk2RetryLeftNumTips(this, mSubId, true));
        } else {
            log("[updateEnableFDNDialog] Use default resId.");
            mButtonEnableFDN.setDialogMessage(getString(resId) + "\n"
                    + TelephonyUtils.getPinPuk2RetryLeftNumTips(this, mSubId, true));
        }
    }
    ...
    private void updateWholeScreen(int subId) {
        ...
        if (iccCard != null) {
            if(!iccCard.getIccFdnAvailable()||(TelephonyUtils.getPuk2RetryNumber(subId) == 0)) {//modified by maolikui at 2015-09-25
                log("[updateWholeScreen] FDN not support!");
