...
//fengyaling add 20150703 for providing dbm values begin
import android.telephony.SignalStrength;
//fengyaling add 20150703 for providing dbm values end
...
public class PhoneInterfaceManager extends ITelephony.Stub {
    ...
    private final class MainThreadHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
           ...
           //fengyaling add 20150703 for providing dbm values begin 
	Phone phones[] = null;
	SignalStrength signalStrength1 = null;
	SignalStrength signalStrength2 = null;
	int simCounts = 0;
	String sim1Signal = null;
	String sim2Signal = null;

	simCounts = TelephonyManager.from(mPhone.getContext())
			.getSimCount();
	phones = new Phone[simCounts];
	log("default simCounts " + simCounts);
	if (simCounts == PhoneConstants.MAX_PHONE_COUNT_SINGLE_SIM) {
		log("sim card one ");
		phones[PhoneConstants.SIM_ID_1] = PhoneFactory
				.getPhone(PhoneConstants.SIM_ID_1);
		if (phones[PhoneConstants.SIM_ID_1] != null) {
			signalStrength1 = phones[PhoneConstants.SIM_ID_1]
					.getSignalStrength();
			if (signalStrength1 != null) {
				if (signalStrength1.isGsm()) {
					sim1Signal = "SIM1:"
							+ signalStrength1.getGsmSignalStrengthDbm();
				} else {
					sim1Signal = "SIM1:" + signalStrength1.getCdmaDbm();
				}
				SystemProperties.set("ril.dbm.sim1", sim1Signal);
			}
		}

	} else if (simCounts == PhoneConstants.MAX_PHONE_COUNT_DUAL_SIM) {
		log("sim card two ");
		phones[PhoneConstants.SIM_ID_1] = PhoneFactory
				.getPhone(PhoneConstants.SIM_ID_1);
		phones[PhoneConstants.SIM_ID_2] = PhoneFactory
		...
		if (phones[PhoneConstants.SIM_ID_2] != null) {
			signalStrength2 = phones[PhoneConstants.SIM_ID_2]
					.getSignalStrength();
			if (signalStrength2 != null) {
				if (signalStrength2.isGsm()) {
					sim2Signal = "SIM2:"
							+ signalStrength2.getGsmSignalStrengthDbm();
				} else {
					sim2Signal = "SIM2:" + signalStrength2.getCdmaDbm();
				}
				SystemProperties.set("ril.dbm.sim2", sim2Signal);
			}
		}
	}
	  //fengyaling add 20150703 for providing dbm values end
    ...
    public boolean isIdleForSubscriber(int subId) {
		//HQ_pangxuhui modify for HQ01296325 20150731
        //return (getPhone(subId).getState() == PhoneConstants.State.IDLE);
        if(getPhone(subId)!=null){
			return (getPhone(subId).getState() == PhoneConstants.State.IDLE);
		}
		return false;
		//End
    ...
    public int[] supplyPinReportResultForSubscriber(int subId, String pin) {
        ...
        if (getPhone(subId) != null) {
            ...
            int[] arr = checkSimPin.unlockSim(null, pin);
    ...
    public int[] supplyPukReportResultForSubscriber(int subId, String puk, String pin) {
        ...
        int[] arry = checkSimPuk.unlockSim(puk, pin);
	     return arry;
    ...
    public int getDataState() {
		...
		if(phone == null) return 0;//add by majian for fatal error
    ...
    public Bundle getCellLocation() {
        ...
        if (checkIfCallerIsSelfOrForegroundUser()) {
            ...
            //HQ_jiazaizheng 20150808 modify for HQ01299684 start
            if (shouldBlockLocation()){
                Log.d(LOG_TAG,"getCellLocation is blocked by permission manager uid:" + Binder.getCallingUid() +" pid:"+Binder.getCallingPid());
                return data;
            }
            //HQ_jiazaizheng 20150808 modify for HQ01299684 end
            //[ALPS01971678]-start
            try {
    ...
    //HQ_jiazaizheng 20150804 modify for HQ01299684 start
    public boolean shouldBlockLocation() {
        Method method = null;
        Log.w(LOG_TAG, "shouldBlockLocation running ...");
      try {
          /* <DTS2014042818262 xiongshiyi/00165767 20140428 begin */
          Class<?> ctrlrClazz = Class.forName("com.huawei.hsm.permission.LocationPermission");
          /* DTS2014042818262 xiongshiyi/00165767 20140428 end>*/
          method = ctrlrClazz.getMethod("isLocationBlocked", null);
          Constructor objctor = ctrlrClazz.getConstructor(Context.class);
          Object inst = objctor.newInstance(mApp.getApplicationContext());

          Boolean b = (Boolean)method.invoke(inst, null);
          boolean ret = b.booleanValue();
          Log.i(LOG_TAG, "shouldBlockLocation  ret:" + ret);
          return ret;
      } catch (IllegalArgumentException ex) {
          Log.w(LOG_TAG, "shouldBlockLocation", ex);
      } catch (RuntimeException ex) {
          Log.w(LOG_TAG, "shouldBlockLocation", ex);
      } catch (java.lang.InstantiationException e) {
          Log.w(LOG_TAG, "shouldBlockLocation", e);
    ...
    public List<NeighboringCellInfo> getNeighboringCellInfo(String callingPackage) {
        ...
        //HQ_jiazaizheng 20150804 modify for HQ01299684 start
        if (shouldBlockLocation()) {
            Log.d(LOG_TAG,"getNeighboringCellInfo is blocked by permission manager uid:"+Binder.getCallingUid());
            return (List <NeighboringCellInfo>) new ArrayList<NeighboringCellInfo>(1);
        }
        //HQ_jiazaizheng 20150804 modify for HQ01299684 start
        if (checkIfCallerIsSelfOrForegroundUser()) {
    ...
    public int getActivePhoneTypeForSubscriber(int subId) {
		if(getPhone(subId) == null) return 0; //add by majian for fatal error
    ...
    public int getNetworkTypeForSubscriber(int subId) {
        ...
        /// M: C2K OM solution2 modification to fix network type fetching problem. @{
        if (SystemProperties.get("ro.mtk_svlte_support").equals("1")) {
			if(subId>0){
            	state = getPhone(subId).getSvlteServiceState();
			}
        }
        //@}
		try{
			if (state == null) {
            state = getPhone(subId).getServiceState();
        }		
        ...
        if (state.getDataNetworkType() != ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN) {
            return state.getDataNetworkType();
        } else {
            // return voice network type for better backward compatible
    ...
    public int getDataNetworkTypeForSubscriber(int subId) {
        /// M: [C2K][SVLTE] Merge the data type of dual connection. @{
        if (SystemProperties.get("ro.mtk_svlte_support").equals("1")) {
            ServiceState ss = getPhone(subId).getSvlteServiceState();
            if (ss != null) {
        ...
        int dataNetworkType = getPhone(subId).getServiceState().getDataNetworkType();
        log("getDataNetworkType=" + dataNetworkType);
        return dataNetworkType;
    }
	...
	/**add by liruihong for HQ01355489 to pass permission check for huawei startup guide  begin*/
	 ...
	 Log.i("liruihong", "cpname:" + cpname);

        if (cpname != null) {
            packagename = cpname.getPackageName();
            Log.i("liruihong", "packagename:" + packagename);
        } 

	if (packagename != null && packagename.equals("com.huawei.hwstartupguide")) {
        	Log.i("liruihong", "is huawei start guide");		
    	}else{
	        enforceModifyPermission();
	...
	/**add by liruihong for HQ01355489 to pass permission check for huawei startup guide  end*/
        ...
        if (phone != null) {
            ...
            int newDefaultDataSubId = -1;
            int[] subIds = SubscriptionManager.from(context).getActiveSubscriptionIdList();
            TelephonyManager telephonyManager = TelephonyManager.from(context);
            if (subIds.length > 1) {
                for (int i : subIds) {
                    loge("setDataEnabled: all subid:"+i);
                }                
                for (int id : subIds) {
                    loge("setDataEnabled: active subId:"+id);
                    if (id != subId && telephonyManager.getDataEnabled(id)) {
                        loge("setDataEnabled: data enabled active subId:"+id);
                        newDefaultDataSubId = id;
                        break;
                    }
                }
                if (newDefaultDataSubId == -1) {
                    loge("setDataEnabled: enable subId[0]: "+subIds[0]);
                    newDefaultDataSubId = subIds[0];
                }
            }
            else if (subIds.length == 1) {
                loge("setDataEnabled: only 1 active subId: "+subIds[0]);
                newDefaultDataSubId = subIds[0];
            }
            else {
                loge("setDataEnabled: no active subId");
            }
            if (newDefaultDataSubId != -1) {
                loge("setDataEnabled: newDefaultDataSubId:"+newDefaultDataSubId);
                mSubscriptionController.setDefaultDataSubId(newDefaultDataSubId);
                setDataEnabled(newDefaultDataSubId, enable);            
            }
