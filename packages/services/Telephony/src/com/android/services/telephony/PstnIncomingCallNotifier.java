...
/*HQ_xionghaifeng 20150917 add for HWstartupguide start*/
...
final class PstnIncomingCallNotifier {
    ...
    PstnIncomingCallNotifier(PhoneProxy phoneProxy) {
        ...
        /* remove by shanlan for HQ02051113 begin */
		//add by zhangjinqiang for block blacklist--start
		...
		//add by zhangjinqiang end
        /* remove by shanlan for HQ02051113 end */
    }
    ...
    private void handleNewRingingConnection(AsyncResult asyncResult) {
        ...
        if (connection != null) {
			...
			if (Settings.Secure.getInt(mPhoneProxy.getContext().getApplicationContext().getContentResolver(), Settings.Global.DEVICE_PROVISIONED, 1) == 0) {
				Log.i("xionghaifeng", "handleNewRingingConnection  return");
			...
			String incomingNumber = connection.getAddress();
			Call ringing = connection.getCall();
			try {
					Bundle bundle = new Bundle();
					bundle.putString("HANDLE_CALLINFO", incomingNumber);
                    /* add by shanlan for HQ02051113 begin */
                    IBinder harassmentIntercepterService = ServiceManager.getService(HARASSMENT_INTERCEPTION_SERVICE);
                    hisStub = IHarassmentInterceptionService.Stub.asInterface(harassmentIntercepterService);
                    if (null == hisStub) {
                        return;
                    }
                    /* add by shanlan for HQ02051113 end */
					int isBlock = hisStub.handleIncomingCallAction(bundle);
					if(isBlock == 1){
						PhoneUtils.hangupRingingCall(ringing);
						Bundle bundleBlock = new Bundle();
						bundleBlock.putString("BLOCK_PHONENUMBER", incomingNumber);
						hisStub.sendCallBlockRecords(bundleBlock);
						return;
					}
				}catch (Exception e) {
