...
public class EmergencyCallHelper {
    ...
    private void startSequenceInternal(Phone phone, Callback callback) {
        ...
        /// M: [ALPS02225265]The register/unregister operation will be useless
        /// when active phone switches. @{
        mPhone = ((com.android.internal.telephony.PhoneProxy) phone).getActivePhone();
        Log.d(this, "phone:" + phone + ", mPhone:" + mPhone);
        /// @}
        mCallback = callback;
        ...
        createDialog();/* HQ_guomiao 2015-10-20 modified for HQ01442667 */
    ...
    private boolean isOkToCall(int serviceState, PhoneConstants.State phoneState) {
        /// M: [ALPS02185470] Only retry once for WFC ECC. @{
        // To retrieve WiFi calling registration status.
        boolean bWiFiCallingIsRegistered = false;
        boolean wfcIsEnabled = ImsManager.getInstance(mContext, mPhone.getPhoneId()).isWfcEnabledByPlatform(mContext)
                       && ImsManager.getInstance(mContext, mPhone.getPhoneId()).isWfcEnabledByUser(mContext);
        
        if (wfcIsEnabled) {
           int wfcStatusCode = ImsManager.getInstance(mContext, mPhone.getPhoneId()).getWfcStatusCode();
           Log.d(this, "[isOkToCall]WiFi calling status code = " + wfcStatusCode);
    ...
    /* HQ_guomiao 2015-10-20 modified for HQ01442667 begin */
    private AlertDialog dialog;

    private void createDialog(){
        if (dialog == null) {
            dialog = new AlertDialog.Builder(mContext)
            .setTitle(R.string.emergency_enable_radio_dialog_title)
            .setMessage(R.string.emergency_enable_radio_dialog_message)
            .create();
    ...
    private void cleanup() {
        ...
        dismissDialog();/* HQ_guomiao 2015-10-20 modified for HQ01442667 */
        onComplete(false);
