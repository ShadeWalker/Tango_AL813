...
public abstract class MmsRequest {
    ...
    public void execute(Context context, MmsNetworkManager networkManager) {
        ...
        Log.d(MmsService.TAG, "MmsRequest: execute");
        if (!ensureMmsConfigLoaded()) { // Check mms config
            ...
            for (int i = 0; i < RETRY_TIMES; i++) {
                try {
                    ...
                    if (mMmsServiceCancelDownloadPlugin.isCancelDownloadEnable()) {
                        ...
                        if (mMmsServiceCancelDownloadPlugin.needBeCanceled(locationUri)) {
                            ...
                            Log.e(MmsService.TAG, "set result = MMS_ERROR_UNSPECIFIED");
                            break;
                        }
                    }
                    /// @}
                    ...
                    networkManager.releaseNetworkImmediately();
                ...
                try {
                    Log.d(MmsService.TAG, "sleep: " + retryDelaySecs * 1000);
            ...
            if (isRequestSuccessed && networkManager.isNetworkReleased()) {
                try {
                    Log.d(MmsService.TAG, "success sleep 2s");
                    Thread.sleep(2000, 0/*nano*/);
                } catch (InterruptedException e) {}
            }
        ...
        if(result != Activity.RESULT_OK) {
            Log.d(MmsService.TAG, "pdp connect FAIL. result = "+result);
        }
    ...
    public void processResult(Context context, int result, byte[] response, int httpStatusCode) {
        ...
        Log.d(MmsService.TAG, "processResult: prepare");
        ...
        if (pendingIntent != null) {
            ...
            if (messageUri != null) {
                Log.d(MmsService.TAG, "processResult: uri = " + mMessageUri);
