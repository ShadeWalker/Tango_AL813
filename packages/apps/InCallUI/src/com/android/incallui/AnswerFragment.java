...
public class AnswerFragment extends BaseFragment<AnswerPresenter, AnswerPresenter.AnswerUi>
	...
	if(InCallApp.gIsHwUi){
		mGlowpad = (GlowPadWrapper) inflater.inflate(R.layout.answer_fragment,
                     container, false);
            	Log.d(this, "answer ui");
	}else{
            mGlowpad = (GlowPadWrapper) inflater.inflate(R.layout.answer_fragment,
                    container, false);
        }
			
        //final View parent = inflater.inflate(R.layout.answer_fragment,container, false);//modify by zjq
        // mGlowpad = (GlowPadWrapper) parent.findViewById(R.id.glow_pad_view);//modify by zjq
        //mPromptView = (TextView) parent.findViewById(R.id.incoming_call_prompt_view);//modify by zjq
    ...
    public void showCustomMessageDialog() {
        ...
        builder.setCancelable(true).setView(et)
                ...
                .setNegativeButton(R.string.custom_message_cancel,
                    ...
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        /// M: ALPS01856137, need to start ping when back from reject with sms dialog.
                        if (mGlowpad != null) {
                            mGlowpad.startPing();
                        }
                        /// @}
                ...
                .setOnCancelListener(
                        /// M: ALPS01770425. add listener for back key.
                        new DialogInterface.OnCancelListener() {
                            @Override
                            public void onCancel(DialogInterface dialogInterface) {
                                if (mGlowpad != null) {
    ...
    /**
        if (mPromptView == null) {
            Log.e(this, "updatePromptMessage, PromptView is null");
            return;
        }

        if (show) {
            mPromptView.setVisibility(View.VISIBLE);
            String message = getView().getResources()
                    .getString(R.string.accept_incoming_and_disconnect_current);
