...
public class PointCloud {
    ...
    private static final int INNER_POINTS = 4;//modify by zjq
	...
	//add by zjq
	private boolean mShowTextPoint = true;
    ...
    class Point {
        ...
        public Point(float x2, float y2, float r,float e) {
            x = (float) x2;
            y = (float) y2;
            radius = r;
			eta =e;
...
//modify by zjq
    public void makePointCloud(float innerRadius, float outerRadius,int count) {
        if (innerRadius == 0) {
        ...
        for (int b = 0; b <= bands; b++, r += dr) {
            ...
            final int pointsInBand = count;//(int) (circumference / ds);
			float eta = PI;//add by zjq
            float dEta = PI/2.0f;
            //float dEta = 2.0f * PI / pointsInBand;
            for (int i = 0; i < pointsInBand; i++) {
                float x = r * FloatMath.cos(eta);
                float y = r * FloatMath.sin(eta);
				mPointCloud.add(new Point(x, y, r,eta));
                eta += dEta;
    ...
    public void draw(Canvas canvas) {
        ...
        for (int i = 0; i < points.size(); i++) {
            ...
            if (alpha == 0 || (!mShowTextPoint && (point.eta == (3.0f*PI)/2.0f))){
                continue;
            }
	...
	/*Added  by yudehai for SW00109712 (1701) 20150131,begin*/
    public void showTextPointCloud(boolean show){
        mShowTextPoint = show;
    }
    /*Added  by yudehai for SW00109712 (1701) 20150131,end*/
}
