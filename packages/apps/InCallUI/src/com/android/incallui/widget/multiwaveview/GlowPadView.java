...
public class GlowPadView extends View {
    ...
    //private static final boolean DEBUG = false;modify by zjq
    ...
    //private static final int WAVE_ANIMATION_DURATION = 1350;modify by zjq
	private static final int WAVE_ANIMATION_DURATION = 2700;
	...
	//add by zhangjinqiang for HQ01456696
    ...
    public GlowPadView(Context context, AttributeSet attrs) {
		...
		//mPointCloud.makePointCloud(mInnerRadius, mOuterRadius); //modify by zjq
        mPointCloud.makePointCloud(mInnerRadius, mOuterRadius,mTargetDrawables.size() -1);
    ...
    private void switchToState(int state, float x, float y) {
        switch (state) {
            ...
            case STATE_TRACKING:
                mHandleDrawable.setAlpha(0.0f);
 	       		showGlow(REVEAL_GLOW_DURATION , REVEAL_GLOW_DELAY, 1.0f, null);// MODIFY BY ZJQ
                break;
    ...
    public void ping() {
        if (mFeedbackCount > 0) {
            ...
            if (waveAnimations.size() > 0 && waveAnimations.get(0).animator.isRunning()) {
                ...
                if (t < WAVE_ANIMATION_DURATION/4) {//modify by zjq(2)
...
//modify by zjq
    private void startWaveAnimation() {
        ...
        mWaveAnimations.add(Tweener.to(mPointCloud.waveManager, WAVE_ANIMATION_DURATION,
                "ease", Ease.Sine.easeOut,//(Quad--Sine)
                "delay", 0,
                "radius", 1.1f * mOuterRadius,//2.0f--1.1f
                "onUpdate", mUpdateListener,
                "onComplete",
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                handled = true;
			   isTouchMove = true;
			   //add by zhangjinqiang for HQ01458319
			   handleMoveView(event.getX(),event.getY());
			   //add by zjq end
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                handled = true;
			   //add by zhangjinqiang for HQ01458319
			   mOuterRing.setPositionX(localX);
        		   mOuterRing.setPositionY(localY);
			   //add by zjq end
                break;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		localX = newWaveCenterX;
		localY = newWaveCenterY;
		android.util.Log.d("Location-onlayout",localX+"--"+localY);
        if (DEBUG) dump();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < ntargets; i++) {
         ...
         if(!isTouchMove){
			...
			canvas.drawCircle(localX, localY, 4.0f, p);
         }
		//add by zjq end
		
    	//mHandleDrawable.draw(canvas);//modify by zjq
    }
	...
	//add by zhangjinqiang for HQ01458319
	 private void handleMoveView(float x, float y){   
	    float Radius = mOuterRadius;  
	    if (Math.sqrt(dist2(x - localX, y - localY)) > Radius)       
	    {  
	        x = (float) ((Radius / (Math.sqrt(dist2(x - localX, y - localY)))) * (x - localX) + localX);  
	        y = (float) ((Radius / (Math.sqrt(dist2(x - localX, y - localY)))) * (y - localY)  + localY);  
	    }  
	    mOuterRing.setPositionX(x);  
	    mOuterRing.setPositionY(y);  
	}   
	//add by zjq end
}
