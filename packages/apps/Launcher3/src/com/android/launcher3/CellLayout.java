    ...
    void setPressedIcon(BubbleTextView icon, Bitmap background, int padding) {
        if (icon == null || background == null) {
            ...
            if (mTouchFeedbackView.setBitmap(background)) {
                ...
                ///M. ALPS02008198, for two cores performance issue.
                long duration = FastBitmapDrawable.CLICK_FEEDBACK_DURATION;

                Log.d(TAG, "setPressedIcon,Processors:"
                    + Runtime.getRuntime().availableProcessors());

                if (Runtime.getRuntime().availableProcessors() <= 2) {
                    duration = FastBitmapDrawable.CLICK_FEEDBACK_LESS_DURATION;
                }
                ///M.
