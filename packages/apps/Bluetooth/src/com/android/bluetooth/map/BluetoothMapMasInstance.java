...
public class BluetoothMapMasInstance {
    private static final String TAG = "[MAP]BluetoothMapMasInstance";
    ...
    private class SocketAcceptThread extends Thread {
        ...
        void shutdown() {
            Log.d(TAG, "shutdown " + mServerSocket);
            stopped = true;
            if(mServerSocket != null) {
                try {
                    mServerSocket.close();
    ...
    public void shutdown() {
        if (D) Log.d(TAG, "MAP Service shutdown " + mServerSession);
    ...
    public void restartObexServerSession() {
        if (D) Log.d(TAG, "MAP Service restartObexServerSession" + mMasInstanceId);
    ...
    private final synchronized void closeServerSocket() {
        Log.d(TAG, "closeServerSocket " + mServerSocket);
