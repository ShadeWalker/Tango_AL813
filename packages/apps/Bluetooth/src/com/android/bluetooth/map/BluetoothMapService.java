...
public class BluetoothMapService extends ProfileService {
    ...
    private final void startObexServerSessions() {
        if (DEBUG) Log.d(TAG, "[startObexServerSession] Map Service START ObexServerSessions()");
    ...
    private void stopObexServerSessions(int masId) {
        if (DEBUG) Log.d(TAG, "MAP Service STOP ObexServerSessions() " + masId);
        ...
        if (getRemoteDevice().equals(device)) {
            switch (mState) {
                case BluetoothMap.STATE_CONNECTED:
                    ...
                    mSessionStatusHandler.removeCallbacksAndMessages(null);
                    closeService();
                    Log.d(TAG, "[disconnectMap] mas size = " + mMasInstances.size());
