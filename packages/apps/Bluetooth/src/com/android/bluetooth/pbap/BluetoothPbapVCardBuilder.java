...
public class BluetoothPbapVCardBuilder extends VCardBuilder {
    ...
    public BluetoothPbapVCardBuilder appendNameProperties(final List<ContentValues> contentValuesList) {
        ...
        final String suffix = contentValues.getAsString(StructuredName.SUFFIX);
        final String displayName = contentValues.getAsString(StructuredName.DISPLAY_NAME);
        Integer nameStyle = contentValues.getAsInteger(StructuredName.FULL_NAME_STYLE);
        int fullNameStyle = FullNameStyle.UNDEFINED;
        if (nameStyle != null) {
            fullNameStyle = nameStyle;
        }
        ...
        if (!TextUtils.isEmpty(familyName) || !TextUtils.isEmpty(givenName)) {
            ...
            if (mUsePbapNameImprove && !TextUtils.isEmpty(displayName)) {
                ...
                boolean hasMiddleName = !TextUtils.isEmpty(middleName);
                ...
                reGivenName = sb.toString();

                final String encodedreGivenName;
                if (!mRefrainsQPToNameProperties &&
                            !(VCardUtils.containsOnlyNonCrLfPrintableAscii(reGivenName))) {
                    encodedreGivenName = encodeQuotedPrintable(reGivenName);
                } else {
                    encodedreGivenName = escapeCharacters(reGivenName);
                }
                mBuilder.append(VCARD_DATA_SEPARATOR);
                mBuilder.append(encodedFamily);
                mBuilder.append(VCARD_ITEM_SEPARATOR);
                mBuilder.append(encodedreGivenName);
                mBuilder.append(VCARD_ITEM_SEPARATOR);
                mBuilder.append(VCARD_ITEM_SEPARATOR);
