...
public class GattService extends ProfileService {
    ...
    private Boolean mStartContinueSearch = Boolean.valueOf(false);

    private ServiceDeclaration addDeclaration(int serverIf) {
        synchronized (mServiceDeclarations) {
            mServiceDeclarations.add(new ServiceDeclaration(serverIf));
            Log.d(TAG, "addDeclaration: mServiceDeclarations=" + mServiceDeclarations.toString());
        }
    ...
    private void removePendingDeclaration() {
        synchronized (mServiceDeclarations) {
            Log.d(TAG, "mServiceDeclarations.size()=" + mServiceDeclarations.size());
            if (mServiceDeclarations.size() > 0) {
    ...
    private ServiceDeclaration getLastestDeclaration(int serverIf) {
        ServiceDeclaration declaration = null;
        synchronized (mServiceDeclarations) {
            Log.d(TAG, "getLastestDeclaration: mServiceDeclarations.size()=" + mServiceDeclarations.size());
            for (int i = mServiceDeclarations.size() - 1; i >= 0 ; i--) {
                if (mServiceDeclarations.get(i).mServerIf == serverIf) {
                    declaration = mServiceDeclarations.get(i);
                    Log.d(TAG, "getLastestDeclaration: found=" + declaration);
                    break;
                }
            }
        }
    ...
    private boolean testContinueServiceDeclarationStartAndSet() {
        boolean bStart = false;
        synchronized (mStartContinueSearch) {
            bStart = mStartContinueSearch;
            mStartContinueSearch = true;
        }
        return bStart;
    }

    private void resetContinueServiceDeclarationStart() {
        synchronized (mStartContinueSearch) {
            mStartContinueSearch = false;
            Log.d(TAG, "resetContinueServiceDeclarationStart");
        }
    }
    ...
    private static class BluetoothGattBinder extends IBluetoothGatt.Stub implements IProfileServiceBinder {
        ...
        public void endServiceDeclaration(int serverIf) {
            Log.v(TAG, "endServiceDeclaration: serverIf =" + serverIf);
            GattService service = getService();
            if (service == null) {
                Log.v(TAG, "endServiceDeclaration: service == null, return directly!");
                return;
            }
    ...
    void onSearchResult(int connId, int srvcType,
        ...
        if (VDBG) Log.d(TAG, "onSearchResult() - address=" + address + ", uuid=" + uuid + ", connId=" + connId);
    ...
    void onGetCharacteristic(int connId, int status, int srvcType,
        ...
        if (VDBG) Log.d(TAG, "onGetCharacteristic() - address=" + address
            + ", status=" + status + ", charUuid=" + charUuid + ", prop=" + charProp + ", connId=" + connId);
    ...
    void onGetDescriptor(int connId, int status, int srvcType,
        ...
        if (VDBG) Log.d(TAG, "onGetDescriptor() - address=" + address
            + ", status=" + status + ", descUuid=" + descUuid + ", connId=" + connId);
    ...
    void onGetIncludedService(int connId, int status, int srvcType,
        ...
        if (VDBG) Log.d(TAG, "onGetIncludedService() - address=" + address
            ...
            + ", inclUuid=" + inclSrvcUuid + ", connId=" + connId);
    ...
    List<BluetoothDevice> getDevicesMatchingConnectionStates(int[] states) {
        ...
        Log.i(TAG, "getDevicesMatchingConnectionStates: mClientMap");
        connectedDevices.addAll(mClientMap.getConnectedDevices());
        Log.i(TAG, "getDevicesMatchingConnectionStates: mServerMap");
        ...
        for (Map.Entry<BluetoothDevice, Integer> entry : deviceStates.entrySet()) {
            for(int state : states) {
                Log.i(TAG, "getDevicesMatchingConnectionStates: address=" + entry.getKey() + ",state=" + entry.getValue());
    ...
    void onServiceAdded(int status, int serverIf, int srvcType, int srvcInstId,
        ...
        if (DBG) Log.d(TAG, "onServiceAdded() UUID=" + uuid + ", status=" + status
            + ", handle=" + srvcHandle + ",serverIf=" + serverIf);
        if (status == 0) {
    ...
    void beginServiceDeclaration(int serverIf, int srvcType, int srvcInstanceId,
        ...
        ServiceDeclaration serviceDeclaration = addDeclaration(serverIf);
    ...
    void addIncludedService(int serverIf, int srvcType, int srvcInstanceId,
        ...
        if (serverIf != getActiveDeclaration().mServerIf) {
            if (DBG) Log.d(TAG, "addIncludedService() - getLastestDeclaration");
            /// it must find a record, otherwise it's a bug
            getLastestDeclaration(serverIf).addIncludedService(srvcUuid, srvcType, srvcInstanceId);
        } else {
    ...
    void addCharacteristic(int serverIf, UUID charUuid, int properties,
        ...
        if (serverIf != getActiveDeclaration().mServerIf) {
            if (DBG) Log.d(TAG, "addCharacteristic() - getLastestDeclaration");
            /// it must find a record, otherwise it's a bug
            getLastestDeclaration(serverIf).addCharacteristic(charUuid, properties, permissions);
        } else {
    ...
    void addDescriptor(int serverIf, UUID descUuid, int permissions) {
        ...
        if (serverIf != getActiveDeclaration().mServerIf) {
            if (DBG) Log.d(TAG, "addDescriptor() - getLastestDeclaration");
            /// it must find a record, otherwise it's a bug
            getLastestDeclaration(serverIf).addDescriptor(descUuid, permissions);
        } else {
    ...
    void endServiceDeclaration(int serverIf) {
        ...
        if (!testContinueServiceDeclarationStartAndSet()) {
            try {
                if (null != getPendingDeclaration() && serverIf != getPendingDeclaration().mServerIf) {
                    serverIf = getPendingDeclaration().mServerIf;
                    Log.d(TAG, "endServiceDeclaration(): serverIf != getPendingDeclaration().mServerIf");
                }
                Log.v(TAG, "endServiceDeclaration: begin a record serverIf=" + serverIf);
                continueServiceDeclaration(serverIf, (byte)0, 0);
            } catch (RemoteException e) {
                resetContinueServiceDeclarationStart();
                Log.e(TAG,""+e);
            }
        }
    ...
    private void continueSearch(int connId, int status) throws RemoteException {
        if (DBG) Log.d(TAG, "continueSearch() - connId=" + connId);
        if (status == 0 && !mSearchQueue.isEmpty(connId)) {
            /** M: ALPS01881507 - discoverService Concurrency Issue */ 
            SearchQueue.Entry svc = mSearchQueue.popByConnId(connId);
            ...
            ClientMap.App app = mClientMap.getByConnId(connId);
            if (DBG) Log.d(TAG, "continueSearch() - onSearchComplete app=" + app);
            if (app != null) {
    ...
    private void continueServiceDeclaration(int serverIf, int status, int srvcHandle) throws RemoteException {
        if (mServiceDeclarations.size() == 0) {
            if (DBG) Log.d(TAG, "continueServiceDeclaration() - mServiceDeclarations.size() == 0, return directly");
            return;
        }
        if (DBG) Log.d(TAG, "continueServiceDeclaration() - srvcHandle=" + srvcHandle + ",serverIf=" + serverIf);
        ...
        if (entry != null) {
            ...
            switch(entry.type) {
                case ServiceDeclaration.TYPE_SERVICE:
                    if (DBG) Log.d(TAG, "continueServiceDeclaration() - gattServerAddServiceNative: serverIf=" + serverIf +
                                        ",uuid=" + entry.uuid + ",srvcHandle=" + srvcHandle);
                ...
                case ServiceDeclaration.TYPE_CHARACTERISTIC:
                    if (DBG) Log.d(TAG, "continueServiceDeclaration() - gattServerAddCharacteristicNative: serverIf=" + serverIf +
                                        ",uuid=" + entry.uuid + ",srvcHandle=" + srvcHandle);
                ...
                case ServiceDeclaration.TYPE_DESCRIPTOR:
                    if (DBG) Log.d(TAG, "continueServiceDeclaration() - gattServerAddDescriptorNative: serverIf=" + serverIf +
                                        ",uuid=" + entry.uuid + ",srvcHandle=" + srvcHandle);
                ...
                case ServiceDeclaration.TYPE_INCLUDED_SERVICE:
                {
                    ...
                    if (inclSrvc != 0) {
                        if (DBG) Log.d(TAG, "continueServiceDeclaration() - gattServerAddIncludedServiceNative: serverIf=" + serverIf +
                                            ",uuid=" + entry.uuid + ",srvcHandle=" + srvcHandle);
            ...
            if (DBG) Log.d(TAG, "continueServiceDeclaration() - gattServerStartServiceNative serverIf=" + serverIf + ",srvcHandle=" + srvcHandle);
            gattServerStartServiceNative(serverIf, srvcHandle,
                (byte)BluetoothDevice.TRANSPORT_BREDR | BluetoothDevice.TRANSPORT_LE);
            finished = true;
            Log.v(TAG, "continueServiceDeclaration: end a record serverIf=" + serverIf);
            dump();
        }
        ...
        if (finished) {
            ...
            if (getPendingDeclaration() != null) {
                if (DBG) Log.d(TAG, "continueServiceDeclaration() - getPendingDeclaration()=" + getPendingDeclaration() +
                                    ",mServerIf=" + getPendingDeclaration().mServerIf);
                Log.v(TAG, "continueServiceDeclaration: begin a record serverIf=" + getPendingDeclaration().mServerIf);
                continueServiceDeclaration(getPendingDeclaration().mServerIf, (byte) 0, 0);
            } else {
                resetContinueServiceDeclarationStart();
                Log.d(TAG, "continueServiceDeclaration() - getPendingDeclaration()== null, no more declaration!");
                dump();
            }
        }
    }
    ...
    private void deleteServices(int serverIf) {
        ...
        List<Integer> handleList = null;
        boolean bConcurrentModificationException = false;
        do {
            bConcurrentModificationException = false;
            try {
                /*
                 * Figure out which handles to delete.
                 * The handles are copied into a new list to avoid race conditions.
                 */
                handleList = new ArrayList<Integer>();
                List<HandleMap.Entry> entries = mHandleMap.getEntries();
                for (HandleMap.Entry entry : entries) {
                    if (entry.type != HandleMap.TYPE_SERVICE ||
                        entry.serverIf != serverIf)
                ...
                Log.e(TAG, "deleteServices: retry it!" + e);
                // wait a moment to avoid the race condition
                SystemClock.sleep(100);
                bConcurrentModificationException = true;
            }
        } while(bConcurrentModificationException) ;

        /* Now actually delete the services.... */
        for(Integer handle : handleList) {
            gattServerDeleteServiceNative(serverIf, handle);
            if (DBG) Log.d(TAG, "deleteServices() - serverIf=" + serverIf + ", handle=" + handle);
        }
    }
