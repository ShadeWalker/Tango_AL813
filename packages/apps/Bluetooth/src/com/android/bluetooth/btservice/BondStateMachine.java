...
final class BondStateMachine extends StateMachine {
    ...
    private class PendingCommandState extends State {
        ...
        public boolean processMessage(Message msg) {
            ...
            infoLog("processMessage: msg = " + msg.what + ", mDevices.contains(dev) = " + mDevices.contains(dev));
    ...
    private void sendDisplayPinIntent(byte[] address, int pin, int variant) {
        infoLog("sendDisplayPinIntent: device = " + mRemoteDevices.getDevice(address)
                + ", pin = " + pin + ", variant = " + variant);
        Intent intent = new Intent(BluetoothDevice.ACTION_PAIRING_REQUEST);
        ...
        infoLog("sendDisplayPinIntent: Pairing Request intent is sent");
    }
