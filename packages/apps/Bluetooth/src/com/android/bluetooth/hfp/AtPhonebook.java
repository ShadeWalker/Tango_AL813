...
public class AtPhonebook {
    ...
    private int getDefaultSim() {
        ...
        if (subId != SubscriptionManager.INVALID_SUBSCRIPTION_ID &&
            ...
            simSlotId = SubscriptionManager.getSlotId(subId);
        } else {
            simSlotId = SubscriptionManager.getSlotId(PhoneFactory.getDefaultPhone().getSubId());
        }

        log("getDefaultSim:" + simSlotId);
    ...
    private int simStatusToCme() {
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            status = TelephonyManager.getDefault().getSimState(getDefaultSim());
        }
