...
final class HeadsetStateMachine extends StateMachine {
    ...
    /// M: DSDA : to judge if it is first time HFP connected
    IState mPrevState;
    ...
    private class Connected extends State {
        ...
        public void enter() {
            ...
            /// M: DSDA : inform BluetoothPhoneService to do call restriction
            /// for first HFP connected @{
            if (mPrevState != mAudioOn && mPrevState != mMultiHFPending) {
                if (mPhoneProxy != null) {
                    try {
                        mPhoneProxy.restrictDualTalk();
                    } catch (RemoteException e) {
                        Log.e(TAG, Log.getStackTraceString(new Throwable()));
                    }
                } else {
                    Log.e(TAG, "Handsfree phone proxy null for restrict DSDA");
                }
            }
            /// @}
            ...
            if (mPhoneProxy != null) {
                try {
                    ...
                    if (isVirtualCallInProgress()) {
                        log("update virtual call again after SLC connected to" +
                            " trigger SCO connection");
    ...
    boolean isAudioConnected(BluetoothDevice device) {
        synchronized(this) {
            ...
            /// M: It may has transitioin delay when state transition from mConnected
            ///    to mAudioOn. Add OR case to eliminate this mismatch problem @{
            if ((getCurrentState() == mAudioOn && device.equals(mCurrentDevice)
                  && mAudioState != BluetoothHeadset.STATE_AUDIO_DISCONNECTED)
               || (getCurrentState() == mConnected && device.equals(mCurrentDevice)
                  && mAudioState == BluetoothHeadset.STATE_AUDIO_CONNECTED))
            {
                if (DBG && getCurrentState() == mConnected) {
                    Log.d(TAG, "isAudioConnected() returned true when state is in mConnected");
                }
                return true;
            }
            /// @}
    ...
    private void processDialCall(String number, BluetoothDevice device) {
        ...
        if ((number == null) || (number.length() == 0)) {
            ...
            if (DBG) {
                log("processDialCall, memory dial is supported by MTK");
            }
            ...
            if (dialNumber == null) {
                if (DBG) {
                    log("processDialCall, memory dial number null");
                }
                atResponseCodeNative(HeadsetHalConstants.AT_RESPONSE_ERROR, 0,
                                       getByteAddress(device));
                return;
        ...
        log("broadcast for outgoing call: " + dialNumber);
