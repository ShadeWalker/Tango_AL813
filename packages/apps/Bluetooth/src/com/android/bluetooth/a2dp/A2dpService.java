...
public class A2dpService extends ProfileService {
    ...
    int getConnectionState(BluetoothDevice device) {
        ...
        /// M: param check. {@
        if (mStateMachine == null) {
            Log.e(TAG, "[getConnectionState] mStateMachine is NULL, return STATE_DISCONNECTED");
    ...
    /**
    * If HFP is connected, GAP will try to connect A2dp Source.
    * If the A2dp Sink is connected, cannot execute the source connect action
    * If A2dp Sink is not support, return true;
    * If A2dp SInk is CONNECTED or CONNECTING state, return false
    * Otherwise return true.
    */
    public boolean isAbleToConnectWhileHfpConnected(BluetoothDevice device) {
/*        if (device == null) {
            Log.e(TAG, "[isAbleToConnectWhileHfpConnected] device is NULL");
            return false;
        }*/
        A2dpSinkService sinkService = A2dpSinkService.getA2dpSinkService();
        if (sinkService == null) {
            Log.e(TAG, "[isAbleToConnectWhileHfpConnected] sinkService is NULL");
            return true;
        }
        int[] states = {BluetoothProfile.STATE_CONNECTED, BluetoothProfile.STATE_CONNECTING};
        List<BluetoothDevice> lists = sinkService.getDevicesMatchingConnectionStates(states);
        Log.d(TAG, "[isAbleToConnectWhileHfpConnected] lists size : " + lists.size());
        if (lists.size() != 0) {
            Log.d(TAG, "[isAbleToConnectWhileHfpConnected] sink has device connected state, return false");
            return false;
        }
        Log.d(TAG, "[isAbleToConnectWhileHfpConnected] return true");
        return true;
    }
