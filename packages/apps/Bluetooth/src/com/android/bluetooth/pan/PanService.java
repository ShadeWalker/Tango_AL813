    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mLocalPanuConnected && isLocalPanu) {
                ...
                if (cm == null) {
                    Log.d(TAG, "ConnectivityManager cm is null");
                    return;
                }
                NetworkInfo netInfo = cm.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
                if (netInfo == null) {
                    Log.d(TAG, "[onReceive] network infor is null");
                    return;
                }
...
/*        ConnectivityManager cm =
            (ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE);
        if (cm == null) {
            Log.d(TAG, "[connect] ConnectivityManager is null");
            return false;
        }
        NetworkInfo netInfo = cm.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
        if (netInfo == null) {
            Log.d(TAG, "[connect] network info is null");
            return false;
        }
        State wifiState = netInfo.getState();
        if (wifiState != null) {
    ...
    */
    ...
    private String enableTethering(String iface) {
        ...
        ConnectivityManager cm =
            (ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE);
        if (cm == null) {
            Log.e(TAG, "[enableTethering] ConnectivityManager is null");
            return null;
        }
