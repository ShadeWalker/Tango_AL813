...
static jboolean confirmRequestNative(JNIEnv *env, jobject object, jbyteArray address, jint acceptOrReject) {
    ...
    ALOGE("Source acceptOrReject : %d", acceptOrReject);
    ...
    if ((status = sBluetoothA2dpInterface->response_rmt_conn(((bt_bdaddr_t *)addr), acceptOrReject)) != BT_STATUS_SUCCESS) {
        ALOGE("Source response_rmt_conn failed, status: %d", status);
    }
