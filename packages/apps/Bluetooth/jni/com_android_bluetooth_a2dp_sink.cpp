...
#if defined(MTK_BT_PROFILE_A2DP_SINK)
static jboolean setAudioStateNative(JNIEnv *env, jobject object,jbyteArray address, jint state) {
    ...
    ALOGI("%s: sBluetoothAvrcpInterface: %p", __FUNCTION__, sBluetoothA2dpInterface);
    ALOGI("state: %d", state);
    ...
    if ((status = sBluetoothA2dpInterface->set_audio_state((bt_bdaddr_t *)addr,(btav_audio_state_t)state) ) != BT_STATUS_SUCCESS) {
        ALOGE("Failed set audio state: %d", status);
    }
...
static jboolean confirmRequestNative(JNIEnv *env, jobject object, jbyteArray address, jint acceptOrReject) {
    jbyte *addr;
    bt_status_t status;
    ...
    ALOGE("Sink acceptOrReject : %d", acceptOrReject);
    ...
    if ((status = sBluetoothA2dpInterface->response_rmt_conn(((bt_bdaddr_t *)addr), accept)) != BT_STATUS_SUCCESS) {
        ALOGE("Sink response_rmt_conn failed, status: %d", status);
    }
...
#if defined(MTK_BT_PROFILE_A2DP_SINK)
    {"confirmRequestNative", "([BI)Z", (void *) confirmRequestNative},
    {"setAudioStateNative", "([BI)Z", (void *) setAudioStateNative},
#endif
};
