...
public class MailPhotoViewActivity extends PhotoViewActivity implements
    ...
    static final String EXTRA_IS_VIEW_ONLY = "is_view_only";
    /// @}
    ...
    /**
     * M: Start a new MailPhotoViewActivity to view the given images.
     *
     * @param photoIndex
     *            The index of the photo to show first.
     * @param isViewOnlyMode
     */
    public static void startMailPhotoViewActivity(final Context context,
            final String account, final String accountType,
            final ConversationMessage msg, final int photoIndex,
            final boolean isViewOnlyMode) {
        final Intents.PhotoViewIntentBuilder builder = Intents
                .newPhotoViewIntentBuilder(context,
        ...
        Intent i = wrapIntent(builder.build(), account, accountType, msg);
        i.putExtra(EXTRA_IS_VIEW_ONLY, isViewOnlyMode);
    ...
    private static Intent wrapIntent(
        ...
        /**
         * M: display a different photo while there is an existing activity
         * instance for that list of photos. Since the initial photo is
         * specified as an extra, without FLAG_ACTIVITY_CLEAR_TOP, the activity
         * instance would just get restarted and it would display whatever photo
         * it was last displaying. FLAG_ACTIVITY_CLEAR_TOP causes a new instance
         * to be created, and it will display the new initial photo. @{
         */
