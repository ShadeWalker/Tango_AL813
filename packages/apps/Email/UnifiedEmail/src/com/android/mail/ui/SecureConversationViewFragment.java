...
public class SecureConversationViewFragment extends AbstractConversationViewFragment
    ...
    private class SecureConversationWebViewClient extends AbstractConversationWebViewClient {
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            /**
             * M: It should re-judge if the fragment had been added to the
             * activity again, because the fragment may be removed from activity
             * in some case. For example, view the mail first, but before the
             * webview's callback came back, this mail was deleted, and then
             * after the callback came back, the operations in this function
             * would remove the fragment from activity, so the JE would occur.
             *
             * @{
             */
            if (!isAdded()) {
                LogUtils.d(LOG_TAG, "ignoring SCVF.onPageFinished, url=%s fragment=%s, conversation=%s", url,
                        SecureConversationViewFragment.this, getConversation());
                return;
            }
            /** @}*/
    ...
    public void setupConversationHeaderView(ConversationViewHeader headerView) {
        ...
        /// M: It should call setSubject first, otherwise it would result in NPE.
        ...
        /// M: Update conversation for header view
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (null != mViewController) {
            LogUtils.v(LogTag.getLogTag(), "SecureConvViewFrag dispatchKeyEvent event: %s", event);
            return mViewController.dispatchKeyEvent(event);
        }
        return false;
    }

   /// M: call the SecureConversationViewController's onDestroyView method.
    @Override
