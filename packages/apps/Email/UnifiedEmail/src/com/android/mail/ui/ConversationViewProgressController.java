...
public class ConversationViewProgressController {
    ...
    ///M: add log for progress's abnormal display
    private static String TAG = "CVPC";
    ...
    public ConversationViewProgressController(Fragment fragment, Handler handler) {
        ...
        mDelayedShow = new FragmentRunnable("mDelayedShow", mFragment) {
            ...
            public void go() {
                ...
                ///M: add log for progress's abnormal display
                LogUtils.d(TAG, "FragmentRunnable Set progress %s visible", mProgressView.hashCode());
            }
        };
    }
    ...
    public void showLoadingStatus(boolean isFragmentUserVisible) {
        ...
        ///M: consume touch event, rather than deliver it to Webview
        ...
        ///M: add log for progress's abnormal display
        LogUtils.d(TAG, "showLoadingStatus bg_view set %s visible ", mBackgroundView.hashCode());
    ...
    private void dismiss(final Runnable doAfter) {
        ...
        ///M: add log for progress's abnormal display
        LogUtils.d(TAG, "dismiss set progress %s invisible", mProgressView.hashCode());
        if (mBackgroundView.getVisibility() == View.VISIBLE) {
            animateDismiss(doAfter);
            ///M: unregister the touch listener
            mBackgroundView.setOnTouchListener(null);
        } else {
            if (doAfter != null) {
                doAfter.run();
            }
    ...
    private void animateDismiss(final Runnable doAfter) {
        ...
        if (!mFragment.isAdded()) {
            ...
            ///M: add log for progress's abnormal display
            LogUtils.d(TAG, "animateDismiss set bg_view %s invisible", mBackgroundView.hashCode());
            return;
        }
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                mBackgroundView.setVisibility(View.GONE);
                ///M: add log for progress's abnormal display
                LogUtils.d(TAG, "onAnimationEnd set bg_view %s invisible",
                        mBackgroundView.hashCode());
