...
public class MimeType {
    ...
    /**
     * M: If the default content type could not be viewed,
     * we try the content type inferred by MediaFile again.
     */
    public static boolean isViewable(Context context, Uri contentUri,
            String contentType, String attName) {
        if (isViewable(context, contentUri, contentType)) {
            return true;
        } else {
            // try other type again.
            return isViewable(context, contentUri,
                    getAttachmentExtensionMimeType(attName));
        }
    }
    ...
    /**
     * M: normalize MimeType by lower case and remove quotes just in case
     * @param type MimeType of attachment
    ...
    public static String normalize(final String type) {
        ...
        if (!TextUtils.isEmpty(mimeType)) {
            ...
            if (mimeType.startsWith("\"") && mimeType.endsWith("\"")) {
                return mimeType.substring(1, mimeType.length() - 1);
            } else {
                return mimeType;
            }
        }
