...
public class MessageInviteView extends LinearLayout implements View.OnClickListener {
    ...
    /// M: Define meeting request mime type
    private static final String INVITATION_MIME_TYPE = "email/invitation";
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.invite_calendar_view) {
            if (!Utils.isEmpty(mMessage.eventIntentUri)) {
                ...
                /// M: Set the mime type, thus only Email app can handle the
                 // calendar invitation viewing request
                intent.setDataAndType(mMessage.eventIntentUri, INVITATION_MIME_TYPE);
    ...
    private class InviteCommandHandler extends AsyncQueryHandler {
        ...
        public void sendCommand(ContentValues params) {
            ...
            if (mMessage.uri == null) {
                LogUtils.w(LogUtils.TAG, "InviteCommandHandler sendCommand" +
                        " but mMessage.uri is null !!!");
                return;
            }
            /** @} */
