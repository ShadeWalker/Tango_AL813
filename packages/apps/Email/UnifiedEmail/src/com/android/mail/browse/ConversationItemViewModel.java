...
public class ConversationItemViewModel {
    ...
    /**
     * M: Get first displayable sender email.
     * @param account
     * @return
     */
    public String getFirstDisplayableSenderEmail(Account account) {
        final ArrayList<ParticipantInfo> partInfos = conversation.conversationInfo.participantInfos;
        String senderEmail = "";
        if (partInfos.size() > 0) {
            final ParticipantInfo currentParticipant = partInfos.get(0);
            final String currentEmail = currentParticipant.email;
            final String currentName = currentParticipant.name;
            senderEmail = TextUtils.isEmpty(currentName) ? account.getEmailAddress() :
