...
public class MimeMessage extends Message {
    ...
    public Address[] getRecipients(RecipientType type) throws MessagingException {
        if (type == RecipientType.TO) {
            if (mTo == null) {
                /// M: force detecting charset and decoding strings.
                mTo = Address.parse(MimeUtility.unfold(getFirstHeader("To")), true);
            }
            return mTo;
        } else if (type == RecipientType.CC) {
            if (mCc == null) {
                /// M: force detecting charset and decoding strings.
                mCc = Address.parse(MimeUtility.unfold(getFirstHeader("CC")), true);
            }
            return mCc;
        } else if (type == RecipientType.BCC) {
            if (mBcc == null) {
                /// M: force detecting charset and decoding strings.
                mBcc = Address.parse(MimeUtility.unfold(getFirstHeader("BCC")), true);
            }
            return mBcc;
        } else {
    ...
    public Address[] getReplyTo() throws MessagingException {
        if (mReplyTo == null) {
            /// M: force detecting charset and decoding strings.
            mReplyTo = Address.parse(MimeUtility.unfold(getFirstHeader("Reply-to")), true);
        }
        return mReplyTo;
    }
