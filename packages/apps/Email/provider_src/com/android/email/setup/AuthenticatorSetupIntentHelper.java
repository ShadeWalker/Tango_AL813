...
public class AuthenticatorSetupIntentHelper {
    ...
    /**
     * M: Direct to {@link AccountSetupChooseMailProvider} if plug-in(VendorEmail/OP Plugin) is supported.
     * @param context
     * @return
     */
    public static Intent actionNewAccountIntent(final Context context) {
        final Intent i = new Intent();
        if (EmailPluginUtils.isSupportProviderList(context)) {
            i.setComponent(new ComponentName(context,
                    "com.mediatek.email.extension.AccountSetupChooseMailProvider"));
        } else {
            i.setComponent(new ComponentName(context,
