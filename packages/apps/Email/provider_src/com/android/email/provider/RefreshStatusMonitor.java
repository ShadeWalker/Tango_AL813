...
public class RefreshStatusMonitor {
    ...
    /// M: Because the sync is parallel for accounts of same variety.
     // Prolong the timeout threshold in case of possible bad network occurs.
    public static final long REMOVE_REFRESH_TIMEOUT_MS = 3 * DateUtils.MINUTE_IN_MILLIS;
    ...
    private RefreshStatusMonitor(Context context) {
        ...
        StorageLowState.registerHandler(new StorageLowState
                ...
                @Override
                public void onStorageLowAfterCheck(int checkReason) {
                }
            });
    }
        ...
        public void run() {
            synchronized (mMailboxSync) {
                ...
                if (Boolean.FALSE.equals(isSyncRunning)) {
                    ...
                    /// M: Disable this line, It look useless but make the refresh status incorrect.
                    //mCallback.onRefreshCompleted(
