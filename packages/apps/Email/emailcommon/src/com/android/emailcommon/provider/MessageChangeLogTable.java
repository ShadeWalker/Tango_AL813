...
public abstract class MessageChangeLogTable {
    ...
    /** M: Selection string prefix for deleting range with last id */
    private static final String SELECTION_BY_LAST_ID_PREFIX = " and " + ID + "<=";

    /**
     * M: Operations on table MessageMove should be done only if their id less than the lastId
     * Create a selection string for all messages in a set.
     * @param messageKeys The set of messages we're interested in.
     * @param count The number of messages we're interested in.
    ...
    private static String getSelectionForMessages(final long[] messageKeys, final int count,
            final long lastId) {
        final StringBuilder sb = new StringBuilder(getSelectionForMessages(messageKeys, count));
        sb.append(SELECTION_BY_LAST_ID_PREFIX).append(String.valueOf(lastId));
        return sb.toString();
    }
     ...
     */
    protected static int deleteRowsForMessages(final ContentResolver cr, final Uri uri,
            final long[] messageKeys, final int count, final long lastId) {
        if (count == 0) {
            return 0;
        }
        final String selection = getSelectionForMessages(messageKeys, count, lastId);
     ...
     */
    private static int updateStatusForMessages(final ContentResolver cr, final Uri uri,
            final long[] messageKeys, final int count, final int status, final long lastId) {
        if (count == 0) {
            return 0;
        }
        final ContentValues cv = new ContentValues(1);
        cv.put(STATUS, status);
        final String selection = getSelectionForMessages(messageKeys, count, lastId);
        return cr.update(uri, cv, selection, null);
    }

    /**
     * M: Set a set of messages to status = retry.
     * @param cr A {@link ContentResolver}.
     * @param uri The {@link Uri} for the update.
     * @param messageKeys The messages to update.
     * @param count The number of messageKeys.
     ...
     */
    protected static int retryMessages(final ContentResolver cr, final Uri uri,
            final long[] messageKeys, final int count, long lastId) {
        return updateStatusForMessages(cr, uri, messageKeys, count, STATUS_NONE, lastId);
    }

    /**
     * M: Set a set of messages to status = failed.
     * @param cr A {@link ContentResolver}.
     * @param uri The {@link Uri} for the update.
     * @param messageKeys The messages to update.
     * @param count The number of messageKeys.
    ...
    protected static int failMessages(final ContentResolver cr, final Uri uri,
            final long[] messageKeys, final int count, long lastId) {
        return updateStatusForMessages(cr, uri, messageKeys, count, STATUS_FAILED, lastId);
    }
}
