...
public class AccountSetupFragment extends Fragment implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (viewId == R.id.next) {
            ...
            /** M: Avoid states disorder, block double-presses here @{ */
            blockNextButton();
            /** @} */
    ...
    /**
     * M: Disable "next" button for a period of time (1s), this is a walk
    ...
    public void blockNextButton() {
        ...
        if (enabledBefore) {
            new UiHandler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    // set next button enabled after attached
    ...
    /** M: hide the soft keyboard in case of blocking when back from CREDENTIALS
     * during manual setup @{ */
    @Override
    public void onPause() {
