...
class P2pLinkManager implements Handler.Callback, P2pEventListener.Callback {
    ...
    /// M: @ {
    //get package name of topActivity
    ...
    public void setNdefCallback(IAppCallback callbackNdef, int callingUid) {
        synchronized (this) {
            ...
            //debug usage
            try {
            Log.d(TAG, "setNdefCallback() callingUid:"+callingUid);
            mValidCallbackPackages = mPackageManager.getPackagesForUid(callingUid);
            } catch (Exception e) {
                Log.e(TAG, "PackageManager.getPackagesForUid Exception:"+e);
                e.printStackTrace();
            }            
            /// }
    ...
    public void onManualBeamInvoke(BeamShareData shareData) {
        ...
        Log.i(TAG, "onManualBeamInvoke() shareData:"+shareData);
        synchronized (P2pLinkManager.this)    {
            ...
            if (mMessageToSend == null && mUrisToSend == null && shareData != null) {
                ...
                Log.d(TAG, "mMessageToSend == null && mUrisToSend == null && shareData != null , shareData.userHandle:"+shareData.userHandle);
    ...
    void prepareMessageToSend(boolean generatePlayLink) {
        synchronized (P2pLinkManager.this) {
            ...
            /// M: @ {
            //debug usage
            Log.d(TAG, "prepareMessageToSend() generatePlayLink:" + generatePlayLink);

            if(mValidCallbackPackages != null){

                try{    
                    Log.d(TAG, "mValidCallbackPackages.size():" + mValidCallbackPackages.length);
                    for (String packagesString : mValidCallbackPackages) {
                        Log.d(TAG, "Valid packagesString " + packagesString);
                    }
                } catch (Exception e) {
                    Log.e(TAG, " mValidCallbackPackages Exception:"+e);
                    e.printStackTrace();
                }                
            }
            /// }
            ...
            try {
                ...
                Log.d(TAG, "ActivityManager.getRunningTasks  tasks.size():"+tasks.size());
                if (tasks.size() > 0) {
                    runningPackage = tasks.get(0).topActivity.getPackageName();
                    Log.d(TAG, "topActivity.getPackageName():"+runningPackage);
                    Log.d(TAG, "topActivity.toSring():"+tasks.get(0).topActivity.toString());
                    Log.d(TAG, "topActivity.getClassName():"+tasks.get(0).topActivity.getClassName());
                    if(runningPackage.equalsIgnoreCase("com.android.documentsui")){
                        Log.d(TAG, " transform to package name,  runningPackage = com.android.providers.downloads.ui");
                        runningPackage = "com.android.providers.downloads.ui";
                    }
                }
            } catch (Exception e) {
                Log.d(TAG, "topActivity.getPackageName()  Exception:"+e);
                e.printStackTrace();
            }
            
            Log.d(TAG, "mCallbackNdef:"+mCallbackNdef+"  mNdefCallbackUid:" + mNdefCallbackUid);                        
            /// }

            List<Integer> foregroundUids = mForegroundUtils.getForegroundUids();
            if (foregroundUids.isEmpty()) {
                /// M: @ {
                if(allowCertHandover(tasks))
                    return;
                /// }
                ...
                if (foregroundUids.contains(mNdefCallbackUid)) {
                    try {
                        ...
                        mSendFlags = shareData.flags;
                         Log.d(TAG, "createBeamShareData mMessageToSend = " + mMessageToSend);
                         Log.d(TAG, "createBeamShareData mUrisToSend = " + mUrisToSend);                        
                        return;
                    } catch (Exception e) {
            ...
            Log.e(TAG, "!!!! go to Default  createDefaultNdef() !!!!");
            ...
            if (pkgs != null && pkgs.length >= 1) {
                if (!generatePlayLink || beamDefaultDisabled(pkgs[0])
                    ...
                    /// M: @ {
                    Log.d(TAG, "   google L,  pkgs[0]:"+pkgs[0]);
                    mMessageToSend = createDefaultNdef(runningPackage); //pkgs[0] to runningPackage
                    /// }
    ...
    final class SendTask extends AsyncTask<Void, Void, Void> {
        ...
        int doHandover(Uri[] uris, UserHandle userHandle) throws IOException {
            ...
            /// M: @ {
            NdefMessage request;
            request = MtkNfcEntry.getInstance().createRequestMessage();
            if(request == null){
            ...
            /// M: @ {
            int mScenario = MtkNfcEntry.getInstance().parseHandoverMessage(response);
            if (mScenario == MtkNfcEntry.SCENARIO_BEAMPLUS_P2P ||
                mScenario == MtkNfcEntry.SCENARIO_WIFI_LEGACY  ||
                mScenario == MtkNfcEntry.SCENARIO_WFA_P2P) {
                //do our funciton, deal selectMessage
                MtkNfcEntry.getInstance().doHandoverUri(mScenario, uris, response);
                return HANDOVER_SUCCESS;
            }
            Log.d(TAG, " beamManager.startBeamSend(..)");
            /// }
    ...
    final HandoverServer.Callback mHandoverCallback = new HandoverServer.Callback() {
        ...
        public void onHandoverBusy() {
            Log.d(TAG, "call P2pLinkManager.this.onHandoverBusy()");
    ...
    final SnepServer.Callback mDefaultSnepCallback = new SnepServer.Callback() {
        ...
        public SnepMessage doGet(int acceptableLength, NdefMessage msg) {
            ...
            int mScenario = MtkNfcEntry.getInstance().parseHandoverMessage(msg); //tryHandoverRequest(NdefMessage m)
            if (mScenario == MtkNfcEntry.SCENARIO_BEAMPLUS_P2P ||
                mScenario == MtkNfcEntry.SCENARIO_WIFI_LEGACY  ||
                mScenario == MtkNfcEntry.SCENARIO_WFA_P2P) {
                //do our funciton,  return selectMessage;
                response = MtkNfcEntry.getInstance().tryHandoverRequest(mScenario, msg);
            }else{
            ...
            case MSG_RECEIVE_HANDOVER:
                ...
                synchronized (this) {
                    ...
                    /// M: @ { WPS/P2P PlugFest Certification
                        MtkNfcEntry.getInstance().notifyHandoverComplete();
    ...
    boolean allowCertHandover(List<RunningTaskInfo> runningTask) {
        boolean result=false;
        try {

            String className = runningTask.get(0).topActivity.getClassName();

            Log.d(TAG, " allowCertHandover() String:" + className);

            String wpsActivity = "NfcForegroundDispatchActivity";
            String p2pActivity = "NfcWfaForegroundDispatchActivity";
            String sigmsActivity = "NfcSigmaForegroundDispatchActivity";

            if(className.endsWith(wpsActivity) || className.endsWith(p2pActivity) || className.endsWith(sigmsActivity)){
                result = true;
                Log.d(TAG, " result = true , allow Handover" + result);
            ...
            //if (callbackValid) {
                ...
                mUrisToSend = shareData.uris;
                mSendFlags = shareData.flags;
                Log.d(TAG, "allowCertHandover(), createBeamShareData mMessageToSend = " + mMessageToSend);
                Log.d(TAG, "allowCertHandover(), createBeamShareData mUrisToSend = " + mUrisToSend);    

            }
            //}

        } catch (Exception e) {
                Log.e(TAG, "allowCertHandover(), Failed NDEF callback: " + e.getMessage());
        }
