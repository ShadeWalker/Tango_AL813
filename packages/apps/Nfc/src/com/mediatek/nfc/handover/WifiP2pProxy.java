...
public class WifiP2pProxy implements IWifiP2pProxy {
    ...
    private class GroupInfoObserver implements GroupInfoListener {
        ...
        public void onGroupInfoAvailable(WifiP2pGroup group) {
            ...
            if (mIsOwner) {
                ...
                Log.e(TAG, "GO deviceAddress : " + group.getOwner().deviceAddress);

                    
                for (WifiP2pDevice device : group.getClientList()) {
                    for (WifiP2pProxyListener listener : mListeners) {
                        listener.onConnected(new TheWifiP2pDevice(device),group.getOwner().deviceAddress);
                    }
                }
                ...
                // Dispatch Event
                Log.e(TAG, "mIsOwner=false onGroupInfoAvailable(), Time is " + System.currentTimeMillis());
                for (WifiP2pProxyListener listener : mListeners) {
                    listener.onConnected(new TheWifiP2pDevice(device),group.getOwner().deviceAddress);
                }
            }
    ...
    private boolean mIsStopWifiScan = false;
    ...
    //reference call timing of broadcastBeamPlusStatus()
    public int stopReconnectAndScan(boolean value) {

        Log.d(TAG, " stopReconnectAndScan  value:" + value + "    mIsStopWifiScan:" + mIsStopWifiScan);

        if (mIsStopWifiScan != value) {
            Log.d(TAG, " call stopReconnectAndScan  value:" + value);
            mWifiManager.stopReconnectAndScan(value, 0);
            mIsStopWifiScan = value;
        } else {
            Log.d(TAG, " multiple calling stopReconnectAndScan, ignore");
        }
    ...
    private void handleConnectionChange(Intent intent) {
        ...
        try {
            ...
            //P2pStatus , sendP2pConnectionChangedBroadcast() in WifiP2pService.java
            ...
            if (info.groupFormed) {
                ...
                Log.d(TAG, "handleConnectionChange(), ===> Group removed,   reason:" + reason);
                for (WifiP2pProxyListener listener : mListeners) {
            ...
            Log.d(TAG, "handleConnectionChange  Exception:"+e);
            e.printStackTrace();
        }
    }
    ...
    private void handleThisDeviceChange(Intent intent) {
        ...
        try {
            ...
            Log.d(TAG, "handleThisDeviceChange  Exception:"+e);
            e.printStackTrace();
        }
    }
