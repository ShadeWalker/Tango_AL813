...
public class FilePushClient implements IClient {
    ...
    private class ClientSession implements Runnable {
        ...
        private boolean sendFile(Uri uri, SocketChannel channel,
            ...
            Log.d(TAG, "fileNameBytes: " + Util.printNdef(fileNameBytes));
            ...
            if (!mIsRunning) {
                sendCancelOrError(job, false);
                FilePushRecord.getInstance().insertWifiOutgoingRecord(Util.getFilePathByContentUri(TAG, mContext, uri), false, sendBytes, fileSize);
                return false;
            } else {
                FilePushRecord.getInstance().insertWifiOutgoingRecord(Util.getFilePathByContentUri(TAG, mContext, uri), true, sendBytes, fileSize);
...
*/
    ...
    private File getFile(Uri uri) {
        try {
            File file = new File(Util.getFilePathByContentUri(TAG, mContext, uri));
