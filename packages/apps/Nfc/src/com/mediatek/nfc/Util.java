...
public class Util {
    ...
    */
    ...
    /**
    *   return null means it's not a file
    *
    */
    public static String getFileStringByUri(String tag, Context context, Uri uri) {
        Log.d(tag, "getFileStringByUri(), uri.toString() = " + uri.toString());
        Log.d(tag, "                      uri.getPath(): " + uri.getPath());
        ...
        if (uri.getScheme().toString().compareTo("content") == 0) {
            ...
            try {
                ...
                for(String print:result){
                    Log.d(tag, " return i:"+i+" cursor.getColumnNames() : " + print);
                    i++;
                }
                ...
                if (cursor.moveToFirst()) {
                    try {
                        column_index = cursor.getColumnIndexOrThrow(MediaStore.Images.Media.DATA); //Instead of "MediaStore.Images.Media.DATA" can be used "_data"
                    } catch (IllegalArgumentException e) {
                        Log.e(tag, "catch IllegalArgumentException:"+e);
                        Log.e(tag, "_data not exist , should not a file, return null");
                        return null;
                    }
                ...
                Log.d(tag, "getFileStringByUri() exception...");
                e.printStackTrace();
            } finally {
                if (cursor != null) {
                    cursor.close();
        ...
        Log.d(tag, "getFileStringByUri() uri.getScheme() is not content,try direct getPath");
