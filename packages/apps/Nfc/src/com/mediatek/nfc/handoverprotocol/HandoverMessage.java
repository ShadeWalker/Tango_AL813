...
public class HandoverMessage {
    ...
    static final String TAG = "HandoverMessage";
    ...
    // Specific record type, Beam+ Handover Request collision
    public static final byte SPECIFIC_RECORD_TYPE_BEAMPLUS_HANDOVER_REQUEST_COLLISION = 0x0;

    // Specific record type, WFA Handover Request collision
    public static final byte SPECIFIC_RECORD_TYPE_WFA_HANDOVER_REQUEST_COLLISION = 0x1;
    ...
    public void appendAlternativeCarrier(byte cps, CarrierData data,
        byte[][] auxiliary) {
        appendAlternativeCarrier(cps, data, auxiliary, (byte[]) null);
    }
    ...
    ac.id = Integer.toString(mCurrentRecordID++).getBytes();
            Log.d(TAG, "appendAlternativeCarrier (hasAcId == null)  ac.id:" + Util.bytesToString(ac.id));
        } else {
            Log.d(TAG, "appendAlternativeCarrier hasAcId:" + Util.bytesToString(hasAcId));
            ac.id = hasAcId; //Integer.toString(hasAcId).getBytes();
        }

    if (auxiliary == null || auxiliary.length == 0) {
             Log.d(TAG, "appendAlternativeCarrier auxiliary == null  :" + auxiliary);
    } else {
     ...
     */
    public void appendAlternativeCarrier(byte cps, CarrierData data) {
    Log.d(TAG, "appendAlternativeCarrier (cps,data)");
    appendAlternativeCarrier(cps, data, (byte[][]) null);
    }
