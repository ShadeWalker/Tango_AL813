...
public class WifiCarrierConfiguration extends CarrierConfigurationRecord {
    ...
    public static class PasswordToken {
        ...
        private void parse(byte[] buffer, int offset, int len) {
            ...
            Log.d(TAG, "pwdToken.parse offset:" + offset);


            System.arraycopy(buffer, offset, publicKeyHash, 0, 20);
            passwordID = buffer[offset + 20] & 0xFF;
            passwordID = (passwordID << 8) | (buffer[offset + 21] & 0xFF);

            password = new byte[passwordLen];
            System.arraycopy(buffer, offset + 22, password, 0, passwordLen);

            Log.d(TAG, "passwordID:" + Util.bytesToString(Util.intToByteCountArray(passwordID, (byte) 2)));
    ...
    public static class Credential {
         ...
         * @param len
         */
        private void parse(byte[] buffer, int offset, int len) throws FormatException {
            ...
            while (cursor < offset + len) {
                ...
                if (innerTagArray[1] != 0x10) {
                    if (innerTag != WPS_ATTRIBUTE_TYPE_VENDOR_ID &&
                        innerTag != WPS_ATTRIBUTE_TYPE_GC_IP &&
                         innerTag != WPS_ATTRIBUTE_TYPE_GO_IP) {
                        throw new FormatException("Tag ID of TLV format invalid: " + Util.bytesToString(innerTagArray));
                    }
                }

                if ((cursor + innerLen) > (offset + len))
                    throw new FormatException("Tag Length of TLV  invalid: (" + cursor + "+" + innerLen + ") > buf position:(" + offset + "+" + len + ")");
    ...
    // UUID-E
    public static final short WPS_ATTRIBUTE_TYPE_UUID_E = 0x1047;

    // Default empty Version
    private static final byte EMPTY_VERISON = 0x00;
    // Default Version
    ...
    // Version
    private byte mVersion = EMPTY_VERISON; //DEFAULT_WIFI_VERISON;
    // Crendential TLV
    ...
    //WSC length
    private byte[] mWscLength = new byte[]{0x0, 0x0};
     ...
     * @return NDEF Message
     */
    public static NdefMessage getWiFiOOB(NdefMessage message) {
        ...
        for (NdefRecord mRecord : message.getRecords()) {
            if (mRecord != null) {
                ...
                if (mRecord.getTnf() == NdefRecord.TNF_MIME_MEDIA && Arrays.equals(mRecord.getType(), TYPE.getBytes())) {
                    Log.i(TAG, " getWiFiOOB hit  mRecordCount:" + mRecordCount);
                    return new NdefMessage(mRecord);
                }
        ...
        Log.i(TAG, " mRecordCount:" + mRecordCount + "  return null;");
    ...
    private static void parseTLV(short tag, byte[] buffer, int offset, int len,
            WifiCarrierConfiguration ccr) throws FormatException  {
    ...
    public byte[] getPayload() {
        ...
        if (Util.byteArrayToint(mWscLength) != 0x0) {
            Log.d(TAG, " mWscLength:" + Util.bytesToString(mWscLength));
            dataSize += mWscLength.length; //mPasswordToken.measureSize();
        }

        if (mVersion != EMPTY_VERISON) {
            dataSize += FIX_FIELDS_LENGTH;
        }

        Log.d(TAG, "mVersion:" + mVersion + "  ByteBuffer dataSize:" + dataSize);
        ...
        if (Util.byteArrayToint(mWscLength) != 0x0)
            ccrPayload.put(mWscLength); // Length
    ...
    public void setId(byte[] recordId) {
        mId = recordId;
    }
