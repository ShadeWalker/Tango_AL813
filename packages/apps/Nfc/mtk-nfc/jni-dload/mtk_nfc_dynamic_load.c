...
)
{
    ...
    #ifdef DEBUG_LOG
    ALOGD("[TIMER]timer_slot(%d) start, handle(%p)\r\n", timer_slot, nfc_timer_table[timer_slot].handle);
    #endif
}
...
)
{
    ...
    #ifdef DEBUG_LOG
    ALOGD("[TIMER]timer_slot(%d) stop, handle(%p)\r\n", timer_slot, nfc_timer_table[timer_slot].handle);
    #endif
}
...
MTK_NFC_CHIP_TYPE_E mtk_nfc_get_chip_type (void)
{
    ...
    // Enable clock buffer
    ioctl_result = ioctl(handle, MTK_NFC_IOCTL_WRITE, (MTK_NFC_GPIO_IOCTL<<8) | (MTK_NFC_IOCTL_CMD_CLOCK_BUF_ENABLE));
    if (ioctl_result != 0) {
        ALOGD("enable clock buffer : node,%d , ioctl fail: %d, %s\n", handle, errno, strerror(errno));
    } else {
        ALOGD("enable clock buffer : Success.");
    }
    ...
    ioctl_result = ioctl(handle, MTK_NFC_IOCTL_WRITE, (MTK_NFC_GPIO_IOCTL<<8) | (MTK_NFC_IOCTL_CMD_CLOCK_BUF_DISABLE));
    if (ioctl_result != 0) {
        ALOGD("disable clock buffer : node,%d , ioctl fail: %d, %s\n", handle, errno, strerror(errno));
    } else {
        ALOGD("disable clock buffer : Success.");
    }
...
int query_nfc_chip(void)
{
    ...
    if(handle <0)
    {
        // open nfc chip is fail !
        ALOGD("mtk_nfc_sys_ioctl_command1 : node,%d , open fail: %d, %s\n", handle, errno, strerror(errno));
    }
    else
    {
        //open nfc chip is success !
        version = ioctl(handle,0xFEFE, 0x00);    
        if (version == -1) 
        {
            ALOGD("mtk_nfc_sys_ioctl_command1 : node,%d , ioctl fail: %d, %s\n", handle, errno, strerror(errno));
            version = ioctl(handle, MTK_NFC_IOCTL_WRITE, (MTK_NFC_GPIO_IOCTL<<8)|(MTK_NFC_IOCTL_CMD_GET_CHIP_ID));
        }
...
void update_nfc_chip(int type)
{
    ...
    {
        //open success !
        version = ioctl(handle,0xFEFD, type);
        close(handle); 
    }
    #ifdef DEBUG_LOG
