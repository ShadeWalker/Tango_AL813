...
extern nfc_jni_discovery_info_t gDiscoveryInfo;  // L migration
...
static jint com_android_nfc_NativeNfcSecureElement_doOpenSecureElementConnection(JNIEnv *e, jobject o)
{
    ...
    else
    {
        ...
        ALOGD("%s: nfc_jni_deactivate done, ret %d", __FUNCTION__, ret);  
    }
    //-------------------------------------------------------
   ...
   jobject o,jint handle, jbyteArray data)
{
	...
	TRACE("com_android_nfc_NativeNfcSecureElement_doTransceive, conn id,%d ",handle);
...
clean_and_return:
    ...
    TRACE("com_android_nfc_NativeNfcSecureElement_doTransceive, done");
