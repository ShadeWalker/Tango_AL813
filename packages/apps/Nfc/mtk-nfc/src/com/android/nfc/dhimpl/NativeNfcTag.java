...
/// M: @ add debug{
import com.mediatek.nfc.addon.Util;
/// }
...
public class NativeNfcTag implements TagEndpoint {
    ...
    public synchronized byte[] transceive(byte[] data, boolean raw, int[] returnCode) {
        ...
        Log.d(TAG, "transceive  data:" + Util.printNdef(data));
        byte[] result = doTransceive(data, raw, returnCode);
        Log.d(TAG, "transceive  return, raw in NCI Data Rx " );
    ...
    private synchronized int checkNdefWithStatus(int[] ndefinfo) {
        ...
        Log.d(TAG, "checkNdef  status:" + status);
    ...
    public synchronized byte[] readNdef() {
        ...
        byte[] result = doRead();
        Log.d(TAG, "readNdef  result:" + Util.printNdef(result));
    ...
    public synchronized boolean writeNdef(byte[] buf) {
        ...
        Log.d(TAG, "writeNdef  buf:" + Util.printNdef(buf));
        boolean result = doWrite(buf);
        Log.d(TAG, "writeNdef  result:" + result);
        if (mWatchdog != null) {
            mWatchdog.doResume();
        }
        return result;
    ...
    public void addNdefFormatableTechnology(int handle, int libnfcType) {
        synchronized (this) {
            Log.d(TAG, "addTechnology() TagTechnology.NDEF_FORMATABLE  handle:" + handle);
            Log.d(TAG, "addTechnology() TagTechnology.NDEF_FORMATABLE  libnfcType:" + libnfcType);
    ...
    public void addNdefTechnology(NdefMessage msg, int handle, int libnfcType,
        ...
        synchronized (this) {
            ...
            Log.d(TAG, "addTechnology() TagTechnology.NDEF  handle:" + handle);
            Log.d(TAG, "addTechnology() TagTechnology.NDEF  libnfcType:" + libnfcType);
            Log.d(TAG, "addTechnology() TagTechnology.NDEF  maxLength:" + maxLength + "  cardState:" + cardState);
    ...
    public NdefMessage findAndReadNdef() {
        ...
        Log.d(TAG, "findAndReadNdef()  foundFormattable:" + foundFormattable);
        Log.d(TAG, "findAndReadNdef()  ndefMsg:" + Util.printNdef(ndefMsg));
