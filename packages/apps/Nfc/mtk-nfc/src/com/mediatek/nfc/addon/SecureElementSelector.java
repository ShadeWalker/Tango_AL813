...
public class SecureElementSelector
    ...
    public static final String NFC_MULTISE_ACTIVE_NUM = "nfc_multise_active_num";
    ...
    private void notifySmartcardSerive() {
        ...
        if (mActiveSe.mSettingString.contains(MySecureElement.SETTING_STRs[USER_ESE])) {
    ...
    private boolean restorePreviousSe() {
        ...
        try {
            ...
            mContentProviderUtil.setActiveSeNumber(mActiveSe.mUserSe);
    ...
    public void deinit(boolean deselect) {
        if (mIsMultiSeSupported) {
            ...
            mContentProviderUtil.setActiveSeNumber(MySecureElement.getLastSe().mUserSe);
        }
    ...
    private class MyContentObserverUtil {
        ...
        private final ContentObserver mObserver = new ContentObserver(null) {
            ...
            public void onChange(boolean selfChange, Uri uri) {
                ...
                do {
                    activeSe = mContentProviderUtil.getActiveSeString(); 
                    if (activeSe != null)
                        break;
                    try {
                ...
                } while (SystemClock.elapsedRealtime() - startTime < WAIT_FOR_GET_ACTIVE_SE_STRING_OPERATIONS_MS);
                ...
                if(activeSe == null){
                    Log.d(TAG, "activeSe is still null, bypass the ContentObserver onchanged routine!");
                    return;
                }
    ...
    private class MyContentProviderUtil {
        ...
        public boolean getNfcEnabled() {
            boolean isNfcEnabled = Settings.Global.getInt(mContext.getContentResolver(), NFC_MULTISE_NFC_ENABLED, 0) != 0;
            Log.d(TAG, "getNfcEnabled, isNfcEnabled = " + isNfcEnabled);
            return isNfcEnabled;
        }
        ...
        public void setActiveSeNumber(int seNumber) {
            Log.d(TAG, "setActiveSeNumber(), seNumber = " + seNumber);
            Settings.Global.putInt(mContext.getContentResolver(), NFC_MULTISE_ACTIVE_NUM, seNumber);
        }
    }
        ...
        public void run() {
            synchronized (mSyncLock) {
                ...
                try {
                    ...
                    mContentProviderUtil.setActiveSeNumber(candidateSe.mUserSe);
        ...
        if (mConfigFileParser.get(1, userConfig)) { /// SWP1
            try {
                if (userConfig[0] == 1) { /// SIM1
                    ...
                    Log.d(TAG, " special config SWP1 to eSE");
                    userSe = USER_ESE;
                } else {
                    Log.d(TAG, "wrong value in nfcse.cfg");
                    userSe = USER_OFF;
                ...
                if (userSe != USER_OFF) {
                    if ((userSe == USER_SIM1 && NfcSimStateObserver.getInstance().isSimReady(PhoneConstants.GEMINI_SIM_1) == true) ||
                        (userSe == USER_SIM2 && NfcSimStateObserver.getInstance().isSimReady(PhoneConstants.GEMINI_SIM_2) == true) ||
                        userSe == USER_ESE) {
    ...
    private void showAlternativeSeDialog(String originalSe) {
        ...
        if (MySecureElement.getFirstSe().isTheSame(MySecureElement.getLastSe())) {
            /* Change Feature: Remove the "No available security elements. NFC Card emulation will be turned off." popup diaglog.
