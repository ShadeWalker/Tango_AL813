...
public class MGLES20Canvas implements MGLCanvas {
    ...
    public void syncState(float[] matrics, int matricsIndex,
            float[] projectMatrics,
            int width, int height, int screenWidth, int screenHeight, float alpha) {
        reset();
    ...
    private int assembleProgram(int vertexShader, int fragmentShader, ShaderParameter[] params) {
        ...
        if (mLinkStatus[0] != GLES20.GL_TRUE) {
            MtkLog.e(TAG, "<assembleProgram> Could not link program: ");
            MtkLog.e(TAG, "<assembleProgram>" + GLES20.glGetProgramInfoLog(program));
    ...
    @Override
    public void drawTexture(MBasicTexture texture, float[] textureTransform, int x, int y, int w,
            int h) {
        if (w <= 0 || h <= 0) {
            return;
        }
    ...
    @Override
    public void drawMixed(MBasicTexture texture, int toColor, float ratio, int x, int y, int w, int h) {
        copyTextureCoordinates(texture, mTempSourceRect);
        mTempTargetRect.set(x, y, x + w, y + h);
    ...
    @Override
    public void drawMixed(MBasicTexture texture, int toColor, float ratio, RectF source, RectF target) {
        if (target.width() <= 0 || target.height() <= 0) {
            return;
        }
    ...
    public void dumpStatisticsAndClear() {
        ...
        mCountFillRect = 0;
        mCountDrawLine = 0;
        MtkLog.d(TAG, "<dumpStatisticsAndClear>" + line);
    }
    ...
    public static void checkError() {
        ...
        if (error != 0) {
            ...
            MtkLog.e(TAG, "<checkError> GL error: " + error, t);
        }
    }
    ...
    private static void printMatrix(String message, float[] m, int offset) {
        ...
        MtkLog.v(TAG, "<printMatrix> " + b.toString());
    }
