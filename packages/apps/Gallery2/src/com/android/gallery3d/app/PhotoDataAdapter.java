...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    /// M: [FEATURE.MODIFY] camera active @{
    ...
    private void fireDataChange() {
        ...
        /// M: [BUG.ADD] if we're at camera preview now, @{
    ...
    public void pause() {
        ...
        for (ImageEntry entry : mImageCache.values()) {
            ...
            /// M: [BEHAVIOR.ADD] @{
            if (entry.highQualityImageTask != null) entry.highQualityImageTask.cancel();
            if (entry.highQualityScreenNail != null) entry.highQualityScreenNail.recycle();
            /// @}
        }
        mImageCache.clear();
        mTileProvider.clear();
        ...
        /// M: [BUG.ADD] @{
        // remove message to avoid JE when exit camera quickly
    ...
    private void updateCurrentIndex(int index) {
        ...
        /// M: [BUG.ADD] @{
        // if we're entering camera preview, make sure "the lights are out";
    ...
    private void uploadScreenNail(int offset) {
        ...
        if (e.highQualityScreenNail != null ) {
            ...
            Log.d(TAG, "<uploadScreenNail> highQualityScreenNail "+s.getWidth() +"  " +s.getHeight());
        } else {
            s = e.screenNail;
        }
        /// @}
    ...
    private void updateTileProvider(ImageEntry entry) {
        ...
        if (entry.highQualityScreenNail != null ) {
            ...
            Log.d(TAG, "<updateTileProvider> highQualityScreenNail  "+screenNail.getWidth() +"  " +screenNail.getHeight());
        }
        /// @}
    ...
    private Future<?> startTaskIfNeeded(int index, int which) {
        ...
        if (which == BIT_SCREEN_NAIL && entry.requestedScreenNail != version) {
            /// M: [BUG.ADD] @{
            //a shot cut for loading camera screen nail. As CameraScreennail is
        ...
        if (which == BIT_FULL_IMAGE && entry.requestedFullImage != version
                ...
                && !mActivity.getIsSmartBookConnected()) {
                /// @}
        ...
        //should no use ReginDecoder to decode tile . @{
                ...
                && !item.getMimeType().endsWith("gif") && !item.getMimeType().endsWith("mpo")
                && ((((item.getSupportedOperations()
                        & MediaItem.SUPPORT_FULL_IMAGE) == 0 )) || mActivity.getIsSmartBookConnected())) {
    ...
    private class UpdateContent implements Callable<Void> {
        ...
        public Void call() throws Exception {
            ...
            if (info.target == mItemPath) {
                ...
                mCurrentIndex = info.indexHint;
                if (mCurrentIndex != oldIndex && oldIndex == mCameraIndex) {
                    mPhotoView.leaveCameraPreview();
                    // Adjust index value, set background when leave camera preview.
                    mPhotoView.setTopStateBackgroundColor(new float[] { 1, 0, 0, 0 });
                }
            }
            /// @}
    ...
    private class ReloadTask extends Thread {
            ...
            while (mActive) {
                ...
                */
                info.items = mSource.getMediaItem(info.contentStart,
                        (info.contentEnd - info.contentStart + 1));
                /// @}
        ...
        private int getIndexOfItemQuickStop(Path path, int hint) {
            int MEDIAITEM_BATCH_FETCH_COUNT = 200;
            ...
            if (index != MediaSet.INDEX_NOT_FOUND) return start + index;
            ...
            while (mActive) {
                ...
                if (index != MediaSet.INDEX_NOT_FOUND) return start + index;
                if (list.size() < MEDIAITEM_BATCH_FETCH_COUNT) return MediaSet.INDEX_NOT_FOUND;
                start += MEDIAITEM_BATCH_FETCH_COUNT;
                list = mSource.getMediaItem(start, MEDIAITEM_BATCH_FETCH_COUNT);
            }
            Log.i(TAG, "<ReloadTask.getIndexOfItemQuickStop> mActive = false, return MediaSet.INDEX_NOT_FOUND");
            return MediaSet.INDEX_NOT_FOUND;
        }
        /// @}
    }
