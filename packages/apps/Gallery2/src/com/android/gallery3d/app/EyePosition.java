...
*/
    ...
    public int getLayoutType() {
        if (mDisplay == null) return SlotView.INVALID_LAYOUT;

        // Always use fancy layout in multi window
        MultiWindowProxy mwProxy = MultiWindowProxy.getInstance();
        if (MultiWindowProxy.isFeatureSupport() && mwProxy != null &&
                mwProxy.getFloatingState()) {
            Log.d(TAG, "getLayoutType return default layout for multi window");
            return SlotView.DEFAULT_LAYOUT;
        }
        ...
        switch (mDisplay.getRotation()) {
            ...
            case Surface.ROTATION_0:
                //PORT
                return SlotView.FANCY_LAYOUT;
            default:
                return SlotView.INVALID_LAYOUT;
        }
    }
    /// @}
}
