...
public class LocalImage extends LocalMediaItem {
    ...
    public static class LocalImageRequest extends ImageCacheRequest {
        ...
        public Bitmap onDecodeOriginal(JobContext jc, final int type) {
            ...
            if (type == MediaItem.TYPE_MICROTHUMBNAIL) {
                /// M: [DEBUG.ADD] @{
                TraceHelper.traceBegin(">>>>LocalImage-onDecodeOriginal-decodeFromExif");
                TraceHelper.traceBegin(">>>>LocalImage-new ExifInterface");
                /// @}
                ...
                try {
                    /// M: [DEBUG.ADD] @{
                    TraceHelper.traceBegin(">>>>LocalImage-exif.readExif");
                    /// @}
                    exif.readExif(mLocalFilePath);
                    /// M: [DEBUG.ADD] @{
                    TraceHelper.traceEnd();
                    TraceHelper.traceBegin(">>>>LocalImage-exif.getThumbnail");
                    /// @}
            ...
            if (type == MediaItem.TYPE_FANCYTHUMBNAIL) {
                ...
                if (type == MediaItem.TYPE_HIGHQUALITYTHUMBNAIL)
                    options.inPostProc = FeatureConfig.supportPictureQualityEnhance
    ...
    public int getSupportedOperations() {
        ...
        if (BitmapUtils.isSupportedByRegionDecoder(mimeType)) {
            ...
            if (Math.max(0, Utils.ceilLog2((float) width / thumbWidth)) == 0
                    || (FeatureConfig.isLowRamDevice && width * height > REGION_DECODER_PICTURE_SIZE_LIMIT)) {
                ...
                Log.i(TAG, "<getSupportedOperations> isLowRamDevice "
                        + FeatureConfig.isLowRamDevice + ", width * height is "
                        + width * height);
    ...
    public void rotate(int degrees) {
        ...
        mApplication.getContentResolver().update(baseUri, values, "_id=?",
                new String[]{String.valueOf(id)});
        /// M: [BUG.ADD] rotate picture, need update camera thumbnail @{
        mApplication.getDataManager().broadcastUpdatePicture();
        /// @}
    }
    ...
    // When new LocalImage in ContainerSet, need parameters about motion &
    // continuous shot
    private boolean mIsMotionTrack = false;
    private boolean mIsConshot = false;

    public LocalImage(Path path, GalleryApp application, String filePath, boolean isMotionTrack) {
        super(path, nextVersionNumber());
        mIsMotionTrack = isMotionTrack;
        mApplication = application;
        ...
        try {
            ...
            Log.w(TAG, "<new> Could not find file to read exif: " + filePath, e);
        } catch (IOException e) {
            Log.w(TAG, "<new> Could not read exif from file: " + filePath, e);
        }
    ...
    private boolean updateMediaData(Cursor cursor) {
        ...
        } else if (mIsMotionTrack) {
            synchronized (mMediaDataLock) {
                mMediaData = MediaDataParser.parseLocalImageMediaData(LocalImage.this);
                mMediaData.mediaType = MediaData.MediaType.NORMAL;
                mMediaData.subType = MediaData.SubType.MOTRACK;
                mExtItem = new ExtItem(mMediaData);
            }
