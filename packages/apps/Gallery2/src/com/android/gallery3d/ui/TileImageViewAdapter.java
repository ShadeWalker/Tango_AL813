...
public class TileImageViewAdapter implements TileImageView.TileSource {
    ...
    public Bitmap getTile(int level, int x, int y, int tileSize) {
        ...
        /// M: [FEATURE.ADD]  PQ @{
        if (mExtItem != null && mExtItem.needHistogram()) {
            int[] hist = mExtItem.getHist();
            if (hist != null) {
                mExtItem.addHistFlag(options);
            } else {
                Log.d(TAG, "<getTile> get not hist for file ="
                        + mExtItem.getImageDCFilePath());
            }
        } else if (mExtItem == null) {
            Log.d(TAG, "<getTile> mExtItem is null" );
        }
        if (FeatureConfig.supportPictureQualityEnhance) {
        ...
        bitmap = BitmapUtils.replaceBackgroundColor(bitmap, 0xFF000000, true);
        /// @}
    ...
    private Bitmap getTileWithoutReusingBitmap(
        ...
        if (mExtItem != null && mExtItem.needHistogram()) {
            ...
            if (hist != null) {
                ...
                Log.d(TAG, "<getTileWithoutReusingBitmap> get not hist for file ="
                        + mExtItem.getImageDCFilePath());
            }
        }  else if (mExtItem == null) {
            Log.d(TAG, "<getTileWithoutReusingBitmap> mExtItem is null" );
        }
        /// @}
        /// M: [PERF.MARK] @{
        //  Do region decode in multi-thread, so delete synchronized
