...
public class SettingsActivity extends PreferenceActivity {
    ...
    private int getSelectedApnKey() {
        ...
        try {
            ...
            MtkLog.v(TAG, "getSelectedApnKey() Uri=" + uri + " cursor = " + cursor);
    ...
    private void refreshSIMInfo() {
        ...
        if (LOG && mSimInfo != null) {
            MtkLog.i(TAG, "refreshSIMInfo() simid=" + mSimInfo.getSubscriptionId()
    ...
    private boolean isGemini() {
        final boolean gemini = "1".equals(SystemProperties.get("ro.mtk_gemini_support"));
        if (LOG) {
