...
public class MovieDrmExtensionImpl extends DefaultMovieDrmExtension {
    ...
    private static boolean handleDrmFile(final Context context, final Uri uri,
        ...
        if (info != null && info.isDrm() && info.getMethod() != OmaDrmStore.DrmMethod.METHOD_FL) {
            ...
            try {
                rightsStatus = client.checkRightsStatus(uri, OmaDrmStore.Action.PLAY);
            ...
            switch (rightsStatus) {
                case OmaDrmStore.RightsStatus.RIGHTS_VALID:
                    mHandler.post(new Runnable() {
                        @Override
                    public void run() {
                        OmaDrmUiUtils.showConsumeRightsDialogForVideoPlayer(client, context,
                                listener);
                    }
                    });
                result = true;
                ...
                case OmaDrmStore.RightsStatus.RIGHTS_INVALID:
                    mHandler.post(new Runnable() {
                        @Override
                    public void run() {
                        OmaDrmUiUtils.showRefreshLicenseDialogForVideoPlayer(client, context, uri,
                                listener);
                    }
                    });
                result = true;
                ...
                case OmaDrmStore.RightsStatus.SECURE_TIMER_INVALID:
                    mHandler.post(new Runnable() {
                        @Override
                    public void run() {
                        OmaDrmUiUtils.showSecureTimerInvalidDialogForVideoPlayer(client, context,
                                listener);
                    }
                    });
                result = true;
