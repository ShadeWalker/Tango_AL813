...
public class InstallAppProgress extends Activity implements View.OnClickListener, OnCancelListener {
    ...
    public void onClick(View v) {
        /* HQ_tanyali 2015-10-22 modified for HQ01456773 begin*/
    	PackageManager pm = this.getPackageManager();
    	String packageName = mAppInfo.packageName;
    	String apps_name = null;
    	try {
            apps_name = pm.getApplicationLabel(pm.getApplicationInfo(packageName,PackageManager.GET_META_DATA)).toString();
        } catch (NameNotFoundException e) {
    	    e.printStackTrace();
    	}
        
        Float size = Float.valueOf((float) new File(mPackageURI.getPath()).length());
        Float apps_size = size/1024/1024;

        if (this.getApplication() == null || this.getApplication().getResources() == null) {
            return;
        }
        String delete_apps_text = this.getApplication().getResources().getString(R.string.delete_apps_text, apps_name, apps_size);
        ...
        if(v == mDoneButton) {
            ...
            if (sourceFile.exists() && size != 0) {
                new AlertDialog.Builder(this)
                .setTitle(R.string.delete_apps_title)
                .setMessage(delete_apps_text)
                .setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        finish();
                    }
    ...
    /* HQ_tanyali 2015-10-22 modified end */  
