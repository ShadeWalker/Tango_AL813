...
public class ActionBarController {
    ...
    private final AnimationCallback mFadeOutCallback = new AnimationCallback() {
        ...
        public void onAnimationEnd() {
            ...
            /** M: Remove the animate listener after animate finished. @{ */
            if (mSearchBox != null) {
                mSearchBox.animate().setListener(null);
            }
            /** @} */
        }
    ...
    public void setAlpha(float alphaValue) {
        /// M: Clear the listener first, otherwise the old listener will be called.
        mSearchBox.animate().setListener(null);
    ...
    public void setHideOffset(int offset) { 
        ...
        /** M: [ALPS01940938] when slide down, show the actionbar @{ */
        if (!mIsActionBarSlidUp && mSearchBox.getVisibility() == View.INVISIBLE) {
    ...
    public void saveInstanceState(Bundle outState) {
        if (DEBUG) {
            Log.d(TAG, "saveInstanceState: mIsActionBarSlidUp=" + mIsActionBarSlidUp);
        }
    ...
    public void restoreInstanceState(Bundle inState) {
        ...
        if (DEBUG) {
            Log.d(TAG, "restoreInstanceState: mIsActionBarSlidUp=" + mIsActionBarSlidUp);
        }
     ...
     */
    public void restoreActionBarOffset(boolean isListsFragmentResumed) {
        if (DEBUG) {
            Log.d(TAG, "restoreActionBarOffset: mIsActionBarSlidUp=" + mIsActionBarSlidUp
                    + ", isListsFragmentResumed=" + isListsFragmentResumed
                    + ", mActivityUi.isInSearchUi()=" + mActivityUi.isInSearchUi()
                    + ", mActivityUi.shouldShowActionBar()=" + mActivityUi.shouldShowActionBar());
        }

        boolean shouldActionBarSlidUp = !mActivityUi.shouldShowActionBar();
        if (isListsFragmentResumed && !mActivityUi.isInSearchUi()
                && mIsActionBarSlidUp != shouldActionBarSlidUp) {
            Log.d(TAG, "restoreActionBarOffset: Conflict!!!!! adjust to shouldActionBarSlidUp="
                    + shouldActionBarSlidUp);
            slideActionBar(shouldActionBarSlidUp /* slideUp */, false /* animate */);
        } else {
            slideActionBar(mIsActionBarSlidUp /* slideUp */, false /* animate */);
        }
        /** @} */

        /** M: [ALPS01940938] if not slide up, show the search box @{ */
        if (!mIsActionBarSlidUp && mSearchBox.getVisibility() == View.INVISIBLE) {
            mSearchBox.setVisibility(View.VISIBLE);
        }
        /** @} */
