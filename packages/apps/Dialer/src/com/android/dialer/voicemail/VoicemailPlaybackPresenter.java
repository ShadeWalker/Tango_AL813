...
public class VoicemailPlaybackPresenter {
    ...
    private boolean mStartPlayingImmediately;
    ...
    public VoicemailPlaybackPresenter(PlaybackView view, MediaPlayerProxy player,
        ...
        mAudioManager = (AudioManager) mView.getDataSourceContext()
    ...
    public void onCreate(Bundle bundle) {
        ...
        /** M: [ALPS01811630] restore the player's position for restarting @{ */
        mStartPosition = bundle == null ? 0 : bundle.getInt(CLIP_POSITION_KEY);
        mStartPlayingImmediately = bundle == null ? mStartPlayingImmediately : !bundle
                .containsKey(PAUSED_STATE_KEY);
        /** @} */
        ...
        mAsyncTaskExecutor.submit(Tasks.PREPARE_MEDIA_PLAYER,
                new AsyncTask<Void, Void, Exception>() {
                    ...
                    public Exception doInBackground(Void... params) {
                        try {
                            /// M: [ALPS01829200] log the bg working status
                            Log.d(TAG, "postSuccessfullyFetchedContent bging");
                            mPlayer.reset();
                    ...
                    public void onPostExecute(Exception exception) {
                        /// M: [ALPS01850932] mPlayer may be released during task's bg running
                        if (exception == null && mPlayer.isReadyToPlay()) {
    ...
    private void postSuccessfulPrepareActions() {
        ...
        /// M: [ALPS01811630] set SeekBar's position
        mView.setClipPosition(mStartPosition, mDuration.get());
        mView.playbackStopped();
        // Always disable on stop.
        mView.disableProximitySensor();
        if (mStartPlayingImmediately) {
            /// M: [ALPS01811630] use mStartPosition to begin with the new play
            resetPrepareStartPlaying(mStartPosition);
        }
    ...
    public void onDestroy() {
        ...
        /// M: [ALPS01829200] don't release here due to indeterminate destroy invoke,
        // and do release in Fragment's destroy function.
        //mPlayer.release();
    ...
    private class AsyncPrepareTask extends AsyncTask<Void, Void, Exception> {
        ...
        public Exception doInBackground(Void... params) {
            try {
                /// M: [ALPS01829200] log the bg working status
                Log.d(TAG, "AsyncPrepareTask bging");
                mPlayer.reset();
                mPlayer.setDataSource(mView.getDataSourceContext(), mVoicemailUri);
                mPlayer.setAudioStreamType(PLAYBACK_STREAM);
                mPlayer.prepare();
    ...
    private class StartStopButtonListener implements View.OnClickListener {
        ...
        public void onClick(View arg0) {
            if (mPlayer.isPlaying()) {
                /// M: [ALPS01907063] work around for vs's flash to beginning
                // during quick switch between start and stop
                stopPlaybackAtPosition(mView.getDesiredClipPosition(), mDuration.get());
            } else {
                resetPrepareStartPlaying(mView.getDesiredClipPosition());
            }
        }
        ...
        private final Runnable mSetClipPostitionRunnable = new Runnable() {
            ...
            public void run() {
                ...
                synchronized (mLock) {
                    ...
                    /// M: [ALPS01783851] don't update the SeekBar, when player has paused
                    if (!mPlayer.isPlaying()) {
                        return;
                    }
                    currentPosition = mPlayer.getCurrentPosition();
                    /** M: [ALPS01783851] only player's position has advance than
                     * the desired position can it update the SeekBar.
                     * SeekBar will be reset by player's completeListener @{ */
                    if (mView.getDesiredClipPosition() > currentPosition) {
                        return;
                    }
                    /** @} */
                }
    ...
    public void onPause() {
        if (mPlayer.isPlaying()) {
            /// M: [ALPS01920347] remove the audio listener in time
            mAudioManager.abandonAudioFocus(mAudioFocusListener);
            stopPlaybackAtPosition(mPlayer.getCurrentPosition(), mDuration.get());
        }
        if (mPrepareTask != null) {
    ...
    private OnAudioFocusChangeListener mAudioFocusListener =
        ...
        public void onAudioFocusChange(int focusChange) {
            ...
            case AudioManager.AUDIOFOCUS_GAIN_TRANSIENT:
                ...
                //Do not restart playing because the activity maybe paused
//                if (!mPlayer.isPlaying()) {
//                    resetPrepareStartPlaying(mView.getDesiredClipPosition());
//                }
                break;
    ...
    /** M: [ALPS01811630] save the player's position for restarting @{ */
