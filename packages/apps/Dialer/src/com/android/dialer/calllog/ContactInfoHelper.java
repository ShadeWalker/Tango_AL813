...
public class ContactInfoHelper {
    ...
    public static Uri createTemporaryContactUri(String number) {
        try {
            ...
            if (!PhoneNumberHelper.isUriNumber(number)) {
                contactRows.put(Phone.CONTENT_ITEM_TYPE, new JSONObject().put(Phone.NUMBER, number)
                        .put(Phone.TYPE, Phone.TYPE_CUSTOM));
            /// M: for ALPS01809680 @{
            // build sip type contact uri
            } else {
                contactRows.put(SipAddress.CONTENT_ITEM_TYPE, new JSONObject().put(
                        SipAddress.SIP_ADDRESS, number)
