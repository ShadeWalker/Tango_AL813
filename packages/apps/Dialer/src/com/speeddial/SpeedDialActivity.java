...
public class SpeedDialActivity extends ListActivity implements View.OnClickListener,
    ...
    public static void actionStart(Context context){
        Intent intent = new Intent(context, SpeedDialActivity.class);
    ...
    // / Modified by guofeiyao
    public Uri mUri = Uri.parse("content://hq_speed_dial/numbers");
    // / End
    ...
    // / Explained by guofeiyao:
    // the following id's in
    // mtk_speed_dial_list_item.xml
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        // / Added by guofeiyao 2016/1/6
        // if the key is in specialKeySet, we don't handle it.
//        if (position >= 1) {
//            Log.e("duanze", SpeedDialPlugin.identifierNameArr[position - 1]);
//        }
        if (position >= 1
                && null != SpeedDialPlugin.specialKeySet
                && SpeedDialPlugin.specialKeySet.contains(SpeedDialPlugin.identifierNameArr[position - 1])) {
            return;
        }

        if (2 == position && SpeedDialPlugin.consumedThree) {
            return;
        }
        // / End
    ...
    private void populateMatrixCursorEmpty(Context cnx, MatrixCursor cursor, int id, String number) {
        ...
        if (TextUtils.isEmpty(number)) {
            // / Added by guofeiyao 2016/1/6
            // if the key is in specialKeySet, we don't handle it.
            if (SpeedDialPlugin.consumedThree && 3 == id) {
                cursor.addRow(new String[]{
                        String.valueOf(id),
                        "*611", "", "", "", ""});
            } else if (2 <= id && id <= 9
                    && null != SpeedDialPlugin.specialKeySet
                    && SpeedDialPlugin.specialKeySet.contains(SpeedDialPlugin.identifierNameArr[id - 2])) {

                cursor.addRow(new String[]{
                        String.valueOf(id),
                        SpeedDialPlugin.specialKeyMap.get(SpeedDialPlugin.identifierNameArr[id - 2]), "", "", "", ""});
            } else {
                cursor.addRow(new String[]{
                        String.valueOf(id),
    ...
    private void populateMatrixCursorRow(int row, Cursor cursor) {
        ...
        if (type == 0) {
            ...
            label = (String) Phone.getTypeLabel(getResources(), type, null);
        }
