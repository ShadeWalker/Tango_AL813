...
class InputMethodPreference extends SwitchPreference implements OnPreferenceClickListener,
    ...
    private String getSummaryString() {
        ...
        for (final InputMethodSubtype subtype : subtypes) {
            ...
            if (label.toString().startsWith("Српски")) {
                subtypeLabels.add(label.toString().replace("Српски", "Srpski"));
            } else {
                //HQ_hushunli 2015-11-16 add for HQ01496783 begin
                int startIndex = label.toString().indexOf("(");
                if (SystemProperties.get("ro.hq.aosp.nocountryinfo").equals("1") && startIndex != -1) {
                    subtypeLabels.add(label.toString().substring(0, startIndex));
                } else {
