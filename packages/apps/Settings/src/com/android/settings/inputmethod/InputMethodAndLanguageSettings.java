...
public class InputMethodAndLanguageSettings extends SettingsPreferenceFragment
    ...
    //add by hushunli for digital switch
    private static final String KEY_DIGITAL_SWITCH_STRING = "digital_switch";
    private static final String ARABIC_NUMERALS = "persist.sys.hq.arabic.numerals";
    ...
    private SwitchPreference mDigitalSwitchPref;//add by hushunli for digital switch
    ...
    public void onCreate(Bundle icicle) {
        ...
        Log.d(TAG, "curLanguageString is " + curLanguageString + ", persist.sys.hq.arabic.numerals is " + SystemProperties.get(ARABIC_NUMERALS));
        mDigitalSwitchPref = (SwitchPreference) findPreference(KEY_DIGITAL_SWITCH_STRING);
        mDigitalSwitchPref.setChecked(SystemProperties.get(ARABIC_NUMERALS).equals("1"));
        mDigitalSwitchPref.setOnPreferenceChangeListener(this);
        //if (!(curLanguageString.equals("ar") || curLanguageString.equals("fa"))) {
            getPreferenceScreen().removePreference(mDigitalSwitchPref);
        //}
        //HQ_hushunli 2015-08-24 add for HQ01303115 end

        if (SystemProperties.get("ro.hq.date.format").equals("1")) {
           if(curLanguageString.equals("ar") || curLanguageString.equals("fa") || curLanguageString.equals("iw") ||  curLanguageString.equals("he")){
    ...
    private void updateUserDictionaryPreference(Preference userDictionaryPreference) {
        ...
        /// @}
		userDictionaryPreference.setWidgetLayoutResource(R.layout.arrow_img_layout);
        final Activity activity = getActivity();
    ...
    public boolean onPreferenceChange(Preference preference, Object value) {
        Log.d(TAG, "in onPreferenceChange, click " + preference.getKey());
        //HQ_hushunli 2015-10-15 add for HQ01303115
        if (preference == mDigitalSwitchPref) {
            boolean isChecked = (boolean)value;
            SystemProperties.set(ARABIC_NUMERALS, isChecked ? "1" : "0");
            Log.d(TAG, "onPreferenceChange, set value is " + (isChecked ? "1" : "0") + ", persist.sys.hq.arabic.numerals is " + SystemProperties.get(ARABIC_NUMERALS));
            return true;
        }
        //HQ_hushunli 2015-10-15 add for HQ01303115
        if (SHOW_INPUT_METHOD_SWITCHER_SETTINGS) {
        ...
        if (curPref != null) {
            ...
            if (!TextUtils.isEmpty(curIme)) {
                synchronized (this) {
                    ...
                    if (curIme.toString().startsWith("Српски")) {
                        curPref.setSummary(curIme.toString().replace("Српски", "Srpski"));
                    } else {
                        if (SystemProperties.get("ro.hq.aosp.nocountryinfo").equals("1")) {//HQ_hushunli 2015-11-16 add for HQ01496783
                            String values[] = curIme.toString().split("-");
                            if (values.length >= 2 && !TextUtils.isEmpty(values[0])) {
                                int startIndex = values[0].indexOf("(");
                                int endIndex = values[0].indexOf(")");
                                if (startIndex != -1 && endIndex != -1) {
                                    curPref.setSummary(values[0].substring(0, startIndex) + curIme.toString().substring(endIndex + 1));
                                } else {
