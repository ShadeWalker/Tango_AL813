...
import android.provider.ChildMode;//add by lihaizhou for ChildMode at 2015-08-09
...
public class WifiEnabler implements SwitchBar.OnSwitchChangeListener  {
    ...
    public void onSwitchChanged(Switch switchView, boolean isChecked) {
        ...
        if (!mWifiManager.setWifiEnabled(isChecked)) {
            // Error
            mSwitchBar.setEnabled(true);
                /* lihaizhou add for forbid checked when child mode times up by begin at 2015-08-09*/
              	if(mWifiManager.isChildModeOn() && mWifiManager.isProhibitWlan())
	  	{
	                     Toast.makeText(mContext, R.string.wifi_dont_use_wifi_childmode_on,Toast.LENGTH_SHORT).show(); 
	                    mSwitchBar.setChecked(false);
	                    return;
	  	}
		else if (mWifiManager.isChildModeOn() && mWifiManager.isWlanTimeUp())
	  	{
		             mSwitchBar.setChecked(false);
		             return;
	  	}
		 /* lihaizhou add for forbid checked when child mode times up by end at 2015-08-09*/
