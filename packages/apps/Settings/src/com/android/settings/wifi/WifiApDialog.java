...
public class WifiApDialog extends AlertDialog implements View.OnClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mWifiConfig != null) {
			...
			CharSequence text = mSsid.getText();
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.show_password) {
	     ...
	     Log.d("yulifeng","ssid_default = " + s);
	     Log.d("yulifeng","ro.hq.wifi.ap.name.pwd = " + SystemProperties.get("ro.hq.wifi.ap.name.pwd") );
	     ...
	     if(SystemProperties.get("ro.hq.wifi.ap.name.pwd").equals("1")){
                    ...
                    if(projetcStr.equals("TAG-L01")) {
                        ...
                        String imei = ((TelephonyManager)mContext.getSystemService(mContext.TELEPHONY_SERVICE)).getDeviceId();
                            if(imei != null){                              
                                   srandom  =imei.substring(imei.length()-4,imei.length());
						    }
                        //add by wanghui for al813 wifi-ap end
                           mSsid.setText("HUAWEI TAG-L01_" + srandom);
                    ...
                    //add by wanghui for al813 wifi-ap begin                      
                        ...
                        //add by wanghui for al813 wifi-ap end
                           mSsid.setText("HUAWEI_TAG-L21_" + srandom);//add by xiemin for HQ02057384
                    } else {
                           //modify by yulifeng for wifi ap name HQ01395154 20150930
                           String productStr = SystemProperties.get("ro.product.name", ""); 
                           Log.d("yulifeng","{WifiApDialog]_productStr:"+productStr);
                           mSsid.setText("HUAWEI " + productStr + "_" +srandom);
                           //end
                    }	
	     	}else{
		      mSsid.setText(s);
	     ...
	     /* SW_GLOBAL_COMMON_043, yulifeng, 20150809, wifiap,HQ01301811,b*/
	     if(SystemProperties.get("ro.hq.wifi.ap.name.pwd").equals("1")){
                      mPassword.setText(randomUUID.substring(0, 8));
             }else{
