...
class AccessPoint extends Preference {
    ...
    static final int SECURITY_EAP = AccessPointExt.SECURITY_EAP;
    ...
    AccessPoint(Context context, WifiConfiguration config) {
        super(context,null, R.attr.wifiPreferenceStyle);
        //add mtk feature
    ...
    private void loadResult(ScanResult result) {
        ...
        ///M: add Passpoint
        mSupportedPasspoint = FeatureOption.MTK_PASSPOINT_R1_SUPPORT
            && mScanResult.capabilities.contains(HOTSPOT);
    }

    @Override
    protected void onBindView(View view) {
        //getContext().setTheme(R.style.preferenceStyle_hw);
        super.onBindView(view);
        updateIcon(getLevel(), getContext());

        mSummaryView = (TextView) view.findViewById(com.android.internal.R.id.summary);
        /*add by wanghui for al812 HQ01386734 20151012 */
        mSummaryView.setTextSize(12);
    ...
    public int compareTo(Preference preference) {
        ...
        int order = mAccessPointExt.compareTo(this.ssid, this.security, other.ssid, other.security);
        if (order != 0) {
            return order;
        }
        /// @}
    ...
    boolean update(ScanResult result) {
        ...
        if (ssid.equals(result.SSID) && security == getSecurity(result)) {
            ...
            ///M: add Passpoint
            mSupportedPasspoint = FeatureOption.MTK_PASSPOINT_R1_SUPPORT
                && result.capabilities.contains(HOTSPOT);
            refresh();
            return true;
        }
        return false;
    ...
    private boolean isInfoForThisAccessPoint(WifiInfo info) {
        if (networkId != WifiConfiguration.INVALID_NETWORK_ID) {
            return networkId == info.getNetworkId();
            //HQ_YangPing fixed wifi issue for HQ02036849  start
        }else if (info != null && info.getNetworkId() != WifiConfiguration.INVALID_NETWORK_ID){

            Log.d(TAG, "info.getNetworkId() = " + info.getNetworkId() + "ssid = " + ssid);
            ...
            if (configs != null) {
                for (WifiConfiguration config : configs) {
                    if ( info.getNetworkId() == config.networkId &&
                        ...
                        return ssid.equals(removeDoubleQuotes(info.getSSID())) && security == getSecurity(config);
                    }
                }
            }
            return ssid.equals(removeDoubleQuotes(info.getSSID()));  
            
            //HQ_YangPing fixed wifi issue for HQ02036849  end   
        } else {
            // Might be an ephemeral connection with no WifiConfiguration. Try matching on SSID.
            // (Note that we only do this if the WifiConfiguration explicitly equals INVALID).
            // TODO: Handle hex string SSIDs.
    ...
    void update(WifiInfo info, NetworkInfo networkInfo) {
        ...
        if ((info != null && isInfoForThisAccessPoint(info))
                || PasspointUtils.shouldUpdate(info, bssid, mSupportedPasspoint)) {
            reorder = (mInfo == null);
            mRssi = info.getRssi();
            mInfo = info;
    ...
    private void refresh() {
        setTitle(ssid);
        ///M: add Passpoint
        PasspointUtils.setTitle(this, getContext(), mSupportedPasspoint, getState());
        ...
        /*add by wanghui for al812 HQ01386734 20151012 begin*/
        if (security != SECURITY_NONE) {
                String securityStrFormat;
                if (summary.length() == 0) {
                    securityStrFormat = context.getString(R.string.wifi_secured_first_item);
                    if(getSecurityString(true) != null){
       ...
       /*add by wanghui for al812 HQ01386734 20151012 end*/
        if (summary.length() > 0) {
