...
public class ManageApplications extends Fragment implements
    ...
    //wuhuihui add
    private static final int MENU_DEFAULT_APP  = Menu.FIRST + 10;
                ...
                if (mContainerService != null) {
                    try {
                        final long[] stats = mContainerService.getFileSystemStats(
                                Environment.getExternalStorageDirectoryExtraSd().getPath());//changed for install app sd.2015.03.07.zhuchao.
                        mTotalStorage = stats[0];
                        mFreeStorage = stats[1];
                    } catch (RemoteException e) {
                        Log.w(TAG, "Problem in container service", e);
                    } catch(IllegalStateException e){
                       Log.w(TAG, "Problem in IllegalStateException", e);
                    }//chenwenshuai
                }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            synchronized (entry) {
                ...
                if (mFilterMode == FILTER_APPS_SDCARD) {
					...
					holder.arrow.setVisibility(View.GONE);//added by chenwenshuai for SDcar app
                    holder.checkBox.setChecked((entry.info.flags
                            & ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0);
                } else {
                    holder.checkBox.setVisibility(View.GONE);
					holder.arrow.setVisibility(View.VISIBLE);//added by chenwenshuai for SDcar app
                }
            }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //if (!Environment.isExternalStorageEmulated()//changed for install app sd.chenwenshuai
        if(!Environment.isExternalStorageEmulatedEx()
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
	   ...
	   menu.add(Menu.NONE, MENU_DEFAULT_APP, 0, R.string.title_preferred_list_settings)
				.setIcon(R.drawable.default_app_menu)
				.setEnabled(true)
        ...
        menu.add(0, SHOW_RUNNING_SERVICES, 3,  R.string.show_running_services)
                .setIcon(R.drawable.ic_menu_apps_show_service_dark)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if ((menuId == SORT_ORDER_ALPHA) || (menuId == SORT_ORDER_SIZE)) {
            ...
            buildResetDialog();
        } else if (menuId == MENU_DEFAULT_APP) {
            final Context context = getActivity();
			Intent defaultAppIntent = new Intent(context,com.android.settings.applications.PreferredListSettingsActivity.class);
			startActivity(defaultAppIntent);
   ...
   private String getSdDesc(String path) {
       ...
       for (int i = 0; i < len; i++) {
           if (volumes[i].getPath().equals(path)) {
               ...
               String ProjectConfigName = SystemProperties.get("ro.config.project"); //wuhuihui add for HQ01597654
             //zhouyoukun fix HQ01533986
               if(Locale.getDefault().getLanguage().equals("en")){
            	   sdDesc="On "+sdDesc;
               }else if (Locale.getDefault().getLanguage().equals("es") && "l03_latin_atandt".equals(ProjectConfigName)){
                   sdDesc="En "+sdDesc;
               }
               break;
             } else {
