...
public final class DynamicIndexableContentMonitor extends PackageMonitor implements
    ...
    public void onPackageModified(String packageName) {
        ...
        try {
            final int state = mContext.getPackageManager()
        ...
        } catch (IllegalArgumentException e) { //M: JE ALPS02044659
            Log.e(TAG, "happen exception , Unknown package: " + packageName);
        }
    }
    ...
    private void handlePackageAvailable(String packageName) {
        if (!mAccessibilityServices.contains(packageName)) {
            ...
            List<ResolveInfo> resolveInfos = mContext.getPackageManager().queryIntentServices(intent, 0);
            Log.d(TAG, "packageName = " + packageName + ", resolveInfos = " + resolveInfos);
        ...
        if (mHasFeatureIme) {
            if (!mImeServices.contains(packageName)) {
                ...
                List<ResolveInfo> resolveInfoIMEs = mContext.getPackageManager()
                           .queryIntentServices(intent, 0);
                Log.d(TAG, "mHasFeatureIme packageName = " + packageName
                        + ", resolveInfoIMEs = " + resolveInfoIMEs);
