...
public class BatteryHistoryChart extends View {
    ...
    void setStats(BatteryStats stats, Intent broadcast) {
        ...
        mDischarging = true;
        /* HQ_fengyaling_2015-12-15 modified for battery usage information item crashing in Burmese start */
        String code = getResources().getConfiguration().locale.getCountry();
        /* HQ_fengyaling_2015-12-15 modified for battery usage information item crashing in Burmese end */
        if (mBatteryBroadcast.getIntExtra(BatteryManager.EXTRA_PLUGGED, 0) == 0) {
            ...
            if (drainTime > 0) {
              ...
              /* HQ_fengyaling_2015-12-15 modified for battery usage information item crashing in Burmese start */
               if(code.equals("ZG")){
                mChargeLabelString = batteryPercentString+"-"+timeString;
               }else{
            ...
            if (chargeTime > 0 && status != BatteryManager.BATTERY_STATUS_FULL) {
               ...
               /* HQ_fengyaling_2015-12-15 modified for battery usage information item crashing in Burmese end */
                ...
                /* HQ_fengyaling_2015-12-15 modified for battery usage information item crashing in Burmese start */
                if(code.equals("ZG")){
                  mChargeLabelString = batteryPercentString +"-"+statusLabel;
                }else{
                  mChargeLabelString = getContext().getResources().getString(
    ...
    void finishPaths(int w, int h, int levelh, int startX, int y, Path curLevelPath,
        ...
        if (curLevelPath != null) {
            if (lastX >= 0 && lastX < w) {
                if (lastPath != null) {
                    lastPath.lineTo(w-1, y);
                }
                curLevelPath.lineTo(w-1, y);
            }
            curLevelPath.lineTo(w-1, mLevelTop+levelh+2);
            curLevelPath.lineTo(startX-2, mLevelTop+levelh+2);
            curLevelPath.close();
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (h > ((textHeight*10)+mChartMinHeight)) {
            ...
            if (h > (textHeight*15)) {
                ...
                mLineWidth = textHeight/6;
            } else {
                // Compress lines to make more room for chart.
                mLineWidth = textHeight/9;
            }
        } else {
            mLargeMode = false;
        ...
        int fullBarOffset = textHeight + mLineWidth*3;

        if (mLargeMode) {
            mChargingOffset = mLineWidth*3;
                    ...
                    if (lastX != x) {
                        ...
                        if (lastY != y) {
                            ...
                            if (curLevelPath == null) {
                                ...
                                curLevelPath.moveTo(x-4, y);
                                startX = x-1;
                            } else {
                                curLevelPath.lineTo(x-4, y);
                            }
                            lastX = x;
                            lastY = y;
                        }
        ...
        if (DEBUG) Log.d(TAG, "while done lastY = " + lastY + ",lastX = " + lastX);
        if (lastY < 0 || lastX < 0) {
            // Didn't get any data...
            x = lastX = mLevelLeft;
        ...
        if (x < w) {
            ...
            // that part of the UI.
            mTimeRemainPath.moveTo(x, lastY+2);
            int fullY = mLevelTop + levelh - ((100-batLow)*(levelh-1))/batChange;
            int emptyY = mLevelTop + levelh - ((0-batLow)*(levelh-1))/batChange;
            if (mDischarging) {
                mTimeRemainPath.lineTo(mLevelRight, emptyY+2);
            } else {
                mTimeRemainPath.lineTo(mLevelRight, fullY+2);
                mTimeRemainPath.lineTo(mLevelRight, emptyY+2);
            }
            mTimeRemainPath.lineTo(x, emptyY+2);
