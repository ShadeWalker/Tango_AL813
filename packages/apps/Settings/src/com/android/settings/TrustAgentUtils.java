...
public class TrustAgentUtils {
    ...
    //Created by libeibei on 17-3-1 ,cause of HQ02067101 ,begin
    public static void checkTrustAgents(Context mContext){
        final String PackageName = "com.google.android.gms";
        final String ClassName = "com.google.android.gms.auth.trustagent.GoogleTrustAgent";

        LockPatternUtils mLockPatternUtils=new LockPatternUtils(mContext);
        ArraySet<ComponentName> mActiveAgents = new ArraySet<ComponentName>();
        ...
        int count = mActiveAgents.size();
        for(int i =0;i<count;i++){
            String mPackageName = mActiveAgents.valueAt(i).getPackageName();
            String mClassName = mActiveAgents.valueAt(i).getClassName();
            Log.d("TrustAgentReceiver","packageName = "+ mPackageName +", className = "+mClassName);
            if(PackageName.equals(mPackageName) && ClassName.equals(mClassName)){
                mActiveAgents.removeAt(i);
            }
        }
        mLockPatternUtils.setEnabledTrustAgents(mActiveAgents);
    }
    //Created by libeibei on 17-3-1 ,cause of HQ02067101 ,end
}
