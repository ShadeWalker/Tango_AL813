...
import android.telephony.TelephonyManager;//modified by maolikui at 2015-11-17 
import android.content.Context;//modified by maolikui at 2015-11-17
public class CreateShortcut extends LauncherActivity {
    ...
    private static final String SIM_SETTINGS = "com.android.settings.Settings$SimSettingsActivity";//modified by maolikui at 2015-11-17
    /// @}
        ...
        if (FeatureOption.MTK_AUDIO_PROFILES) {
            for (int i = 0; i < listSize; i++) {
                if (SOUND.equals(list.get(i).className)) {
                    ...
                    //list.remove(i); /*HQ_jiangchao modified for shortcut at 20151009*/
                    break;
                }
            }
	   //modified by maolikui at 2015-11-17 start 
	    TelephonyManager tm = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
            int SIMCount = tm.getSimCount();
            if(SIMCount == 1){
            for (int i = 0; i < listSize; i++) {
                if (SIM_SETTINGS.equals(list.get(i).className)) {
                    Log.d(TAG, "Not support SIM_SETTINGS ,remove it");
                    list.remove(i); 
                    break;
                }
            }
          }
          //modified by maolikui at 2015-11-17 end
        } else {
            for (int i = 0; i < listSize; i++) {
        ...
        ///M: ALPS01475094 remove DreamSettings when MTK_GMO_RAM_OPTIMIZE is true
