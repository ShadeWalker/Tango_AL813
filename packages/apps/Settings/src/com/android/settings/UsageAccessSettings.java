...
public class UsageAccessSettings extends SettingsPreferenceFragment implements
	...
	//add by HQ_caoxuhao at 20151001 HQ01410378 begin
	//add to change all icons in the same size
        private void updatePreference(PackageEntry pe) {
	    Drawable icon = pe.packageInfo.applicationInfo.loadIcon(mPackageManager);

	    //Log.i(TAG,"icon.getIntrinsicHeight() org = " + icon.getIntrinsicHeight());
	    //Log.i(TAG,"icon.getIntrinsicWidth() org = " + icon.getIntrinsicWidth());

	    icon = zoomDrawable(icon, 240, 240);
            //Log.i(TAG,"icon.getIntrinsicHeight() = " + icon.getIntrinsicHeight());
	    //Log.i(TAG,"icon.getIntrinsicWidth() = " + icon.getIntrinsicWidth());

            pe.preference.setIcon(icon);
	...
	//add by HQ_caoxuhao at 20151001 HQ01410378 end
