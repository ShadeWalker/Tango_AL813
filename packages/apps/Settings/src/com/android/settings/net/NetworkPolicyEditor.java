...
public class NetworkPolicyEditor {
    ...
    public void read() {
        ...
        Log.d(TAG, "read()... polices size: " + (policies == null ? 0 : policies.length));
    ...
    public NetworkPolicy getOrCreatePolicy(NetworkTemplate template) {
        ...
        if (policy == null) {
            ...
            Log.d(TAG, "getOrCreatePolicy()... policy: " + policy);
            mPolicies.add(policy);
        }
        return policy;
    }
    ...
    public NetworkPolicy getPolicy(NetworkTemplate template) {
        Log.d(TAG, "template: " + template + ", mPolices size() : " + mPolicies.size());
    ...
    private void dumpPolicys(NetworkPolicy[] policies) {
        if (policies != null) {
            for (NetworkPolicy networkPolicy : policies) {
                if (networkPolicy != null) {
                    Log.d(TAG, "networkPolicy: " + networkPolicy.toString());
                }
            }
        }
    }
