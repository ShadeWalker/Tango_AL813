...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    private TextView percentage_bar_text;
    ...
    public void init() {
        ...
        //HQ_wuhuihui_20151012 add for HQ01429893 start
        if (mVolume != null) {
            mUsageBarPreference.setVolume(mVolume);
        }
        //HQ_wuhuihui_20151012 add for HQ01429893 end
        mUsageBarPreference.setOrder(ORDER_USAGE_BAR);
        addPreference(mUsageBarPreference);

        //HQ_wuhuihui_2015_0619 modified for HQ01176163 start
        /*HQ_xupeixin at 2015-09-17 modified settings->storage page likes p8 begin*/
        if (mVolume != null && mVolume.isRemovable()) {
            //mItemTotal = buildItem(R.string.memory_size_total, 0);
            mItemTotal = buildItem(R.string.storage_internal_storage_title, 0);
        } else {
            //mItemTotal = buildItem(R.string.memory_size, 0);
            mItemTotal = buildItem(R.string.storage_internal_storage_title, 0);
        }
        /*HQ_xupeixin at 2015-09-17 modified end*/
        //HQ_wuhuihui_2015_0619 modified for HQ01176163 end
        ...
        mMountTogglePreference.setWidgetLayoutResource(R.layout.arrow_img_layout);
        if (isRemovable) {
    ...
    private void updatePreferencesFromState() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(state)
            ...
            addPreference(mUsageBarPreference);
            //addPreference(mItemTotal);    modify by HQ_zhouguo at 20150915
            //addPreference(mItemAvailable);
        } else {
        ...
        if (mUsbConnected && (UsbManager.USB_FUNCTION_MTP.equals(mUsbFunction) ||
            ...
            //HQ_zhouyoukun_2015_0821 modified for HQ01332024 start
          //  mMountTogglePreference.setEnabled(false);
            //HQ_zhouyoukun_2015_0821 modified for HQ01332024 end
	    if (SystemProperties.get("ro.hq_sdcard_usb_display").equals("1")) {
		mMountTogglePreference.setEnabled(false);
	    }
            if (Environment.MEDIA_MOUNTED.equals(state)
                    || Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)) {
    ...
    public void updateApproximate(long totalSize, long availSize) {
        ...
        mUsageBarPreference.updateSpace(formatSize(totalSize),formatSize(availSize));//chenwenshuai add for HQ01413789
    ...
    public void updateDetails(MeasurementDetails details) {
        ...
        if(mUsageBarPreference != null){
            mUsageBarPreference.updateSpace(formatSize(details.totalSize),formatSize(details.availSize));
        }
