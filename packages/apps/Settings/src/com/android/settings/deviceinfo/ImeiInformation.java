...
public class ImeiInformation extends PreferenceActivity {
    ...
    private static final String TAG = "ImeiInformation";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mExt = UtilsExt.getStatusExtPlugin(this);
    ...
    private void initPreferenceScreen(int slotCount) {
        ...
        for (int slotId = 0; slotId < slotCount; slotId ++) {
            ...
            setNewKey(slotId);
            ///M: Add for op01 single imei feature.
            mExt.customizeGeminiImei("_" + KEY_IMEI + String.valueOf(slotId),
                    "_" + KEY_IMEI_SV + String.valueOf(slotId),
                    getPreferenceScreen(), slotId);
        }
    }

    private void setPreferenceValue(int phoneId) {
        //HQ_wuhuihui_20150731 modified for HQ01283966 start
        Phone phone = null; //PhoneFactory.getPhone(phoneId);
        try {
            phone = PhoneFactory.getPhone(phoneId);
        } catch (IllegalStateException e) {
            Log.d(TAG, "Default phones haven't been made yet!");
        }
        //HQ_wuhuihui_20150731 modified for HQ01283966 end
        if (phone != null) {
            if (phone.getPhoneType() == TelephonyManager.PHONE_TYPE_CDMA) {
				...
				/*HQ_liugang modify for HQ01325146 20150813*/
				...
				String imei_sv = telephonyManager.getDeviceSoftwareVersion(phoneId);
				phoneId++;
				String prop = "gsm.imei" + phoneId;
				if (productName.equalsIgnoreCase("TAG-TL00")&& OperatorIsoCountry.equals("cn") && phoneId == 1){
	                setSummaryText(KEY_IMEI, SystemProperties.get(prop, null));
				    setSummaryText(KEY_IMEI_SV, imei_sv);
				} else if (!productName.equalsIgnoreCase("TAG-TL00") || (productName.equalsIgnoreCase("TAG-TL00")&& !OperatorIsoCountry.equals("cn"))){
	                setSummaryText(KEY_IMEI, SystemProperties.get(prop, null));
				    setSummaryText(KEY_IMEI_SV, imei_sv);				
				}
				/*End modify*/
