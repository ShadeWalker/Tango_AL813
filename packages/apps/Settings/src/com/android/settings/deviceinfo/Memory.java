...
//Add by wuhuihui for device info start
import android.text.format.Formatter;
import android.os.StatFs;
import android.os.Environment;
//Add by wuhuihui for device info end
...
public class Memory extends SettingsPreferenceFragment implements Indexable {
    ...
    private static final String KEY_ROM_INFO = "hw_rom_info";
    ...
    private Preference mCleanerPreference;
    ...
    public void onCreate(Bundle icicle) {
        ...
        //wuhuihui add for rom info start
        Preference romPreference = findPreference(KEY_ROM_INFO);
        StatFs stat;
        long romTotalSize;
        Log.d(TAG,"read sdcard0");
        stat = new StatFs("/storage/sdcard0");
        long blockSize = stat.getBlockSizeLong();
        long totalBlocks = stat.getBlockCountLong();
        long totalSize = blockSize * totalBlocks;
        if (totalSize > 8 * 1024 * 1024 * 1024L) {
            romTotalSize = 16 * 1024 * 1024 * 1024L;
        } else {
            romTotalSize = 8 * 1024 * 1024 * 1024L;
        }
        Log.d(TAG,"get primary volume total size:" + formatSize(totalSize));
        romPreference.setSummary(getActivity().getResources().getString(R.string.hw_rom_info_summary)
                                 + formatSize(romTotalSize));
        //wuhuihui add for rom info end
       ...
       for (StorageVolume volume : storageVolumes) {
            Log.d(TAG, "volume path:" + volume.getPath());
        ...
        addCleanerPreference(context);
        setHasOptionsMenu(true);
    }

    //add by majian for rom display
    private String formatSize(long size) {
       return Formatter.formatFileSize(getActivity(), size);
    }

    /*HQ_xupeixin at 2015-09-17 modified settings->storage page likes p8 begin*/
    public void addCleanerPreference(Context context) {
        mCleanerPreference = new Preference(context);
        //mCleanerPreference.setLayoutResource(R.layout.preference_storage_space_cleaner);
        mCleanerPreference.setTitle(R.string.storage_cleaner_preference_title);
        mCleanerPreference.setWidgetLayoutResource(R.layout.arrow_img_layout);
        Intent intent = new Intent();
        intent.setClassName("com.huawei.systemmanager", "com.huawei.systemmanager.spaceclean.spacemain.SpaceCleanActivity");
        mCleanerPreference.setIntent(intent);
        getPreferenceScreen().addPreference(mCleanerPreference);
    }
    /*modified by xupeixin at 2015-09-17 modified end*/
