...
public class LbsReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (FeatureOption.MTK_AGPS_APP && FeatureOption.MTK_GPS_SUPPORT) {
           ...
           if ((AGPS_OMACP_PROFILE_UPDATE).equals(action)) {
                handleAgpsOmaProfileUpdate(context, intent);
            } else if ((ACTION_OMA_CP).equals(action)) {
                handleOmaCpSetting(context, intent);
            } else if ((ACTION_OMA_CP_CAPABILITY).equals(action)) {
    ...
    private void handleOmaCpSetting(Context context, Intent intent) {
        ...
        int slotId = intent.getIntExtra("slotId", PhoneConstants.SIM_ID_1);
        ...
        try {
            ...
            //M: because the TTL port is Fixed and the message doesn't include the information about port number, we fix it.
    ...
    private void initSIMStatus(int slotId) {
        ...
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            ...
            if (TelephonyManager.SIM_STATE_READY == simStatus) {
                int subId = SubscriptionManager.getSubId(slotId)[0]; //TODO::MSIM
        ...
        Log.d(TAG, "SlotId : " + slotId   +  " Status : " + simStatus + " OperatorCode : " + mCurOperatorCode);
    }
