...
public class WifiGprsSelector extends WifiSettings implements Preference.OnPreferenceChangeListener {
    ...
    private boolean init() {         
        ...
        mIsSIMExist = mTelephonyManager.hasIccCard();//iTel.hasIccCard(PhoneConstants.GEMINI_SIM_1) ||
    ...
    private void initPhoneState() {
        ...
        Xlog.d(TAG, "GEMINI_SIM_ID_KEY = " + mSubId + ";");
    ...
    private void fillList(int subId) {
        ...
        Xlog.d(TAG, "fillList(), simSlot=" + subId + ";");
        String where;
        ...
        boolean simReady = (TelephonyManager.SIM_STATE_READY == mTelephonyManager.getSimState(slotId));
        ...
        if (KEY_ADD_WIFI_NETWORK.equals(key)) {
            ...
            .setSingleChoiceItems(mAdapter, mInitValue, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        Xlog.d(TAG,"mSubId = " + mSubId);
                        Xlog.d(TAG,"mIsSim=" + simItem.mIsSim + ",mState=" + simItem.mState
                            + ",SIM_INDICATOR_LOCKED=" + 1);

                        if (simItem.mIsSim) {
                            int state = mCellConnMgr.getCurrentState(simItem.mSubId, CellConnMgr.STATE_SIM_LOCKED);
    ...
    private void updateDataEnabler() {
        if (FeatureOption.MTK_GEMINI_SUPPORT) {
            Xlog.d(TAG,"updateDataEnabler, mSubId=" + mSubId);
            fillList(mSubId);
    ...
    public Dialog onCreateDialog(int id) {
        ...
        if (id == DIALOG_WAITING) {
            ...
            simStatusStrings = mCellConnMgr.getStringUsingState(mSelectedDataSubId, CellConnMgr.STATE_SIM_LOCKED);
