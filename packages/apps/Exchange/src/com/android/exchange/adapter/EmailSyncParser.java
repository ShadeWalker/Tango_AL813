...
public class EmailSyncParser extends AbstractSyncParser {
    ...
    private void bodyParser(EmailContent.Message msg) throws IOException {
        ...
        /**
         * M: What if a mail with bodyTruncated tag, but both body html and text are empty?
         * In this case, client should set a suitable load flag: FLAG_LOADED_COMPLETE.
         * @{
         */
        if (TextUtils.isEmpty(msg.mHtml)
                && TextUtils.isEmpty(msg.mText)) {
    ...
    public void commitImpl(int maxOpsPerBatch)
        ...
        if (mMailbox.mId == Exchange.sBadSyncKeyMailboxId
            ...
            Cursor c = mContentResolver.query(Message.CONTENT_URI, new String[] {
                    MessageColumns._ID, MessageColumns.TIMESTAMP },
                    selection, null, null);
            try {
                if (c != null) {
        ...
        for (Long id : deletedEmails) {
            /**
             * M: 1. The attachments were belonged to the com.android.email
             * application, delete attachments here, it would fail, so mask the
             * function call to AttachmentUtilities.deleteAllAttachmentFiles
             * here and do the attachment delete operation on Email provider.
             * 2. Add a new parameter (account id) to URI for delete attachments on
             * Email side.
             *
             * @{
             */
            ...
            builder.appendQueryParameter(AttachmentUtilities.KEY_ACCOUNT_ID, String.valueOf(mAccount.mId));
