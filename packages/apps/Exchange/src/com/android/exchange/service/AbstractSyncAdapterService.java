...
public abstract class AbstractSyncAdapterService extends Service {
    ...
    // In onPerformSync, if we don't yet have our EasService, we will wait for up to 20
    // seconds for it to appear. If it takes longer than that, we will fail the sync.
    /// M: adjust from 10 to 20 seconds.
    private static final long MAX_WAIT_FOR_SERVICE_MS = 20 * DateUtils.SECOND_IN_MILLIS;
        ...
        synchronized(mConnection) {
            if (mEasService == null) {
                ...
                try {
                    ...
                    /// M: Return false out and just logging for this error is harmless
