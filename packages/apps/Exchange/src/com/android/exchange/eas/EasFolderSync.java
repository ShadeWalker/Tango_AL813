...
public class EasFolderSync extends EasOperation {
    ...
    public int performOperation() {
        if (mStatusOnly) {
            ...
            if (EmailContent.isInitialSyncKey(cacheAccount.mSyncKey)) {
                Account account = loadAccount(mContext, cacheAccount.getId());
                if (account == null
                        || !EmailContent.isInitialSyncKey(account.mSyncKey)) {
                    LogUtils.w(LOG_TAG,
                            "Skip dirty initial FolderSync for account [%s]", getAccountId());
