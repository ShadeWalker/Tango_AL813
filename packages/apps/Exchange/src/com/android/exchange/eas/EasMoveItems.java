...
public class EasMoveItems extends EasOperation {
    ...
    public int upsyncMovedMessages() {
        ...
        for (final MessageMove move : moves) {
            ...
            lastId = (mMove.getLastId() > lastId) ? mMove.getLastId() : lastId;
        }

        final ContentResolver cr = mContext.getContentResolver();
        /** M: Only move records less than the lastId could be deleted in this turn @{ */
        MessageMove.upsyncSuccessful(cr, messageIds[0], counts[0], lastId);
        MessageMove.upsyncFail(cr, messageIds[1], counts[1], lastId);
        MessageMove.upsyncRetry(cr, messageIds[2], counts[2], lastId);
        /** M: @} */
    ...
    private void processResponse(final MessageMove request, final MoveResponse response) {
        ...
        if (response.sourceMessageId == null) {
            ...
            LogUtils.e(LOG_TAG,
                    "MoveItems response for message %d has no SrcMsgId, using request's server id[%s]",
                    request.getMessageId(), request.getServerId());
            ...
            if (!sourceMessageId.equals(request.getServerId())) {
                ...
                LogUtils.e(LOG_TAG,
                        "MoveItems response for message %d has SrcMsgId != request's server id[%s]",
                        request.getMessageId(), request.getServerId());
            }
        }
        ...
        if (response.moveStatus == MoveItemsParser.STATUS_CODE_REVERT) {
            LogUtils.w(LOG_TAG, "MoveItems Revert to original mailbox [%d] with serverId[%s]",
                    request.getSourceFolderKey(), request.getSourceFolderId());
