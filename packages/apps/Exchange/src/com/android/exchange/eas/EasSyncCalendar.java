...
public class EasSyncCalendar extends EasSyncCollectionTypeBase {
    ...
    private void sendEvent(final Context context, final Account account, final Entity entity,
        ...
        for (final Entity.NamedContentValues ncv: subValues) {
            ...
            if (ncvUri.equals(ExtendedProperties.CONTENT_URI)) {
                ...
                /// M: avoid NPE, since propertyName maybe null. @{
                if (EXTENDED_PROPERTY_CATEGORIES.equals(propertyName)) {
                /// @}
    ...
    private void handleExceptionsToRecurrenceRules(final Serializer s, final Context context,
        ...
        /// M: Close the cursor to avoid cursor leak. @{
        try {
            ...
            while (exIterator.hasNext()) {
                ...
                if (getInt(exValues, Events.DIRTY) == 1) {
                    // This is a new/updated exception, so we've got to notify
                    // our attendees about it
                    ...
                    if ((getInt(exValues, Events.DELETED) == 1)
                        ...
                        if (!selfOrganizer) {
                            ...
                            // Since CalendarProvider2 sets the organizer of
                            // exceptions to the user, we have to reset it first
                            // to the original organizer
                            exValues.put(Events.ORGANIZER,
                                    entityValues.getAsString(Events.ORGANIZER));
                            sendDeclinedEmail(context, account, exEntity,
                                    clientId);
                        }
                    } else {
                        flag = Message.FLAG_OUTGOING_MEETING_INVITE;
                    }
                    // Add the eventId of the exception to the uploaded id list,
                    // so that the dirty/mark bits are cleared
                    mUploadedIdList.add(exEventId);

                    // Copy version so the ics attachment shows the proper
                    // sequence #
                    ...
                    if (selfOrganizer) {
                        final Message msg = CalendarUtilities
                                .createMessageForEntity(context, exEntity,
                                        flag, clientId, account);
                        if (msg != null) {
                            LogUtils.d(TAG, "Queueing exception update to %s",
                                    msg.mTo);
                            mOutgoingMailList.add(msg);
                        }

                        // Also send out a cancellation email to removed
                        // attendees
                        ...
                        for (final Entity.NamedContentValues ncv : exEntity
                                .getSubValues()) {
                            if (ncv.uri.equals(Attendees.CONTENT_URI)) {
                                exAttendeeEmails.add(ncv.values
                                        .getAsString(Attendees.ATTENDEE_EMAIL));
                            }
                        }
                        // Find the ones left out from the previous event and
                        // add them to the new entity
                        for (final Entity.NamedContentValues ncv : entity
                                .getSubValues()) {
                            if (ncv.uri.equals(Attendees.CONTENT_URI)) {
                                final String attendeeEmail = ncv.values
                                        .getAsString(Attendees.ATTENDEE_EMAIL);
                                if (!exAttendeeEmails.contains(attendeeEmail)) {
                                    removedEntity.addSubValue(ncv.uri,
                                            ncv.values);
                                }
                            }
                        }
                        ...
                        final Message removedMessage = CalendarUtilities
                                .createMessageForEntity(context, removedEntity,
                                        Message.FLAG_OUTGOING_MEETING_CANCEL,
                                        clientId, account);
                        if (removedMessage != null) {
                            LogUtils.d(TAG,
                                    "Queueing cancellation for removed attendees");
    ...
    private void updateAttendeesAndSendMail(final Context context, final Account account,
        ...
        for (final Entity.NamedContentValues ncv: entity.getSubValues()) {
            if (ncv.uri.equals(ExtendedProperties.CONTENT_URI)) {
                final ContentValues ncvValues = ncv.values;
                final String propertyName = ncvValues.getAsString(ExtendedProperties.NAME);
                /// M: propertyName may be null. @{
                if (EXTENDED_PROPERTY_ATTENDEES.equals(propertyName)) {
                    attendeeString = ncvValues.getAsString(ExtendedProperties.VALUE);
                    attendeeStringId = ncvValues.getAsLong(ExtendedProperties._ID);
                } else if (EXTENDED_PROPERTY_USER_ATTENDEE_STATUS.equals(propertyName)) {
    ...
    private boolean handleEntity(final Serializer s, final Context context, final Account account,
        ...
        for (final Entity.NamedContentValues ncv: entity.getSubValues()) {
            if (ncv.uri.equals(ExtendedProperties.CONTENT_URI)) {
                final ContentValues ncvValues = ncv.values;
                /// M: avoid NPE, since propertyName maybe null. @{
                if (EXTENDED_PROPERTY_UPSYNC_PROHIBITED.equals(ncvValues
