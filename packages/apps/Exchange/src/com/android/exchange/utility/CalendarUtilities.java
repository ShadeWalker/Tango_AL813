...
public class CalendarUtilities {
    ...
    static public String timeZoneToTziString(TimeZone tz) {
        ...
        if (tziString != null) {
            if (Eas.USER_LOG) {
                LogUtils.d(TAG, "TZI string for %s", tz.getDisplayName() + " found in cache.");
            }
            return tziString;
        }
    ...
    static TimeZone tziStringToTimeZone(String timeZoneString, int precision) {
        ...
        if (timeZone != null) {
            if (Eas.USER_LOG) {
                LogUtils.d(TAG, " Using cached TimeZone %s", timeZone.getID());
            }
        } else {
            ...
            if (timeZone == null) {
                ...
                LogUtils.d(TAG, "TimeZone not found using default: %s", timeZoneString);
        ...
        if (zoneIds.length > 0) {
            ...
            if (dstEnd == null) {
                ...
                if (!defaultTimeZone.useDaylightTime() &&
                    ...
                    if (Eas.USER_LOG) {
                        LogUtils.d(TAG, "TimeZone without DST found to be default: %s",
                ...
                for (String zoneId: zoneIds) {
                    ...
                    if (!timeZone.useDaylightTime()) {
                        if (Eas.USER_LOG) {
                            LogUtils.d(TAG, "TimeZone without DST found by offset: %s",
                                    timeZone.getID());
                        }
                        return timeZone;
                    }
                ...
                if (Eas.USER_LOG) {
                    LogUtils.d(TAG,
                            ...
                            (name ? "name" : (lenient ? "lenient" : "first")) + ": %s",
                                    timeZone.getID());
                }
                return timeZone;
            }
    ...
    static public String rruleFromRecurrence(int type, int occurrences, int interval, int dow,
        ...
        if (Eas.USER_LOG) {
            LogUtils.d(Logging.LOG_TAG, "Created rrule: %s", rrule);
        }
