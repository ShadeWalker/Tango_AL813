...
import com.mediatek.contacts.ext.IContactAccountExtension.FeatureName;
...
public abstract class BaseAccountType extends AccountType {
    ...
    protected DataKind addDataKindSipAddress(Context context) throws DefinitionException {
        /// M:[for op09] fix CR: ALPS01284180 @{
        if (!ExtensionManager.getInstance().getContactAccountExtension()
                .enableFeature(null/**not special account type for all accouts**/,
                        FeatureName.VOIP)) {
            return null;
        }
        // @}
