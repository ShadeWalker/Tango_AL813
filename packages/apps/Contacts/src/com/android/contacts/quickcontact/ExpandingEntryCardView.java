...
public class ExpandingEntryCardView extends CardView {
    ...
    public static final class Entry {
		...
		//add by zhangjinqiang for call_duration\
        ...
        public Entry(int id, Drawable mainIcon, String header, String subHeader,
                Drawable subHeaderIcon, String text, String duration,int callType,int subId,Drawable textIcon,
            ...
            this(id, mainIcon, header, subHeader, subHeaderIcon, text, duration,callType,subId,textIcon, null, null,
...
//    private CooTekSmartdialerOemModule csom;
    ...
    public ExpandingEntryCardView(Context context, AttributeSet attrs) {
        ...
        //HQ_wuruijun add for HQ01359238 & HQ01369041 start
//        csom = PeopleActivity.getCsom();
    ...
    private void insertEntriesIntoViewGroup() {
        ...
        /// M: Bug fix ALPS01771310, google issue, google's removeView method is not standard.
    ...
    //HQ_wuruijun add for HQ01352450 start
    ...
    //HQ_wuruijun add for HQ01359238 & HQ01369041 start
//    public String entryNumberLocation(String s) {
//        if (null == csom) {
//            Log.d(TAG, "ExpandingEntryCardView null == csom !!!");
//            return null;
//        }
//        csom.setNetworkAccessible(true);
//        return csom.getPhoneAttribute(s);
//    }
...
//        Toast.makeText(getContext(), "号码助手Service通信连接失败！！", Toast.LENGTH_LONG).show();
//    }
    //HQ_wuruijun add end

    //HQ_wuruijun add for HQ01430339 start
    ...
    //HQ_wuruijun add for HQ01435331 start
    ...
    private View createEntryView(LayoutInflater layoutInflater, final Entry entry,
            int iconVisibility) {
		//add by xiatao ------------------
        /* begin: add by donghongjing for HQ01410797 */
        boolean isCallLog = false;
        if (!TextUtils.isEmpty(entry.getText())) {
            String date = entry.getText();
            int hour_index = date.lastIndexOf(" ");
            int min_index = date.lastIndexOf(":");
            if (hour_index != -1 && min_index != -1) {
                isCallLog = true;
            }
        }
        /* end: add by donghongjing for HQ01410797 */
		...
		if(entry.getAlternateIcon()!=null){
            ...
            /* begin: change by donghongjing for HQ01410797 */
            if (isCallLog) {
			    view = (EntryView)layoutInflater.inflate(
                    R.layout.expanding_entry_call_log_item, this, false);
            } else {
			    view = (EntryView)layoutInflater.inflate(
                    R.layout.expanding_entry_card_item, this, false);
            }
            /* end: change by donghongjing for HQ01410797 */
		}
		//end xiatao
        ...
        //header.setTextSize(16);//add  by niubi tang for fucking huawei ui
        ...
        if (!TextUtils.isEmpty(entry.getSubHeader())) {
        	String subName=entry.getSubHeader();
        	if (mFamily.contains(subName)) {
        		subName= getContext().getResources().getString(R.string.family_group);
    		} else if (mColleague.contains(subName)) {
    			subName= getContext().getResources().getString(R.string.colleague_group);
    		} else if (mFriend.contains(subName)) {
    			subName= getContext().getResources().getString(R.string.friend_group);
    		} else if (mSchoolmate.contains(subName)) {
    			subName= getContext().getResources().getString(R.string.schoolmate_group);
    		} 
        	
            subHeader.setText(subName);
        } else {
...
//            //HQ_wuruijun modify for HQ01352450 start
//            String date = entry.getText();
//            int hour_index = date.lastIndexOf(" ");
//            int min_index = date.lastIndexOf(":");
//            if (hour_index == -1 || min_index == -1) {
//                if (!TextUtils.isEmpty(entry.getHeader())) {
//                    String mHeader = entry.getHeader();
//                    if (!isEmail(mHeader) && !isZipNO(mHeader)) {
//                        String lo =  entryNumberLocation(mHeader);
...
//        }
        ...
        if (entry.getSimIcon() != null) {
		   int slotId = SubscriptionManager.getSlotId(entry.getSubId());
		   if(slotId == 0){
           	 simIcon.setBackgroundResource(R.drawable.sim_one);
		   }else if(slotId == 1){
			 simIcon.setBackgroundResource(R.drawable.sim_two);
		   }
        } else {
	    ...
	    final TextView callTimeText = (TextView) view.findViewById(R.id.call_time);
        if (!TextUtils.isEmpty(entry.getDuration())) {
			String time = formatLongTime(entry.getDuration());
			...
			if (isTL) {
			    ...
			    //add by zhangzhihua for HQ02065728 --start
			    if(!time.equals(INVALID_INT)){
				if(entry.getCallType()== Calls.INCOMING_TYPE){
					
					callTimeText.setText(getContext().getString(R.string.incoming_type)+time);
				}else if(entry.getCallType()==Calls.OUTGOING_TYPE){
					callTimeText.setText(getContext().getString(R.string.outgoing_calltype)+time);
				}else if(entry.getCallType()== Calls.MISSED_TYPE) {
					callTimeText.setText(getContext().getString(R.string.missed_type)+time);
				}
			    	}
				//add by zhangzhihua for HQ02065728 --end
			}
        } else {
            callTimeText.setVisibility(View.GONE);
        }
        ...
        final TextView alternateIcon = (TextView) view.findViewById(R.id.icon_alternate);
        ...
        if (entry.getAlternateIcon() != null && entry.getAlternateIntent() != null) {
			...
			// / Commented out by guofeiyao
            //alternateIcon.setText(getContext().getString(R.string.send_sms));
            // / End
...
//add by zhangjinqiang for al812 call duration --start
//add by zhangzhihua for HQ02065728 --start
	private static final String INVALID_INT = "invalid int";
	private String formatLongTime(String stringTime){
		try{
		...
		String hour_hw = getResources().getString(R.string.hour_hw);
		String minute_hw = getResources().getString(R.string.minute_hw);
		String second_hw = getResources().getString(R.string.seconds_hw);
		//return  hour +hour_hw + min + minute_hw+sec +second_hw;
		//dongkai 当前系统语言为法语的时候加空格
		Locale locale = getResources().getConfiguration().locale;
		String language = locale.getLanguage();
		if (language.endsWith("fr")){
			return   "  "+min+minute_hw+sec+second_hw;
		}else{
			return   min+minute_hw+sec+second_hw;
		}
			}catch(NumberFormatException e){
			    Log.i("zzh", "invalid number");
			}
			return INVALID_INT;
		//add by zhangzhihua for HQ02065728 --end
		
	}
//add by zhangjinqiang for al812 call duration --end
    ...
    private static final class EntryTouchListener implements View.OnTouchListener {
        ...
        private final TextView mAlternateIcon;
        ...
        public EntryTouchListener(View entry, TextView alternateIcon, ImageView thirdIcon) {
            mEntry = entry;
            mAlternateIcon = alternateIcon;
            mThirdIcon = thirdIcon;
