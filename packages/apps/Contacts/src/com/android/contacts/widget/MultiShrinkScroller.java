...
//						add by niubi tanghuaizhe for f*g huawei UI start						
						 mMaximumHeaderHeight = mPhotoViewContainer.getWidth()/2+36;
                    mMinimumHeaderHeight = mMaximumHeaderHeight ;
//						add by niubi tanghuaizhe for f*g huawei UI end						
...
//                + largeTextLayoutParms.bottomMargin) * TITLE_GRADIENT_SIZE_COEFFICIENT);
    ...
    public void scrollUpForEntranceAnimation(boolean scrollToCurrentPosition) {
        ...
        final int desiredValue = currentPosition + (scrollToCurrentPosition ? currentPosition
         ...
         /*add by liruihong to cancel animator when is full screen mode begin*/
	 if(!scrollToCurrentPosition){
		setScroll(desiredValue);
		return;
	 }		 
         /*add by liruihong to cancel animator when is full screen mode begin*/
...
//        mLargeTextView.setScaleY(scale);
 			mLargeTextView.setScaleX(0.4f);
          mLargeTextView.setScaleY(0.9f);
