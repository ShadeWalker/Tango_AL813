...
public class CellBroadcastAlertService extends Service {
    ...
    private static final class MessageServiceCategoryAndScope {
        ...
        MessageServiceCategoryAndScope(int serviceCategory, int serialNumber,
                SmsCbLocation location, boolean isPrimary) {
        ...
        public String toString() {
            return "{mServiceCategory: " + mServiceCategory + " serial number: " + mSerialNumber +
                    " location: " + mLocation.toString() + " isPrimary " + mIsPrimary + '}';
        }
    }
    ...
    private void handleCellBroadcastIntent(Intent intent) {
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            //int slotId = intent.getIntExtra(PhoneConstants.SLOT_KEY/*GEMINI_SIM_ID_KEY*/, -1);
        	subId = intent.getIntExtra(PhoneConstants.SUBSCRIPTION_KEY/*GEMINI_SIM_ID_KEY*/, -1);
            Xlog.d(TAG, "subid:"+subId);
            //SIMInfo si = SIMInfo.getSIMInfoBySlot(this.getApplicationContext(), slotId);
            //List<SubInfoRecord> si = SubscriptionManager.getSubInfoUsingSlotId(slotId);
            //if (si != null) {
                Xlog.d(TAG, "sub-id from intent is:"+ subId);
                //simId = 0; //(int)si.mSimId;
                //subId = si.get(0).mSubId;
           // }
        }
        ...
        MessageServiceCategoryAndScope newCmasId = new MessageServiceCategoryAndScope(
                message.getServiceCategory(), message.getSerialNumber(), message.getLocation(), isPrimary);
    ...
    private void openEmergencyAlertNotification(CellBroadcastMessage message) {
        ...
        if (message.isCmasMessage()) {
            ...
            duration = 10500;
        } else if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            duration = Integer.parseInt(prefs.getString(CellBroadcastSettings.KEY_ALERT_SOUND_DURATION+"_"+message.getSubId(),
                                        CellBroadcastSettings.ALERT_SOUND_DEFAULT_DURATION)) * 1000;
            vibrate = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_VIBRATE + "_" + message.getSubId()/*getSimId()*/, true);
        } else {
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            audioIntent.putExtra("subscription", message.getSubId()/*getSimId()*/);
        }
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            enableAlertSpeech = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_SPEECH
                    + "_" + message.getSubId()/*getSimId()*/, CellBroadcastSettings.ENABLE_ALERT_SPEECH_DEFAULT);
        } else {
            enableAlertSpeech = prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ALERT_SPEECH,
    ...
    private boolean isMessageEnabledByUserGemini(CellBroadcastMessage message) {
        ...
        if (message.isEtwsTestMessage()) {
            return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_ETWS_TEST_ALERTS
                    + "_" + message.getSubId()/*getSimId()*/, CellBroadcastSettings.ENABLE_ETWS_TEST_ALERTS_DEFAULT);
        }

        if (message.isCmasMessage()) {
            switch (message.getCmasMessageClass()) {
                case SmsCbCmasInfo.CMAS_CLASS_EXTREME_THREAT:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_EXTREME_THREAT_ALERTS+"_"+message.getSubId()/*getSimId()*/,
                                        CellBroadcastSettings.ENABLE_CMAS_EXTREME_THREAT_ALERTS_DEFAULT);
                case SmsCbCmasInfo.CMAS_CLASS_SEVERE_THREAT:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_SEVERE_THREAT_ALERTS+"_"+message.getSubId()/*getSimId()*/,
                                        CellBroadcastSettings.ENABLE_CMAS_SEVERE_THREAT_ALERTS_DEFAULT);
                case SmsCbCmasInfo.CMAS_CLASS_CHILD_ABDUCTION_EMERGENCY:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_AMBER_ALERTS+"_"+message.getSubId()/*getSimId()*/,
                                        CellBroadcastSettings.ENABLE_CMAS_AMBER_ALERTS_DEFAULT);
                case SmsCbCmasInfo.CMAS_CLASS_REQUIRED_MONTHLY_TEST:
                case SmsCbCmasInfo.CMAS_CLASS_CMAS_EXERCISE:
                    return prefs.getBoolean(CellBroadcastSettings.KEY_ENABLE_CMAS_TEST_ALERTS+"_"+message.getSubId()/*getSimId()*/,
