...
public class CellBroadcastAlertReminder extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // add for gemini
        int subId = intent.getIntExtra("subscription", 0);
    ...
    static boolean queueAlertReminder(Context context, boolean firstTime, int subId) {
        ...
        log("queueAlertReminder firstTime " + firstTime + " subid " + subId);
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        String prefStr;
        // add for gemini
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1")) {
            prefStr = prefs.getString(CellBroadcastSettings.KEY_ALERT_REMINDER_INTERVAL + "_" + subId, null);
        } else {
        ...
        playIntent.putExtra("subscription", subId);
