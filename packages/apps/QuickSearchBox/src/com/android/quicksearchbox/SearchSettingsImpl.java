...
public class SearchSettingsImpl implements SearchSettings {
     ...
     * @return the saved SearchEngine favicon.
    ...
    /**
     * M: Get the default SearchEngine favicon.
     * @return the default SearchEngine favicon
     */
    public String getDefaultSearchEngineFavicon() {
        ...
        SearchEngine temp = searchEngineManager.getDefault();
        if (temp == null) {
            Log.d(TAG, "getDefaultSearchEngineFavicon(), getDefault is null.");
            return "";
        }

        String favicon = temp.getFaviconUri();

        IPreferenceSetting handler = MPlugin.createInstance(IPreferenceSetting.class.getName(), mContext);
        if (handler != null) {
            SearchEngine engine = handler.getDefaultSearchEngine(mContext);
            if (engine != null) {
                favicon = engine.getFaviconUri();
            }
        } else {
            Log.d(TAG, "getDefaultSearchEngineFavicon(), get IPreferenceSetting instance from MPlugin failed.");
        }
     ...
     * @return the SearchEngine favicon.
     *         if null, return getDefaultSearchEngineFavicon().
     */
    public String getSearchEngineFaviconByName(String name) {
        SearchEngineManager searchEngineManager = (SearchEngineManager) mContext
        .getSystemService(Context.SEARCH_ENGINE_SERVICE);
        return searchEngineManager.getByName(name) != null
                ? searchEngineManager.getByName(name).getFaviconUri()
     ...
     * @return the SearchEngine name
     */
    public String getSearchEngineNameByFavicon(String favicon) {
        SearchEngineManager searchEngineManager = (SearchEngineManager) mContext
        .getSystemService(Context.SEARCH_ENGINE_SERVICE);
        SearchEngine searchEngineInfo
                = searchEngineManager.getBestMatch("", favicon) != null
                        ? searchEngineManager.getBestMatch("", favicon)
                        : searchEngineManager.getBestMatch("",
                                getDefaultSearchEngineFavicon());
