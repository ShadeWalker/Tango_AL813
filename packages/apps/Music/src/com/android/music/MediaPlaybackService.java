...
public class MediaPlaybackService extends Service {
    ...
    public void open(long [] list, int position) {
        synchronized (this) {
            /// M: If there is TRACK_WNET_TO_NEXT msg hasn't be dealt when open song,clear it.ALPS01768949 @{
            ...
            if (mediaPlayerHandler != null && mediaPlayerHandler.hasMessages(TRACK_WENT_TO_NEXT)) {
                MusicLogUtils.d(TAG, "removeMessages(TRACK_WENT_TO_NEXT)");
    ...
    private void openCurrentAndNext() {
        synchronized (this) {
            if(mLossesBTEnable){
                MusicLogUtils.d(TAG, "openCurrentAndNext(),lossBTEnable,sleep 100");
                SystemClock.sleep(100);
            }
            if (mCursor != null) {
                mCursor.close();
    ...
    private void setNextTrack() {
        ...
        if (mPlayListLen == 0) {
            MusicLogUtils.d(TAG, "setNextTrack with an empty playlist!");
            return;
        }
       ...
       //     just return, to avoid it will be play twice.
        ...
        MusicLogUtils.d(TAG, "afterSize = " + afterSize + ", preSize = " + preSize);
        if (afterSize > 0) {
    ...
    private void gotoIdleState() {
        ...
        Message msg = mDelayedStopHandler.obtainMessage();
        mDelayedStopHandler.sendMessageDelayed(msg, IDLE_DELAY);
        /// M: when music is removed form recent app,and do stopForeground,music process will be kill.
            ...
            if (gotonext) {
                if (mPlayListLen == 0) {
                    ...
                    if (mQuietMode && mPlayPos == -1 && mPlayListLen == 0) {
                        MusicLogUtils.d(TAG, "removeTracksInternal with no audio in playlist, set mQuietMode to false");
                        mQuietMode = false;
                    }
                }
            }
    ...
    private class MultiPlayer {
        ...
        private void handlePlaySongFail(MediaPlayer mediaPlayer) {
            ...
            if (mPlayPos < 0) {
                MusicLogUtils.e(TAG, "handlePlaySongFail with mPlayPos is -1, return");
                return;
            }
            if (mShuffleMode == SHUFFLE_NORMAL && mOpenFailedCounter++ < OPEN_FAILED_MAX_COUNT
                    && mPlayListLen > 1 && mHistory.size() >= mPlayListLen) {
                MusicLogUtils.e(TAG, "the last song play failed at shuffle model");
        ...
        MediaPlayer.OnPreparedListener preparedlistener = new MediaPlayer.OnPreparedListener() {
            public void onPrepared(MediaPlayer mp) {
                synchronized (MediaPlaybackService.this) {
                    ...
                    MusicLogUtils.v(TAG, "mQuietMode: " + mQuietMode + ",mIsSupposedToBePlaying:" + mIsSupposedToBePlaying);
                    if (!mQuietMode) {
                        if(mNeedSeekCauseLossBT){
                            MusicLogUtils.v(TAG, "onPrepared: mNeedSeekCauseLossBT: " + mNeedSeekCauseLossBT);
                        ...
                        if(mNeedSeekCauseLossBT){
                            MusicLogUtils.v(TAG, "mQuietMode==true,onPrepared: mNeedSeekCauseLossBT: " + mNeedSeekCauseLossBT);
                    ...
                    /// @}
                    MusicLogUtils.d(TAG, "mLossesBTEnablee = " + mLossesBTEnable);
                    if (!mLossesBTEnable) {
                        setNextTrack();
                    }
    ...
    private class AlbumArtWorker extends AsyncTask<Long, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Long... albumId) {
            ...
            MusicLogUtils.d(TAG, "AlbumArtWorker: getArtwork returns " + bm + " for album " + id);
            return bm;
        }
        ...
        protected void onPostExecute(Bitmap bm) {
            MusicLogUtils.d(TAG, "AlbumArtWorker.onPostExecute: " + mIsSupposedToBePlaying);
            if (mIsSupposedToBePlaying) {
