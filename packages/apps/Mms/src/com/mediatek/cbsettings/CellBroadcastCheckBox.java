...
import android.os.SystemProperties;//add by zhangjing
...
public class CellBroadcastCheckBox extends CheckBoxPreference {
    ...
    public void init(int subId, TimeConsumingPreferenceListener listener, boolean skipReading) {
        ...
        MmsLog.d(LOG_TAG, "LP pMccMnc = " + pMccMnc);
	...
	/*HQ_2015-11-30 modified for cellbroadcast for al813 begin*/
	...
	private boolean isShouldCheckd( Context context,String mccMnc ){
		Boolean shouldChecked = true;
		...
		MmsLog.d(LOG_TAG,"cellbroadcastcheckbox should check:" + shouldChecked);
		return shouldChecked;
	}
	private boolean shouldOpenForSouthEast(Context context,String mccMnc ){
		boolean defaultOpen  = false;
		int shouldOpen = Settings.System.getInt(context.getContentResolver(),
				"should_open", 0);
		defaultOpen = (shouldOpen == 1);
		
		MmsLog.d(LOG_TAG,"cellbroadcastcheckbox for southeast should check:" + defaultOpen);
		return defaultOpen;
		
	}
	/*HQ_2015-11-30 modified for cellbroadcast for al813 end*/
            ...
            new Thread(new Runnable() {
                public void run() {
                        RecoverChannelSettings setting =
                                new RecoverChannelSettings(mSubId, getContext().getContentResolver());
