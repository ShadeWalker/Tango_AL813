...
public class SlideEditorActivity extends Activity implements IMmsTextSizeAdjustHost {
    ...
    private void updateTitle() {
    	if ( locale.startsWith("fa")) {
    		setTitle(getString(R.string.slide_show_part, mSlideshowModel.size(), (mPosition + 1)));
		}
		else{
    ...
    private void showDurationDialog() {
        ...
        // add by wangmingyue for ar langruage
		if (locale.equals("ar") || locale.equals("fa")||locale.startsWith("iw")) {
			builder.setTitle(title+mSlideshowModel.size()+ "/"+ (mPosition + 1));
		}
		else{
			builder.setTitle(title + (mPosition + 1) + "/" + mSlideshowModel.size());
		}
		// add by wangmingyue end
    ...
    */
        ...
        if (mSlideshowModel.get(mPosition) != null) {
            if (mSlideshowModel.get(mPosition).hasAudio()) {
                ...
                if (hasContent && hasRight) {
                    mDrmAudioLock.setImageDrawable(res.getDrawable(MessageResource.drawable.drm_green_lock));
                    mDrmAudioLock.setVisibility(View.VISIBLE);
                } else if (hasContent) {
                    mDrmAudioLock.setImageDrawable(res.getDrawable(MessageResource.drawable.drm_red_lock));
                    mDrmAudioLock.setVisibility(View.VISIBLE);
                }
            }
            if (mSlideshowModel.get(mPosition).hasImage()) {
                ...
                if (hasContent && hasRight) {
                      mDrmImageVideoLock.setImageDrawable(res.getDrawable(MessageResource.drawable.drm_green_lock));
                      mDrmImageVideoLock.setVisibility(View.VISIBLE);
                } else if (hasContent) {
                    mDrmImageVideoLock.setImageDrawable(res.getDrawable(MessageResource.drawable.drm_red_lock));
                    mDrmImageVideoLock.setVisibility(View.VISIBLE);
                }
            }
    ...
    private void showSizeDisplay() {
        ...
        //modify by wangingyue for mms ui begin
        String mediaSize = this.getResources().getString(R.string.media_size_unit);
        mTextView.setText(showSize + mediaSize+"/" + mSizeLimit + mediaSize);
        //modify by wangingyue for mms ui end
        showDrmLock();
    }
    /// @}
    ...
    private void initPlugin(Context context) {
        mMmsTextSizeAdjustPlugin = (IMmsTextSizeAdjustExt) MmsPluginManager.getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_TEXT_SIZE_ADJUST);
    }
