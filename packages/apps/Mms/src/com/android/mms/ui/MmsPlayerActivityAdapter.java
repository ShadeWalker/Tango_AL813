...
public class MmsPlayerActivityAdapter extends BaseAdapter {
    ...
    private IMmsUnsupportedFilesExt mMmsUnsupportedFilesPlugin = null;
    ...
    public View getView(int arg0, View arg1, ViewGroup arg2) {
        ...
        if (imageUri != null) {
            ...
            if (MmsConfig.isUnsupportedFilesOn()) {
                if (!mMmsUnsupportedFilesPlugin.isSupportedFile(imageType, null)) {
                    mMmsUnsupportedFilesPlugin.setImageUnsupportedIcon(mImage, imageType, null);
                    mMmsUnsupportedFilesPlugin.setUnsupportedMsg((LinearLayout) mImage.getParent()
                            .getParent(), (View) mImage.getParent(), true);
                    MmsLog.d(TAG, "add image unsupported view");
                } else {
                    MmsLog.d(TAG, "remove image unsupported view");
                    mMmsUnsupportedFilesPlugin.setUnsupportedMsg((LinearLayout) mImage.getParent(),
                        (View) mImage.getParent(), false);
                }
            }
            /// @}
        } else {
        ...
        if (t != null) {
            ...
            if (MmsConfig.isUnsupportedFilesOn()) {
                if (!mMmsUnsupportedFilesPlugin.isSupportedFile(videoType, null)) {
                    mMmsUnsupportedFilesPlugin.setVideoUnsupportedIcon(mVideo, videoType, null);
                    mMmsUnsupportedFilesPlugin.setUnsupportedMsg((LinearLayout) (mVideo.getParent()
                            .getParent().getParent()), (View) (mVideo.getParent().getParent()),
                        true);
                    MmsLog.d(TAG, "add video unsupported view");
                } else {
                    MmsLog.d(TAG, "remove video unsupported view");
                    mMmsUnsupportedFilesPlugin.setUnsupportedMsg((LinearLayout) (mVideo.getParent()
                            .getParent().getParent()), (View) (mVideo.getParent().getParent()),
                        false);
                }
        ...
        if (audioName != null) {
            ...
            if (MmsConfig.isUnsupportedFilesOn()) {
                if (!mMmsUnsupportedFilesPlugin.isSupportedFile(audioType, audioName)) {
                    mMmsUnsupportedFilesPlugin.setAudioUnsupportedIcon(mAudioIcon, audioType,
                        audioName);
                    mMmsUnsupportedFilesPlugin.setUnsupportedMsg((LinearLayout) itemView
                            .getChildAt(0), mAudio, true);
                    MmsLog.d(TAG, "add audio unsupported view");
                } else {
                    MmsLog.d(TAG, "remove audio unsupported view");
                    mMmsUnsupportedFilesPlugin.setUnsupportedMsg((LinearLayout) itemView
                            .getChildAt(0), mAudio, false);
                }
            }
            /// @}
        } else {
    ...
    class mediaClick implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (MessageUtils.checkUriContainsDrm(MmsPlayerActivityAdapter.this.mContext, mUri)) {
                MessageUtils.showDrmAlertDialog(MmsPlayerActivityAdapter.this.mContext);
                return;
            }
            /// @}
            setIsClick(true);
    ...
    private void initPlugin(Context context) {
        /// M: OP09 Feature: UnsupportedFiles; @{
        mMmsUnsupportedFilesPlugin = (IMmsUnsupportedFilesExt) MPlugin.createInstance(
            IMmsUnsupportedFilesExt.class.getName(), context);
        if (mMmsUnsupportedFilesPlugin == null) {
            mMmsUnsupportedFilesPlugin = new DefaultMmsUnsupportedFilesExt(context);
            MmsLog.d(TAG, "default mMmsUnsupportedFilesPlugin = " + mMmsUnsupportedFilesPlugin);
        }
        /// @}
    }
}
