...
public class MmsThumbnailPresenter extends Presenter {
    ...
    private IMmsUnsupportedFilesExt mMmsUnsupportedFilesExt = null;
    ...
    private void presentVideoThumbnail(SlideViewInterface view, VideoModel video) {
        ...
        if (MmsConfig.isUnsupportedFilesOn()
            && !mMmsUnsupportedFilesExt.isSupportedFile(video.getContentType(), video.getSrc())) {
            view.setVideoThumbnail(video.getSrc(), mMmsUnsupportedFilesExt
                    .getUnsupportedVideoIcon());
            return;
        }
        /// @}
    ...
    private void presentImageThumbnail(SlideViewInterface view, ImageModel image) {
        ...
        /// M: OP09 Feature: Unsupported Files; @{
        if (MmsConfig.isUnsupportedFilesOn()
            && !mMmsUnsupportedFilesExt.isSupportedFile(image.getContentType(), image.getSrc())) {
            view.setImage(image.getSrc(), mMmsUnsupportedFilesExt.getUnsupportedImageIcon());
            return;
        }
        /// @}
        if (FeatureOption.MTK_DRM_APP) {
    ...
    private void initPlugin(Context context) {
        mMmsUnsupportedFilesExt = (IMmsUnsupportedFilesExt) MPlugin.createInstance(
            IMmsUnsupportedFilesExt.class.getName(), context);
        MmsLog.d(TAG, "Operator mMmsUnsupportedFilesExt = " + mMmsUnsupportedFilesExt);
        if (mMmsUnsupportedFilesExt == null) {
            mMmsUnsupportedFilesExt = new DefaultMmsUnsupportedFilesExt(context);
            MmsLog.d(TAG, "default mMmsUnsupportedFilesExt = " + mMmsUnsupportedFilesExt);
        }
    }
}
