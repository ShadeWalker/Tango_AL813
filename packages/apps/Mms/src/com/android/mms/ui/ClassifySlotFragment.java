...
public class ClassifySlotFragment extends PreferenceFragment implements Preference.OnPreferenceChangeListener {
    ...
    private IMmsUtilsExt mMmsUtisExt = (IMmsUtilsExt) MmsPluginManager
    ...
    private void setMessagePreferences() {
        ...
        /// M: for OP09 feature.@{
        if (MmsConfig.isDeliveryReportInRoamingEnable()
            && !MmsConfig.isAllowDRWhenRoaming(mActivity.getApplicationContext(), mSubId)) {
            /*HQ_zhangjing modified for CQ HQ01363866 begin*/
            //mSmsDeliveryReport.setEnabled(false);
            /*HQ_zhangjing modified for CQ HQ01363866 end*/
        }
        ///@}
        ...
        if (MmsConfig.isStringReplaceEnable() && location != null
            && mMmsUtisExt.isCDMAType(mActivity.getApplicationContext(), mSubId)) {
    ...
    private void changeSingleCardKeyToSimRelated() {
        ...
        SharedPreferences spr = mActivity.getSharedPreferences(MMS_PREFERENCE,
        ...
        SubscriptionInfo sfr = mMmsUtisExt.getSubInfoBySubId(mActivity, mSubId);
        if (MmsConfig.isStringReplaceEnable() && sfr != null && ctString != null
                && sfr.getSimSlotIndex() == 0) {
        ...
        if (preference == mManageSimPref) {
            ...
            if (mSmsDeliveryReport.isChecked()
                ...
                if (mMmsFailedNotifyPlugin != null) {
                    mMmsFailedNotifyPlugin.popupToast(mActivity.getApplicationContext(),
            ...
            if (FeatureOption.MTK_C2K_SUPPORT && MessageUtils.isUSimType(mSubId)
                ...
                mNumberTextDialog = dialog.setIcon(R.drawable.ic_dialog_info_holo_light).setTitle(
                    ...
                    R.string.OK, new PositiveButtonListener()).setNegativeButton(R.string.Cancel,
            ...
            if (FeatureOption.MTK_C2K_SUPPORT && MessageUtils.isUSimType(mSubId)) {
                ...
                SubscriptionInfo sfr = mMmsUtisExt.getSubInfoBySubId(mActivity, mSubId);
                String displayName = "";
                if (sfr != null && sfr.getDisplayName() != null) {
                    displayName = sfr.getDisplayName().toString();
                }
    ...
    private CharSequence getVisualTextName(int subId, Context context, String enumName,
        ...
        if (MmsConfig.isStringReplaceEnable()) {
            if (mMmsUtisExt.isCDMAType(mActivity, subId)
    ...
    public boolean onPreferenceChange(Preference arg0, Object arg1) {
        ...
        if ((Long.toString(mSubId) + "_" + SMS_SAVE_LOCATION).equals(key)
            ...
            if (!mActivity.getResources().getBoolean(R.bool.isTablet)) {
                Log.d(TAG, "enter");
                mSmsLocation.setSummary(getVisualTextName(mSubId, mActivity, stored,
    ...
    private void setListPrefSummary() {
        ...
        if (!mActivity.getResources().getBoolean(R.bool.isTablet)) {
            mSmsLocation.setSummary(getVisualTextName(mSubId, mActivity, saveLocation,
                R.array.pref_sms_save_location_choices, R.array.pref_sms_save_location_values));
        } else {
