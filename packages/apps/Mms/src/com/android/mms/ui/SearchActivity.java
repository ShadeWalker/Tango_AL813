...
//HQ_zhangjing 2015-10-10 modified for CQ HQ01431762
...
public class SearchActivity extends ListActivity {
    ...
    private IMmsAdvanceSearchExt mMmsAdvanceSearchPlugin;
    /// @}
    ...
    public static class TextViewSnippet extends TextView {
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            ...
            while (m.find(start)) {
				...
				/*HQ_zhangjing 2015-10-10 modified for CQ HQ01431762 begin */
                //spannable.setSpan(new StyleSpan(sTypefaceHighlight), m.start(), m.end(), 0);
                spannable.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.search_result_color)),
                        m.start(), m.end(), 0);
				/*HQ_zhangjing 2015-10-10 modified for CQ HQ01431762 begin */
                start = m.end();
            }
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (MmsConfig.isAdvanceSearchEnable()) {
            mIsAdvancedQuery = getIntent().getBooleanExtra(IMmsAdvanceSearchExt.ADVANCED_SEARCH_QUERY, false);
        }
        /// @}
    ...
    private void initPlugin(Context context, Intent intent) {
        mMmsAdvanceSearchPlugin = (IMmsAdvanceSearchExt) MPlugin.createInstance(IMmsAdvanceSearchExt.class.getName(), context);
        if (mMmsAdvanceSearchPlugin == null) {
            mMmsAdvanceSearchPlugin = new DefaultMmsAdvanceSearchExt(context);
    ...
    private class  SearchResultCursorAdapter extends CursorAdapter {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            if (address!= null && address.startsWith("9+++")) {
				address = address.substring(4);
			}
            Contact contact = address != null ? Contact.get(address, false) : null;

            String titleString = contact != null ? contact.getNameAndNumber() : "";
			if (titleString.startsWith(IpMessageConsts.GROUP_START)) {
				titleString = IpMessageUtils.getContactManager(
						SearchActivity.this).getNameByNumber(titleString);
			}
			if (Build.TYPE.equals("eng")) {
				MmsLog.w(TAG, "Joyn  number" + address + "title" + titleString);
			}
            /// @}
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {  
                   /// M: add for ALPS01766374 ipMessage merge to L @{               
                    ...
                    if (MmsConfig.isServiceEnabled(SearchActivity.this)) {
                    ...
                    if (FeatureOption.MTK_WAPPUSH_SUPPORT == true) {
                        ...
                        if (threadType == Telephony.Threads.IP_MESSAGE_GUIDE_THREAD) {
                    ...
                    /// @}
                    if (MmsConfig.getMmsDirMode() && msgBox == 1 && m_type == 130) {
                        return;
                    } else {
