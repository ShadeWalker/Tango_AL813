...
import android.widget.Toast;//HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient
...
/*HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient begin*/
	...
	private int mLastCont = 0;
	private boolean mIsMms = false;
    ArrayList<String> dialoged = new ArrayList<String>();
/*HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient end*/
 ...
 /// M:       setDropDownWidth(LayoutParams.MATCH_PARENT);
 		mContext = context;//HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient
...
//    }
    ...
    public ContactList constructContactsFromInput(boolean blocking) {
     ...
     /*add by wanghui for al813 request begin*/
	public boolean isNumMatches(String number, boolean isMms) { 
	    if(Mms.isEmailAddress(number)){
			Log.d("wanghui","is email");
			return true;
		}           
			...
			for (int i = 0; i < len; i++) {
				...
				// In strict mode, we only let space and the first '+' pass
	            if ((c == ' ') || ((c == '-') && !(i == 0)) || ((c == '+') && (i == 0))) {
	                continue;
	            }
	...
	/*add by wanghui for al813 request end*/
    ...
    private boolean isValidAddress(String number, boolean isMms) {
        if (isMms) {
            ...
            /// character can use +;@{
            if (commonValidValue && MmsConfig.isMoreStrictValidateForSmsAddr()) {
...
//            }
        ...
        public List<String> getNumbers() {
            ...
            String langage = Locale.getDefault().getLanguage();//add by lipeng

            int start = 0;
            int i = 0;
            while (i < len + 1) {
                char c;
                //add by lipeng for number format
                if ((i == len) || ((c = sp.charAt(i)) == ',') || (c == ';')) {
                    if (i > start) {                 	
                    	if(langage.startsWith("ar")||langage.startsWith("fa")||langage.startsWith("iw")){///RTL language
                            String dataInfo = getNumberAt(sp, start, i, getContext())+"";
                        	String dataValue =""+Integer.toHexString(dataInfo.charAt(0));//get number index 0 ,if it is 202d or 202c
                        	String samplePre = "202d";
                        	String sampleAfter = "202c";
                        	
                    		if( dataValue.equals(samplePre)	|| dataValue.equals(sampleAfter)){                   
                                int lens = dataInfo.length();
                        		dataInfo = dataInfo.substring(1,lens-1);
                    		}
                    		list.add(dataInfo);
                    	}else{
    ...
    public void parseRecipientsFromChipWatcher(ArrayList<RecipientEntry> recipientList, ArrayList<String> changedNumbers, String lastRecipient, int limit) {
        ContactList list = new ContactList();
/*HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient begin*/
		mLastCont = mCurrentCount;
		mCurrentCount = recipientList.size();
/*HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient end*/
        ...
        if ((changedNumbers == null || (changedNumbers != null && changedNumbers.size() == 0))
            ...
            if(SystemProperties.get("ro.hq.sms.remove.invalid").equals("1")){
            	/*HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient begin*/
                for (int i = 0;i < changedNumbers.size();i++) {
    				String num = changedNumbers.get(i);
                    num = PhoneNumberUtils.replaceUnicodeDigits(num);
                    String title = getResourcesString(R.string.invalid_recipient,
                    		num);
    				if( !isNumMatches(num, mIsMms)&& (i == (changedNumbers.size() -1))
    					&& (mCurrentCount > mLastCont) ){
    					if(!dialoged.contains(num)){
    					 dialoged.add(num);
    					...
    					if( recipientList.size() == 1 ){
    						Log.d(TAG,"recipientList.size()= 1 and invalid nedd delete the recipient");
    						//populate(getContactsFromChipWatcher());
    						return;
    					}
    					return;
    				}	
    ...
    /*HQ_zhangjing 2015-11-12 modified for CQ HQ01455134 remove the invalid recipient end*/
            ...
            for (int i = 0; i < recipientList.size(); i++) {
                ...
                String number = recipient.getDestination();
                number = PhoneNumberUtils.replaceUnicodeDigits(number);
               /*HQ_sunli 20151127 HQ01508468 begin*/
               /* if(SystemProperties.get("ro.hq.sms.recipient.check").equals("1")&& number.length()<=2) {
		Toast.makeText(mContext,
	                       mContext.getResources().getString(R.string.invalid_number),
			       Toast.LENGTH_SHORT).show();
		continue;
		}*/
                /*HQ_sunli 20151127 HQ01508468 end*/
                numbers.add(number);
