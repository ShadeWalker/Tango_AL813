...
public class SlideshowPresenter extends Presenter {
    ...
    private IMmsUnsupportedFilesExt mMmsUnsupportedFilesExt =
        (IMmsUnsupportedFilesExt) MmsPluginManager
            .getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_UNSUPPORTED_FILES);
    ...
    protected void presentImage(SlideViewInterface view, ImageModel image,
                ...
                if (lowerName.endsWith(".gif")) {
                    ...
                    if (FeatureOption.MTK_GMO_ROM_OPTIMIZE
                        ...
                        view.setImage(image.getSrc(),
                                image.getBitmap(PREVIEW_IMAGE_SIZE, PREVIEW_IMAGE_SIZE));
                    } else {
                    ...
                    if (MmsConfig.isUnsupportedFilesOn()
                        && !mMmsUnsupportedFilesExt.isSupportedFile(image.getContentType(), image
                                .getSrc())) {
                        view.setImage(image.getSrc(), mMmsUnsupportedFilesExt
                                .getUnsupportedImageIcon());
                    }
                    /// @}
                    /* @} */
                }
        ...
        /// M: OP09 Feature: UnsupportedFiles; @{
        if (MmsConfig.isUnsupportedFilesOn()) {
            if (!mMmsUnsupportedFilesExt.isSupportedFile(image.getContentType(), image.getSrc())) {
                mMmsUnsupportedFilesExt.setUnsupportedViewVisibilityForImage(image.isVisible());
            } else {
                mMmsUnsupportedFilesExt.setUnsupportedViewVisibilityForImage(false);
            }
        }
        /// @}
    }
