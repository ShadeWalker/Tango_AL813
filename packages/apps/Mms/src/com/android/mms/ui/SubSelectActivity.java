...
public class SubSelectActivity extends ListActivity {
	...
	try {
        ...
        //add OP01 Plugin,set title
        mMmsPreferencePlugin.configSelectCardPreferenceTitle(this);
        //add action bar
    ...
    protected void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (mMmsPreferencePlugin.handleSelectCardPreferenceTreeClick(this, mCurrentSubId)) {
            return;
        }
        ...
        if (SmsPreferenceActivity.SMS_MANAGE_SIM_MESSAGES.equals(mPreferenceKey)) {
            ...
            if (FeatureOption.MTK_C2K_SUPPORT
                    && MessageUtils.isUSimType(mCurrentSubId)
                    /// M: modify for OP09 feature, not allow some SMS setting in WCDMA.
                    && !MessageUtils.isCSIMInGsmMode(mCurrentSubId)) {
    ...
    private void setSaveLocation(int subId) {
        ...
        final String saveLocationKey = Long.toString(subId) + "_" + SmsPreferenceActivity.SMS_SAVE_LOCATION;
...
//                        mSim.setNotifyChange(context);
    ...
    public void setServiceCenter(int subId) {
        if (FeatureOption.MTK_C2K_SUPPORT && MessageUtils.isUSimType(subId) &&
            ...
            !MessageUtils.isCSIMInGsmMode(subId)) {
            Toast.makeText(getApplicationContext(), getString(R.string.cdma_not_support),
                Toast.LENGTH_SHORT).show();
        } else {
            ...
            if (result != null
                ...
                mNumberText.setSelection(scNumber.length());
            ...
            new Thread(new Runnable() {
                public void run() {
                	if(langage.startsWith("ar")||langage.startsWith("fa")||langage.startsWith("iw")){///RTL language
                      TelephonyManagerEx.getDefault().setScAddress(mCurrentSubId, scNumber.replaceAll("\u202D", "").replaceAll("\u202C", ""));
               	}else{
    ...
    private final BroadcastReceiver mSimStateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SUBINFO_RECORD_UPDATED)) {
                int nowSubCount = SubscriptionManager.from(MmsApp.getApplication())
                        .getActiveSubscriptionInfoCount();
                if (nowSubCount != mOldSubCount) {
