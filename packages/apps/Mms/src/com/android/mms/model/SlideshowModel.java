        ...
        for (int i = 0; i < partNum; i++) {
           ...
           /// @}
            ...
            if (MmsConfig.isSupportAttachEnhance()) {
                ...
                if (MmsContentType.isImageType(new String(part.getContentType())) && !isInSmil(filename, slides, PartUri)) {
                ...
                } else if (MmsContentType.isVideoType(new String(part.getContentType())) && !isInSmil(filename, slides, PartUri)) {
                ...
                } else if (MmsContentType.isAudioType(new String(part.getContentType())) && !isInSmil(filename, slides, PartUri)) {
                    FileAttachmentModel fam = new FileModel(context, new String(part.getContentType()),
                    filename, part.getDataUri());
                    attachFiles.add(fam);
                    ...
                    if (FileAttachmentModel.isTxtType(part) && !isInSmil(filename, cid, slides)) {
                        FileAttachmentModel fam = new FileModel(context, MmsContentType.TEXT_PLAIN,
                        filename, part.getData());
                        attachFiles.add(fam);
                    } else if (FileAttachmentModel.isHtmType(part) && !isInSmil(filename, cid, slides)) {
                        FileAttachmentModel fam = new FileModel(context, MmsContentType.TEXT_HTML,
                            filename, part.getData());
                        attachFiles.add(fam);
                    }
                }
    ...
    private static boolean isInSmil(String filename, ArrayList<SlideModel> slides, String PartUri) {
        for (SlideModel slide : slides) {
            for (int j = 0; j < slide.size(); j++) {
                ...
                if (filename != null && slide.get(j).getSrc() != null && filename.compareTo(slide.get(j).getSrc()) == 0) {
            ...
            if (fileAttachment.isSupportedFile()) {
                ...
                if (MmsConfig.isSupportAttachEnhance()) {
                    if (fileAttachment.getContentType().equals("text/plain")
                        ...
                        part.setData(fileAttachment.getData());
                    }
                }
            }
