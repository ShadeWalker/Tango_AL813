    ...
    public static void startQuery(AsyncQueryHandler handler, int token, String selection) {
        ...
        if (selection != null && selection.equals("allunread")) {
            if (MmsConfig.isMassTextEnable()) {
                handler.startQuery(token, null, Uri.parse("content://mms-sms/conversations/group/unread_count"), null,
            ...
            handler.startQuery(token, null, sAllThreadsUriExtend, ALL_THREADS_PROJECTION_EXTEND, selection, null,
                Conversations.DEFAULT_SORT_ORDER);
        }
    }
...
//                }
    ...
    private static void fillFromCursor(Context context, Conversation conv,
        ...
        synchronized (conv) {
            ...
            // get unread count
            if (conv.mType == Telephony.Threads.IP_MESSAGE_GUIDE_THREAD) {
        ...
        if (c.getColumnCount() > SPAM && sIsActivated && recipients.size() == 1) {
            ...
            if (conv.mSpamStatus) {
                long spamDate = IpMessageUtils.getContactManager(context).getSpamTime(recipients.get(0).getNumber());
                if (spamDate == 0) {
                    conv.mSpamDate = conv.mDate;
                } else {
                    conv.mSpamDate = spamDate;
                }
            }
        }
        /// @}
