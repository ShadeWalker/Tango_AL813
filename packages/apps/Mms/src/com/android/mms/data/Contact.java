...
//                /// @}
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
            ...
            /// M: add for ip message
            if (MmsConfig.isServiceEnabled(mContext)
    ...
    public static String getValidNumber(String numberOrEmail) {
        ...
        /// M: add for ipmessage
        if (MmsConfig.isServiceEnabled(MmsApp.getApplication()) &&
    ...
    public synchronized Drawable getGroupAvatar(final Handler mUiHandler,
            final Context context, final long threadId) {
        MmsLog.d(M_TAG, "Contact.getGroupAvatar(): mAvatar is null ?= "
                + (mAvatar == null));
        if (mAvatar == null) {
            final Object lock = new Object();
            mIpMessageAvatarBitmap = null;
            sContactCache.pushTask(new Runnable() {
                public void run() {
                    mIpMessageAvatarBitmap = IpMessageUtils.getContactManager(
                            context).getAvatarByThreadId(threadId);
                    if (null != mIpMessageAvatarBitmap) {
                        mAvatar = new BitmapDrawable(mIpMessageAvatarBitmap);
                    }
                    if (null != mUiHandler) {
                        mUiHandler.sendEmptyMessage((int) threadId);
                    }
                }
            });
