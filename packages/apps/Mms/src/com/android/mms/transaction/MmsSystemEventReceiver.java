...
public class MmsSystemEventReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
    	/*HQ_zhangjing add for al812 HQ01331697 for security red line begin*/
		if( null == intent ){
			return;
		}
    	/*HQ_zhangjing add for al812 for security red line end*/
        if (Log.isLoggable(LogTag.TRANSACTION, Log.VERBOSE)) {
      ...
      //add by lipeng
        ...
        //add by lipeng for sim card change
        if(action.equals(TelephonyIntents.ACTION_SIM_DETECTED)){
        	Log.v(TAG, " Intent received: " + intent);
        	MessageUtils.setMmsLimitSize(context);
        }//end by lipeng 
