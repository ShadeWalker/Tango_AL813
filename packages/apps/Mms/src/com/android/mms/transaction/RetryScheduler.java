        ...
        if (cursor != null) {
            try {
                if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
                    ...
                    DefaultRetryScheme scheme = null;
                    if (MmsConfig.isMmsRetrySchedulerEnable()) {
                        scheme = new DefaultRetryScheme(mContext, retryIndex, msgType);
                    } else {
                        scheme = new DefaultRetryScheme(mContext, retryIndex);
                    }
                    ...
                    if ((retryIndex < scheme.getRetryLimit()) && retry) {
                        ...
                        IMmsFailedNotifyExt mmsFailedNotifyPlugin = (IMmsFailedNotifyExt) MmsPluginManager
                                .getMmsPluginObject(MmsPluginManager.MMS_PLUGIN_TYPE_FAILED_NOTIFY);
                        /// @}

                        if (retryIndex > notifyIndex && !MmsConfig.isTransactionFailedNotifyEnable()) {
