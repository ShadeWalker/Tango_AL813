...
public class GLUtil {
    ...
    public static float[] createTopGraphicRect(int width, int height, float toTop) {
        ...
        float topGrapihcEdgeLength = (float) minValue * PIPCustomization.TOP_GRAPHIC_DEFAULT_EDGE_RELATIVE_VALUE;
        float left = 0;
        float top  = 0;
        if (width >= height) {
            // landscape
            left = toTop;
            top = minValue * (1 - PIPCustomization.TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE) - topGrapihcEdgeLength;
        } else {
            // portrait
    ...
    private static float[] createVerticalFlipTexCoord(float lowWidth, float highWidth,
