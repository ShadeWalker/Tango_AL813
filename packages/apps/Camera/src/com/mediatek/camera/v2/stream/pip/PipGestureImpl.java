 ...
 * Not preview view area, But gestures are based on preview area, so we need make 
 * a transform from 'preview area gesture' to 'preview size gesture'.
 */
public class PipGestureImpl implements PipGestureController {
    ...
    private RectF                          mPreviewArea = null; // used to transform gesture 
    ...
    public void setPreviewSize(Size previewSize) {
        ...
        Log.i(TAG, "[setPreviewSize]+ : " + previewSize.getWidth() + " x " + previewSize.getHeight());
        ...
        if(mPreviewWidth ==0 || mPreviewHeight == 0) {
            ...
            mEditSquareSideLength = Math.min(width, height) / PIPCustomization.TOP_GRAPHIC_EDIT_BUTTON_RELATIVE_VALUE;
            float[] topGraphicPosition = GLUtil.createTopRightRect(width, height, mRelativeRectToTop);
            mTopGraphicRectInPortrait.setRendererSize(Math.min(width, height), Math.max(width, height));
    ...
    public boolean onDown(float x, float y) {
        ...
        mTopGraphicRectInPortrait.setHighLightEnable(isTranslateAnimationEnable || isScaleRotateAnimationEnable);
    ...
    private void initVertexData(float dx, float dy, int animationtype) {
        ...
        case ANIMATION_SCALE_ROTATE:
            ...
            AnimationRect animationRect = mTopGraphicRectInPortrait.copy();
            animationRect.rotate(-mCurrentGsensorOrientation + animationRect.getCurrrentRotationValue());
            ...
            mTopGraphicRectInPortrait.rotate(mTopGraphicRectInPortrait.getCurrrentRotationValue());
            // rotate
            mTopGraphicRectInPortrait.rotate(degress + mTopGraphicRectInPortrait.getCurrrentRotationValue());
            break;
        }
    }
