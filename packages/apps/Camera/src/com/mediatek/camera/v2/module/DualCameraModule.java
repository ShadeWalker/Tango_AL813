...
public class DualCameraModule extends AbstractCameraModule {
    ...
    public DualCameraModule(AppController app) {
        super(app);
        mAdditionManager = new AdditionManager(app, this, SettingCtrl.BACK_CAMERA);
        mAaaManager = new AaaManager(app, this, true, SettingCtrl.BACK_CAMERA);
        
        mSubCamAdditionManager = new AdditionManager(app, this, SettingCtrl.FRONT_CAMERA);
        mSubCamAaaManager = new AaaManager(app, this, true, SettingCtrl.FRONT_CAMERA);
    }
    ...
    public void open(Activity activity, boolean isSecureCamera,
        ...
        mSubCamAaaManager.open(activity, mAppUi.getModuleLayoutRoot(), isCaptureIntent);
        mSubCamAdditionManager.open(activity, mAppUi.getModuleLayoutRoot(), isCaptureIntent);
    ...
    public void close() {
        Log.i(TAG, "[close]+");
        super.close();
        mSubCamAaaManager.close();
        mSubCamAdditionManager.close();
    ...
    public void resume() {
        Log.i(TAG, "[resume]+");
        super.resume();
        mSubCamAaaManager.resume();
        mSubCamAdditionManager.resume();
    ...
    public void pause() {
        Log.i(TAG, "[pause]+");
        super.pause();
        mSubCamAaaManager.pause();
        mSubCamAdditionManager.pause();
        closeCamera();
        Log.i(TAG, "[pause]-");
    }
    ...
    public void onPreviewAreaChanged(RectF previewArea) {
        ...
        mSubCamAaaManager.onPreviewAreaChanged(previewArea);
        mSubCamAdditionManager.onPreviewAreaChanged(previewArea);
    }
    ...
    public void onSettingChanged(Map<String, String> result) {
        ...
        if (includeCameraId) {
            ...
            if (cameraId != mCameraId) {
                ...
                mCameraId = cameraId;
                requestChangeCaptureRequets(false/*sync*/,getDefaultRepeatingRequest(), CaptureType.REPEATING_REQUEST);
    ...
    public void onOrientationChanged(int orientation) {
        ...
        mAaaManager.onOrientationChanged(newOrientation);
        mSubCamAaaManager.onOrientationChanged(newOrientation);
        mAdditionManager.onOrientationChanged(newOrientation);
        mSubCamAdditionManager.onOrientationChanged(newOrientation);
    ...
    public boolean onSingleTapUp(float x, float y) {
        ...
        if(mIsDeviceNeedSwitch) {
            mSubCamAaaManager.onSingleTapUp(x, y);
            mSubCamAdditionManager.onSingleTapUp(x, y);
        } else {
            mAdditionManager.onSingleTapUp(x, y);
            mAaaManager.onSingleTapUp(x, y);
        }
    ...
    public boolean onLongPress(float x, float y) {
        ...
        if(mIsDeviceNeedSwitch) {
            mSubCamAdditionManager.onLongPressed(x, y);
        } else {
            mAdditionManager.onLongPressed(x, y);
        }
        return false;
    }
    ...
    @Override
    public void requestChangeCaptureRequets(boolean isMainCamera, boolean sync, RequestType requestType, 
            CaptureType captureType) {
    ...
    private CameraSessionCallback mMainCamSessionCallback = new CameraSessionCallback() {
        ...
        public void onSessionConfigured() {
            requestChangeCaptureRequets(true,getDefaultRepeatingRequest(), CaptureType.REPEATING_REQUEST);
        }
        ...
        public CaptureCallback configuringSessionRequests(Builder requestBuilder,
            ...
            requestBuilder.set(CaptureRequest.SCALER_CROP_REGION, Utils.cropRegionForZoom(
                    ...
                    1f));
            Map<RequestType, CaptureRequest.Builder> requestBuilders = new HashMap<RequestType, CaptureRequest.Builder>();
            requestBuilders.put(requestType, requestBuilder);
            // 1. apply 3A control 
            mAaaManager.configuringSessionRequests(requestBuilders, captureType, true);
            // 2. apply addition parameter
            mAdditionManager.configuringSessionRequests(requestBuilders,captureType);
    ...
    private CaptureCallback                  mMainSessionCapProgressCallback = new CaptureCallback() {
        ...
        public void onCaptureStarted(CameraCaptureSession session,
            ...
            if (!mIsDeviceNeedSwitch) {
                mAaaManager.onPreviewCaptureStarted(request, timestamp, frameNumber);
                mAdditionManager.onCaptureStarted(request, timestamp, frameNumber);
            }
        ...
        public void onCaptureProgressed(CameraCaptureSession session, 
            ...
            if (!mIsDeviceNeedSwitch) {
                mAaaManager.onPreviewCaptureProgressed(request, partialResult);
            }
        };
        ...
        public void onCaptureCompleted(CameraCaptureSession session,
            ...
            if (!mIsDeviceNeedSwitch) {
                mAaaManager.onPreviewCaptureCompleted(request, result);
                mAdditionManager.onCaptureCompleted(request, result);
            }
    ...
    private CameraSessionCallback mSubCamSessionCallback = new CameraSessionCallback() {
        ...
        public CaptureCallback configuringSessionRequests(Builder requestBuilder,
            ...
            requestBuilders.put(requestType, requestBuilder);
            // 1. apply 3A control 
            mSubCamAaaManager.configuringSessionRequests(requestBuilders, captureType, true);
            // 2. apply addition parameter
            mSubCamAdditionManager.configuringSessionRequests(requestBuilders,captureType);
            // 3. apply mode parameter
    ...
    private CaptureCallback                  mSubSessionCapProgressCallback = new CaptureCallback() {
        ...
        public void onCaptureStarted(CameraCaptureSession session,
                CaptureRequest request, long timestamp, long frameNumber) {
            if (mIsDeviceNeedSwitch) {
                mSubCamAaaManager.onPreviewCaptureStarted(request, timestamp, frameNumber);
                mSubCamAdditionManager.onCaptureStarted(request, timestamp, frameNumber);
            }
            mCurrentMode.onPreviewCaptureStarted(request, timestamp, frameNumber);
        }
        ...
        public void onCaptureProgressed(CameraCaptureSession session, 
                CaptureRequest request, CaptureResult partialResult) {
            if (mIsDeviceNeedSwitch) {
                mSubCamAaaManager.onPreviewCaptureProgressed(request, partialResult);
            }
        };
        ...
        public void onCaptureCompleted(CameraCaptureSession session,
                CaptureRequest request, TotalCaptureResult result) {
            if (mIsDeviceNeedSwitch) {
                mSubCamAaaManager.onPreviewCaptureCompleted(request, result);
                mSubCamAdditionManager.onCaptureCompleted(request, result);
            }
            mCurrentMode.onPreviewCaptureCompleted(request, result);
        }
    };
    ...
    private void openCamera() {
        ...
        mCameraManager.openSync(SettingCtrl.BACK_CAMERA, mMainCamStateCallback, mMainCamSessionCallback, mCameraHandler);
        mCameraManager.open(SettingCtrl.FRONT_CAMERA, mSubCamStateCallback, mSubCamSessionCallback, mCameraHandler);
        Log.i(TAG, "[openCamera]-");
    }
