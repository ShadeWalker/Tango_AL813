...
public class RecordStreamView {
    ...
    public RecordStreamView(Activity activity, 
            RecordStreamController recordingStream, 
    ...
    private class RecordingCallback implements RecordStreamStatus {
        ...
        public void onInfo(int what, int extra) {
            if (what == RecordStreamController.MEDIA_RECORDER_INFO_START_TIMER) {
                mRecordingStartTime = SystemClock.uptimeMillis();
                updateRecordingTime();
            }
            if (what == RecordStreamController.MEDIA_RECORDER_INFO_RECORDING_SIZE) {
                if (0 < mLimitSize) {
                    int progress = (int) (extra * 100 / mLimitSize);
                    Log.i("mmTAG","extra = " + extra + " : mLimitSize = " + mLimitSize + "  : progress = " + progress);
                    if (100 >= progress) {
    ...
    private View getView() {
        ...
        View viewLayout = mActivity.getLayoutInflater().inflate(R.layout.recording_ext_v2, mParentViewGroup, true);
