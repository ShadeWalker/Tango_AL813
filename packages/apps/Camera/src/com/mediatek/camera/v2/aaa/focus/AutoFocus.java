...
package com.mediatek.camera.v2.aaa.focus;
...
import com.mediatek.camera.v2.aaa.AaaHelper;
import com.mediatek.camera.v2.aaa.IAaaManager.IAaaListener;
...
public class AutoFocus implements IFocus, ISettingChangedListener {
    ...
    private MeteringRectangle[]    mAFRegions = AaaHelper.ZERO_WEIGHT_3A_REGION;
    private MeteringRectangle[]    mAERegions = AaaHelper.ZERO_WEIGHT_3A_REGION;
    ...
    public void onSingleTapUp(float x, float y) {
        ...
        if (!mAfTriggerEnabled) {
            mFaceDetectionValue = mSettingServant.getSettingValue(SettingKeys.KEY_CAMERA_FACE_DETECT);
        }
        ...
        mAERegions = AaaHelper.aeRegionsForNormalizedCoord(points[0], points[1], 
                cropRegion, sensorOrientation);
        mAFRegions = AaaHelper.afRegionsForNormalizedCoord(points[0], points[1], 
                cropRegion, sensorOrientation);
    ...
    private void sendAutoFocusTriggerCaptureRequest() {
        ...
        mAaaListener.requestChangeCaptureRequets(true,requiredRequestType, CaptureType.REPEATING_REQUEST);
    ...
    private void sendAutoFocusCancelCaptureRequest() {
        mAfMode = CameraMetadata.CONTROL_AF_MODE_AUTO;
        // make a single request to trigger auto focus
        mAaaListener.requestChangeCaptureRequets(true,mAaaListener.getRepeatingRequestType(), CaptureType.CAPTURE);
        mIsAfTriggerRequestSubmitted = false;
    }
    ...
    private final Runnable mReturnToContinuousAFRunnable = new Runnable() {
        ...
        public void run() {
            mIsAfTriggerRequestSubmitted = false;
            resetTouchFocus();
            mAFRegions = AaaHelper.ZERO_WEIGHT_3A_REGION;
            mAfMode = CameraMetadata.CONTROL_AF_MODE_CONTINUOUS_PICTURE;
            mAaaListener.requestChangeCaptureRequets(true, mAaaListener.getRepeatingRequestType(), CaptureType.REPEATING_REQUEST);
        }
    };
    ...
    private void autofocusStateChangeDispatcher(CaptureResult result) {
        ...
        if (currentFrameNumber < mLastControlAfStateFrameNumber ||
            ...
            Log.i(TAG, "frame number, last:current " + mLastControlAfStateFrameNumber +
                    ":" + currentFrameNumber + " afState:" + result.get(CaptureResult.CONTROL_AF_STATE));
            return;
        }
    ...
    private void resumeContinuousAF() {
        ...
        mSettingServant.doSettingChange(SettingKeys.KEY_CAMERA_FACE_DETECT, mFaceDetectionValue, false);
    ...
    private void onFocusStatusUpdate(final int resultAFState) {
        Log.i(TAG, "onFocusStatusUpdate resultAFState: " + resultAFState + " cameraId:" + mSettingServant.getCameraId());
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                case CaptureResult.CONTROL_AF_STATE_PASSIVE_SCAN:/**1**/
                    ...
                    if (!AaaHelper.ZERO_WEIGHT_3A_REGION.equals(mAERegions)) {
                        mAERegions = AaaHelper.ZERO_WEIGHT_3A_REGION;
                        mAaaListener.requestChangeCaptureRequets(false,mAaaListener.getRepeatingRequestType(), CaptureType.REPEATING_REQUEST);
                    }
                    break;
