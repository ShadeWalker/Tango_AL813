...
public class RendererManager {
    ...
    private int                      mRendererTexWidth = -1; // bottom/top use the same preview texture size
    ...
    public void setPreviewSize(Size previewSize) {
        ...
        if (mRendererTexWidth == previewSize.getWidth() && mRendererTexHeight == previewSize.getHeight()) {
    ...
    public void prepareRecording() {
        Log.i(TAG, "[prepareRecording]+ mRecorderRenderer:" + mRecorderRenderer + 
    ...
    private SurfaceTexture _getMainPvSurfaceTexture() {
        synchronized (mRenderLock) {
            if (mMainPv_StWrapper == null && mEglHandler != null) {
                try {
                    mRenderLock.wait();
                } catch (InterruptedException e) {
                    Log.w(TAG, "unexpected interruption");
            ...
            Log.i(TAG, "_getMainPvSurfaceTexture mPreviewBottomSurfaceTexture = " + mMainPv_StWrapper.getSurfaceTexture()
                    +" mEglHandler = " + mEglHandler);
    ...
    private SurfaceTexture _getSubPvSurfaceTexture() {
        synchronized (mRenderLock) {
            if (mSubPv_StWrapper == null && mEglHandler != null) {
                try {
                    mRenderLock.wait();
                } catch (InterruptedException e) {
                    Log.w(TAG, "unexpected interruption");
            ...
            Log.i(TAG, "_getSubPvSurfaceTexture mPreviewTopSurfaceTexture = " + mSubPv_StWrapper.getSurfaceTexture()
                    +" mEglHandler = " + mEglHandler);
    ...
    private class EglHandler extends Handler{
        ...
        private void doUpdateRenderSize() {
            ...
            if (mBottomGraphicRenderer != null) {
                mBottomGraphicRenderer.setRendererSize(mRendererTexWidth, mRendererTexHeight, false);
            }
        ...
        private void doSetupPictureTextures(Size bPictureSize, Size tPictureSize) {
            ...
            if (mMainCap_StWrapper == null) {
                ...
                mMainCap_StWrapper.setDefaultBufferSize(bPictureSize.getWidth(), bPictureSize.getHeight());
            ...
            if (mSubCap_StWrapper == null) {
                ...
                mSubCap_StWrapper.setDefaultBufferSize(tPictureSize.getWidth(), tPictureSize.getHeight());
        ...
        private boolean doTimestampSync() {
            ...
            if (mMainPv_StWrapper.getBufferTimeStamp() == 0l || mSubPv_StWrapper.getBufferTimeStamp() == 0l) {
                return false;
            }
            // after sync first frame, when high fps's surface texture new frame arrives, draw the frame
            // bottom is the high frame
            if (isBottomHasHighFrameRate && (mBottomCamTimeStamp != mMainPv_StWrapper.getBufferTimeStamp())) {
            ...
            if (!isBottomHasHighFrameRate && (mTopCamTimeStamp != mSubPv_StWrapper.getBufferTimeStamp())) {
                mBottomCamTimeStamp = mMainPv_StWrapper.getBufferTimeStamp();
                mTopCamTimeStamp = mSubPv_StWrapper.getBufferTimeStamp();
                return true;
            }
        ...
        private void draw() {
            synchronized (mRenderLock) {
                if (doTimestampSync() && !mBlockingForPvSizeChange) {
                    ...
                    // ensure time stamp will not backup, always choose one camera's buffer time stamp
                    long effectBufferTimestamp = 
                            pipSwitched? sub_StWrapper.getBufferTimeStamp() : main_StWrapper.getBufferTimeStamp();
                    // draw to encoder
                    if (mRecorderRenderer != null) {
                    ...
                    if (mNeedNotifyFirstFrameForSurfaceChanged) {
                        mRendererCallback.onFristFrameAvailable(main_StWrapper.getBufferTimeStamp());
            ...
            if (mMainCap_StWrapper != null && mMainCap_StWrapper.getBufferTimeStamp() > 0
                ...
                if (mCurrentOrientation % 180 != 0) {
                    ...
                    android.opengl.Matrix.rotateM(texRotateMtxByOrientation, 0,
                            -(mPipSwitched?180:0), 0, 0, 1);// sub camera should rotated in 90/270 g-sensor
                ...
                mTopGraphicRenderer.draw(
                        ...
                        (mCurrentOrientation - (mPipSwitched?270:90) + 360) % 360,// -90/270 for rotate to portrait
                        mPipSwitched);
