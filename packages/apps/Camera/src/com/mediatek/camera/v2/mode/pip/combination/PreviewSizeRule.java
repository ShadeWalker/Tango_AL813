    ...
    public void execute() {
        ...
        if ("on".equalsIgnoreCase(pipKeyValue)) {
            if (isPipSwitched && mCurrentPictureRatio != null) {
                mSettingServant.setSharedPreferencesValue(SettingKeys.KEY_PICTURE_RATIO, mCurrentPictureRatio);
                SettingItem pictureRatioItem = mSettingServant.getSettingItem(SettingKeys.KEY_PICTURE_RATIO);
                pictureRatioItem.setValue(mCurrentPictureRatio);
            } else {
                mCurrentPictureRatio = mSettingServant.getSettingValue(SettingKeys.KEY_PICTURE_RATIO);
                // override another setting servant
                ISettingServant anotherServant = mSettingCtrl.getSettingServant(getAnotherCameraId());
                SettingItem pictureRatioSettingItem = anotherServant.getSettingItem(SettingKeys.KEY_PICTURE_RATIO);
