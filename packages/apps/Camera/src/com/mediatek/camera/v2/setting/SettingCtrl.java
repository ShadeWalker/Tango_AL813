...
import com.mediatek.camera.v2.setting.SettingCharacteristics;
...
public class SettingCtrl{
    ...
    private ISettingRule[][]                    mRuleMatrix = new ISettingRule[SettingKeys.SETTING_COUNT][SettingKeys.SETTING_COUNT];
    private SharedPreferences                   mGlobalPreferences;
    private Map<String, SharedPreferences>      mPreferencesMap   = new HashMap<String, SharedPreferences>();
    private Map<String, SettingCharacteristics> mCharacteristicsMap = new HashMap<String, SettingCharacteristics>();
    private List<ISettingFilterListener>        mISettingFilterListeners = new ArrayList<ISettingFilterListener>();
    ...
    /**
     * Register listener to monitor the filter result after executing setting rules.
     * @param listener
     */
    public void registerSettingFilterListener(ISettingFilterListener listener) {
        Log.i(TAG, "[registerSettingFilterListener], listener:" + listener);
    ...
    public void configurateSetting(Map<String, String> defaultSettings) {
        Log.d(TAG, "[configurateSetting], changedSettings:" + defaultSettings.toString());
        ...
        while (keyIterator.hasNext()) {
            ...
            if (sharePreferencesValue != null && !sharePreferencesValue.equals(defaultValue)) {
                item.setValue(sharePreferencesValue);
                changedSettings.put(key, sharePreferencesValue);
            } else {
                item.setValue(defaultValue);
                setSharedPreferencesValue(key, defaultValue);
            }
            
            item.setDefaultValue(defaultValue);
            if (key.equals(SettingKeys.KEY_PICTURE_RATIO)) {
                if (sharePreferencesValue != null) {
                    changedSettings.put(SettingKeys.KEY_PICTURE_RATIO, sharePreferencesValue);
                } else {
                    changedSettings.put(SettingKeys.KEY_PICTURE_RATIO, defaultValue);
                }
        ...
        changedSettings.put(SettingKeys.KEY_CAPTURE_MODE, captureModeItem.getValue());
        
        // update setting sync to shared preference values.
        doSettingChange(changedSettings);
    ...
    public void doSettingChange(String key, String value, boolean saved) {
        ...
        // Gather result after setting changed.
        List<SettingItem> effectedSettings = queryResultSettings(key);
        Map<String, String> changedSetting = new HashMap<String, String>();
        changedSetting.put(key, value);
        for (int i = 0; i < effectedSettings.size(); i++) {
            SettingItem toChangedSetting = effectedSettings.get(i);
            if (toChangedSetting.getType() == SettingKeys.DO_NOT_APPLY_TO_NATIVE) {
                continue;
            }
            String currentValue = toChangedSetting.getValue();
            if (currentValue != null) {
                changedSetting.put(toChangedSetting.getKey(), currentValue);
            }
        }
        Log.i(TAG, "[doSettingChange], afterSetting:" + changedSetting.toString());
        ...
        // Callback result to UI
        Map<String, String> overrideValues = new HashMap<String, String>();
        Map<String, String> values = new HashMap<String, String>();
        values.put(key, value);
        for (int i = 0; i < effectedSettings.size(); i++) {
            SettingItem effectSetting = effectedSettings.get(i);
            if (!effectSetting.isEnable()) {
                continue;
            }
            overrideValues.put(effectSetting.getKey(), 
                    effectSetting.getOverrideValue());
            values.put(effectSetting.getKey(), effectSetting.getValue());
            List<SettingItem> items = 
                    queryResultSettings(effectSetting.getKey());
            for (SettingItem secondItem : items) {
                overrideValues.put(secondItem.getKey(), secondItem.getOverrideValue());
                values.put(secondItem.getKey(), secondItem.getValue());
            } 
        }
        Log.i(TAG, "[doSettingChanged], changedSettingsForUI:" + overrideValues.toString());
        for (int i = 0; i < mISettingFilterListeners.size(); i++) {
            ISettingFilterListener listener = mISettingFilterListeners.get(i);
    ...
    public void doSettingChange(Map<String, String> changedSettings) {
        ...
        List<SettingItem> effectSettings = new ArrayList<SettingItem>();
        while (keyIterator.hasNext()) {
            String key = keyIterator.next();
            SettingItem item = mSettingGenerator.getSettingItem(key);
            if (!item.isEnable()) {
            ...
            if (!effectSettings.contains(item)) {
                effectSettings.add(item);
            }
            List<SettingItem> resultItems = queryResultSettings(key);
            for (int i = 0; i < resultItems.size(); i++) {
                SettingItem resultItem = resultItems.get(i);
                if (!effectSettings.contains(resultItem)) {
                    effectSettings.add(resultItem);
                }
            }
        ...
        // Gather the result of settings changed.
        final Map<String, String> afterChanged = new HashMap<String, String>();
        for (int i = 0; i < effectSettings.size(); i++) {
            SettingItem effectSetting = effectSettings.get(i);
            String currentValue = effectSetting.getValue();
            if (currentValue == null || effectSetting.getType() != SettingKeys.APPLY_TO_NATIVE) {
                continue;
            }
            afterChanged.put(effectSetting.getKey(), currentValue);
        }
        Log.i(TAG, "[doSettingChange], afterChanged:" + afterChanged.toString());
        ...
        Log.i(TAG, "[doSettingChange], changedSettingsForUI:" + overrideValues.toString());
        for (int i = 0; i < mISettingFilterListeners.size(); i++) {
            ISettingFilterListener listener = mISettingFilterListeners.get(i);
            listener.onFilterResult(values, overrideValues);
        }
    ...
    public void setSharedPreferencesValue(final Map<String, String> keyValues, final String cameraId) {
    ...
    public Size getPreviewSize(String cameraId) {
        ...
        SettingItem pictureRatioItem = mSettingGenerator.getSettingItem(SettingKeys.KEY_PICTURE_RATIO, cameraId);
    ...
    private class SettingServant implements ISettingServant {
        ...
        public void registerSettingChangedListener(
            ...
            Log.i(TAG, "[registerSettingChangedListener], listener:" + listener + ", priority:" + priority);
        ...
        private void postResultToListener(ISettingServant.ISettingChangedListener listener,
            ...
            if (concernedSettings == null && !changedSetting.isEmpty()) {
                Log.i(TAG, "[postResultToListener], all result:" + changedSetting.toString() + ", " +
                        "listener:" + listener);
