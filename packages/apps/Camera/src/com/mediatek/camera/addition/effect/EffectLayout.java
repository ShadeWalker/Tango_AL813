...
public class EffectLayout extends ViewGroup {
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (mOnScrollListener != null) {
                        mOnScrollListener.onScrollOut(EffectLayout.this, OnScrollListener.DIRECTION_UP);
                    }
                }
            });
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP:
            ...
            if (isTouchFocus(mUpPointX, mUpPointY, false) && !mTouchFocused && mPressedView != null) {
                mSelectorRect.setEmpty();
                setPressed(true);
                mPressedView.setPressed(true);
                positionSelector(mDownRow, mPressedView);
            ...
            Log.i(TAG, "mUpPointX:" + mUpPointX + ", mUpPointY:" + mUpPointY + ", distance:" + Math.abs(mUpPointY - mDownPointY));
    ...
    private void scrollViewByDistance(int deltaY) {
        ...
        // the max distance scroll should not exceed mColumnHeight, if the mTopPosition > 0, it is no 
    ...
    private boolean isTouchFocus(int x, int y, boolean isMoving) {
        ...
        if (isMoving) {
            ...
            Log.i(TAG, "Moving, distanceX:" + distanceX + ", distanceY:" + distanceY + ", max:" + max + ", " +
    ...
    private void scrollView(int topPosition) {
        ...
        Log.i(TAG, "topRow:" + topRow + ", mLastTopRow:" + mLastTopRow + ",topPosition:" + topPosition);
        ...
        if (mLastTopRow > topRow) {
            ...
            if (child != null) {
                child.layout(0, inRow * mColumnHeight, mColumnWidth * mColumnCount, (inRow + 1) * mColumnHeight);
                mHandler.postDelayed(new Runnable() {
    ...
    public void setAdapter(BaseAdapter adapter) {
        ...
        if (adapter.getCount() % mColumnCount != 0) { //may be total cell can't divide by column count
    ...
    private void positionSelector(int row, View sel) {
        ...
        selectorRect.set(sel.getLeft(), row * mColumnHeight, sel.getRight(), (row + 1) * mColumnHeight);
