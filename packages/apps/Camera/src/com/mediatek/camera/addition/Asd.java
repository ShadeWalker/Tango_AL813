...
public class Asd extends CameraAddition {
    ...
    private final AsdListener mASDCaptureCallback = new AsdListener() {
        public void onDeviceCallback(int scene) {
            Log.i(TAG, "[onDeviceCallback] onDetected scene.obj = " + scene);
            mICameraAppUi.onDetectedSceneMode(scene);
            ICameraView iCameraView = mICameraAppUi.getCameraView(CommonUiType.PICKER);
            ListPreference pref = mISettingCtrl.getListPreference(SettingConstants.KEY_HDR);
            if (pref != null && mLastScene != scene) {
                if (scene == SCENE_BACK_LIGHT || scene == SCENE_BACK_LIGHT_PORTRAIT) {
                    // the value of 2 presents backlight scene, the value of 8
                    // presents
                    // backlight-portrait scene, but backlight and
                    // backlight-portrait scene
                    // is replaced by hdr scene in ROME chip
                    pref.setEnabled(true);
                    mICameraAppUi.updatePreference();
                    iCameraView.refresh();
                    mICameraAppUi.showToast(mActivity.getString(R.string.asd_hdr_guide));
                } else {
                    pref.setEnabled(false);
                    mICameraAppUi.updatePreference();
                    iCameraView.refresh();
                    mICameraAppUi.hideToast();
                }
            }
            mLastScene = scene;
        }
