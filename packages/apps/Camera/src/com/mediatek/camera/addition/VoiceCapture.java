...
public class VoiceCapture extends CameraAddition {
    ...
    public VoiceCapture(ICameraContext context) {
        ...
        mIsSupportIndicator = mICameraContext.getFeatureConfig().isVoiceUiSupport()
                && (mIModuleCtrl.isNonePickIntent() || mIModuleCtrl.isImageCaptureIntent());
    }
    ...
    private void setVoiceValue() {
        ...
        String value = null;
        if (mISettingCtrl != null && mISettingCtrl.getListPreference(SettingConstants.KEY_VOICE) != null) {
            value = mISettingCtrl.getListPreference(SettingConstants.KEY_VOICE)
                    .getValue();
            if (isSupportVoiceCase()) {
                mISettingCtrl.setSettingValue(SettingConstants.KEY_VOICE, value, mICameraDeviceManager.getCurrentCameraId());
            }
        }
    ...
    private void notifyStateChangedIfNeed() {
        ...
        Log.d(TAG, "[notifyStateChangedIfNeed]...mIsSyncWithNative  = " + mIsSyncWithNative + ", mISettingCtrl" + mISettingCtrl);
        if (!mIsSyncWithNative) {
            if (mVoiceValue != null && mISettingCtrl != null && mISettingCtrl.getListPreference(SettingConstants.KEY_VOICE) != null) {
                mISettingCtrl.getListPreference(SettingConstants.KEY_VOICE)
                        .setValue(mVoiceValue);
                if (checkNeedCaptureCase()) {
                    mISettingCtrl.setSettingValue(SettingConstants.KEY_VOICE, mVoiceValue, mICameraDeviceManager.getCurrentCameraId());
                }
    ...
    private void notifyKeyWordsToUi() {
        Log.d(TAG, "[notifyKeyWordsToUi] mKeywords=" + mKeywords + ", mISettingCtrl = " + mISettingCtrl);
        if (mKeywords != null && mISettingCtrl != null && mISettingCtrl.getListPreference(SettingConstants.KEY_VOICE) != null) {
            mISettingCtrl.getListPreference(SettingConstants.KEY_VOICE)
 ...
 // check the cases capture or not.
    ...
    private boolean isSupportVoiceCase() {
        ...
        isNoNeed = !mIsSupportIndicator
                ...
                || "on".equals(mISettingCtrl.getSettingValue(SettingConstants.KEY_MAV))
                || "on".equals(mISettingCtrl.getSettingValue(SettingConstants.KEY_LIVE_PHOTO))
                || "on".equals(mISettingCtrl.getSettingValue(SettingConstants.KEY_MOTION_TRACK))
