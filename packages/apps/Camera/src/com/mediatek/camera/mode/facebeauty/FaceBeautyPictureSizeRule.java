...
public class FaceBeautyPictureSizeRule implements ISettingRule {
    ...
    public void execute() {
        ...
        if (index != -1) {
            ...
            String nearLimitPictureSize = null;
            long nearLimit = 0l;
            int indexNum;
            int width;
            int height;
            // here get entry values from listPreference and then remove size max limitPictureSize
            for (int i = 0; i < entryValues.length; i++) {
                ...
                height = Integer.parseInt(entryValues[i].toString().substring(
                        indexNum + 1));
                if (limitPictureSize >= width * height) {
                    // remember the maximum size which is not bigger limitPictureSize
                    if (nearLimit < width * height) {
                        nearLimit = width * height;
                        nearLimitPictureSize = "" + width + "x" + height;
                    }
                    overValues.add("" + width + "x" + height);
                }
            }
            // if resultValue is not bigger than limitPictureSize use it or use near limitPictureSize
            if (0 > overValues.indexOf(resultValue)) {
                resultValue = nearLimitPictureSize;
            }

            String[] values = new String[overValues.size()];
            String overrideValue = SettingUtils.buildEnableList(overValues.toArray(values), resultValue);
