...
import java.util.List;
    ...
    public class VideoHdrRule implements ISettingRule {
        ...
        private void setHdrRule() {
            ...
            if (ParametersHelper.getParametersSupportedValues(mParameters, SettingConstants.KEY_HDR) == null
                    || ParametersHelper.getParametersSupportedValues(mParameters,
                            SettingConstants.KEY_HDR).indexOf("on") < 0) {
        ...
        private void restoreHdrRule() {
            Log.i(TAG, "[restoreHdrRule], mLastHdrValue:" + mLastHdrValue + ", mHasOverride:" + mHasOverride);
            if (!mHasOverride) {
                return;
            }
            ...
            SettingItem setting = mISettingCtrl.getSetting(SettingConstants.KEY_HDR);
            int overrideCount = setting.getOverrideCount();
            if (overrideCount > 0 && setting.getOverrideRecord(SettingConstants.KEY_VIDEO) != null) {
                overrideCount--;
