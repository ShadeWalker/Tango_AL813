...
public class PIPOperator implements ImageReaderWrapper.ImageReaderCallback {
    ...
    public void initPIPRenderer() {
        ...
        if (mImageReaderWrapper == null) {
            mImageReaderWrapper = new ImageReaderWrapper(mActivity, this);
        }
    }
    ...
    public void setPreviewTextureSize(int width, int height) {
        ...
        mRendererManager.setPreviewTextureSize(width, height);
    }
    ...
    public SurfaceTexture getBottomSurfaceTexture() {
        ...
        return mRendererManager.getBottomSurfaceTexture();
    }
    ...
    public SurfaceTexture getTopSurfaceTexture() {
        ...
        return mRendererManager.getTopSurfaceTexture();
    }
    ...
    /**
     * get pip bottom surface texture's width.
     * @return bottom surface texture's width
     */
    ...
    /**
     * get pip bottom surface texture's height.
     * @return bottom surface texture's height
     */
     ...
     */
    //TODO consider API2
    public void takePicture(byte[] jpeg, int width, int height, boolean isBottomCamera, int captureOrienation) {
        Log.i(TAG, "takePicture jpeg = " + jpeg +
                   " width = " + width + 
                   " height = " + height + 
                   " isBottomCamera = " + isBottomCamera + 
                   " captureOrienation = " + captureOrienation);
        if (isBottomCamera) {
            mImageReaderWrapper.setUpImageReaderRelated(width, height);
            mRendererManager.setCaptureSurface(mImageReaderWrapper.getSurface());
        }
        mImageReaderWrapper.offerRawData(jpeg,width, height, isBottomCamera, captureOrienation);
    }
    ...
    public void setRecordingSurface(Surface surface) {
        ...
        mRendererManager.setRecordingSurface(surface);
    }
    ...
    public void takeVideoSnapshot(int orientation,boolean isBackBottom) {
        ...
        int max = Math.max(mRendererManager.getPreviewTextureHeight(), mRendererManager.getPreviewTextureWidth());
        int min = Math.min(mRendererManager.getPreviewTextureHeight(), mRendererManager.getPreviewTextureWidth());
        int width = isLandscape? max : min;
        int height = isLandscape?min : max;
        mImageReaderWrapper.takeVideoSnapshot(width, height);
        mRendererManager.setCaptureSurface(mImageReaderWrapper.getSurface());
        mRendererManager.takeVideoSnapShot(orientation,isBackBottom);
    }
    ...
    public static class PIPCustomization {
        ...
        public static final float TOP_GRAPHIC_LEFT_TOP_RELATIVE_VALUE = TOP_GRAPHIC_DEFAULT_EDGE_RELATIVE_VALUE
