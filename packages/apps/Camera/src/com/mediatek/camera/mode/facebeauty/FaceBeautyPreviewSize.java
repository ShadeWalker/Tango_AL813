...
public class FaceBeautyPreviewSize implements ISettingRule {
    ...
    private boolean setVFBPreviewSize() {
        boolean hasChangePreviewSize = false;
        for (int i = 0; i < mSupportedPreviewSizes.size(); i++) {
            if (mSupportedPreviewSizes.get(i).width > FaceBeautyParametersHelper.VIDEO_FACE_BEAUTY_MAX_SOLUTION_WIDTH
                    || mSupportedPreviewSizes.get(i).height > FaceBeautyParametersHelper.VIDEO_FACE_BEAUTY_MAX_SOLUTION_HEIGHT) {
                ...
                hasChangePreviewSize = true;
            }
        }
        leftSupportedPreviewSize(mSupportedPreviewSizes);
        
        double previewRatio = Double.parseDouble(mISettingCtrl
                .getSettingValue(SettingConstants.KEY_PICTURE_RATIO));
        Size optiomSize = SettingUtils.getOptimalPreviewSize(mActivity, mSupportedPreviewSizes,
                previewRatio, false, true);
        
        Log.d(TAG, "[setVFBPreviewSize] hasChangePreviewSize " + hasChangePreviewSize
                + ", width = " + optiomSize.width + ",height = " + optiomSize.height);
        if (hasChangePreviewSize) {
            mICameraDevice.setPreviewSize(optiomSize.width, optiomSize.height);
        }
        
        return hasChangePreviewSize;
    }
