...
public class PipPhotoMode extends PhotoMode implements PipController.Listener,
    ...
    private boolean mIsModeClosed =false;
    ...
    public PipPhotoMode(ICameraContext cameraContext) {
        ...
        mICameraAppUi.setZoomListener(this);
    ...
    public void canDoStartPreview() {
        Log.i(TAG, "[canDoStartPreview]mCameraClosed = " + mCameraClosed
                + ", mIsModeClosed = " + mIsModeClosed);
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if (!mCameraClosed && !mIsModeClosed) {
    ...
    private void pipDenaliZSDRule(int index) {
        ...
        if (!MTK_CHIP_0321.equals(mICameraContext.getFeatureConfig()
                .whichDeanliChip())
    ...
    private boolean enableFD() {
        ...
        // supports FD
        boolean enable = mICameraDeviceManager
                .getCameraDevice(mICameraDeviceManager.getCurrentCameraId()) == mICameraDeviceManager
