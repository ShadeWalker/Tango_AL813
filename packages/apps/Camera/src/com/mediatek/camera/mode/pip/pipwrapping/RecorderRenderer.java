...
//        Log.i(TAG, "draw begin");
        synchronized (mReadyFence) {
            if (!mReady || !mIsCanRecordFirstFrame) {
//                mRecorder.dropVideoFrame(timestamp);
                Log.i(TAG, "mReady = " + mReady + " mIsCanRecordFirstFrame = "
                        + mIsCanRecordFirstFrame);
                return;
            }
            long presentationTimeNs = timestamp;
//            long presentationTimeNs = timestamp - mRecorder.getVideoPausedDurationUs() * 1000;
//            if (presentationTimeNs / 1000 <= mRecorder.getVideoLatestPresentationTimeUs()) {
...
//                    mRecorder.notifyFirstVideoFrameReady(timestamp);
//                    mIsFirstVideoFrameReady = true;
//                }
//                mRecorder.setVideoLatestPresentationTimeUs(presentationTimeNs / 1000);
                mEncoderHandler.obtainMessage(EncoderHandler.MSG_FRAME_AVAILABLE,
                        (int) (presentationTimeNs >> 32), (int) presentationTimeNs, textureId).sendToTarget();
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_QUIT:
                if (mRecordingEGLSurface != null) {
                    ...
                    Log.i(TAG, "RecorderRendere swap buffer total count : " + mSwapVideoBufferCount);
                }
                if (mEglCore != null) {
                    mEglCore.release();
                    mEglCore = null;
