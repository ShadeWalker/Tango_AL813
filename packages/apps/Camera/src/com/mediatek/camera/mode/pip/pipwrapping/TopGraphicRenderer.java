...
public class TopGraphicRenderer extends Renderer {
    ...
    public TopGraphicRenderer(Activity activity) {
        ...
        //TODO init should align to RendererManager's init
    ...
    public void draw(int preTex, final float[] preTexMtx, final float[] texReverseRotateMtx,
        ...
        if (rotation < 0) {
            ...
            lowWidth = isLandScape ? (longer - shorter)
                    * (bottomIsMainCamera ? (1f - topGraphicCropValue) : topGraphicCropValue)
                    / longer : 0;
            highWidth = isLandScape ? ((longer - shorter)
                    * (bottomIsMainCamera ? (1f - topGraphicCropValue) : topGraphicCropValue) + shorter)
                    / longer
                    : 1;
            ...
            case 90:
                mTexCoordBuf = createFloatBuffer(mTexCoordBuf,
                        GLUtil.createRightTexCoord(lowWidth, highWidth, lowHeight, highHeight));
                break;
            case 180:
                mTexCoordBuf = createFloatBuffer(mTexCoordBuf,
                        GLUtil.createStandTexCoord(lowWidth, highWidth, lowHeight, highHeight));
                break;
            case 270:
                mTexCoordBuf = createFloatBuffer(mTexCoordBuf,
                        GLUtil.createLeftTexCoord(lowWidth, highWidth, lowHeight, highHeight));
                break;
            }
        }
//        Log.v(TAG, "isLandScape = " + isLandScape + " rotation = " + rotation + " lowHeight = "
//                + lowHeight + " highHeight = " + highHeight + " lowWidth = " + lowWidth
//                + " highWidth = " + highWidth);
