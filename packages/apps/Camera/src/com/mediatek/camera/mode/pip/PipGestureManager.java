...
//        }
    ...
    public void setRendererSize(int width, int height) {
        ...
        mEditButtonSize = Math.min(width, height) / PIPCustomization.TOP_GRAPHIC_EDIT_BUTTON_RELATIVE_VALUE;
        initEditButtonRect(mTopGraphicRect.getRightBottom()[0], mTopGraphicRect.getRightBottom()[1], mEditButtonSize);
    }
    ...
    public boolean onDown(float x, float y, int relativeWidth, int relativeHeight) {
        ...
        boolean mFboPreviewIsLandscape = (GLUtil.getDisplayOrientation(0, mListener.getButtomGraphicCameraId()) % 90 == 0);
        boolean mRelativeFrameIsLandscape = (GLUtil.getDisplayRotation((Activity) mContext) % 180 == 0);
    ...
    public boolean onScroll(float dx, float dy, float totalX, float totalY) {
        ...
        synchronized (mSyncTopGraphicRect) {
            switch (mListener.getGSensorOrientation()) {
            case 0:
                break;
            case 90:
                float temp = dx;
