...
public class PipController implements PipGestureManager.Listener, PIPOperator.Listener, PipView.Listener {
    ...
    public void onUpdateEffect(int backResourceId, int frontResourceId, 
        ...
        updateEffectTemplates(backResourceId, frontResourceId, effectFrontHighlightId, editButtonResourceId);
    }
     ...
     */
    public void takePicture(byte[] jpeg, int width, int height, boolean isBottomCamera, int captureOrientation) {
        Log.i(TAG, "takePicture jpeg = " + jpeg +
                   " width = " + width + 
                   " height = " + height + 
                   " isBottomCamera = " + isBottomCamera);
        if (mPipOperator != null) {
            mPipOperator.takePicture(jpeg, width, height, isBottomCamera, captureOrientation);
        }
    }
    ...
    public void takeVideoSnapshot(int orientation,boolean isBackBottom) {
        Log.i(TAG, "takeVideoSnapshot orientation = " + orientation + " isBackBottom = " + isBackBottom);
        if (mPipOperator != null) {
    ...
    public void init(ICameraContext cameraContext, Listener listener) {
        ...
        synchronized (mSyncLock) {
            ...
            if (mPipView == null) {
                ...
                mPipView.init(mActivity, cameraContext.getCameraAppUi(), cameraContext.getModuleController());
