...
public class AudioEncoder extends Encoder {
    ...
    private ExecutorService           mEncodingService = Executors.newSingleThreadExecutor(); // re-use mEncodingService
    ...
    public void prepareAudioEncoder(String mimeType, int audioSource, int sampleRate, int channelCount, int bitRate) {
    ...
    public void startAudioRecording() {
        ...
        Thread mRecordingThread = new Thread("AudioEncoder") {
            ...
            public void run() {
                ...
                //when eos arrived, should wait previous buffer handled, and then send EOS to media codec
    ...
    public void offerAudioEncoder(byte[] input, long presentationTimeStampNs) {
        Log.v(TAG, "offerAudioEncoder getEncodeState = " + getEncodeState() + " mIsVideoStarted = " + mIsVideoStarted);
        int encodeState = getEncodeState();
        if (!mIsVideoStarted || (encodeState == ENCODE_STATE_PAUSED) || (encodeState == ENCODE_STATE_STOPPED) || input == null) {
            // in paused state, should keep paused duration
            if (encodeState == ENCODE_STATE_PAUSED) {
                setPausedDurationUs((presentationTimeStampNs - mAVSyncDuration) / 1000 - getLatestPresentationTimeUs());
            }
            if (mAudioSoftwarePoller != null) {
                mAudioSoftwarePoller.recycleInputBuffer(input);
            }
            Log.i(TAG, "offerAudioEncoder getEncodeState = " + getEncodeState() + " mIsVideoStarted = " + mIsVideoStarted + " input = " + input);
            return;
        }
        if (!mIsAudioStarted) {
        ...
        presentationTimeStampNs = presentationTimeStampNs - mAVSyncDuration - getPausedDurationUs() * 1000;
        if ((presentationTimeStampNs / 1000) <= getLatestPresentationTimeUs()) {
            ...
            Log.i(TAG, "presentationTimeStampNs = " + presentationTimeStampNs + 
                    " mAVSyncDuration = " + mAVSyncDuration + " getPausedDurationUs = " + getPausedDurationUs() + 
                    " getLatestPresentationTimeUs = " + getLatestPresentationTimeUs() + 
                    " mEncodingServiceQueueLength = " + mEncodingServiceQueueLength);
            if (mAudioSoftwarePoller != null) {
                mAudioSoftwarePoller.recycleInputBuffer(input);
            }
            return;
        ...
        if (!mEncodingService.isShutdown()) {
            ...
            setLatestPresentationTimeUs(presentationTimeStampNs > 0 ? presentationTimeStampNs / 1000 : getLatestPresentationTimeUs());
        }
    }
        ...
        try {
            ...
            if (inputBufferIndex >= 0) {
                ...
                if (mEosAudioArrived) {
                    ...
                    mMediaCodec.queueInputBuffer(inputBufferIndex, 0, input.length, presentationTimeUs, MediaCodec.BUFFER_FLAG_END_OF_STREAM);
    ...
    private void logStatistics() {
        Log.v(TAG + "-Stats", "audio frames input: " + mTotalInputAudioFrameCount + " output: " + mTotalOutputAudioFrameCount);
    }
    ...
    private class EncoderTask implements Runnable {
        ...
        private void encodeFrame() {
            if (encoder == null || audio_data == null || (presentationTimeNs / 1000 <= mSendToEncoderLastTimeStampUs)) {
