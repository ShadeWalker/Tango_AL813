...
public class ScreenRenderer extends Renderer implements Runnable {
    ...
    private Surface             mPreviewSurface = null; // A surface is used to receive preview buffer, will create EGLSurface by it
    ...
    protected void setSurface(Surface surface, boolean scaled, boolean rotated) {
        Log.i(TAG, "setSurface scaled = " + scaled + " rotated = " + rotated + ", mPreviewSurface = " + mPreviewSurface);
        //TODO this is a dirty workaround when configuration change should rotate texture
        ...
        updateEGLSurface(); 
        mRenderTexWidth = (scaled || rotated)? mPreviewEGLSurface.getWidth() : getRendererWidth();
        mRenderTexHeight = (scaled || rotated)? mPreviewEGLSurface.getHeight() : getRendererHeight();
        mTextureRotation = rotated? getDisplayRotation(getActivity()) : 0;
    ...
    private boolean skipUpdateSurface(Surface surface) {
        ...
        boolean skipSurfaceSize = (mRenderTexWidth == getRendererWidth() && mRenderTexHeight == getRendererHeight());
        boolean skipUnValidSurface = !(surface.isValid());
        return (skipSurface && skipMinimalSize && skipRotation && skipSurfaceSize) || skipUnValidSurface;
    }
    ...
    public void draw(AnimationRect topGraphicRect, int texId, boolean highlight) {
        synchronized (mReadyFence) {
            if (mScreenHandler != null && mIsEGLSurfaceReady) {
                ...
                mScreenHandler.obtainMessage(ScreenHandler.MSG_FRAME_AVAILABLE, texId, highlight? 1 : 0, topGraphicRect).sendToTarget();
    ...
    public void notifySurfaceStatus(Surface surface) {
        synchronized (mReadyFence) {
            if (mScreenHandler != null && mPreviewEGLSurface != null && surface == mPreviewEGLSurface.getSurface()) {
                mScreenHandler.removeCallbacksAndMessages(null);
                mReleaseScreenSurfaceSync.close();
    ...
    private void updateRendererSize(int width, int height) {
        ...
        mEditTexSize = Math.min(width, height) / PIPCustomization.TOP_GRAPHIC_EDIT_BUTTON_RELATIVE_VALUE;
        mEditTexRenderer.setRendererSize(width, height);
        mPressedTexRenderer.setRendererSize(width, height);
    }

    private int getDisplayRotation(Activity activity) {
        //TODO add crop coordinate by Renderer Size and Renderer texture size
    ...
    private class ScreenHandler extends Handler {
        ...
        private void doSetupEGLSurface() {
            Log.i(TAG, "handleSetupSurface  mEglCore = " + mEglCore + 
                    " mPreviewEGLSurface = " + mPreviewEGLSurface + " mPreviewSurface = " + mPreviewSurface);
            if (mEglCore == null) {
        ...
        private void doDraw(AnimationRect topGraphicRect, int texId, boolean highlight) {
            ...
            GLES20.glVertexAttribPointer(maTexCoordHandle, 2, GLES20.GL_FLOAT, false, 4*2, mTexCoordBuf);
            ...
            if (topGraphicRect != null) {
                topGraphicRect.changeCooridnateSystem(mRenderTexWidth, mRenderTexHeight, mTextureRotation);
                mEditTexRenderer.draw(topGraphicRect.getRightBottom()[0], topGraphicRect.getRightBottom()[1], mEditTexSize, null,null);
            }
            // draw highlight texture
            if (topGraphicRect != null && highlight) {
                mTopGraphicPositionBuf = createFloatBuffer(mTopGraphicPositionBuf, GLUtil.createTopRightRect(topGraphicRect));
