...
public class ModeChecker {
    ...
    static {
        ...
        MATRIX_NORMAL_ENABLE[ModePicker.MODE_MAV] = new boolean[] { true, true };
    ...
    public static void updateModeMatrix(CameraActivity camera, int cameraId) {
        ...
        //Current MotionTrack feature table maybe not right,such as the FO is no,
        //so native will remove the the MT code,but the feature table also have this
        //capture mode,so need check the system properties again
        int index = ModePicker.MODE_MOTION_TRACK;
        if (MATRIX_NORMAL_ENABLE[index][cameraId]
                && supported.indexOf(MODE_STRING_NORMAL[index]) > 0) {
            boolean isVisble = FeatureSwitcher.isMotionTrackSupported();
            MATRIX_NORMAL_ENABLE[index][cameraId] = isVisble;
            Log.d(TAG, "MotionTrack mode visible will be set to : " + isVisble);
        }
        ...
        if (cameraId == CameraInfo.CAMERA_FACING_BACK) {
            MATRIX_NORMAL_ENABLE[ModePicker.MODE_LIVE_PHOTO][cameraId] = FeatureSwitcher
                    .isLivePhotoEnabled();
    ...
    public static int modesSupportedByCamera(CameraActivity camera, int cameraId) {
        int count = 0;
        boolean stereo = camera.isStereoMode();
        boolean[][] matrix;
        if (FeatureSwitcher.isStereoSingle3d() && stereo) {
        ...
        for (int i = 0; i < ModePicker.MODE_VIDEO; i++) {
            ...
            // them
            if (matrix[i][cameraId] && i != ModePicker.MODE_ASD && i != ModePicker.MODE_HDR) {
                count++;
            }
        }
        return count;
