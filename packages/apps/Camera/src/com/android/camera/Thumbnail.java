...
public class Thumbnail {
    ...
    private Thumbnail(Uri uri, Bitmap bitmap, int orientation, long id, long dataTaken, String filePath) {
        mUri = uri;
    ...
    private static class Media {
        public Media(long id, int orientation, long dateTaken, Uri uri, int mediaType,
                int isLivePhoto, String filePath) {
            this.id = id;
        ...
        public String toString() {
            return new StringBuilder().append("Media(id=").append(id).append(", orientation=")
                    ...
                    .append(uri).append(", mediaType=").append(mediaType).append(", isLivePhoto=")
                    .append(isLivePhoto).append(", filePath=").append(filePath).append(")")
                    .toString();
        }
    }
    ...
    public static int getLastThumbnailFromContentResolver(ContentResolver resolver,
        ...
        String[] projection = new String[] { ImageColumns._ID,// image and video
                ...
                FileColumns.MEDIA_TYPE, Video.Media.IS_LIVE_PHOTO, // live photo
                                                                   // tag
        };
        String selection = "((" + ImageColumns.MIME_TYPE + "='image/jpeg' OR "
                + ImageColumns.MIME_TYPE + "='image/mpo') AND " + ImageColumns.BUCKET_ID + '='
                + Storage.getBucketId() + ") OR (" + ImageColumns.BUCKET_ID + '='
                + Storage.getBucketId() + ")";
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                if (oldThumbnail == null || id != oldThumbnail.mThumbnailId
                    ...
                    media = new Media(id, cursor.getInt(1), cursor.getLong(2),
                            ...
                            cursor.getInt(5), cursor.getString(3));
                } else {
        ...
        try {
            ...
            if (Util.isUriValid(media.uri, resolver)) {
                result[0] = createThumbnail(media.uri, bitmap, orientation, id, dateTaken, filePath);
