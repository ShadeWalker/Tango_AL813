...
public class CameraDeviceImpl implements ICameraDevice {
    ...
    @Override
    public boolean setMavCallback(MavListener mavListener) {
        mCameraDevice.setMavCallback(mavListener == null ? null : new MavListenerImpl(mavListener));
        return true;
    }
    
    @Override
    public boolean setAutoRamaCallback(PanoramaListener panoramaListener) {
        mCameraDevice.setAutoRamaCallback(panoramaListener == null ? null : new AutoRamaListenerImpl(
                panoramaListener));
        return true;
    }
    ...
    @Override
    public boolean setMotionTrackCallback(Listener motionTrackCallback) {
        mCameraDevice.setMotionTrackCallback(motionTrackCallback == null ? null
                : new MotionTrackListenerImpl(motionTrackCallback));
        return true;
    }
    ...
    public void setObjectTrackingListener(OtListener otListener) {
        mCameraDevice.setObjectTrackingListener(otListener == null ? null : new OTListener(otListener));
    }
    
    @Override
    public boolean setcFBOrignalCallback(cFbOriginalCallback cFbOrillback) {
        mCameraDevice.setFbOriginalCallback(cFbOrillback == null ? null : new cFBCallback(cFbOrillback));
        return true;
    }
    @Override
    public void setStereoJpsCallback(StereoJpsCallback jpsCallback) {
        mCameraDevice.setStereoCameraJpsCallback(jpsCallback == null ? null : new StereoCameraJpsListener(jpsCallback));
    }
    
    @Override
    public void setStereoMaskCallback(StereoMaskCallback maskCallback) {
        mCameraDevice.setStereoCameraMaskCallback(maskCallback == null ? null : new StereoCameraMaskListener(maskCallback));
    }
    
    @Override
    public void setStereoWarningCallback(StereoWarningCallback warningCallback) {
        mCameraDevice.setStereoCameraWarningCallback(warningCallback == null ? null : new StereoCameraWarningListener(warningCallback));
    }
    
    @Override
    public void setStereoDistanceCallback(StereoDistanceCallback distanceCallback) {
        mCameraDevice.setStereoCameraDistanceCallback(distanceCallback == null ? null : new StereoCameraDistanceListener(distanceCallback));
    }

    @Override
    public boolean startMav(int num) {
        mCameraDevice.startMav(num);
        return true;
    }
    
    @Override
    public boolean stopMav(boolean merge) {
        mCameraDevice.stopMav(merge ? 1 : 0);
        return true;
    }
    ...
    @Override
    public boolean startMotionTrack(int num) {
        mCameraDevice.startMotionTrack(num);
        return true;
    }
    
    @Override
    public boolean stopMotionTrack() {
        mCameraDevice.stopMotionTrack();
        return true;
    }
    ...
    private class MavListenerImpl implements MavCallback {
        private MavListener mListener;
        
        MavListenerImpl(MavListener callback) {
            mListener = callback;
        }
    ...
    private class MotionTrackListenerImpl implements MotionTrackCallback {
        private Listener mListener;
        
        MotionTrackListenerImpl(Listener listener) {
            mListener = listener;
        }
        ...
        public void onDataCallback(byte[] data) {
            if (mListener != null) {
                mListener.onDeviceCallback(data);
            }
        }
    ...
    @Override
    public void takePictureAsync(ShutterCallback shutterCallback, PictureCallback rawPictureCallback, 
            PictureCallback postViewPictureCallback, PictureCallback jpegPictureCallback) {
        mCameraDevice.takePictureAsync(shutterCallback, rawPictureCallback, postViewPictureCallback,
                jpegPictureCallback);
    }
