...
public class CameraDeviceProxyImpl implements CameraDeviceProxy {
    ...
    public CameraDeviceProxyImpl(Activity activity, CameraDevice camera, CameraSessionCallback callback, Handler sessionHandler) {
        mActivity      = activity;
                ...
                try {
                    synchronized (mSessionLock) {
                        if (mIsSessionAbortCalled || mIsClosed) {
                            Log.i(TAG, "ignore configure session, mIsSessionAbortCalled:" + mIsSessionAbortCalled
                        ...
                        mCameraDevice.createCaptureSession(outputSurfaces, mSessionStateCallback, mCameraHandler);
                    }
            ...
            case REQUEST_CHANGE_REQUEST:
                synchronized (mSessionLock) {
                    if (mIsSessionAbortCalled || mCameraCaptureSession == null || mIsClosed) {
                        Log.i(TAG, "ignore request change, mIsSessionAbortCalled:" + mIsSessionAbortCalled
                                + " mCameraCaptureSession:" + mCameraCaptureSession
                                + " DeviceClosed:" + mIsClosed);
                        return;
                    }
                    ...
                    CaptureCallback captureCallback = mCameraSessionCallback.configuringSessionRequests(builder, requestType, captureType);
                    // submit capture requests
                    submitCaptureRequests(builder.build(), requestType, captureType, captureCallback);
