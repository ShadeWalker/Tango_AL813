...
public class CaptureAnimManager {
    ...
    private float mCenterX; // The center of the whole view including preview
                            // and review.
    ...
    public abstract class Animation {
        ...
        boolean drawAnimation(GLCanvas canvas, CameraScreenNail preview, RawTexture review, int lx,
                int ly, int lw, int lh) {
            return false;
        }
    ...
    private class FadeInFadeOutAnimation extends Animation {
        private static final float ZOOM_DELTA = 0.2f; // The amount of change
                                                      // for zooming out.
        private static final float ZOOM_IN_BEGIN = 1f - ZOOM_DELTA; // Pre-calculated
                                                                    // value for
                                                                    // convenience.
        private static final float CAPTURE_ANIM_DURATION = 700; // milliseconds.
        private static final float GAP_RATIO = 0.1f; // The gap between preview
                                                     // and review based
        ...
        private float mCenterDelta; // The amount of the center will move after
                                    // whole animation.
        private float mGap; // mGap = (width or height) * GAP_RATIO. (depends on
                            // orientation)
        ...
        boolean drawAnimation(GLCanvas canvas, CameraScreenNail preview, RawTexture review, int lx,
            ...
            if (timeDiff > CAPTURE_ANIM_DURATION)
                return false;
        ...
        private float calculateScale(float fraction) {
            ...
            if (fraction <= 0.5f) {
                ...
                value = 1f - ZOOM_DELTA * mZoomOutInterpolator.getInterpolation(fraction * 2);
            } else {
                // Zoom out for the last.
                value = ZOOM_IN_BEGIN + ZOOM_DELTA
                        * mZoomInInterpolator.getInterpolation((fraction - 0.5f) * 2f);
            }
            return value;
        }
    };
    ...
    private class FlashHoldAnimation extends Animation {
        ...
        boolean drawAnimation(GLCanvas canvas, CameraScreenNail preview, RawTexture review, int lx,
            ...
            if (animStep == ANIM_FLASH) {
                ...
                float fraction = mSlideInterpolator.getInterpolation((float) (timeDiff)
                ...
                preview.directDraw(canvas, (int) mCenterX, (int) mCenterY, mDrawWidth, mDrawHeight);
                mBorder.draw(canvas, (int) x - mShadowSize, (int) y - mShadowSize, (int) mHoldW + 2
                        * mShadowSize, (int) mHoldH + 2 * mShadowSize);
