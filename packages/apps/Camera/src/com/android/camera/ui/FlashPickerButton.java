...
public class FlashPickerButton extends PickerButton {
    ...
    protected int getValidIndexIfNotFind(String value) {
        int index = 0;
        if (FLASH_AUTO.equals(value)) { // if cannot find auto, it video mode,
                                        // match off
            index = mPreference.findIndexOfValue(FLASH_OFF);
        } else if (FLASH_ON.equals(value)) { // if cannot find on, it video
                                             // mode, match torch
            index = mPreference.findIndexOfValue(FLASH_TORCH);
        } else if (FLASH_TORCH.equals(value)) { // if cannot find torch, it
                                                // video mode, match on
