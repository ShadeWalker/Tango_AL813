...
//            mPreference.setLastValue(index == 1);
            if (index == 1) {
                SettingUtils.writePreferredStereoCamera(mContext.getPreferences(), SettingUtils.STEREO_CAMERA_ON);
                if (mListener != null) {
                    mListener.onStereoCameraSettingChanged(this, mPreference, 3, false);
                }
            } else {
                SettingUtils.writePreferredStereoCamera(mContext.getPreferences(), SettingUtils.STEREO_CAMERA_OFF);
                if (mListener != null) {
    ...
    protected void updateView() {
        ...
        if (!needUpdateSwitch() && SettingUtils.readPreferredStereoCamera(mContext.getPreferences()).equals(SettingUtils.STEREO_CAMERA_ON)) {
            mIsSubListItemEnable = true;
            mSwitch.setChecked(true);
        } else if (needUpdateSwitch() && SettingUtils.readPreferredStereoCamera(mContext.getPreferences()).equals(SettingUtils.STEREO_CAMERA_OFF)) {
    ...
    public boolean expendChild() {
        ...
        if (!mShowingChildList) {
            ...
            mSettingLayout = (SettingSwitchVirtualLayout)mContext.inflate(R.layout.setting_switch_virtual_layout,
...
//            mTitle.setTextColor(SettingUtils.getMainColor(getContext()));
    ...
    public void onStereoCameraSettingChanged(int index, boolean isChecked) {
        if (mListener != null) {
            if (!isChecked && SettingUtils.readPreferredStereoCamera(mContext.getPreferences()).equals(SettingUtils.STEREO_CAMERA_ON)) {
                SettingUtils.writePreferredStereoCamera(mContext.getPreferences(), SettingUtils.STEREO_CAMERA_OFF);
            }
            if (isChecked && SettingUtils.readPreferredStereoCamera(mContext.getPreferences()).equals(SettingUtils.STEREO_CAMERA_OFF)) {
                SettingUtils.writePreferredStereoCamera(mContext.getPreferences(), SettingUtils.STEREO_CAMERA_ON);
            }
