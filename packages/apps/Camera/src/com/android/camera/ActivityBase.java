...
public abstract class ActivityBase extends AbstractGalleryActivity implements
    ...
    public void createCameraScreenNail(boolean getPictures) {
        String path = null;
        if (FeatureSwitcher.isApi2Enable(this)) {
            mCameraAppView = findViewById(R.id.camera_view_container);
            path = getPictures ? getCameraActivityBridge().getCameraFolderPath() : 
                                 "/local/all/0";
        } else {
            mCameraAppView = findViewById(R.id.camera_app_root);
            path = getPictures ? Storage.getCameraScreenNailPath() : "/local/all/0";
        }
