...
public final class RichInputConnection {
    ...
    public int getCodePointBeforeCursor() {
        ...
        if (mCommittedTextBeforeComposingText.length() < 1) {
            resetCachesUponCursorMoveAndReturnSuccess(-1, -1, false);
        }
        /// @}
    ...
    public boolean isCursorTouchingWord(final SpacingAndPunctuations spacingAndPunctuations) {
        ...
        return !(Constants.NOT_A_CODE == consideredCodePoint
                ...
                /// M: add for touching a color Emoji
                || Character.getType(consideredCodePoint) == Character.OTHER_SYMBOL);
    }

    public boolean isCursorFollowedByWordCharacter(
            final SpacingAndPunctuations spacingAndPunctuations) {
        /// M: It may be a Emoji, so the length should be 2;
        final CharSequence after = getTextAfterCursor(2, 0);
        ...
        if (spacingAndPunctuations.isWordSeparator(codePointAfterCursor)
                ...
                /// M: add for touching a color Emoji
                || Character.getType(codePointAfterCursor) == Character.OTHER_SYMBOL) {
            return false;
        }
        return true;
    }
    ...
    public boolean hasSelection() {
        /// M: fix the can not delete the selection issue. @{
        if (mExpectedSelEnd == mExpectedSelStart
            && (mExpectedSelEnd == 0 || mExpectedSelEnd == INVALID_CURSOR_POSITION)) {
    ...
    public void fixTheAsyncSelectionUpdate(){
        final CharSequence selectedText = getSelectedText(0);
        if (!TextUtils.isEmpty(selectedText) && (mExpectedSelStart - mExpectedSelEnd) != selectedText.length()) {
            CharSequence textBeforeCursor = getTextBeforeCursor(Constants.EDITOR_CONTENTS_CACHE_SIZE, 0);
            CharSequence textAfterCursor = getTextAfterCursor(Constants.EDITOR_CONTENTS_CACHE_SIZE, 0);
            if (textBeforeCursor != null && textBeforeCursor.length() == 0
                ...
                mExpectedSelStart = 0;
                mExpectedSelEnd = selectedText.length();
            }
        }
    }
    /// @}
