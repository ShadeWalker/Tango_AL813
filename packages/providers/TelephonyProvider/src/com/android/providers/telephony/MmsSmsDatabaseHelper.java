...
//            "  WHERE (new.m_type=132 OR new.m_type=130 OR new.m_type=128) AND threads._id = new." + Mms.THREAD_ID + "; " +
    ...
    private static final String SMS_UPDATE_THREAD_MESSAGE_COUNT_CT = "UPDATE threads SET message_count = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE thread_id = new.thread_id UNION SELECT _id FROM pdu WHERE thread_id = new.thread_id AND (m_type=132 OR m_type=130 OR m_type=128))) "
        + "WHERE _id = new.thread_id;"
        + "UPDATE threads SET readcount = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE (thread_id = new.thread_id AND (read = 1) AND (type != 3)) UNION SELECT _id FROM pdu WHERE (thread_id = new.thread_id AND (m_type=132 OR m_type=130 OR m_type=128) AND read = 1 AND msg_box != 3))) "
        + "WHERE _id = new.thread_id;";
    ...
    private static final String SMS_UPDATE_THREAD_SNIPPET_CT = "UPDATE threads SET snippet = "
        ...
        + "UNION SELECT max(date) as date, body as snippet FROM sms WHERE (thread_id = new.thread_id)) WHERE snippet is not null)) "
        + "WHERE _id = new.thread_id;"
        + "UPDATE threads SET snippet_cs = "
    ...
    private static final String SMS_UPDATE_THREAD_ERROR_CT = "UPDATE threads SET error = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE thread_id = new.thread_id and type = 5 UNION SELECT _id FROM pdu WHERE thread_id = new.thread_id "
    ...
    private static final String SMS_UPDATE_OLD_THREAD_MESSAGE_COUNT_CT = "UPDATE threads SET message_count = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE thread_id = old.thread_id UNION SELECT _id FROM pdu WHERE thread_id = old.thread_id AND (m_type=132 OR m_type=130 OR m_type=128))) "
        + "WHERE _id = old.thread_id;"
        + "UPDATE threads SET readcount = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE (thread_id = old.thread_id AND (read = 1) AND (type != 3)) UNION SELECT _id FROM pdu WHERE (thread_id = old.thread_id AND (m_type=132 OR m_type=130 OR m_type=128) AND read = 1 AND msg_box != 3))) "
        + "WHERE _id = old.thread_id;";
    ...
    private static final String SMS_UPDATE_OLD_THREAD_SNIPPET_CT = "UPDATE threads SET snippet = "
        ...
        + "UNION SELECT max(date), body as snippet FROM sms WHERE (thread_id = old.thread_id)) WHERE snippet is not null)) "
        + "WHERE _id = old.thread_id;"
        + "UPDATE threads SET snippet_cs = "
        + "(SELECT snippet_cs FROM (SELECT max(date), snippet_cs FROM "
    ...
    private static final String SMS_UPDATE_OLD_THREAD_ERROR_CT = "UPDATE threads SET error = "
        + "(SELECT count(_id) FROM (SELECT _id FROM sms WHERE thread_id = old.thread_id and type = 5 UNION SELECT pdu._id FROM pdu WHERE thread_id = old.thread_id "
    ...
    static final int DATABASE_VERSION = 600400;
    ...
    public static void updateThread(SQLiteDatabase db, long thread_id) {
       ...
       try {
            ...
            if (MTK_WAPPUSH_SUPPORT) {
                Cursor pushCursor = db.rawQuery("select * from threads where type=" + Telephony.Threads.WAPPUSH_THREAD + " AND _id=" + thread_id, null);
                if (pushCursor != null) {
                    try {
            ...
            if (MTK_WAPPUSH_SUPPORT) {
                rows = db.delete(MmsSmsProvider.TABLE_THREADS,
                      ...
                      new String[] { String.valueOf(thread_id), String.valueOf(Telephony.Threads.WAPPUSH_THREAD) });
            } else {
                rows = db.delete(MmsSmsProvider.TABLE_THREADS,
                      "_id = ? AND _id NOT IN" +
                      "          (SELECT thread_id FROM sms where thread_id is not null " +
                ...
                removeOrphanedAddresses(db);
                    ...
                    String query = "SELECT thread_id FROM sms WHERE (type = " +
                        Telephony.TextBasedSmsColumns.MESSAGE_TYPE_FAILED + " OR status >= " +
                            Telephony.TextBasedSmsColumns.STATUS_FAILED + ") AND thread_id = " + thread_id;
//                                            + " LIMIT 1";
                    int setError = 0;
    ...
    public static void updateAllThreads(SQLiteDatabase db, String where, String[] whereArgs) {
        Xlog.d(TAG, "updateAllThreads start");
        db.beginTransaction();
        try {
            if (where == null) {
                where = "";
        ...
        Xlog.d(TAG, "updateAllThreads end");
    }
    ...
    protected static void removeOrphanedAddresses(SQLiteDatabase db) {
        ...
        Xlog.d(TAG, "removeOrphanedAddresses, recipient ids = " + ids);
...
//                   "END;");
    ...
    public static void updateMultiThreads(SQLiteDatabase db, long[] deletedThreads) {
        Xlog.d(TAG, "updateMultiThreads start, deletedThreads.length = " + deletedThreads.length);
        if (MTK_WAPPUSH_SUPPORT) {
            db.delete(MmsSmsProvider.TABLE_THREADS,
                  "type <> ? AND _id NOT IN" +
                  "          (SELECT thread_id FROM sms where thread_id is not null " +
        ...
        removeOrphanedAddresses(db);
        Xlog.d(TAG, "delete obsolete threads and addresses end");
                ...
                if (wallpaperPath.exists()) {
                    ...
                    if (i > 0) {
                        for (int j = 0 ; j < i ; j++) {
                            if (threadWallpaperName.equals(oldFile[j])) {
                                boolean d = new File(WALLPAPER_PATH, oldFile[j]).delete();
                                Xlog.d(TAG, "wallpaper " + threadWallpaperName + "isDelete " + d);
                            }
                        }
                    }
                }
        ...
        Xlog.d(TAG, "updateMultiThreads end");
    }
    /// @}
