...
public class WapPushProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        Xlog.d(TAG, "delete end, affectedRows = " + count);
        return count;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (rowId > 0) {
            ...
            notifyChange(uri);
            Xlog.d(TAG, "insert succeed, uri = " + insertUri);
            return insertUri;
        } else {
    ...
    private void updatePushThread(SQLiteDatabase db, long threadId) {
        ...
        Cursor pushCursor = db.rawQuery("select * from threads where type <>"
                + Telephony.Threads.WAPPUSH_THREAD + " AND _id=" + threadId, null);
        if (pushCursor != null) {
            try {
      ...
      */
        ...
        Xlog.i(TAG, "update threads table for wappush: " + updateThreadsInfo);
    ...
    private void updatePushAllThread(SQLiteDatabase db, String where, String[] whereArgs) {
        ...
        db.delete("threads", "_id NOT IN (SELECT DISTINCT thread_id FROM " + TABLE_WAPPUSH + ")"
             + " AND " + Threads.TYPE + " = " + Telephony.Threads.WAPPUSH_THREAD, null);
    }
