...
public class MmsSmsProvider extends ContentProvider {
    ...
    /// @}
    private static final boolean MTK_WAPPUSH_SUPPORT = SystemProperties.get("ro.mtk_wappush_support").equals("1");
    ...
    // For op09 show right number in CoversationList(read and unread) and show right sim indicator in ConversationList.
...
//            "snippet FROM words WHERE index_text MATCH ? ORDER BY snippet LIMIT 50;";
  ...
  /// M: add for op09 of new feature cc @{
    private static final String MMS_CONVERSATION_CONSTRAINT_DISTINCT_INCLUDE_CC = "(" +
        ...
        Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF + " OR " +
        Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND + "))" +
        " AND addr.msg_id = pdu._id AND pdu._id in (select msg_id from addr where addr.type = 130) AND ADDR.type = 130";

    private static final String MMS_CONVERSATION_CONSTRAINT_DISTINCT_NOT_INCLUDE_CC = "(" + Mms.MESSAGE_BOX + " != "
        + Mms.MESSAGE_BOX_DRAFTS + " AND (" + Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_SEND_REQ + " OR "
        + Mms.MESSAGE_TYPE + " = " + PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF + " OR " + Mms.MESSAGE_TYPE + " = "
        + PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND + "))"
        + "AND addr.msg_id = pdu._id AND pdu._id not in (select msg_id from addr where addr.type = 130)";
    /// @}
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        //Xlog.d(LOG_TAG, "query begin, uri = " + uri + ", selection = " + selection);
        switch(URI_MATCHER.match(uri)) {
            ...
            case URI_SEARCH_SUGGEST: {
                ...
                /// M: Code analyze 018, fix bug ALPS00344334, support recipients search
                /// suggestion. @{
                Xlog.d(LOG_TAG, "query().URI_SEARCH_SUGGEST: uriStr = " + uri);
                ...
                Xlog.d(LOG_TAG, "query().URI_SEARCH_SUGGEST: searchString = \"" + searchString + "\"");
            ...
            case URI_SEARCH_ADVANCED: {
                ...
                Xlog.d(LOG_TAG, "query(): URI_SEARCH_ADVANCED, content = " + content + ", name = " + name
                    + ", number = " + number + ", beginDateStr = " + beginDateStr + ", endDateStr = " + endDateStr);
                Xlog.d(LOG_TAG, "query(): URI_SEARCH_ADVANCED, content is null ?= " + (content == null)
                ...
                Xlog.d(LOG_TAG, "query(): URI_SEARCH_ADVANCED, hasContent = " + hasContent + ", hasName = " + hasName
                ...
                if (!hasContent && !hasName && !hasNumber && !hasBeginDate && !hasEndDate) {
                    Xlog.e(LOG_TAG, "query(): URI_SEARCH_ADVANCED, no query parameter.");
                    return null;
                }
                if (hasBeginDate && hasEndDate && beginDate >= endDate) {
                    Xlog.e(LOG_TAG, "query(): URI_SEARCH_ADVANCED, begin data later than end date.");
                    return null;
                }
                ...
                if (TextUtils.isEmpty(searchContacts) && (hasName || hasNumber)) {
                    Xlog.e(LOG_TAG, "query(): URI_SEARCH_ADVANCED, no matched thread");
                    return null;
                }
                ...
                String smsQuery = String.format(Locale.ENGLISH,
                        ...
                        ")",
                        smsProjection);
                Xlog.d(LOG_TAG, "query(): URI_SEARCH_ADVANCED, smsQuery = " + smsQuery);
                ...
                String mmsQuery = String.format(Locale.ENGLISH,
                        ...
                        PduHeaders.TO,
                        Mms.MESSAGE_BOX_INBOX);
                Xlog.d(LOG_TAG, "query(): URI_SEARCH_ADVANCED, mmsQuery = " + mmsQuery);
                ...
                try {
                    ...
                    Xlog.d(LOG_TAG, "query(): URI_SEARCH_ADVANCED, rawQuery = " + rawQuery);
                } catch (Exception ex) {
                    Xlog.e(LOG_TAG, "query(): URI_SEARCH_ADVANCED, got exception: " + ex.toString(), ex);
                    return null;
                }
                break;
                /// @}
            ...
            case URI_SEARCH_FOLDER: {
                ...
                "0 as index_text,words._id,1 as msg_type,sms.type as msg_box,sms.sub_id as sub_id,0 as charset,0 as m_type";
                String smsQuery = String.format(
                ...
                String mmsProjection = "pdu._id,thread_id,addr.address,pdu.sub as " + "" +
                "body,pdu.date,0 as index_text,0 as _id,2 as msg_type,msg_box,sub_id,addr.charset as charset,pdu.m_type as m_type";
            ...
            case URI_UNREAD_SMSMMS_GROUP: {
                ...
                .format("SELECT COUNT(_id) FROM (SELECT ipmsg_id, _id FROM sms WHERE read = 0 AND ipmsg_id = 0 UNION SELECT ipmsg_id, _id FROM sms WHERE read = 0 GROUP BY ipmsg_id HAVING ipmsg_id < 0 UNION SELECT 0 as ipmsg_id, _id FROM pdu WHERE read = 0 AND (m_type=132 OR m_type=130 OR m_type=128) UNION SELECT 1 as ipmsg_id, _id FROM cellbroadcast WHERE read = 0)");
            ...
            case URI_WIDGET_THREAD:
                ...
                try {
                    threadId = Long.parseLong(uri.getLastPathSegment());
                    Xlog.d(LOG_TAG, "query URI_WIDGET_THREAD Thread ID is " + threadId);
                } catch (NumberFormatException e) {
                    Xlog.e(LOG_TAG, "URI_WIDGET_THREAD Thread ID must be a long.");
                    break;
                }
    ...
    private Cursor getSimIdAndMsgCount(SQLiteDatabase db, long threadId) {
        Xlog.d(LOG_TAG, " threadId = " + threadId);
        String smsQuery = String.format("SELECT _id, sub_id, read, date FROM sms where thread_id = " + threadId
            + " ORDER BY date DESC");
        String mmsQuery = String
                .format("SELECT _id, sub_id, read, date * 1000 as date FROM pdu where thread_id = " + threadId
                    + " ORDER BY date DESC");
        ...
        if (mmsCursor != null) {
            try {
                Xlog.d(LOG_TAG, " mmsCursor.getCount() = " + mmsCursor.getCount());
                if (mmsCursor.getCount() > 0) {
                    ...
                    while (mmsCursor.moveToNext()) {
                        if (mmsCursor.getInt(2) == 0) {
                            mmsUnread += 1;
                            Xlog.d(LOG_TAG, " mmsUnread = " + mmsUnread);
                        }
                    }
                }
            } finally {
        ...
        Xlog.d(LOG_TAG, "smsCount = " + smsCount + " smsUnread = " + smsUnread + " mmsCount = " + mmsCount
            + " mmsUnread = " + mmsUnread + " simId = " + simId + " smsSimDate = " + smsSimDate + " mmsSimDate = " + mmsSimDate);
    ...
    private Set<Long> getAddressIds(List<String> addresses) {
        ...
        Cursor cursor = null;
        Xlog.d(LOG_TAG, "getAddressIds begin");
        try {
            SQLiteDatabase db = mOpenHelper.getReadableDatabase();
            db.beginTransaction();
            cursor = db.query(
                ...
                if (!address.equals(PduHeaders.FROM_INSERT_ADDRESS_TOKEN_STR)) {
                                        ...
                                        if(SystemProperties.get("ro.hq.russian.number.combin").equals("1")){
                                           ...
                                           if((refinedAddress.startsWith("+7"))&&(addressValue.startsWith("8"))&&(refinedAddress.length()== (addressValue.length()+1))){
                                               ...
                                               if(temp_refinedAddress != null&&temp_addressValue != null){
                                                   ...
                                                   //MmsLog.d(LOG_TAG, "isEqual2:"+isEqual);
                                                   break;
                                                }
                                         }
                                  } //wanghui for russian end			
                            }
                        }
                    }
                    ...
                    if ((isEqual == true) && (addressId != null) && (addressId != -1L)) {
                        if (Build.TYPE.equals("eng")) {
                           Xlog.d(LOG_TAG, "getAddressIds: get exist id=" + addressId
                                    + ", address=" + address + ", currentNumber="
                                    + addressValue);
                        }
            ...
            long id = -1L;
            if (Build.TYPE.equals("eng")) {
                Xlog.d(LOG_TAG, "getAddressIds: insert new canonical_address for " +
            ...
            for (String addr : refinedAddresses) {
                ...
                if (id != -1L) {
                    ...
                    Xlog.e(LOG_TAG, "getAddressIds: address ID not found for " + addr);
                }
            }
    ...
    private Cursor getUndeliveredMessages(
        ...
        if (includeNonPermanentFail) {
            Xlog.d(LOG_TAG, "getUndeliveredMessages true");
            ...
            Xlog.d(LOG_TAG, "getUndeliveredMessages false");
    ...
    public int delete(Uri uri, String selection,
        ...
        Xlog.d(LOG_TAG, "delete begin, uri = " + uri + ", selection = " + selection);
        ...
        switch(URI_MATCHER.match(uri)) {
            case URI_CONVERSATIONS_MESSAGES:
                ...
                Xlog.d(LOG_TAG, "delete,  deleteConversation end, updateThread start");
            ...
            case URI_CONVERSATIONS:
                ...
                Xlog.d(LOG_TAG, "delete URI_CONVERSATIONS begin, multidelete = " + multidelete);
                if ((multidelete != null) && multidelete.equals("true")) {
                    ...
                    Xlog.d(LOG_TAG, "multi delete URI_CONVERSATIONS end");
                    if (affectedRows > 0) {
            ...
            case URI_OBSOLETE_THREADS:
                ...
                affectedRows += db.delete(TABLE_THREADS,
                                        "recipient_ids = \"\"", null);
                if (affectedRows > 0) {
                    Xlog.d(LOG_TAG, "delete,  delete obsolete threads end, removeOrphanedAddresses start");
            ...
            case URI_OBSOLETE_THREAD_ID:
                ...
                affectedRows += db.delete(TABLE_THREADS,
                                            "recipient_ids = \"\"", null);
                if (affectedRows > 0) {
                    Xlog.d(LOG_TAG, "delete,  delete obsolete thread end, removeOrphanedAddresses start");
                    MmsSmsDatabaseHelper.removeOrphanedAddresses(db);
                }
        ...
        Xlog.d(LOG_TAG, "delete end, affectedRows = " + affectedRows);
        return affectedRows;
    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        Xlog.d(LOG_TAG, "insert begin, uri = " + uri + ", values = " + values);
        /// M: Code analyze 002, new feature, support for quicktext. @{
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
    ...
    public int update(Uri uri, ContentValues values,
        ...
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        int affectedRows = 0;
        Xlog.d(LOG_TAG, "update begin, uri = " + uri + ", values = " + values + ", selection = " + selection);
        switch(URI_MATCHER.match(uri)) {
            case URI_CONVERSATIONS_MESSAGES:
            ...
            case URI_CONVERSATION_SETTINGS_ITEM: {
                ...
                Xlog.d(LOG_TAG, "wallpaperPath: " + wallpaperPath);
                if (wallpaperPath != null) {
                    Xlog.d(LOG_TAG, "wallpaperPath: exsited");
            ...
            case URI_CONVERSATION_SETTINGS: {
                ...
                String path = getContext().getDir("wallpaper", Context.MODE_WORLD_WRITEABLE).getPath();
                Xlog.d(LOG_TAG, "oldWallpaper " + path);
                if (wallpaper != null) {
                    ...
                    for(int i=0 ; i<allFile.length ; i++) {
                        ...
                        Xlog.d(LOG_TAG, "oldWallpaper " + oldWallpaper.getName());
                        if (oldWallpaper.exists()) {
                            boolean isDelete = oldWallpaper.delete();
                            Xlog.d(LOG_TAG, "isDelete " + isDelete);
                        }
                    }
                }*/
        ...
        Xlog.d(LOG_TAG, "update end, affectedRows = " + affectedRows);
        return affectedRows;
    }
    ...
    private Cursor getMsgInfo(SQLiteDatabase db, long threadId, String selection) {
        ...
        Xlog.d(LOG_TAG, "getMsgBox begin rawQuery = " + rawQuery);
    ...
    private static Cursor getAllUnreadCount(SQLiteDatabase db) {
        ...
        /// M: Xlog.d(LOG_TAG, "getAllUnreadCount rawQuery = " +rawQuery);
        return db.rawQuery(rawQuery, null);
    }
    /// @}
    /// M: Code analyze 007, fix bug ALPS00255806, when reply a message, use the same
    ...
    private Cursor getSimidListByThread(SQLiteDatabase db, long threadId) {
        String rawQuery = String.format("SELECT DISTINCT sub_id FROM" +
                ...
                " UNION SELECT DISTINCT sub_id FROM pdu WHERE thread_id=" + threadId + " AND msg_box=1" + ")");
    ...
    private Cursor getInboxMessage(SQLiteDatabase db, String selection) {
         ...
         String mmsNotInPendingQuery = String.format("SELECT pdu._id as _id,pdu.thread_id as thread_id,threads.recipient_ids as address," +
                 ...
                 ") as attachment,pdu.m_type as m_type,pdu.sub_id as sub_id,pdu.msg_box as box_type,pdu.sub_cs as sub_cs" +
                 ",pdu.locked as locked " +
                 " FROM " +
         ...
         String mmsNotInPartQeury = String.format("SELECT pdu._id as _id,pdu.thread_id as thread_id,threads.recipient_ids as address,pdu.sub as body," +
                 "pdu.date * 1000 as date,pdu.read as read,2 as msg_type,pending_msgs.err_type as status,0 as attachment,pdu.m_type as m_type,pdu.sub_id as sub_id," +
         ...
         String mmsNotInBothQeury = String.format("SELECT pdu._id as _id,pdu.thread_id as thread_id,threads.recipient_ids as address,pdu.sub as body,pdu.date * 1000 as date," +
                 "pdu.read as read,2 as msg_type,0 as status,0 as attachment,pdu.m_type as m_type,pdu.sub_id as sub_id,pdu.msg_box as box_type,pdu.sub_cs as sub_cs " +
                 ",pdu.locked as locked " +
                 "FROM pdu," +
...
//                Set<String> recipients = searchRecipients(cursor.getString(1));
    ...
    private Set<String> getRecipientNumbers(String recipientIds, HashMap<Integer, String> contacts) {
        ...
        if (contacts == null || contacts.size() <= 0) {
            Xlog.d(LOG_TAG, "getRecipientNumbers contacts is null");
            return recipients;
        }
    ...
    private synchronized Cursor getWapPushThreadId(List<String> recipients) {
        ...
        String queryString = "SELECT _id FROM threads " + "WHERE type=" + Telephony.Threads.WAPPUSH_THREAD + " AND recipient_ids=?";
        String[] selectionArgs = new String[] { recipientIds };
        SQLiteDatabase db = mOpenHelper.getReadableDatabase();
    ...
    private void insertCBThread(String recipientIds, int numberOfRecipients) {
        ...
        long result = mOpenHelper.getWritableDatabase().insert("threads", null, values);
        Xlog.d(LOG_TAG, "insertThread: created new thread_id " + result +
                " for recipientIds " + recipientIds);
        Xlog.w(LOG_TAG, "insertCBThread!");
        /// M: Code analyze 012, fix bug ALPS00262044, not show out unread message
        /// icon after restore messages. notify mms application about unread messages
        /// number after insert operation.
        notifyChange();
    ...
    private synchronized Cursor getCBThreadId(List<String> recipients) {
        ...
        String queryString = "SELECT _id FROM threads " + "WHERE type=" + Telephony.Threads.CELL_BROADCAST_THREAD + " AND recipient_ids=?";
        String[] selectionArgs = new String[] { recipientIds };
        SQLiteDatabase db = mOpenHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery(queryString, selectionArgs);
    ...
    private int deleteAllInFolderMode(Uri uri, SQLiteDatabase db, String selection,
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    Xlog.d(LOG_TAG, "confirmDeleteThreadDialog max SMS id = " + smsId);
                }
            } finally {
                cursor.close();
                cursor = null;
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    Xlog.d(LOG_TAG, "confirmDeleteThreadDialog max MMS id = " + mmsId);
                }
            } finally {
                cursor.close();
                cursor = null;
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    ...
                    Xlog.d(LOG_TAG, "confirmDeleteThreadDialog max CB id = " + cbId);
                }
            } finally {
                cursor.close();
                cursor = null;
        ...
        Xlog.d(LOG_TAG, "deleteAllInFolderMode query all thread ids " + query);
        ...
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    ...
                    Xlog.d(LOG_TAG, "get threads unread message count = " + threadsUnreadCount);
                }
            } finally {
                c.close();
            }
    ...
    private int deleteAllConversation(SQLiteDatabase db, Uri uri, String selection, String[] selectionArgs) {
        ...
        if (mmsId != null) {
            ...
            db.execSQL("DELETE FROM words WHERE (table_to_use=2" + exceptFromPart
                    + ") OR (table_to_use=" + MmsProvider.TABLE_TO_USE_SUBJECT + exceptFromPdu + ");");
        }
    ...
    private String searchContactsAdvanced(String numberPattern, String namePattern, HashMap<String, String> contactRes) {
        Xlog.d(LOG_TAG, "searchContactsAdvanced(): numberPattern = " + numberPattern
            + ", namePattern = " + namePattern);
        boolean hasNumber = !TextUtils.isEmpty(numberPattern);
        boolean hasName = !TextUtils.isEmpty(namePattern);
        if (!hasNumber && !hasName) {
            Xlog.d(LOG_TAG, "searchContactsAdvanced(): no number and no name");
            return null;
        }
        /* query the related thread ids */
        Set<Long> threadIds = new HashSet<Long>();
        ...
        try {
            while (cursor.moveToNext()) {
                Long threadId = cursor.getLong(0);
                Xlog.d(LOG_TAG, "searchContactsAdvanced(): threadId = " + threadId);
                Set<String> recipients = searchRecipients(cursor.getString(1));
                for (String recipient : recipients) {
                    int compareCount = 0;
        ...
        if (threadIds.size() > 0) {
            ...
            in = in.replace('[', '(');
            in = in.replace(']', ')');
            Xlog.d(LOG_TAG, "searchContactsAdvanced(): searchContacts, in = " + in);
            return in;
        }
        Xlog.w(LOG_TAG, "searchContactsAdvanced(): threadIds.size() = 0.");
        return null;
    }

    private HashMap<String, String> getContactsByName(String namePattern, boolean hasNumber, HashMap<String, String> contactRes) {
        Xlog.d(LOG_TAG, "getContactsByName(): namePattern = " + namePattern + ", hasNumber = " + hasNumber);
    ...
    private static String buildConversationQueryDistinct(String[] projection,
        ...
        /// M: Code analyze 003, new feature, support for cellbroadcast. @{
        String unionQuery = unionQueryBuilder.buildUnionQuery(
                new String[] { smsSubQuery, smsSubQueryOrdinary , mmsSubQuery, mmsSubQueryCC, cbSubQuery },
                handleNullSortOrder(sortOrder), null);
        /// @}
        SQLiteQueryBuilder outerQueryBuilder = new SQLiteQueryBuilder();
  ...
  /// M: For op09 new feature of cc. @{
    ...
    private Cursor getSimpleConversationsExtend(String[] projection, String selection,
        ...
        Xlog.d(LOG_TAG, "extend query selection:" + selection);
    ...
    private void deleteIPMsgWallPaper(SQLiteDatabase db, String delSelectionString) {
        Xlog.d(LOG_TAG, "delete wallpaper from obsolete begin");
