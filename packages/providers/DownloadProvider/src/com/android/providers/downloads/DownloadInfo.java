...
import android.util.Log; //add HQ_wangshiqing
/**
...
public class DownloadInfo {
    ...
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            Log.d(Constants.TAG,"info.mFileName1:"+info.mFileName);
            //add by chiguoqing for Play Store closed
	    if (null != (info.mFileName)){
		  info.mFileName = changeDownloadPath(info.mFileName,
		  Environment.getExternalStorageDirectory().getAbsolutePath());
	    }
            Log.d(Constants.TAG,"info.mFileName2:"+info.mFileName);
            info.mMimeType = Intent.normalizeMimeType(getString(Downloads.Impl.COLUMN_MIME_TYPE));
            if (info.mMimeType != null && !"".equals(info.mMimeType)) {
                info.mMimeType = info.mMimeType.replace("\"", "");
            }
            ...
            sDownloadProviderFeatureExt = Extensions.getDefault(info.mContext);
	...
	private String changeDownloadPath(String path,String defaltPath){
		StringBuffer canonicalOld = new StringBuffer(path);
		if(!path.startsWith(defaltPath) 
			&& defaltPath.equals("/storage/sdcard1") 
			&& path.startsWith("/storage/emulated/0/")){
			path = canonicalOld.replace(0,19,"/storage/sdcard1").toString();
		}
		return path;
	}
