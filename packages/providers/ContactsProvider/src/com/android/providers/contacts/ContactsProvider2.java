...
import com.mediatek.providers.contacts.DataRowHandlerForPhoneNumberEx;
import com.mediatek.providers.contacts.DataRowHandlerForStructuredNameEx;
import com.mediatek.providers.contacts.DataRowHandlerForSipAddress;
...
public class ContactsProvider2 extends AbstractContactsProvider
	...
	private static final int PHONE_LOOKUP2 = 4001;//add by zhaizhanfeng for number match update sqlite at 150923
            ...
            Contacts._ID + " IN "
                    ...
                    + " WHERE " + RawContactsColumns.CONCRETE_ID + " IN "
                            ...
                            + " WHERE " + DataColumns.MIMETYPE_ID + "=?"
                                    ...
                                    + " FROM " + Tables.GROUPS
                                    /**
                                     * M: fix ALPS01002380/ALPS01002519. Group_ID supports special group
    ...
    static {
		...
		matcher.addURI(ContactsContract.AUTHORITY, "phone_lookup2", PHONE_LOOKUP2);//add by zhaizhanfeng for number match update sqlite at 150923
    ...
    private boolean initialize() {
        ...
        mDialerSearchSupport = DialerSearchSupport.getInstance(getContext(), mDbHelper.get());
        /**
         * @}
         */
    ...
    private void initDataRowHandlers(Map<String, DataRowHandler> handlerMap,
        ...
        if (ContactsProviderUtils.isImsCallEnabled() && ContactsProviderUtils.isVOLTEEnabled()) {
            handlerMap.put(ImsCall.CONTENT_ITEM_TYPE, new DataRowHandlerForImsCall(context, dbHelper, contactAggregator));
        }
   ...
   /// @}        
    ...
    protected int deleteInTransaction(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case RAW_CONTACTS:
                ...
                /// TODO: should move this block to another method
                /// @{
                /// M: fix CR:ALPS01534150,enable airplanemode,contact name is different from title
                invalidateFastScrollingIndexCache();
    ...
    protected int updateInTransaction(
        ...
        switch(match) {
			...
			case PHONE_LOOKUP2: {
				Log.d("zhaizhanfeng","PHONE_LOOKUP2 come in");
			    ...
			    while(c.moveToNext()) {
			        ...
			        if (Build.TYPE.equals("eng")) {
			            ...
			            Log.d("zhaizhanfeng", "[PHONE_LOOKUP2]1.normalizeNumber:" + normalizeNumber);
			        }
			        ...
			        if (Build.TYPE.equals("eng")) {
			            ...
			            Log.d("zhaizhanfeng", "[PHONE_LOOKUP2]2.normalizeNumber:" + normalizeNumber + "  oldMinMatch:" + oldMinMatch + "  minMatch:" + minMatch);
			        }
                    /* modify by shanlan for HQ01917125 begin */
			        mDb.update(Tables.PHONE_LOOKUP, phoneValues, PhoneLookupColumns.DATA_ID + "=" + dataId + " AND " + PhoneLookupColumns.RAW_CONTACT_ID + "=" + rawContactId 
                        + " AND " + PhoneLookupColumns.NORMALIZED_NUMBER + "=\"" + normalizeNumber + "\"", null);
                    /* modify by shanlan for HQ01917125 end */
			    }
			    c.close();
			    break;
			}
    ...
    private int updateContactOptions(ContentValues values, String selection,
        ...
        final SQLiteDatabase db = mDbHelper.get().getWritableDatabase();
   ///M: the bellow lines are added by mediatek for facebook sync. @{
        String contactsView = mDbHelper.get().getContactView(ContactsDatabaseHelper.canAccessRestrictedData(getContext()));
   ...
   /// @}
    ...
    private int updateContactOptions(
        ...
        if (inputValues.containsKey(Contacts.FILTER)) {
            values.clear();
            ContactsDatabaseHelper.copyStringValue(values, Contacts.FILTER, inputValues, Contacts.FILTER);
    ...
    protected Cursor queryLocal(final Uri uri, final String[] projection, String selection,
        ...
        switch (match) {
            ...
            case CONTACTS_STREQUENT: {
                ...
                if (phoneOnly) {
                    ...
                    tableBuilder.append(Tables.DATA_USAGE_STAT
                            + " INNER JOIN " + dataView + " " + Tables.DATA
                            + " ON (" + DataUsageStatColumns.CONCRETE_DATA_ID + "="
                                + DataColumns.CONCRETE_ID + " AND "
                            + DataUsageStatColumns.CONCRETE_USAGE_TYPE + "="
                                + DataUsageStatColumns.USAGE_TYPE_INT_CALL + ")");									
                ...
                if (uri.getPathSegments().size() > 2) {
                    ...
                    if (searchPhoneNumber) {
                        ...
                        if (!TextUtils.isEmpty(number)) {
							...
							sb.append("%");//HQ_zhangjing add for CQ HQ01359598
                            sb.append(number);
                            sb.append("%')");
                            hasCondition = true;
                        }
            ...
            /// M: Remove abandoned dialer search entry @{
            /**
             * do not use increment query any more
            ...
            */
            ...
            case DIALER_SEARCH_INIT: {
                ...
                // Do not need initialization again with the new query.
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
        String rawContactsView = mDbHelper.get()
                .getRawContactsEntitiesView(ContactsDatabaseHelper.canAccessRestrictedData(getContext()));
        ...
        if (includeDataUsageStat) {
            sb.append(" ON (" +
                    DbQueryUtils.concatenateClauses(
                            ...
                            RawContacts.CONTACT_ID + "=" + contactsView + "." + Contacts._ID) +
                    ")");
        }
	   /// @}
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
        String dataView = mDbHelper.get().getDataView(ContactsDatabaseHelper.canAccessRestrictedData(getContext()));
        String sql =
                "SELECT " + Photo.PHOTO + " FROM " + dataView +
                " WHERE " + selection;
   /// @}
        try {
...
// ------------- MTK below ----------------------------
    ...
    private boolean isLocalAccount(Cursor c) {
        ...
        if (accountType.equals("SIM Account") ||
            ...
            accountType.equals("com.android.huawei.phone") ||
             ...
             if (cursor != null && cursor.getCount() == 0) {
                 ...
                 if ((ProductCharacteristic != null) && (ProductCharacteristic.equals("tablet"))) {
                     db.execSQL("INSERT INTO " + Tables.SETTINGS + " (" + Settings.ACCOUNT_NAME
                             ...
                             + Settings.UNGROUPED_VISIBLE + ", " + Settings.SHOULD_SYNC
                             + ") VALUES (?, ?, ?, ?, ?)", new String[] {
                             "Tablet", "com.android.huawei.phone", null, "1", "1"
                     });
                 } else {
                 /** @} */
                     db.execSQL("INSERT INTO " + Tables.SETTINGS + " (" + Settings.ACCOUNT_NAME
                             ...
                             + Settings.UNGROUPED_VISIBLE + ", " + Settings.SHOULD_SYNC
                             + ") VALUES (?, ?, ?, ?, ?)", new String[] {
                             "Phone", "com.android.huawei.phone", null, "1", "1"
                     });
                 /** M: Bug Fix for CR: ALPS00351067 @{ */
                 }
                 /** @} */
    ...
    private boolean canDeleteAccount(AccountWithDataSet accountWithDataSet) {
        if (accountWithDataSet.getDataSet() == null) {
            if ((accountWithDataSet.getAccountName().equals("Phone") && accountWithDataSet
                    .getAccountType().equals("com.android.huawei.phone"))
