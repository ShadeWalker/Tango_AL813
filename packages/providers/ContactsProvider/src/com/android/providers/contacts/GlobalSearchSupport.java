...
public class GlobalSearchSupport {
    ...
    public Cursor handleSearchShortcutRefresh(SQLiteDatabase db, String[] projection,
        ...
        c = GlobalSearchSupportEx.processCursor(c, projection, lookupKey, SEARCH_SUGGESTIONS_COLUMNS);
        return c;
        /**
         * @}
         */
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
        String contactsView = mContactsProvider.getDatabaseHelper(mContext)
                .getContactView(ContactsDatabaseHelper.canAccessRestrictedData(mContext));
   ...
   /// @}        
    ...
    private void getCursorField(Cursor c, SearchSuggestion suggestion, int index) {
        suggestion.simIndicator = c.getInt(index);
        suggestion.isSdnContact = c.getInt(index + 1);
        Context context = mContactsProvider.getContext();
        SubscriptionInfo subInfo = SubscriptionManager.from(context).getActiveSubscriptionInfo(suggestion.simIndicator);
