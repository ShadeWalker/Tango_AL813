...
///M: the following lines are added by mediatek for facebook sync. @{
...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
     ...
     * @{
     */
    static final int DATABASE_VERSION = 952;//tang modified for HQ01631894
    /** @} */
    ...
    public interface Views {
        ...
        /// @}
   ///M: the bellow lines are added by mediatek for facebook sync. @{
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
				...
				initialUnrestrictedPkg(context); //add by MTK for facebook sync.
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
                ...
                RawContacts.IS_RESTRICTED + " INTEGER DEFAULT 0," +               //add by MTK for facebook
...
//        ");");
    ...
    private void createContactsViews(SQLiteDatabase db) {
        ...
        String rawEntitiesSelect = "SELECT "
                ...
                + Tables.GROUPS + "." + Groups.SOURCE_ID + " AS " + GroupMembership.GROUP_SOURCE_ID + ", "
                + RawContacts.IS_SDN_CONTACT + ", "  // add by MTK
                + RawContactsColumns.INDICATE_PHONE_SIM + " AS " + RawContacts.INDICATE_PHONE_SIM + ", " // add by MTK
                + RawContactsColumns.INDEX_IN_SIM + " AS " + RawContacts.INDEX_IN_SIM // add by MTK
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
				...
				createrRestrictedContactsViews(db, contactsSelect, dataSelect, rawContactsSelect, rawEntitiesSelect, entitiesSelect);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 952) {// tang add 
            // Adding is_restricted to raw_contacts table.
           	Log.i("tang", "db oldversion is  "+oldVersion);
        	Log.i("tang", "db upgrade to 952");
        	if(!checkColumnExists2(db)){
            	Log.i("tang", "db upgrade to 952  and  add  is_restricted ");
    ...
    * @return
    */
 private boolean checkColumnExists2(SQLiteDatabase db) {
        // Adding is_restricted to raw_contacts table.
		Cursor sqlCursor=db.rawQuery(
				" select sql from sqlite_master where tbl_name='raw_contacts' and type='table'; ",
				null);
		if(sqlCursor.moveToNext()){
			String sql=sqlCursor.getString(0);
			Log.i("tang", "the sql is "+sql);
			if(sql!=null&&sql.toLowerCase().contains("is_restricted")){
				return true;
			}
		}
		if(sqlCursor!=null){
  ...
  /**
    ...
    private void upgradeToVersion666(SQLiteDatabase db) {
        db.execSQL("UPDATE groups SET "
                + " account_type='com.android.huawei.phone', system_id=null, group_is_read_only=0 "
        ...
        db.execSQL("UPDATE raw_contacts SET "
                ...
                + RawContacts.ACCOUNT_TYPE + "='com.android.huawei.phone' "
                + " WHERE ("
        ...
        final Cursor contactIdCursor = db.rawQuery(
                "SELECT DISTINCT contact_id " +
                "FROM raw_contacts " +
                "WHERE deleted=0 AND account_type='com.android.huawei.phone'",
                null);
        try {
            while (contactIdCursor.moveToNext()) {
                final long contactId = contactIdCursor.getLong(0);
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
        ...
        String contactsView = getContactView(ContactsDatabaseHelper.canAccessRestrictedData(mContext));
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
    ...
    public void updateRawContactDisplayName(SQLiteDatabase db, long rawContactId) {
        ...
        if (sortKeyPrimary != null) {
            ...
            //caohaolin modify
            if (phonebookLabelPrimary.equals("#")) {
        ...
        if (sortKeyAlternative != null) {
            ...
            //caohaolin modify
            if (phonebookLabelAlternative.equals("#")) {
    ...
    /// @}
        ...
        ///M: the following lines are added by mediatek for facebook sync. @{
    private void createrRestrictedContactsViews(SQLiteDatabase db, String contactsSelect, String dataSelect,
    ...
    public String getRawContactsView() {
        return getRawContactsView(true);
    }
    ...
    public String getContactView() {
        return getContactView(true);
    }

    public String getContactView(boolean canAccessRestrictedData) {
    ...
    public String getRawContactsEntitiesView() {
        return getRawContactsEntitiesView(true);
    }

    public String getRawContactsEntitiesView(boolean canAccessRestrictedData) {
        return (hasAccessToRestrictedData() && canAccessRestrictedData) ?
    ...
    public static synchronized boolean canAccessRestrictedData(Context context) {
        ...
        // no this process
        if (result == null) {
            Log.v(TAG, "permitedAccessRestrictedData(" + pid + ") can not get result!");
            if (sPermitedProcessNames == null) {
                Log.w(TAG, "[isPermitedAccessRestrictedData], should not call this method before init");
                return false;
            }
            ...
            for (int i = 0; i < size; i++) {
                ...
                Log.v(TAG, "pid=" + runInfo.pid + ", name=" + runInfo.processName + ", allow=" + allow);
            }
            result = sCurrentProcesses.get(pid);
            if (result == null) {
        ...
        if (!result) {
            Log.v(TAG, "permitedAccessRestrictedData(" + pid + ") return " + result);
        }
        return result;
    }
    ///M: the before lines are added by mediatek for facebook sync. @{
    /// @}
}
