...
public class DialerSearchUtils {
       ...
       // Log.d(TAG, "[insert] get default insert number:" + strInsNumber);
        ...
        try {
            ...
            Cursor allCallLogCursorOfSameNum = db.query(Tables.CALLS, new String[] { Calls._ID, Calls.DATE }, " CASE WHEN "
                    + Calls.PHONE_ACCOUNT_ID + "=" + CALL_TYPE_SIP + " THEN " + Calls.NUMBER + "='" + strInsNumber + "'"
   ...
   ///M: the bellow lines are added by mediatek for facebook sync. @{
                String dataView = dbHelper.getDataView(ContactsDatabaseHelper.canAccessRestrictedData(context));
                nameCursor = db.query(dataView, new String[] { Data._ID, Data.RAW_CONTACT_ID }, Data.DATA1 + "='"
   ...
   /// @}                
    ...
    public static Cursor findNumberExactMatchedCursor(Cursor cursor, String normalizedNumber) {
        ...
        if (cursor == null || cursor.getCount() <= 1 || TextUtils.isEmpty(normalizedNumber)
                || (cursor.getColumnIndex(Phone.NUMBER) < 0 && cursor.getColumnIndex(PhoneLookup.NUMBER) < 0)) {
