...
public class CallLogSearchSupport {
    ...
    private interface SearchSuggestionQuery {
        ...
        public static final String TABLE = Tables.CALLS + " LEFT JOIN " + Views.DATA + " ON ("
                ...
                + ") ";
        // M: fix Cr:ALPS01770140,modify match to glob to ensure query call log contacts name in quickSearchBox right.
        ...
        public static final String[] COLUMNS = {
                ...
                // M: fix CR:ALPS01676096,substitute CACHED_NUMBER_TYPE for Data2 to ensure query the latest data
    ...
    private Cursor buildCursorForSearchSuggestions(SQLiteDatabase db, String[] projection,
        ...
        if (haveFilter) {
            if (TextUtils.isEmpty(where)) {
                where = Tables.CALLS + "." + Calls.NUMBER + " GLOB '*" + filter + "*' " + "AND ("
                ...
                where += " AND " + Tables.CALLS + "." + Calls.NUMBER + " GLOB '*" + filter + "*' "
                        + "AND (" + SearchSuggestionQuery.SNIPPET_CONTACT_ID + ">0 AND "
                        + Tables.CALLS + "." + Calls.RAW_CONTACT_ID + ">0)";
            }
        }
        // M: fix CR:ALPS01753330,add tables.calls to solve ambiguous _id columns problems.
        Cursor c = db.query(false, table, SearchSuggestionQuery.COLUMNS, where, null, Tables.CALLS + "." + Calls._ID,
                null, null, limit);
        ...
        try {
            while (c.moveToNext()) {
                ...
                SubscriptionInfo subInfo = SubscriptionManager.from(mContext).getActiveSubscriptionInfo(indicate_phone_sim);
                if (subInfo == null) {
                    suggestion.slotId = -1;
                } else {
                    suggestion.slotId = subInfo.getSimSlotIndex();
                }
