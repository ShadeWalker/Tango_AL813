...
public class PreviewStubActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.v(LOG_TAG, "mWallpaperIntent = " + mWallpaperIntent);
    ...
    public void setLiveWallpaper(View v) {
        Log.d(LOG_TAG, "setLiveWallpaper() ");
        try {
    ...
    public void onResume() {
        ...
        if (mWallpaperConnection != null && mWallpaperConnection.mEngine != null) {
            try {
                Log.v(LOG_TAG, "onResume() inner mWallpaperConnection = " + mWallpaperConnection);
                mWallpaperConnection.mEngine.setVisibility(true);
                Log.d(LOG_TAG, "onResume() setVisibility(true)");
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
    ...
    public void onPause() {
        ...
        if (mWallpaperConnection != null && mWallpaperConnection.mEngine != null) {
            try {
                ...
                Log.d(LOG_TAG, "onPause() setVisibility(false)");
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
        ...
        mView.post(new Runnable() {
            public void run() {
                if (!mWallpaperConnection.connect()) {
                    Log.d(LOG_TAG, "onAttachedToWindow() mWallpaperConnection.connect() fail, should not enter...");
    ...
    class WallpaperConnection extends IWallpaperConnection.Stub implements ServiceConnection {
        ...
        public void disconnect() {
            synchronized (this) {
                ...
                unbindService(this);
                mService = null;
                Log.d(LOG_TAG, "disconnect() successfully");
            }
        }

        public void onServiceConnected(ComponentName name, IBinder service) {
            if (mWallpaperConnection == this) {
                Log.d(LOG_TAG, "connect() successfully");
                ...
                try {
                    ...
                    mService.attach(this, view.getWindowToken(),
                            ...
                            true, root.getWidth(), root.getHeight(), null);
                    Log.v(LOG_TAG, "onServiceConnected end");
        ...
        public void attachEngine(IWallpaperEngine engine) {
            synchronized (this) {
                Log.v(LOG_TAG, "attachEngine start engine = " + engine);
                if (mConnected) {
                    mEngine = engine;
                    try {
                        Log.v(LOG_TAG, "attachEngine engine = " + engine);
