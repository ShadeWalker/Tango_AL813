...
void MtpServer::removeAllStorage() {
    Mutex::Autolock autoLock(mMutex_storage);
    size_t storage_size = mStorages.size();
    for (size_t i = 0; i < storage_size; i++) {
        MtpStorage* storage = mStorages[0];
        if (storage) {
            ALOGI("remove storage %d, id=0x%x", i, storage->getStorageID());
            mStorages.removeAt(0);
            delete storage;
            storage = NULL;
        } else {
            ALOGE("storage %d is NULL", i);
        }
    }
}
