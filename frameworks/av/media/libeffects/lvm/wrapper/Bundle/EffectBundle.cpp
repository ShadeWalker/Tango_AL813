...
#ifdef MTK_AUDIO            
            if (pCmdData == NULL || ( cmdSize != 2 * sizeof(uint32_t) && cmdSize != 3 * sizeof(uint32_t) ) || pReplyData == NULL ||
                    replySize == NULL || *replySize < 3*sizeof(int32_t)) {
                ALOGV("\tLVM_ERROR : Effect_command cmdCode Case: "
