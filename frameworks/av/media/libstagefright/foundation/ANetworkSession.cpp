    ...
    if (mState == DATAGRAM) {
		...
		do {
			...
			if (n < 0) {
				...
				{
					mPreUs = mCurUs;
					ALOGI("ANetworkSession Bandwidth=0.0 (KB/s) at %lld Ms", mPreUs/1000);
				} else {
...
///Add by MTK @{
            ...
            while (mInBuffer.size() >= 4) {
                ALOGD("UIBC remain buffer size:%d", mInBuffer.size());
 	  ...
 	  do {
    	   ...
    	   if(endTimeUs - startTimeUs > 2000ll){
    	   	    ALOGI("[Latency]Send a datagram more than %lld ms,left %d frags",
                    (endTimeUs - startTimeUs)/1000,mOutFragments.size());
    	   }
        ...
        if (err == -EAGAIN) {
            if (!mOutFragments.empty()) {
                ...
                ALOGI("[Latency]%d datagrams remain queued. BytesInSocket = %d/%d", mOutFragments.size(), numBytesQueued, res);
...
int64_t ANetworkSession::Session::getRTPRecvNum(){
	...
	ALOGD("Receive RTP Number is %lld",mRTPCounter);	
	return mRTPCounter;
}
