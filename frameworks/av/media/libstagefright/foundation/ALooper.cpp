...
#ifdef MTK_AOSP_ENHANCEMENT
        //workaround for google shced_policy issue
        if (!setSelfAsForeGround)
        {
            prctl(PR_SET_TIMERSLACK_PID, 50000, gettid());
            setSelfAsForeGround = 1;
        }
#endif//MTK_AOSP_ENHANCEMENT
        return mLooper->loop();
    }
