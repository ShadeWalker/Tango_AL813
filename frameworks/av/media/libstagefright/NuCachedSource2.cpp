...
void NuCachedSource2::fetchInternal() {
    MM_LOGI("fetchInternal, at %lld + %d", mCacheOffset, mCache->totalSize());
    ALOGV("fetchInternal");
	...
	MM_LOGI("reconnectAtOffset: %lld", mCacheOffset + mCache->totalSize());
...
ssize_t NuCachedSource2::readAt(off64_t offset, void *data, size_t size) {
    ...
    if (offset < 0 || size > (size_t)mHighwaterThresholdBytes) {
        ALOGE("Error: offset:%lld size:%u", offset, size);
        return -EINVAL;
    }
    if (data == NULL) {
