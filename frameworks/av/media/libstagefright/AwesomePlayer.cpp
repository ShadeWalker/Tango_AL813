...
void AwesomePlayer::notifyVideoSize_l() {
    ...
    if (mDisplayWidth == 0) { // in case there is no resolution info in the container
    ...
    if (mDisplayHeight == 0) { // in case there is no resolution info in the container
...
void AwesomePlayer::onVideoEvent() {
        ...
        for (;;) {
            ...
            if (err != OK) {
                ...
                if (mSeeking != NO_SEEK) {
                    ...
                    ALOGI("EOS,mFlags=0x%x, mSeekTimeUs=%lld,mDurationUs=%lld", mFlags, mSeekTimeUs, mDurationUs);
    ...
    if (releaseElapse > mThrottleVideoBufRel)  {              // default 6ms
        ALOGD("mVideoBuffer release elapse(us):%lld", releaseElapse);
    }
#endif
    mVideoBuffer = NULL;
    ...
    if (wasSeeking == NO_SEEK) {
        ...
        for (;;) {
            ...
            MM_LOGI("mVideoBuffer len 0 when get next frame, mSinceLastDropped:%d", mSinceLastDropped);        //M: add log
            // Some decoders, notably the PV AVC software decoder
            // return spurious empty buffers that we just want to ignore.
        ...
        if (delayUs <= 0) {
            if (mSinceLastDelay < 10) {
                ...
                ALOGD("next Frame: delayUs:%lld post 1ms", delayUs);
                mSinceLastDelay = 0;
                delayUs = 1000;
            }
        } else {
            ...
            if (mCachedSource != NULL) {
                ...
                for (;;) {
                    ...
                    if (metaDataSize < 0
                        ...
                        MM_LOGI("metaDataSize = %lld bytes", metaDataSize);
                    }
    ...
    //  (!strcasecmp(_mimeX, "video/mp4") ||!strcasecmp(_mimeX, "video/3gp") ))
    {
        ...
        if (!nonInterDisable && fileMeta != NULL && fileMeta->findCString(kKeyMIMEType, &_mimeX) &&
            ...
            if (isNonInterleave) {
                ...
                ALOGD("new NuCachedSource2(config=%s, disconn@highwater=%d, aoff=%lld", configStr.isEmpty() ? "NULL" : configStr.string(), mCachedSource->disconnectAtHighwatermark(), aoff);
    ...
    *pts = &mSystemTimeSource;
    if (!mAudioNormalEOS && mAudioPlayer != NULL && !(mFlags & SEEK_PREVIEW)) {
        ...
        if (mAudioPlayer->reachedEOS(&finalStatus)) {
            ...
            *pts = mAudioPlayer;
            mTimeSourceDeltaUs = *realTimeUs - *mediaTimeUs;
            ALOGV(" real %lld media %lld", *realTimeUs, *mediaTimeUs);
        } else {
            *mediaTimeUs = mSeekTimeUs;
