    ...
    if (mTmpBufLen > 0)
    {
        ...
        else
        {
            ...
            *pCurPos = mFramePosition;
			...
			if (decoderErr != AAC_DEC_OK && decoderErr != AAC_DEC_NOT_ENOUGH_BITS)
			{
				AAC_EXTR_WARN("decoding error(0x%x).", decoderErr);
                return ERROR_END_OF_STREAM;
			}

            if (AAC_DEC_OK != aacDecoder_GetFreeBytes(mAACDecoder, (UINT *)&DecFreeBytes))
            {
                AAC_EXTR_WARN("aacDecoder_GetFreeBytes error.");
                return ERROR_END_OF_STREAM;
            }
...
status_t MtkAACSource::start(MetaData *)
{
    ...
    {
        mFrameDurationUs = (1024 * 1000000ll + (nSampleRate - 1)) / nSampleRate;
...
#ifdef AAC_ADIF_READ_BY_FRAME
		uint32_t nReadSize = 0;
		int64_t frameUs = 0;
		off_t nextPos = 0;
		if (getNextFramePos(&mCurrentPos, &nextPos, &frameUs) == OK)
		{
			nReadSize = nextPos - mCurrentPos;
		}
		else
		{
			nReadSize = 0;
