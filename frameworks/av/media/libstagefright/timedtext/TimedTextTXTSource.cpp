...
status_t TimedTextTXTSource::parse(
          uint8_t* text,
          size_t size,
          int64_t startTimeUs,
          int64_t endTimeUs,
...
{    
    ...
    ALOGE("[--dbg--] parse subtitle  sTime=%lld, eTime=%lld", startTimeUs, endTimeUs);
    extractAndAppendLocalDescriptions(startTimeUs, textBuffer, parcel);
    textBuffer->release();
    return OK;
}
...
status_t TimedTextTXTSource::ex_read(
    ...
    if (err != OK) {
        ALOGE("getText Return error!,err = %d",err);
        return err;
    }
...
void TimedTextTXTSource::adjustPreSubEndTime(int64_t curStartTimeUs, MultiTextInfo info){
	...
	for(int i= vSize -1 ; i >= 0 ; i--){
		...
		if( temp.langIndex == info.langIndex ){
            if ((0x0FFFFFFFFFFFFFFF == temp.endTimeUs) || (temp.endTimeUs >  curStartTimeUs))
            {
...
status_t TimedTextTXTSource::getNextSubtitleInfo(
    ...
    do {
        ...
        if ((err = TimedTextUtil::readNextLine(mExSource, offset, &data, mFileEncodeType)) != OK) {
			ALOGE("Reading Error Return Here!,err = %d",err);
			return err;
        }
		...
		if(data.empty()){	//To skip blank lines
			...
			if(-1 == index){	//skip invalid line
                ...
                //parser the subtitle {starttime}{endtime}text
				info->langIndex = 0;

                int32_t sindex = data.indexOf(MagicString::MAGIC_STRING_LARGE_BRACKETS);
                if (-1 == sindex)
                {
                    continue;
                }

                int32_t endindex = data.indexOf(MagicString::MAGIC_STRING_ANTI_LARGE_BRACKETS,sindex);
                if (-1 == endindex)
                {
                    continue;
                }
				...
				sp<MagicString> startTimeMStr = data.subString(sindex + typeSize,  endindex - typeSize);
				startTimeMStr->trim();
				MagicString::print("[StartTime]", startTimeMStr);
                *startTimeUs = MagicString::getIntValue(startTimeMStr);

                info->endTimeUs = 0x0FFFFFFFFFFFFFFF;	//will be wrote with right value when parsing over next subtitle infomation
                sindex = data.indexOf(MagicString::MAGIC_STRING_LARGE_BRACKETS,endindex);
                if (-1 == sindex)
                {
                ...
                {
                    endindex = data.indexOf(MagicString::MAGIC_STRING_ANTI_LARGE_BRACKETS,sindex);
                    if (-1 == endindex)
                    {
                    ...
                    {
                        sp<MagicString> endTimeMStr = data.subString(sindex + typeSize,  endindex - typeSize);
				        endTimeMStr->trim();
				        MagicString::print("[endTimeMStr]", endTimeMStr);
                        info->endTimeUs = MagicString::getIntValue(endTimeMStr);
                    }                    
                }				

                
				sp<MagicString> subtitle = data.subString(endindex + typeSize);
				MagicString::print("[Get Subtitle]", subtitle);
                if ((1 == *startTimeUs) && (1 == info->endTimeUs))
                {
                    //mframerate = MagicString::getIntValue(subtitle);
                    ALOGE("[TXT mframerate = %d",mframerate);
                    continue;
                }

                *startTimeUs = (*startTimeUs * 1000000ll)/mframerate;
                info->endTimeUs = (info->endTimeUs* 1000000ll)/mframerate;
				info->offset = lineBeginIndex + noneSpaceStartPos + endindex + typeSize;
