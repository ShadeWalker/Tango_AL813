...
status_t TimedTextDVBSource::parse(
          uint8_t* text,
          size_t size,
          int64_t startTimeUs,
          int64_t endTimeUs,
          Parcel *parcel)
{
	...
	ALOGI("timeUS[%lld ~ %lld],size[%d] of this subtitle\n",startTimeUs/1000, endTimeUs/1000, size);
	ALOGI("DVB subtitle content,%02x %02x %02x %02x %02x %02x %02x %02x\n", 
						content[0],content[1],content[2],content[3],
						content[4],content[5],content[6],content[7]);
    CHECK_GE(startTimeUs, 0);
	...
	if (DVBR_OK != i4_ret)
    {
        ...
        if (DVBR_MEM_USAGE_CHANGED == i4_ret)
        {
            ...
            #ifdef DVB_ENABLE_HD_SUBTITLE
            mDvbParser->updateLogicalSize( 720, 576);
            #endif
