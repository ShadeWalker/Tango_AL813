...
FLV_ERROR_TYPE flvParser::ParseflvFile()
{
    ...
    FLV_DEBUG_LOGD("ParseflvFile: flv_parse_header done:cur_file_offset=0x%p\n",mfile->cur_file_offset); 
    ...
    FLV_DEBUG_LOGD("ParseflvFile: flv_parse_script done:cur_file_offset=0x%p\n",mfile->cur_file_offset); 

    ret = flv_setup_seektable();
    if(ret!=FLV_OK)
        return ret;
    FLV_DEBUG_LOGD("ParseflvFile: flv_setup_seektable done:cur_file_offset=0x%p\n",mfile->cur_file_offset); 
...
FLV_ERROR_TYPE flvParser::flv_parse_script()
{
    ...
    while(1){
        ...
        if(pMeta->tag_header.tag_type!=FLV_TAG_TYPE_META){
            FLV_DEBUG_LOGE("flv_parse_script:this not a script,tag_type is %d,cur_file_offset=0x%p\n",
...
READ_TAG_HEADER:
    ...
    if((tag->tag_header.prv_tag_size != tag->tag_header.tag_data_size + FLV_TAG_HEADER_SIZE)&&
        ...
        FLV_DEBUG_LOGE("flv tag size err, file offset=0x%p\n", mfile->cur_file_offset); 
...
int64_t flvParser::flv_seek_to_msec(int64_t msec)
{
	...
	bUpdateSeekTable = true;
#endif
        FLV_DEBUG_LOGD("flv_seek_to_msec: seet to 0, mfile->cur_file_offset=0x%p\n",mfile->cur_file_offset);
        return 0;
        ...
        if(i < mSeekTable->MaxEntries){
            ...
            mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            FLV_DEBUG_LOGD("flv_seek_to_msec: return 1 %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset);
            return newTs;   
        }else{
            ...
            mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            FLV_DEBUG_LOGD("flv_seek_to_msec: return 2 %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset);
            return newTs;
        }
        ...
        if(i < mSeekTable->SetEntries){
            ...
            mfile->cur_file_offset = pEntry[i].ulOffset; //tag start offset
            mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
            FLV_DEBUG_LOGD("flv_seek_to_msec: return 3 %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset);
            return newTs; 
        }else{
            if(mSeekTable->SetEntries == mSeekTable->MaxEntries){
                ...
                mfile->cur_file_offset = pEntry[mSeekTable->MaxEntries-1].ulOffset; //tag start offset
                mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                FLV_DEBUG_LOGD("flv_seek_to_msec: return 4 %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset);
                return newTs;
            }else{
                flv_tag_str  tag;
				...
				if((mSeekTable->SetEntries != 0)){
		            ...
		            FLV_DEBUG_LOGD("flv_seek_to_msec: jump to offset=0x%p first",mfile->cur_file_offset);
				}else{
				    ts = 0;
		            mfile->cur_file_offset = mfile->data_tag_position;
		            mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);	
				...
				FLV_DEBUG_LOGD("flv_seek_to_msec: time diff = 0x%p",(msec - ts));
                ...
                if(mSeekTable->SetEntries==0 && ret!=FLV_OK){
                    ...
                    mfile->cur_file_offset = mfile->data_tag_position;
                    mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                    FLV_DEBUG_LOGE("flv_seek_to_msec: return 5:entry %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset);
                }else{   
                    ...
                    if(i < mSeekTable->SetEntries){
                        ...
                        mfile->cur_file_offset = pEntry[i].ulOffset; //tag start offset
                        mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                        FLV_DEBUG_LOGD("flv_seek_to_msec: return 7 %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset); 
                    }else{
                        newTs =  pEntry[mSeekTable->SetEntries-1].ulTime;
                        mfile->cur_file_offset = pEntry[mSeekTable->SetEntries-1].ulOffset; //tag start offset
                        mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
                        FLV_DEBUG_LOGD("flv_seek_to_msec: return 8 entry %d newTs=%lld, offset=0x%p",i,newTs,mfile->cur_file_offset);
                    }     
                }                         
        ...
        mfile->cur_file_offset  = mfile->data_tag_position ;
        mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
        FLV_DEBUG_LOGD("flv_seek_to_msec: set to 0, mfile->cur_file_offset=0x%p\n",mfile->cur_file_offset);
        return 0;  
    }
...
FLV_ERROR_TYPE flvParser::flv_direct_seek_to_msec(int64_t Trgmsec, int64_t Currmsec, int64_t *Retmsec)
{
    ...
    while(1){
	    ...
	    FLV_DEBUG_LOGD("flv_direct_seek_to_msec : jump to 0x%p/0x%p/0x%p\n",file_offset, beh_offset, fwd_offset); 
		    while(1){
    ...
    FLV_DEBUG_LOGD("flv_direct_seek_to_msec : final offset = 0x%p\n",mfile->cur_file_offset);
    mfile->mIoStream.seek(mfile->mIoStream.source, mfile->cur_file_offset, FLV_SEEK_FROM_SET);
...
int32_t flvParser::flv_search_tag_pattern(uint8_t **data, uint32_t size)
{
    ...
    FLV_DEBUG_LOGD("flv_search_tag_pattern : find offset = 0x%p\n",offset); 
    return offset;
}
#endif
...
void flvParser::flv_set_file_size(uint64_t file_size)
{
    ...
    if(mfile && ((0 == mfile->file_size) || (file_size != mfile->file_size))){
    	  if (file_size != mfile->file_size){
			 ...
			 int32_t read_size = 0;
    	     FLV_DEBUG_LOGD("flv_set_file_size: file size mismatch,meta/real 0x%p/0x%p\n", mfile->file_size, file_size); 
    	     tag_data = (uint8_t*)malloc(FLV_BS_BUFFER_SIZE*sizeof(uint8_t)); 
...
bool  isSupportedStream( FLV_Type stream_type, uint32_t  codec_id,sp<MetaData> StreamMeta)
{
	if(stream_type == VIDEO){	 
		switch (codec_id) {
			case FLV_VIDEO_CODEC_ID_AVC:
			{
#if 0  // remove MTK_GMO_RAM_OPTIMIZE for avc 
			...
			default:
			{
				ALOGE("[FLV capability error]Unsupport video format!!!mStreamType=0x%p ", codec_id);
				return false;
			}
		}
...
#endif 
            return OK;
	} else if (mExtractor->mVideoFrames.size() >= FLV_CACHE_POOL_LOW) { //CacheMore a video frame and dequeue a video frame 
	    retVal = mExtractor->CacheMore(CACHE_FRAME);
	    if (FLV_FILE_EOF == retVal) {
	        FLV_LOGD("FLVSource read eos, but mVideoFrames still has queued %d frames", mExtractor->mVideoFrames.size());
	    } else if (FLV_FILE_READ_ERR == retVal) {
...
uint8_t* FLVExtractor::cutAVCC2Buffer(uint8_t *data, size_t size, int buffer_size) 
{
 ...
 READ_TAG:
	 ...
	 { 
		 ...
		 {
			 byteVal1 = *(mTag->tag_data);
			 FLV_LOGE ("CacheMore not video && not audio tag: skip it,byteVal1=0x%p\n",byteVal1);
		 }
 ...
 MediaBuffer* FLVExtractor::DequeueVideoFrame(int64_t targetSampleTimeUs)
 {
	 ...
	 switch(byteVal & 0x0F)
	 {
		 ...
		 case FLV_VIDEO_CODEC_ID_AVC:
		 {
			  ...
			  int32_t CompositionTime;
			  uint64_t ts;
			  FLV_LOGV("DequeueVideoFrame:bf tag_data_size=%d tag_data_offset =%u, tag_data=0x%p\n",
