...
status_t MtkAVISource::read(
    ...
    *out = NULL;
    ...
    if (newBuffer) {
        ...
        if (mIsVideo) {
if((mSampleSize == 0)&&(isSyncSample)&&!(isSyncSample & kKeySyncFixedMask)){
	if(isSyncFrame((const char*)mBuffer->data(), mBuffer->range_length())){
		mSampleSyncs.replaceAt(1|kKeySyncFixedMask, mCurrentSampleIndex-1);
	}else{
		mSampleSyncs.replaceAt(0, mCurrentSampleIndex-1);
		isSyncSample = 0;
	}
}
