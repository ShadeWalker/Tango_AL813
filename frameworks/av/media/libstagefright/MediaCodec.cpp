...
status_t MediaCodec::queueInputBuffer(
    ...
    MM_LOGV("[%s][mState %d]index %d size %d presentationTimeUs %lld flags %x+++",mComponentName.c_str(),mState,
...
status_t MediaCodec::queueSecureInputBuffer(
    ...
    MM_LOGV("[%s][mState %d]index %d numSubSamples %d presentationTimeUs %lld flags %x+++",mComponentName.c_str(),mState,
...
status_t MediaCodec::dequeueInputBuffer(size_t *index, int64_t timeoutUs) {
   ...
   MM_LOGV("[%s][mState %d]+++",mComponentName.c_str(),mState);
    ...
    MM_LOGV("[%s][mState %d]index %d ---",mComponentName.c_str(),mState,*index );
    return OK;
}
...
#ifdef MTK_AOSP_ENHANCEMENT
            mHasACodecError = true;             // when started, set the flag. It is used by release
#endif
            switch (what) {
...
status_t MediaCodec::queueCSDInputBuffer(size_t bufferIndex) {
    ...
    if (csd->size() > codecInputData->capacity()) {
	    ...
	    MM_LOGI("[error] csd->size()  %d  > codecInputData->capacity() %d", csd->size() , codecInputData->capacity());
...
#ifdef MTK_AOSP_ENHANCEMENT
    if (newState != UNINITIALIZED && mHasACodecError == true ) {
        mHasACodecError = false;
    }
#endif
    mState = newState;
...
status_t MediaCodec::onQueueInputBuffer(const sp<AMessage> &msg) {
    ...
    if (index >= mPortBuffers[kPortIndexInput].size()) {
	    ...
	    MM_LOGI("[error RANGE]index %d > mPortBuffers[%d].size() %d",
    ...
    if (offset + size > info->mData->capacity()) {
	    MM_LOGI("[error VAL] offset=%d , size %d >info->mData->capacity() %d  ",offset,size,info->mData->capacity());
    ...
    if (mCrypto != NULL) {
        if (size > info->mEncryptedData->capacity()) {
	        ...
	        MM_LOGI("[error RANGE] size %d >info->mEncryptedData->capacity() %d  ",size,info->mEncryptedData->capacity());
...
ssize_t MediaCodec::dequeuePortBuffer(int32_t portIndex) {
    ...
    if (availBuffers->empty()) {
	    ...
	    //MM_LOGI("[%s]portIndex=%d no buffer avail",__FUNCTION__,portIndex);
...
status_t MediaCodec::setNativeWindow(
    ...
    if (surfaceTextureClient != NULL) {
        ...
        if(err == -EEXIST ||err == -EINVAL)
        {
          ...
          native_window_api_disconnect(surfaceTextureClient.get(),NATIVE_WINDOW_API_MEDIA);
          err = native_window_api_connect(surfaceTextureClient.get(),NATIVE_WINDOW_API_MEDIA);
        }
#endif
        if (err != OK) {
...
status_t MediaCodec::amendOutputFormatWithCodecSpecificData(
    ...
    if (!strcasecmp(mime.c_str(), MEDIA_MIMETYPE_VIDEO_AVC)) {
        ...
        if (csdIndex != 2) {
	     MM_LOGI("[%s][state %d] csdIndex = %d", mComponentName.c_str(),mState, csdIndex);
