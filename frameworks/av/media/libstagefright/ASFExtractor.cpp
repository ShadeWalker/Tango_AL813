...
ASFSource::ASFSource(const sp<ASFExtractor> &extractor, size_t index)
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_MPEG)) {
        ...
        ALOGD("mMP3Header=0x%p", mMP3Header);
    }
    //AVC
...
status_t ASFSource::findMP3Header(int32_t *pHeader) {
    ...
    while(0 == *pHeader) {
        ...
        if ((ASF_END_OF_FILE == retVal) && (0 != header1)) {
            ALOGE("[ASF_ERROR]ASFSource::findMP3Header failed, fake header = 0x%p", header1);
                ...
                if (j < 4) {
                    ...
                    if (ASF_SUCCESS == mExtractor->GetNextMediaFrame(&packet2,
                        ...
                        if(packet2->range_length() < (size_t)left) {
                            ALOGD("ASF The packet(= %d < left= %d)is too small to check MP3 Header.",
                        ...
                        ALOGD("[ASF_ERROR]ASFSource::findMP3Header End of stream,fake header = 0x%p", header1);
                        *pHeader = (int32_t)header1;
                        mExtractor->ASFSeekTo(0);
                        return OK;
                    }
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {//seek and find an I
        ALOGI("ASFSource::read seek to %lld us (mType=%d, mode=%d,mHasVideo=%d)",
             seekTimeUs, mType, mode, mExtractor->mHasVideo);
        mSeeking = true;
            ...
            if (mType == ASF_VIDEO) {
                ...
                if (ret < 0) {
                    ...
                    if (lastSeekTime != -1) {
                        ...
                        repeat_cnt = 10;
                        ALOGI("seek to last seek Time:%lld", lastSeekTime);
                    } else {
                        return ERROR_END_OF_STREAM;
                    }
                }
                ...
                while (!bIsKeyFrame) {
                    ...
                    if ((ASF_END_OF_FILE == retVal) || (ASF_ERROR_MALFORMED == retVal)) {
                        ...
                        if (lastSeekTime != -1 && eosTimes < 2) {
                            ...
                            bIsKeyFrame = false;
                            repeat_cnt = 10;
                            ALOGI("seek to last seek Time %lld when ASF_END_OF_FILE, eosTimes=%zu", lastSeekTime, eosTimes);
                        } else {
                            ALOGI("return EOS, eosTimes=%zu", eosTimes);
                            ...
                            if (((mode == ReadOptions::SEEK_PREVIOUS_SYNC && (diff > 0 || diff < -500000))) ||
                                ...
                                if (mode == ReadOptions::SEEK_PREVIOUS_SYNC) {
                                    if ((lastDiff > 0 && diff < 0)) {
                                        ...
                                        bIsKeyFrame = false;
                                        repeat_cnt = 10;
                                        ALOGI("seek to last seek Time:%lld, lastDiff:%lld, diff:%lld", lastSeekTime, lastDiff, diff);
                                        continue;
                                    }
                                }
                                ...
                                ALOGI(
                                        ...
                                        keyTimeUs / 1000, seekTimeUs / 1000,
                                        keyTimeUs / 1000 - seekTimeUs / 1000, repeat_cnt);
                                lastDiff = diff;
...
status_t ASFSource::assembleAVCSizeNalToFrame(MediaBuffer **out) {
    ...
    while(mBuffer->range_length() != 0 ) {
        ...
        memcpy(data + 3, mBuffer->data() + mBuffer->range_offset(), nalLength);
        memcpy(tempBuffer->data() + tempBuffer->range_offset(),data, 3 + nalLength);
    ...
    if (mBuffer->range_length() == 0) {
        ...
        memcpy(reBuffer->data() + reBuffer->range_offset(),
                tempBuffer->data() + tempBuffer->range_offset(),tempBuffer->range_length());
...
status_t ASFSource::assembleMp3Frame(MediaBuffer **out) {
    ...
    if((mBuffer != NULL) && (mBuffer->range_length() < 4)) {
        ALOGI("ASFSource::read() mBuffer size(%d) < 4 bytes, appending buffer for detecting MP3 header",
        ...
        ALOGI(
             "ASFSource::read() new mBuffer size(%d), including original mBuffer size(%d) + new packet size(%d)",
    ...
    if (mMP3Header >= 0) {
        ALOGD("[ASF_Read]mMP3Header=0x%p", mMP3Header);
    }
    ...
    ALOGI("[ASF_Read]mp3 start %d header %x frameSize %d length %d bitrate %d",
        ...
        if (frameSize + (unsigned)start > length) {
            ...
            if(readret == OK) {
                ...
                *out = tmp;
                ...
                ALOGE("readNextASFframe return 0x%p", readret);
                tmp->release();
                tmp = NULL;
                nextBuffer->release();
                nextBuffer = NULL;
...
status_t ASFSource::assembleMjpegFrame(MediaBuffer **out) {
    ...
    clone->meta_data()->setInt32(kKeyIsSyncFrame, keyIsSyncFrame);
    uint8_t *data = (uint8_t *)clone->data();
    memcpy(data, mBuffer->data() + mBuffer->range_offset() + (size_t)start, mjpeg_frame_size);
    CHECK(clone != NULL);
...
// ]
    ...
    *out = buffer;
    ...
    ALOGI("GetNextMediaFrame StreamId=%d, ts=%d, IsKey=%d, replicated_size=%d, (*out)->range_length()=%d, current_frame_size=%d",
    ...
    if (mAsfParser != NULL) {
        ...
        do {
            ...
            if (!pNextPacket->payload_count) {
                ...
                ALOGI("GetNextMediaPayload: curTrackIndex = %d, find a new packet, contain payloads = %d, ret = %d",
...
bool ASFExtractor::ParserVC1CodecPrivateData(uint8_t*input,
    ...
    ALOGI("-----[ASF]CodecPrivateData is 0x%p-----\n",reserved);
...
bool ASFExtractor::RetrieveWmaCodecSpecificData(
    ...
    if(pStreamProp->flags & ASF_STREAM_FLAG_AVAILABLE) {
        ...
        switch (wfx->wFormatTag) {
            ...
            {
                ALOGE(
                     "[VC-1 Playback capability Error] capability not support as:Unknown audio format MSADPCM=0x%p\n",
                     wfx->wFormatTag);
                return false;;
            }
            ...
            {
                ALOGE(
                     "[VC-1 Playback capability Error] capability not support as:Unknown audio format=0x%p\n",
                     wfx->wFormatTag);
                return false;
            }
        }
        ALOGI("-----[ASF]RetrieveWmaCodecSpecificData -----\n");
        ALOGI("wfx->wFormatTag = 0x%p\n", wfx->wFormatTag);
...
status_t ASFExtractor::addMPEG4CodecSpecificData(const sp<MetaData>& mMeta) {
    ...
    ALOGI("volStart = %u, vopStart = %u", volStart, vopStart);
    for (uint32_t i = volStart ; i < vopStart; i++) {
        ALOGI("VOS[%d] = 0x%p", i, *((uint8_t *)buffer->data() + i));
    }
...
status_t ASFExtractor::addAVCCodecSpecificData(const sp<MetaData>& mMeta) {
    ...
    ALOGI("asfDataPos = %lld, read %d byte\n",asfDataPos, n);
    if (n < (ssize_t)size) {
