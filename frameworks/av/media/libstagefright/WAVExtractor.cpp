...
#ifdef MTK_AOSP_ENHANCEMENT
	off64_t streamSize;
	if ((mDataSource->getSize(&streamSize) == OK) && (streamSize - 8 > totalSize) )
	{
	    ALOGD("totalSize(%d), streamSize(%d)", totalSize, streamSize);
	    totalSize = streamSize - 8;
	}
#endif
...
//				return ERROR_MALFORMED;
            ...
            if (mValidFormat) {
				...
				{
					if (mDataSize + mDataOffset > streamSize)
					{
						mDataSize = streamSize - mDataOffset;
					}
				}
#endif
...
//				mTrackMeta->setInt32(kKeyExtraDataSize, mExtraDataSize);
				...
				if(mWaveFormat == WAVE_FORMAT_MSGSM)
				{
					...
					durationUs = 1000000LL * (mDataSize / 65 * 320) / mSampleRate;
					SXLOGD("mDataSize is %d, duration is %0.2f s, mSampleRate(%d)", mDataSize, durationUs / 1E6, mSampleRate);
				}
				else
                	durationUs =
...
#ifdef MTK_AOSP_ENHANCEMENT
#ifdef MTK_AUDIO_RAW_SUPPORT
	SXLOGV("PCM Data pass PCM Component !");
#else
