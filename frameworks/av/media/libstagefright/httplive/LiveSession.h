...
private:
    ...
    bool canSwitchUp();
#else
    bool canSwitchUp(bool upwardsFlag);
    void checkBufferingIfNecessary(bool bufferingFlag);

public:
    bool allTracksPresent();
    status_t getTextStreamFormat(sp<AMessage> *format);
private:
    void dumpPlaylist(sp<ABuffer> &buffer);
    void onPictureReceived(const sp<AMessage> &msg);
    ssize_t pickBandwidthIndex(int32_t bandwidthBps);
    
    bool mForceDisconnect;
    bool mIsBuffering;
    Mutex mLock;
    sp<ALooper> mPlaylistfetcherLooper;
    uint32_t mReadSucessCount[kMaxStreams];
#endif
