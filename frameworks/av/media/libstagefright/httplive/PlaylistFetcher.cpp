...
#define PLAYLISTFETCHER_USE_XLOG
#ifdef PLAYLISTFETCHER_USE_XLOG
...
// static
#ifdef MTK_AOSP_ENHANCEMENT
const int64_t PlaylistFetcher::kMinBufferedDurationUs = 11000000ll;
const int64_t PlaylistFetcher::kMaxBufferedDurationUs = 15000000ll;
const int64_t PlaylistFetcher::kThresholdBufferedDurationUs = 30000000ll;
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
//Hypothesis we support bandwith-chage min bandwith is 100Kbps(100 * 1024 /8;)
const int32_t PlaylistFetcher::kDownloadBlockSize = 12800 ;
#else
...
#ifdef MTK_AOSP_ENHANCEMENT
      ALOGI("PlaylistFetcher,mURI = %s",mURI.c_str());
#endif
...
void PlaylistFetcher::startAsync(
    ...
    msg->post();
#ifdef MTK_AOSP_ENHANCEMENT
	ALOGI("startAsync,streamTypeMask=0x%x, startTimeUs=%" PRId64 " us, segmentStartTimeUs=%" PRId64 " us,startDiscontinuitySeq=%d,adaptive=%d",\
				streamTypeMask,startTimeUs,segmentStartTimeUs,startDiscontinuitySeq,adaptive);
#endif
}
...
status_t PlaylistFetcher::onResumeUntil(const sp<AMessage> &msg) {
...
#ifdef MTK_AOSP_ENHANCEMENT
            ALOGI("onResumeUntil uri=%s",mURI.c_str());
            ALOGI("onResumeUntil,latestTimeUs =%" PRId64 " us,stopTimeUs=%" PRId64 " us",latestTimeUs,stopTimeUs);
#endif
            stop = true;
        }
    }
    ...
    mStopParams = params;
#ifdef MTK_AOSP_ENHANCEMENT
    //fast trigger before next gear download the second segment.
    onMonitorQueue();
#else	
    postMonitorQueue();
#endif
...
void PlaylistFetcher::onMonitorQueue() {
    bool downloadMore = false;
		...
		if (refreshPlaylist() != OK) {
		 		ALOGW("onMonitorQueue,refresh playlist fail");
		        return;
		 }
#else
    refreshPlaylist();
...
#ifdef MTK_AOSP_ENHANCEMENT
         //buffer bewteen min and max
    if (durationToBufferUs < kMinBufferedDurationUs)  {
        durationToBufferUs = kMinBufferedDurationUs;
    }

    if (durationToBufferUs > kMaxBufferedDurationUs)  {
        durationToBufferUs = kMaxBufferedDurationUs;
    }
...
#ifdef MTK_AOSP_ENHANCEMENT // use min duration as judge downloadMore
        int64_t maxbufferedStreamDurationUs = 0;
        for (size_t i = 0; i < mPacketSources.size(); ++i) {
            ...
            ALOGD("buffered %" PRId64 " for stream %d",
                    bufferedStreamDurationUs, mPacketSources.keyAt(i));
	     if (bufferedDurationUs == 0){
                bufferedDurationUs = bufferedStreamDurationUs;
            }else if (bufferedStreamDurationUs < bufferedDurationUs) {
                bufferedDurationUs = bufferedStreamDurationUs;
            }
            if (bufferedStreamDurationUs > maxbufferedStreamDurationUs) {
                maxbufferedStreamDurationUs = bufferedStreamDurationUs;
            }

        }
		
        if (maxbufferedStreamDurationUs >=kThresholdBufferedDurationUs){
	    ALOGE("maybe error cache too much, maxbufferedStreamDurationUs = %lld, bufferedDurationUs = %lld ", maxbufferedStreamDurationUs, bufferedDurationUs);
	    bufferedDurationUs = maxbufferedStreamDurationUs;
        }
#else
    ...
    if (!mPrepared && bufferedDurationUs > targetDurationUs && downloadMore) {
        ...
        ALOGD("prepared, buffered=%" PRId64 " > %" PRId64 "",
    ...
    if (finalResult == OK && downloadMore) {
        ALOGD("monitoring, buffered=%" PRId64 " < %" PRId64 "",
        ...
        ALOGD("prepared, buffered %lld", bufferedDurationUs);
        sp<AMessage> msg = mNotify->dup();
        msg->setInt32("what", kWhatTemporarilyDoneFetching);
        msg->post();

        int64_t delayUs = mPrepared ? kMaxMonitorDelayUs : targetDurationUs / 2;
        ALOGD("pausing for %" PRId64 ", buffered=%" PRId64 " > %" PRId64 "",
    ...
    if (delayUsToRefreshPlaylist() <= 0) {
        bool unchanged;
         ...
         sp<M3UParser> playlist = mSession->fetchPlaylist(
                mURI.c_str(), mPlaylistHash, &unchanged, &errcode);
         ALOGI("refreshPlaylist,mURI =%s,unchanged =%d,errcode=%d",mURI.c_str(),unchanged,errcode);
#else
        sp<M3UParser> playlist = mSession->fetchPlaylist(
...
//#ifdef MTK_AOSP_ENHANCEMENT
//                if(errcode != OK) {
//                    ALOGE("failed to load playlist at url '%s' cuz force disconnection", mURI.c_str());
//                    return ERROR_IO;//cancel download without notifyerror
//                }
//#endif
...
void PlaylistFetcher::onDownloadNext() {
...
#ifdef MTK_AOSP_ENHANCEMENT
				selectSeqNumber(firstSeqNumberInPlaylist,lastSeqNumberInPlaylist);
#else
...
#ifndef MTK_AOSP_ENHANCEMENT	//use absolute timestamp 			
...
#ifdef MTK_AOSP_ENHANCEMENT
								ALOGD("sequence number high: %d from (%d .. %d), "
    ...
    if (itemMeta->findInt32("discontinuity", &val) && val != 0) {
        ...
        discontinuity = true;
#ifdef MTK_AOSP_ENHANCEMENT
        ALOGI("onDownloadNext,playlist has EXT-X-DISCONTINUITY,mDiscontinuitySeq=%d",mDiscontinuitySeq);
#endif 
    }
    ...
    ALOGD("this:%p,fetching segment %d from (%d .. %d) for:%s",
          this,mSeqNumber, firstSeqNumberInPlaylist, lastSeqNumberInPlaylist,uri.c_str());
    ...
    do {
#ifdef MTK_AOSP_ENHANCEMENT
        int64_t fetchfileStartTimeus = ALooper::GetNowUs();
#endif
        bytesRead = mSession->fetchFile(
                uri.c_str(), &buffer, range_offset, range_length, kDownloadBlockSize, &source);
 #ifdef MTK_AOSP_ENHANCEMENT
        TotalfetchfileTimeUs += (ALooper::GetNowUs() - fetchfileStartTimeus);
 #endif
        if (bytesRead < 0) {
...
#ifdef MTK_AOSP_ENHANCEMENT
        if ((!anchorTimeFlag) && (mPlaylist->isComplete() || mPlaylist->isEvent())) {
            mNextPTSTimeUs = getSegmentStartTimeUs(mSeqNumber);
            //ALOGD("got mNextPTSTimeUs:%lld",mNextPTSTimeUs);
            anchorTimeFlag = true;
        }
#endif    
...
#endif
#ifdef MTK_AOSP_ENHANCEMENT
        if((mSeqNumber == 0 || mStartup) && discontinuity) {
            if(mStartup)
                ALOGD("Skip queueDiscontinuity while playlist start with explicitDiscontinuity");
        }
...
#ifdef MTK_AOSP_ENHANCEMENT
    ALOGD("fetching done fetch file rate [%d] %lld , TSparserate[%d] %lld, totaltime = %lld", 
        (TotalfetchfileTimeUs * 100 /(ALooper::GetNowUs() - fetchStartTimeUs))  ,TotalfetchfileTimeUs, 
        (TotalParseTime * 100 /(ALooper::GetNowUs() - fetchStartTimeUs)), TotalParseTime,
        (ALooper::GetNowUs() - fetchStartTimeUs));
#endif
    ALOGD("this:%p,fetching done segment %d from (%d .. %d) for:%s",this,
...
#ifdef MTK_AOSP_ENHANCEMENT
        //post mStartTimeUsNotify after mStreamTypeMask revised
        if (mStartTimeUsNotify != NULL) {
            uint32_t streamMask = 0;
            mStartTimeUsNotify->findInt32("streamMask", (int32_t *) &streamMask);
            if (streamMask == mStreamTypeMask) {
                ALOGD("post mStartTimeUsNotify");
...
status_t PlaylistFetcher::extractAndQueueAccessUnitsFromTs(const sp<ABuffer> &buffer) {
    ...
    if (mNextPTSTimeUs >= 0ll) {
        ...
        mTSParser->signalDiscontinuity(
                    ATSParser::DISCONTINUITY_HTTPLIVE_MEDIATIME, extra);
#else
        mTSParser->signalDiscontinuity(
    ...
    for (size_t i = mPacketSources.size(); i-- > 0;) {
        ...
        while (source->hasBufferAvailable(&finalResult)
            ...
            if (mStartup) {
#ifdef MTK_AOSP_ENHANCEMENT
				ALOGI("Startup,first %s,timeUs =%" PRId64 " us,mStartTimeUs=%" PRId64 " us,mSeqNumber=%d",\
					key,timeUs,mStartTimeUs,mSeqNumber);
#endif
                if (!mFirstPTSValid) {
                    mFirstTimeUs = timeUs;
                    mFirstPTSValid = true;
...
#ifdef MTK_AOSP_ENHANCEMENT
					ALOGD("timeUs %" PRId64 " us < mStartTimeUs %" PRId64 " us",
                            timeUs, mStartTimeUs);
#else
            ...
            CHECK(accessUnit->meta()->findInt64("timeUs", &timeUs));
#ifdef MTK_AOSP_ENHANCEMENT
			ALOGV("extractAndQueueAccessUnitsFromTs,accu timeUs = %" PRId64 " us",timeUs);
      ...
      if (mStartTimeUsNotify != NULL && timeUs > mStartTimeUs) {
...
#ifdef MTK_AOSP_ENHANCEMENT
					ALOGI("start time too high,re-start from mSeqNumber=%d",mSeqNumber);
#endif 
                ...
                if (!mStartTimeUsNotify->findInt64(key, &startTimeUs)) {
                    ...
                    if (streamMask == mStreamTypeMask) {
#ifdef MTK_AOSP_ENHANCEMENT
						ALOGI("all track has get a buffer,notify startTimeUsNotify");
#endif 
                        mStartup = false;
                        mStartTimeUsNotify->post();
                        mStartTimeUsNotify.clear();
...
#ifdef MTK_AOSP_ENHANCEMENT
				ALOGI("extractAndQueueAccessUnits,queue FormatChange,unitTimeUs =%" PRId64 " us,stopTimeUs=%" PRId64 " us",\
					unitTimeUs,stopTimeUs);
#endif 
                packetSource->queueAccessUnit(mSession->createFormatChangeBuffer());
                mStreamTypeMask = 0;
                mPacketSources.clear();
...
#ifdef MTK_AOSP_ENHANCEMENT
void PlaylistFetcher::selectSeqNumber(const int32_t firstSeqNumberInPlaylist,const int32_t lastSeqNumberInPlaylist) {
    ...
    ALOGD("selectSeqNumber,Start with Seq(%d), targetDuration(%d), Penultimate(%d)", mSeqNumber,targetDurationSecs, i);
}

bool PlaylistFetcher::checkIfHaveMetaPicture() {
...
#ifdef MTK_AOSP_ENHANCEMENT
								ALOGI("checkIfHaveMetaPicture,has Picture,mimeType =%s",mimeType.c_str());
#endif 
                sp<AMessage> notify = mNotify->dup();
                notify->setInt32("what", kWhatPicture);
                notify->setBuffer("buffer", accessUnitMeta);
                //not set format OR seq
                notify->post();
            }
        }
        return true;
