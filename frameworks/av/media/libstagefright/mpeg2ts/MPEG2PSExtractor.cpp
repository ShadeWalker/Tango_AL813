...
#define PS_SNIFF_LENGTH SNIFF_LENGTH_1K	//enlarge from 1k to 900k for .dat files, reference BD experience
                                        //for performance return 1k
#else
    ...
    if (pSource->hasBufferAvailable(&finalResult)) { 
       ...
       if (firstMeta == NULL) {
           ...
           if (timeDuration > 2000000LL) {  //2s
               ALOGV("duration :%lld >2s", timeDuration);
               return true;
           }
           else {
               ALOGV("duration :%lld", timeDuration);
               return false;
           }
       }
       int64_t timeUs2;
...
#endif //defined(MTK_AOSP_ENHANCEMENT) && defined(MTK_AUDIO_CHANGE_SUPPORT)
        ...
        if (index < 0 && mScanning) {
            ...
            if (streamType != ATSParser::STREAMTYPE_RESERVED) {
                ...
                if(streamType == ATSParser::STREAMTYPE_MPEG1_VIDEO ||
                    ...
                    ALOGD("PES - add video track %02X,index=%d",stream_id,index);
                }
                    ...
                    mhasATrack= true;
                    index = mTracks.add(stream_id, new Track(this, stream_id, streamType));
                    ALOGD("PES - add audio track %02X,index=%d",stream_id,index);
                }
            }
#else
             if (streamType != ATSParser::STREAMTYPE_RESERVED) {
...
void MPEG2PSExtractor::parseMaxPTS() {
	...
	mDataSource->getSize(&mFileSize);
    ALOGD("File length :%lld ", mFileSize);
    setDequeueState(false);
	signalDiscontinuity();
    ...
    for (off64_t i = 1; mOffsetSearch >= (kChunkSize/100); i++) {
        ...
        if (mOffsetSearch > 100 * i * kChunkSize) {
        ...
        ALOGD("Parse %lld times, Search offset %lld, Valid Offset: %lld, %2.2f%% of Total File", i, mOffsetSearch, mOffset, (float)(100 * mOffsetSearch / mFileSize));
        mFinalResult = OK;
        mBuffer->setRange(0, 0);
    	while (feedMore() == OK) {
    	    ...
    	    if(((mOffset - mOffsetSearch) > 100 * kChunkSize) && (0 == getMaxPTS()) ) {
                ALOGD("stop feedmore (no PES) mOffset=%lld  mOffsetSearch=%lld",mOffset,mOffsetSearch);
                break;                    
             }

             if((mOffset - mOffsetSearch) > 100 * i * kChunkSize) {
                ALOGD("stop feedmore (enough data) mOffset=%lld  mOffsetSearch=%lld",mOffset,mOffsetSearch);
                break;                    
             }
    	}
        mDurationUs = getMaxPTS();
        if (mDurationUs) {
            ALOGD("parseMaxPTS:mDurationUs=%lld, mOffset=%lld", mDurationUs, mOffset);
            break;
        }
    }
