...
MP3Source::MP3Source(
	...
	if (meta->findInt32(kKeyIsMtkMusic, &isMtkMusic) && (isMtkMusic == 1) ) {
		mEnableTOC = true;
		ALOGD("Mtk Music, enable TOC");
		isMtkMusic = 0;
	}
	else 
	{
		mEnableTOC = false;
		ALOGD("Not Mtk Music, disable TOC");
	}
...
#ifdef MTK_AOSP_ENHANCEMENT
		int64_t readheader_start = ALooper::GetNowUs();
        ssize_t n = mDataSource->readAt(mCurrentPos, buffer->data(), 4);
		int64_t readheader_finish = ALooper::GetNowUs();
		if((readheader_finish - readheader_start) > 500)
		{
		   ALOGE("Read mp3 header may timeout , time consumed = %lld", readheader_finish - readheader_start);
		}
#else
		ssize_t n = mDataSource->readAt(mCurrentPos, buffer->data(), 4);
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT
    int64_t readframe_start = ALooper::GetNowUs();
    ssize_t n = mDataSource->readAt(mCurrentPos, buffer->data(), frame_size);
    int64_t readframe_finish = ALooper::GetNowUs();
    if((readframe_finish - readframe_start) > 500)
    {
	ALOGE("Read mp3 frame may timeout , time consumed = %lld", readframe_finish - readframe_start);
    }
#else
    ssize_t n = mDataSource->readAt(mCurrentPos, buffer->data(), frame_size);
#endif
