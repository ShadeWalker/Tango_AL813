...
bool FLACSource::getSeekPosforStreaming(int64_t targetTimeUS, off_t *mCurrentSeekPos, int *seekFrm)
{
    ...
    while (1)
    {
        if (TargetFrmPos >= filesize) {
            ALOGD("TargetFrmPos(%d), filesize(%d)", TargetFrmPos, filesize);
		    break;
        }
        ...
        if (((targetFrmNum - SeekFrmOffset) <= (int64_t)new_frmnum) &&
                ((int64_t)new_frmnum <= (targetFrmNum + SeekFrmOffset)))
        {
...
{
    if (mObserver != NULL && mCallback != NULL && bTocEnable) {
      ...
      mMetaInfo(MetaInfo)
{
	...
	mObserver = NULL;
	mCallback = NULL;
	if (mTrackMetadata->findPointer(kKeyDataSourceObserver, &ptr)) {
	    mObserver = ptr;
	}
	if (mTrackMetadata->findPointer(kKeyUpdateDuraCallback, &ptr) ) {
    ...
    MediaBuffer **outBuffer, const ReadOptions *options)
{
    ...
    *outBuffer = NULL;
    ...
    if (options != NULL && options->getSeekTo(&seekTimeUs, &mode))
    {
        ...
        {
            status_t stat = getFramePos(seekTimeUs, &mCurrentTimeUs, &pActualPos, true);
