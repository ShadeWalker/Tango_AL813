...
void AudioFlinger::ThreadBase::TrackBase::releaseBuffer(AudioBufferProvider::Buffer* buffer)
{
    ...
    if(!mIsOut){
        ...
        const int SIZE = 256;
        char fileName[SIZE];
        sprintf(fileName,"%s_%p.pcm",gaf_track_pcm,this);
                                                    ...
                                                    int triggerSession __unused)
{
    ...
    if (thread != 0) {
        ...
        //ALOGD("%s mState %d", __FUNCTION__, mState);
        ...
        mRsmpInUnrel(0), mRsmpInFront(0), mFramesToDrop(0), mResamplerBufferProvider(NULL)
{
    ...
    if (thread->mSampleRate != sampleRate && thread->mChannelCount <= FCC_2 &&
        ...
        #else
        uint32_t sourceSampleRate = thread->mSampleRate;
        mResampler = AudioResampler::create(AUDIO_FORMAT_PCM_16_BIT,
                thread->mChannelCount, sampleRate,AudioResampler::DEFAULT_QUALITY,sourceSampleRate);
        #endif
        // source SR
