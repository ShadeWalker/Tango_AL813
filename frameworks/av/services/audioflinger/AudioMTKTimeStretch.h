...
//============================================================//
    ...
    void volumeRamp(short* out, int frameCount, short* temp, int* UpDown);
    int SetFirstRamp(int NeedRamp);
    typedef enum
    {
        RAMP_DOWN,
        RAMP_NORMAL,
    ...
    {
        TS_STATE_INIT = 0,
        TS_STATE_RAMP_DOWN_FROM_NORMAL,
        TS_STATE_RAMP_DOWN_FROM_TS,    
        TS_STATE_SILENCE,
        TS_STATE_NORMAL,
        TS_STATE_RAMP_UP
    } MTK_TS_PROCESS_STATUS;
...
private:
	...
	unsigned int mTempBufferSize;
         int mFraction_remain; // leftover if outputbuffer can't hold, only used in 16x
         int mRampFlag; // ramp down :0, normal: 0, rampUp 2
         int mProcessState;
         int mRatio_Cache;
         int mNeedRamp;
