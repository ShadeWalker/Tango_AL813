...
status_t Parameters::initialize(const CameraMetadata *info, int deviceVersion) {
    ...
    //
    for( uint32_t i = 0; i < sizeof(encoder_list)/sizeof(video_encoder); i++ ) {
        int32_t width = videoEncoderProfiles->getVideoEncoderParamByName(
                "enc.vid.width.max", encoder_list[i]);
        int32_t height = videoEncoderProfiles->getVideoEncoderParamByName(
                "enc.vid.height.max", encoder_list[i]);
        ALOGD("encoder %d: %dx%d", encoder_list[i], width, height);
        if( width > maxVideoWidth || height > maxVideoHeight ) {
            maxVideoWidth = width;
            maxVideoHeight = height;
        }
    }
    ALOGD("max encoder size: %dx%d", maxVideoWidth, maxVideoHeight);
#endif
