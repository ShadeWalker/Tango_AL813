...
status_t CameraService::connect(
    ...
    LOG1("CameraService::connect E (pid %d \"%s\", id %d)", callingPid,
            clientName8.string(), cameraId);
		LOG1("xubang add for performance opti , force open 8 cpu 3s");//niyangadd
	if(handle==NULL)
	{
		if(load_api() != 0)
		{
		...
		mPerfHandle = perfUserScnReg(8, 0); //force open 8 cpu
		perfUserScnEnableTimeout(mPerfHandle, 3); //8 cpu timeout after 3s
	}
...
#ifdef POWER_GENIE_POWER_LOG_INTERFACE
        POWERPUSH(129, "", "", "");//define in LogPower.java, 129 stand for camera start now
#endif
...
void CameraService::removeClientByRemote(const wp<IBinder>& remoteBinder) {
    ...
    if(mPerfHandle!=-1)
    {
        LOG1("CameraService::removeClientByRemote  mPerfHandle!=-1");
        perfUserScnDisable(mPerfHandle);
        perfUserScnUnreg(mPerfHandle);
        mPerfHandle=-1;
    }
    if(handle!=NULL)
    {
	    LOG1("CameraService::removeClientByRemote  handle!=NULL");
        dlclose(handle);
        handle=NULL;
    }
...
//!--
    ...
    if (mSoundRef++) return;
//!++ xubang modify for performance opti
#if 0
...
//!++ xubang add for performance opti
...
void* CameraService::loadSoundThread(void* arg) {
	...
	if(!pCameraService)
	{
      ALOGE("pCameraService is null error!!!");
	  return NULL;
	}
...
CameraService::Client::~Client() {
    ...
    /*<DTS2014112601808 huangwen 00181596 20141126 begin */
#ifdef POWER_GENIE_POWER_LOG_INTERFACE
    POWERPUSH(134, "", "", "");//define in LogPower.java, 134 stand for camera disconnect now
#endif
    /* DTS2014112601808 huangwen 00181596 20141126 end>*/
    Client::disconnect();
}
