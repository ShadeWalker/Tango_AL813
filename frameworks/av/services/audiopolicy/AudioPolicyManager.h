...
//MTK_AUDIO_ADD>
		...
		audio_io_handle_t getPrimaryOutput();
...
//        bool AdjustMasterVolumeIncallState(audio_stream_type_t stream)    ;
//        bool AdjustMasterVolumeNormalState(audio_stream_type_t stream);
//        bool AdjustMasterVolumeRingState(audio_stream_type_t stream);
//        bool StreamMatchPhoneState(audio_stream_type_t stream);
//        int Audio_Match_Force_device(audio_policy_forced_cfg_t  Force_config);
...
public:
        ...
        status_t checkAndSetGainTableAnalogGain(int stream, int index, audio_io_handle_t output,
                                          audio_devices_t device, int delayMs = 0, bool force = false );
        int selectGainTableActiveStream(int requestStream);
        float computeGainTableCustomVolume(int stream, int index, audio_io_handle_t output,audio_devices_t device);
private:
       int                 mVolumeStream;
       int                 mVolumeIndex;
