                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_SONIFICATION_RESPECTFUL:
        if (isInCall()) {
            ...
            device = getDeviceForStrategy(STRATEGY_MEDIA, false /*fromCache*/);//Recovery Goodgle code by lihaizhou at 20151030 
                        
			//add by HQ_zhouguo at 20150902 start
			// when media is not playing anymore, fall back on the sonification behavior 
			//device = getDeviceForStrategy(STRATEGY_SONIFICATION, false /*fromCache*/); //delete by lihaizhou for HQ01476013 to Recovery Goodgle code
			//user "safe" speaker if available instead of normal speaker to avoid triggering 
			//other acoustic safety mechanisms for notification 
			//if (device == AUDIO_DEVICE_OUT_SPEAKER && (availableOutputDeviceTypes & AUDIO_DEVICE_OUT_SPEAKER_SAFE)) //delete by lihaizhou for HQ01476013 to Recovery Goodgle code
				//device = AUDIO_DEVICE_OUT_SPEAKER_SAFE;//delete by lihaizhou for HQ01476013 to Recovery Goodgle code
			//add by HQ_zhouguo at 20150902 end
    ...
    case STRATEGY_MEDIA: {
        ...
        if ((device2 == AUDIO_DEVICE_NONE) &&
                (mForceUse[AUDIO_POLICY_FORCE_FOR_MEDIA] != AUDIO_POLICY_FORCE_NO_BT_A2DP) &&
                (getA2dpOutput() != 0) && !isInCall()
#ifdef MTK_AUDIO
                         && !bA2DPForeceIgnore
#endif
) {
			//start modify FmRadio for wiredheadset by HQ_zhouguo at 20150903
			if(!bPlayFmRadio) 
			//end modify FmRadio for wiredheadset by HQ_zhouguo at 20150903
                                           ...
                                           audio_devices_t device)
{
	...
	//add for HQ01456361 by lizhao at 20152319 begain
	if(stream == AUDIO_STREAM_NOTIFICATION) {
		on = false;
		checkAndSetVolume(stream, 15, output, device, delayMs);
	}
	//add for HQ01456361 by lizhao at 20152319 end
...
status_t AudioPolicyManager::SetPolicyManagerParameters(int par1,int par2 ,int par3,int par4)
{
    ...
    switch(par1){
		 ...
		 //start modify FmRadio for wiredheadset by HQ_zhouguo at 20150903
		case POLICY_SET_FM_RADIO_WIREDHEADSET: 
		{ 
			bPlayFmRadio = (par2?1:0); 
			ALOGD("ddd setPolicyManagerParameters POLICY_SET_FM_RADIO_WIREDHEADSET bPlayFmRadio= [%d]",bPlayFmRadio); 
			break; 
		} 
		//end modify FmRadio for wiredheadset by HQ_zhouguo at 20150903
