...
///M:
    ...
    private void updateSettings() {
        ...
        logd("mPoorNetworkDetectionEnabled="+mPoorNetworkDetectionEnabled);
...
//                            + txbad + " txgood=" + txgood);
                    ...
                    if (now - mCurrentBssid.mLastTimeSample < LINK_SAMPLING_INTERVAL_MS * 2) {
                        ...
                        if (DBG) logd("Fetch RSSI succeed, rssi=" + rssi + " mrssi=" + mrssi + " txbad="
                            + txbad + " txgood=" + txgood + " linkspeed=" + linkspeed + " dbad=" + dbad
                            + " dRetryCount=" + dRetryCount + " dMultipleRetryCount=" + dMultipleRetryCount);


                        if (dtotal > 0) {

                            if (mWeightAlgo == false) {
                                ...
                                double loss_withRetry = ((double) (dbad * 16 + dRetryCount)) /
                                    ((double) (dgood + dbad * 16 + dRetryCount));
                                int dtotalRetry = (dgood + dbad * 16 + (int) dRetryCount);


                                if (mLinkSpeed < 5 && rssi < -85) {  //when link speed is fast, it shoud have error torarrent
                                    //recalculate  packet loss rate by using only 1-layer retry count
                                    loss_withRetry = ((double) (dbad * 16 + dRetryCount)) / ((double) (dgood + dbad * 16 + dRetryCount));
                                    dtotalRetry = (dgood + dbad * 16 + (int) dRetryCount);
                                } else {
                                ...
                                logd("conclusion, LinkSpeed=" + mLinkSpeed + " Current loss=" + df.format(mCurrentLoss.mValue * 100) + "%" + " rssi=" + rssi + " txgood=" + dgood + " txbad=" + dbad + " dtotal=" + dtotal +
                            ...
                            if (mCurrentLoss.mValue > mPoorLinkThreshold
                                ...
                                if (++mSampleCount >= POOR_LINK_SAMPLE_COUNT) {
                                    ...
                                    if (dataenabled == true) {
                                        ...
                                        logd("dont' count cause mobile data not enable");
                                    }
                                }
    ...
    private void hideLatencyPanel() {
     ...
     private final Runnable mPoorLinkProfilingInfo = new Runnable() {
        ...
        public void run() {
            ...
            if (mCurrentBssid == null) {
                logd("mPoorLinkProfilingInfo mCurrentBssid=null return");
            } else {
                double clValue = 0;
                double clVolume = 0;
                if (mCurrentLoss != null) {
                    clValue =  mCurrentLoss.mValue;
                    clVolume = mCurrentLoss.mVolume;
                }
                      mTextView.setTextColor(0xffffffff);
                mTextView.setText(
                    "Link Status: " + mLinkStatus + "\n" +
                    "RSSI: " + mLatestRssi + "\n" +
                    "LinkSpeed: " + mLinkSpeed  + "\n" +
                    "GoodLinkTargetRssi: " + mCurrentBssid.mGoodLinkTargetRssi + "\n" +
                    "GoodLinkTargetCount: " + mCurrentBssid.mGoodLinkTargetCount  + "\n" +
                    "SampleCount: " + mSampleCount   + "\n" +
                    "CurrentLossValue:" +  df.format(clValue * 100) + "%" + "\n" +
                    "CurrentLossVolume:" +  df.format(clVolume)  + "\n" +
                    "\n" +
                    "p2pStart:" + p2pStart + "\n" +
                    "PoorLinkThreshold: " + mPoorLinkThreshold + "\n" +
                    "GoodLinkThreshold: " + mGoodLinkThreshold + "\n"
                );
            }
