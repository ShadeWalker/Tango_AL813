...
public class WifiMonitor {
   ...
   /* Network connection completed */
    ...
    ///M: add
     ...
     public static final int HS_R1_CONNECTED_EVENT                  = BASE + 89;
     /** @} */
    ...
    /** @} */
    ///M: passpoint R1
    //passpoint string e.g.  IFNAME=wlan0 HS20R1-CONNECTED c4:01:7c:15:c3:d8
    private static final String PASSPOINT_CONNECTED_STRING = "HS20R1-CONNECTED";
    private static final int PASSPOINT_CONNECTED_STRING_LEN = PASSPOINT_CONNECTED_STRING.length();
	...
	public void quitMonitoring() {
		WifiMonitorSingleton.sInstance.quitMonitoring();
    ...
    private static class WifiMonitorSingleton {
		...
		private boolean mQuitMonitorFlag = false;
        ...
        public synchronized void killSupplicant(boolean p2pSupported) {
            ...
            ///ALPS02040057, quit wifimonitor thread 
            quitMonitoring();
        }
		public synchronized void quitMonitoring() {	
			Xlog.d(TAG, "quit Monitoring!");
			mQuitMonitorFlag = true;
		}
        ...
        public void run() {
            ...
            for (;;) {
                ...
                if (mWifiMonitorSingleton.dispatchEvent(eventStr)
					||mWifiMonitorSingleton.mQuitMonitorFlag) {
                    if (DBG) Log.d(TAG, "Disconnecting from the supplicant, no more events");
					if(mWifiMonitorSingleton.mQuitMonitorFlag) Log.d(TAG,"Disconnecting from the supplicant, by mQuitMonitorFlag");
                    break;
                }
            }
        }
    ...
    private boolean dispatchEvent(String eventStr, String iface) {
        ...
        if (!eventStr.startsWith(EVENT_PREFIX_STR)) {
            ...
            /** } */
            ///M: passpoint R1
            } else if (eventStr.startsWith(PASSPOINT_CONNECTED_STRING)) {
                if (mMtkPasspointR1Support) {
                     Log.i(TAG, "PASSPOINT_CONNECTED_STRING sub=" +
                        eventStr.substring(PASSPOINT_CONNECTED_STRING_LEN + 1));
                     mStateMachine.sendMessage(HS_R1_CONNECTED_EVENT,
                        eventStr.substring(PASSPOINT_CONNECTED_STRING_LEN + 1));
                }
            }
            ///@}
