...
public class WifiStateMachine extends StateMachine {
   ...
   /* Set the frequency band */
    ...
    ///M: for passpoint R1 @{
    private static final int M_CMD_ENABLE_HS                 = BASE + 223;
    private static final int M_CMD_ADD_CREDENTIAL            = BASE + 224;
    private static final int M_CMD_SET_CREDENTIAL            = BASE + 225;
    private static final int M_CMD_SET_HS_NETWORK            = BASE + 226;
    private static final int M_CMD_DEL_HS_NETWORK            = BASE + 227;
    private static final int M_CMD_GET_CREDENTIAL            = BASE + 228;
    private static final int M_CMD_DEL_CREDENTIAL            = BASE + 229;
    private static final int M_CMD_GET_HS_STATUS             = BASE + 230;
    private static final int M_CMD_GET_HS_NETWORK            = BASE + 231;
    private static final int M_CMD_SET_HS_PREFERED_NETWORK  = BASE + 232;
    //@}
   ...
   private boolean mIsSupplicantFirstScan = false;
    ...
    //M: for passpoint R1
    private String mPasspointConnectedBssid = null;
    private boolean mIsPasspointConnection;
  ...
  //      Xlog.d(TAG, "setScanResults, scanResults:" + scanResults);
    ...
    boolean shouldSwitchNetwork(int networkDelta) {
        ...
        if (mMtkPasspointR1Support && mIsPasspointConnection) {
            Xlog.d(TAG, "passpoint connection Don't");
            return false;
        }
    ...
    class DefaultState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case M_CMD_ADD_CREDENTIAL:
                case M_CMD_SET_CREDENTIAL:
                case M_CMD_SET_HS_NETWORK:
                case M_CMD_DEL_HS_NETWORK:
                case M_CMD_GET_CREDENTIAL:
                case M_CMD_DEL_CREDENTIAL:
                case M_CMD_GET_HS_STATUS:
                case M_CMD_GET_HS_NETWORK:
                case M_CMD_SET_HS_PREFERED_NETWORK:
                    break;
                ///@}
                default:
    ...
    class SupplicantStartedState extends State {
        ...
        public void enter() {
            ...
            if (mMtkPasspointR1Support) {
                log("SupplicantStartedState, enter, mMtkPasspointR1Support = " +
                    mMtkPasspointR1Support);
                doHsWhenWifiOn();
            }
            ///@}
            ///M: if have special network selection and wifi is connected, don't try to associate.
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case M_CMD_ENABLE_HS:
                    ...
                    break;
                case M_CMD_ADD_CREDENTIAL:
                    ArrayList<String> credential = (ArrayList<String>) message.obj;

                    int addCredResult =
                            mWifiNative.addHsCredentialCommand(credential.get(0),
                            credential.get(1), credential.get(2), credential.get(3),
                            credential.get(4), credential.get(5), credential.get(6),
                            credential.get(7), credential.get(8), credential.get(9),
                            credential.get(10), credential.get(11), credential.get(12));

                    replyToMessage(message, message.what, addCredResult);
                    break;
                case M_CMD_SET_CREDENTIAL:
                    String credEntity = (String) message.obj;
                    String[] creds = credEntity.split(",");
                    int setCredIndex = -1;
                    try {
                        setCredIndex = Integer.parseInt(creds[0]);
                    } catch (NumberFormatException e) {
                        loge("Failed to read " + creds[0] + "'");
                        replyToMessage(message, message.what, false);
                        break;
                    }
                    boolean setCredentialResult =
                            mWifiNative.setHsCredentialCommand(setCredIndex, creds[1], creds[2]);
                    replyToMessage(message, message.what, setCredentialResult);
                    break;
                case M_CMD_SET_HS_NETWORK:
                    String setHsNetwork = (String) message.obj;
                    String[] networkParam = setHsNetwork.split(",");
                    int setHsNetworkIndex = -1;
                    try {
                        setHsNetworkIndex = Integer.parseInt(networkParam[0]);
                    } catch (NumberFormatException e) {
                        loge("Failed to read " + networkParam[0] + "'");
                        replyToMessage(message, message.what, false);
                        break;
                    }
                    boolean setHsNetworkResult =
                            mWifiNative.setHsNetworkCommand(setHsNetworkIndex,
                            networkParam[1], networkParam[2]);
                    replyToMessage(message, message.what, setHsNetworkResult);
                    break;
                case M_CMD_DEL_HS_NETWORK:
                    int delHsNetworkIndex = message.arg1;
                    boolean delHsNetworkResult =
                            mWifiNative.delHsNetworkCommand(delHsNetworkIndex);
                    replyToMessage(message, message.what, delHsNetworkResult);
                    break;
                case M_CMD_GET_CREDENTIAL:
                    String credentials = mWifiNative.getHsCredentialCommand();
                    replyToMessage(message, message.what, credentials);
                    break;
                case M_CMD_DEL_CREDENTIAL:
                    int delCredIndex = message.arg1;
                    boolean delCredentialResult = mWifiNative.delHsCredentialCommand(delCredIndex);
                    replyToMessage(message, message.what, delCredentialResult);
                    break;
                case M_CMD_GET_HS_STATUS:
                    String hsStatus = mWifiNative.getHsStatusCommand();
                    replyToMessage(message, message.what, hsStatus);
                    break;
                case M_CMD_GET_HS_NETWORK:
                    String getHsNetwork = mWifiNative.getHsNetworkCommand();
                    replyToMessage(message, message.what, getHsNetwork);
                    break;
                case M_CMD_SET_HS_PREFERED_NETWORK:
                    boolean hsPreferedresult =
                            mWifiNative.setHsPreferredNetworkCommand(message.arg1, message.arg2);
                    replyToMessage(message, message.what, hsPreferedresult);
                    break;
                ///@}
                default:
                    return NOT_HANDLED;
            ...
            switch (message.what) {
                ...
                case WifiMonitor.NETWORK_CONNECTION_EVENT:
                    ...
                    if (mMtkPasspointR1Support) {
                        ...
                        //Important: When Sigma is testing, because sigma will trigger dhcp
                        //client to get IP, it conflicts with obtainingIPStae,
                        //so we avoid it enter .
                        //the obtainingIpState, just jump to the connectedState
                        if (isHsSigmaTesting()) {
                            log("ConnectModeState, isHsSigmaTesting() == 1");
                            //Help sigma test to enabel IPV6 here
                            if (mMtkDhcpv6cWifi) {
                                log("ConnectModeState, Supplicant state is " +
                                    mWifiInfo.getSupplicantState() +
                                    " before enable IPV6 (this is for sigma test usage)");
                                try {
                            ...
                            //Break it, don't do below original "enter ObtainingIpState"
    ...
    class ConnectedState extends State {
        ...
        public void enter() {
            ...
            if (mMtkPasspointR1Support) {
                if (mPasspointConnectedBssid != null &&
                    mPasspointConnectedBssid.equals(mLastBssid)) {
                    //it is passpoint connection, stop auto join
                    mIsPasspointConnection = true;
                } else {
                    ///reset
                    mIsPasspointConnection = false;
                    mPasspointConnectedBssid = null;
                }
            }
        }
        @Override
        ...
        public boolean processMessage(Message message) {
			...
			///M: ALPS01872204 After disabling wifi should go to WaitP2pDisable first @{
                ...
                case WifiP2pServiceImpl.DISCONNECT_WIFI_REQUEST:
                    ...
                    break;
                ///M: passpoint R1 @{
                case WifiMonitor.HS_R1_CONNECTED_EVENT:
                    if (mMtkPasspointR1Support) {
                        mPasspointConnectedBssid = (String) message.obj;
                    }
                    break;
                ///@}
                default:
   ...
   /**
       ...
       * @return 1  if it's success,0 otherwise
        * @hide
        */
     public int syncAddHsCredential(AsyncChannel channel, String type, String username,
         ...
         log("syncAddHsCredential");

         List<String> credential = new ArrayList<String>();
         credential.add(type);
         credential.add(username);
         credential.add(passwd);
         credential.add(imsi);
         credential.add(root_ca);
         credential.add(realm);
         credential.add(fqdn);
         credential.add(client_ca);
         credential.add(milenage);
         credential.add(simslot);
         credential.add(priority);
         credential.add(roamingconsortium);
         credential.add(mcc_mnc);
         Message resultMsg =
                 channel.sendMessageSynchronously(M_CMD_ADD_CREDENTIAL, credential);
         int result = resultMsg.arg1;
         resultMsg.recycle();
         log("syncAddHsCredential, result = " + result);
    ...
    /**
        ...
        * @hide
        */
     public boolean syncSetHsCredential(AsyncChannel channel, int index,
             String name, String value) {
         log("syncSetHsCredential, index = " + index + " name = " + name + " value = " + value);

         String credEntity = index + "," + name + "," + value;
         Message resultMsg = channel.sendMessageSynchronously(M_CMD_SET_CREDENTIAL, credEntity);
         boolean result = (Boolean) resultMsg.obj;
         resultMsg.recycle();
         log("syncSetHsCredential, result = " + result);
    ...
    /**
        ...
        * @hide
        */
     public String syncGetHsCredential(AsyncChannel channel) {
         log("syncGetHsCredential");

         Message resultMsg = channel.sendMessageSynchronously(M_CMD_GET_CREDENTIAL);
         String result = (String) resultMsg.obj;
         resultMsg.recycle();
         log("syncGetHsCredential, result = " + result);
    ...
    /**
        ...
        * @hide
        */
     public boolean syncDelHsCredential(AsyncChannel channel, int index) {
         log("syncDelHsCredential, index = " + index);

         Message resultMsg = channel.sendMessageSynchronously(M_CMD_DEL_CREDENTIAL, index);
         boolean result = (Boolean) resultMsg.obj;
         resultMsg.recycle();
         log("syncDelHsCredential, result = " + result);
   ...
   /**
        ...
        * @hide
        */
     public String syncGetHsStatus(AsyncChannel channel) {
         log("syncGetHsStatus");

         Message resultMsg = channel.sendMessageSynchronously(M_CMD_GET_HS_STATUS);
         String result = (String) resultMsg.obj;
         resultMsg.recycle();
         log("syncGetHsStatus, result = " + result);
    ...
    /**
        ...
        * @hide
        */
     public String syncGetHsNetwork(AsyncChannel channel) {
         log("syncGetHsNetwork");

         Message resultMsg = channel.sendMessageSynchronously(M_CMD_GET_HS_NETWORK);
         String result = (String) resultMsg.obj;
         resultMsg.recycle();
         log("syncGetHsNetwork, result = " + result);
    ...
    /**
        ...
        * @hide
        */
     public boolean syncSetHsNetwork(AsyncChannel channel, int index,
             String name, String value) {
         log("syncSetHsNetwork, index = " + index + " name = " + name + " value = " + value);

         String hsNetwork = index + "," + name + "," + value;
         Message resultMsg = channel.sendMessageSynchronously(M_CMD_SET_HS_NETWORK, hsNetwork);
         boolean result = (Boolean) resultMsg.obj;
         resultMsg.recycle();
         log("syncSetHsNetwork, result = " + result);
    ...
    /**
        ...
        * @hide
        */
     public boolean syncDelHsNetwork(AsyncChannel channel, int index) {
         log("syncDelHsNetwork, index = " + index);

         Message resultMsg = channel.sendMessageSynchronously(M_CMD_DEL_HS_NETWORK, index);
         boolean result = (Boolean) resultMsg.obj;
         resultMsg.recycle();
         log("syncDelHsNetwork, result = " + result);
     ...
     public boolean doHsWhenWifiOn() {
         ...
         //Check if passpoint settings is enabled, if yes,
         //send command to supplicant to enable passpoint
         if (mMtkPasspointR1Support) {
             try {
                 if (Settings.Global.getInt(mContext.getContentResolver(),
                    Settings.Global.WIFI_PASSPOINT_ON) == 1) {
                     isSettingOK = true;
                 }
             } catch (Settings.SettingNotFoundException e) {
             ...
             if (isSettingOK) {
                 ...
                 // TODO: Conflict with Passpoint R2 switch in PasspointStateMachine
                 //enableHS(true);
                 log("enableHS, enabled = true");
                 sendMessage(M_CMD_ENABLE_HS, 1);
                 return true;
             }
    ...
    /**
        ...
        * @hide
        */
     public boolean syncSetHsPreferredNetwork(AsyncChannel channel, int index,
             int value) {
         log("syncSetHsPreferredNetwork, index = " + index + " value = " + value);

         Message resultMsg = channel.sendMessageSynchronously(
            M_CMD_SET_HS_PREFERED_NETWORK, index, value);
         boolean result = (Boolean) resultMsg.obj;
         resultMsg.recycle();
         log("syncSetHsPreferredNetwork, result = " + result);
