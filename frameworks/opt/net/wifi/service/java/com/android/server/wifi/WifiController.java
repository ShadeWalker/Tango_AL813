...
class WifiController extends StateMachine {
    ...
    ///M: ePDG
    private boolean mShouldIgnoreCmdSetAp = false;
    private boolean mCmdSetApDefered = false;
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case CMD_AIRPLANE_TOGGLED:
                    ...
                    && mSettingsStore.isScanAlwaysAvailable() && mSettingsStore.isAirplaneModeOn() == false) {
                ...
                case CMD_SET_AP:
                    if (msg.arg1 == 1) {
                        ///M: ePDG @{
                        if (mShouldIgnoreCmdSetAp) {
                            SXlog.d(TAG, "ignore CMD_SET_AP: " + CMD_SET_AP);
                            mShouldIgnoreCmdSetAp = false;
                        } else {
                        ///@}
    ...
    class StaEnabledState extends State {
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                case CMD_WIFI_TOGGLED:
                    ///M: ePDG @{
                    if (! mSettingsStore.isWifiToggleEnabled() && mCmdSetApDefered) {
                        int uid = msg.arg2;
                        if (uid != EPDG_UID) {
                            SXlog.d(TAG, "not ePDG wifi off, should ignore CMD_SET_AP");
                            mShouldIgnoreCmdSetAp = true;
                        } else {
                            SXlog.d(TAG, "ePDG wifi off, no need to ignore CMD_SET_AP");
                            mShouldIgnoreCmdSetAp = false;
                        }
                        mCmdSetApDefered = false;
                    }
                    ///@}
                    ///M: add WifiIpoOff@{
                    boolean wifiIpoOff = (msg.arg1 == 1) ? true : false;
                ...
                case CMD_EMERGENCY_MODE_CHANGED:
                    ...
                    return NOT_HANDLED;
                ///M: When enable ap, wifi is off delayed by ePDG @{
                case CMD_SET_AP:
                    if (msg.arg1 == 1) {
                        log("deferMsg CMD_SET_AP");
                        SXlog.d(TAG, "deferMsg CMD_SET_AP");
                        mCmdSetApDefered = true;
                        deferMessage(msg);
                    }
                    break;
                ///@}
