...
///M: for gbk ssid
static jbyteArray doByteArrayCommand(JNIEnv* env, jstring javaCommand) {
    char reply[REPLY_BUF_SIZE]= {0};
    if (!doCommand(env, javaCommand, reply, sizeof(reply))) {
        return NULL;
    }
    ...
    env->SetByteArrayRegion(ret, 0, byteSize, (jbyte*)reply);
    return ret;
}
...
///M: for gbk ssid
static jbyteArray android_net_wifi_waitForEventInByteArray(JNIEnv* env, jobject, jstring jIface)
{
    char buf[EVENT_BUF_SIZE]= {0};
    int nread = ::wifi_wait_for_event(buf, sizeof buf);
    if (nread > 0) {
        ...
        int byteSize = nread;
        jbyteArray ret = env->NewByteArray(byteSize);
        env->SetByteArrayRegion(ret, 0, byteSize, (jbyte*)buf);
...
///M: for gbk ssid
static jbyteArray android_net_wifi_doByteArrayCommand(JNIEnv* env, jobject, jstring javaCommand) {
    return doByteArrayCommand(env,javaCommand);
}
...
static JNINativeMethod gWifiMethods[] = {
    ...
    { "waitForEventInByteArrayNative", "(Ljava/lang/String;)[B", (void*)android_net_wifi_waitForEventInByteArray },
    { "doByteArrayCommandNative", "(Ljava/lang/String;)[B",
            (void*) android_net_wifi_doByteArrayCommand }
