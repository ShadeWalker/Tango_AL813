...
bool AudioGroup::DeviceThread::threadLoop()
{
    ...
    char value[PROPERTY_VALUE_MAX];
    int ret;
    property_get("audiogroup.pcm.dump", value, "0");
    int pcm_dump = atoi(value);
    FILE *mPCMDumpFileUL = NULL;
    FILE *mPCMDumpFileDL = NULL;
    ...
    if (pcm_dump)
    {
        char BufUL[10], BufDL[10];
        String8 mDumpFileNameUL, mDumpFileNameDL;
        snprintf(BufUL, sizeof(BufUL), "%d.pcm", mDumpFileNum);

        mDumpFileNameUL = String8("/sdcard/SIPUplink_Dump.pcm");
        mDumpFileNameUL.append((const char *)BufUL);

        mPCMDumpFileUL = NULL;
    
        mPCMDumpFileUL = fopen(mDumpFileNameUL, "wb");

        if (mPCMDumpFileUL != NULL)
        {
            ALOGD("%s DumpFileName UL = %s", __FUNCTION__, mDumpFileNameUL.string());
        }

        snprintf(BufDL, sizeof(BufDL), "%d.pcm", mDumpFileNum);
        mDumpFileNameDL = String8("/sdcard/SIPDownlink_Dump.pcm");
        mDumpFileNameDL.append((const char *)BufDL);

        mPCMDumpFileDL = NULL;
    
        mPCMDumpFileDL = fopen(mDumpFileNameDL, "wb");

        if (mPCMDumpFileDL != NULL)
        {
            ALOGD("%s DumpFileName DL = %s", __FUNCTION__, mDumpFileNameDL.string());
        }
        ...
        while (--chances > 0 && (toWrite > 0 || toRead > 0)) {
            if (toWrite > 0) {
                ...
                if (status == NO_ERROR) {
                    ...
                    if (pcm_dump)
                    {
                        fwrite((void *)buffer.i8, sizeof(char), buffer.size, mPCMDumpFileDL);
                    }
            ...
            if (toRead > 0) {
                ...
                if (status == NO_ERROR) {
                    ...
                    if (pcm_dump)
                    {
                        fwrite((void *)buffer.i8, sizeof(char), buffer.size, mPCMDumpFileUL);
                    }
...
exit:
    ...
    if (mPCMDumpFileUL)
    {
        fclose(mPCMDumpFileUL);
        mPCMDumpFileUL = NULL;
    }
    
    if (mPCMDumpFileDL)
    {
        fclose(mPCMDumpFileDL);
        mPCMDumpFileDL = NULL;
    }
    return true;
}
