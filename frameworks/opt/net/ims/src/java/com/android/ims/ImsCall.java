...
public class ImsCall implements ICall {
    ...
    private static final boolean FORCE_DEBUG = true; /// M: for debug. false; /* STOPSHIP if true */ ///@}
    ...
    public static class Listener {
        ...
        public void onCallStarted(ImsCall call) {
         ...
         ///M: WFC @{
         /**
         * Called when the call type is changed.
         *
         * @param call the call object that carries out the IMS call and rat type
         */
        public void onCallTypeChanged(ImsCall call, int rat) {
    ...
    public void close() {
        synchronized(mLockObj) {
            if (mSession != null) {
                /// M: ALPS02033202. @{
                // reset listener to prevent leak.
    ...
    private void processCallTerminated(ImsReasonInfo reasonInfo) {
        ...
        synchronized(ImsCall.this) {
            ...
            // point.
            /// M: ALPS02033202. Workaround for modem. @{
            // When 1A (id:2) + 1H (id:1) merge, MTK modem behavior is different with Android.
            // MTK modem: merge result -> ECPI 2, 133 -> ECPI 1, 133.
            // Then ECPI 2, 133 will cause the active connection (host) to be destroyed.
            if (isCallSessionMergePending() || mTransientConferenceSession != null) {
                /// @}
    ...
    private void processMergeComplete() {
        ...
        synchronized(ImsCall.this) {
            ...
            if (isMultiparty()) {
                ...
                /// M: ALPS02046774. @{
                // we always consider it is case-3.
                /*
        ...
        synchronized(ImsCall.this) {
            ...
            if (!isMultiparty()) {
                if (mTransientConferenceSession == null) {
                    ...
                    /// M: ALPS02064616. @{
                    /// Merge failed might happened before callSessionMergeStarted.
                    //return;
                } else {
                    /// @}
    ...
    private class ImsCallSessionListenerProxy extends ImsCallSession.Listener {
        ...
        public void callSessionHandover(ImsCallSession session,
                ...
                ImsReasonInfo reasonInfo)  {
            log("callTypeChanged :: source =" + srcAccessTech + ",  new rat=" + targetAccessTech);
            ...
            if (listener != null) {
                try {
                    listener.onCallTypeChanged(ImsCall.this, targetAccessTech);
                } catch (Throwable t) {
                    loge("callTypeChanged :: ", t);
                }
            }
        }
        /// @}
        ...
        public void callSessionTerminated(ImsCallSession session, ImsReasonInfo reasonInfo) {
            ...
            /// M: ALPS02064606. @{
            /// Conference host session might be disconnected during merging.
            if (isCallSessionMergePending() && isMultiparty()) {
                log("this is a conference host during merging, and is disconnected..");
                processMergeFailed(reasonInfo);
            }
            /// @}
        ...
        public void callSessionHeld(ImsCallSession session, ImsCallProfile profile) {
            ...
            synchronized(ImsCall.this) {
                ...
                /// M: ALPS02041739. @{
                // This function might be called during merge(). So only reset for hold() case.
        ...
        public void callSessionResumed(ImsCallSession session, ImsCallProfile profile) {
            ...
            synchronized(ImsCall.this) {
                ...
                mCallProfile = profile;
                /// M: ALPS02041739. @{
                // This function might be called during merge(). So only reset for resume() case.
                if (mUpdateRequest == UPDATE_RESUME) {
                    mUpdateRequest = UPDATE_NONE;
                }
                /// @}
                mHold = false;
        ...
        public void callSessionUpdated(ImsCallSession session, ImsCallProfile profile) {
            ...
            synchronized(ImsCall.this) {
                ...
                mCallProfile = profile;
                /// M: ALPS02041739. @{
                // Only reset for update() case.
                if (mUpdateRequest == UPDATE_UNSPECIFIED) {
                    mUpdateRequest = UPDATE_NONE;
                }
                /// @}
            }
