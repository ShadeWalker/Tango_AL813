...
public abstract class InboundSmsHandler extends StateMachine {
    ...
    static final String HANDLE_KEY_AIDL_SMSINTENT = "HANDLE_SMS_INTENT";
    static final String HARASSMENT_INTERCEPTION_SERVICE = "com.huawei.harassmentinterception.service.HarassmentInterceptionService";
    /*  DTS2013120203869   lixue/00214442 20131202 end > */
	...
	/*HQ_sunli HQ01445367 20151015 begin*/
	int subId = mPhone.getSubId();
        if (SubscriptionManager.isValidSubscriptionId(subId)){
	    intent.putExtra("subscription", subId);
            }
	/*HQ_sunli HQ01445367 20151015 end*/
        ...
        if (destPort == -1) {
			...
			/*< DTS2013120203869   lixue/00214442 20131202 begin */
			//[HSM]
			//Send the sms to HwSystemManager to determine if the sms should be intercepted first
			try {
					IBinder harassmentIntercepterService = ServiceManager.getService(HARASSMENT_INTERCEPTION_SERVICE);
					IHarassmentInterceptionService hisStub = IHarassmentInterceptionService.Stub.asInterface(harassmentIntercepterService);
					Bundle smsInfo = new Bundle();
					smsInfo.putParcelable(HANDLE_KEY_AIDL_SMSINTENT, intent);
					/* < DTS2013121207896 lixue/00214442 20131212 begin */
					//if the return value from HwSystemManager is "Block" ,the sms is record as a spam message ,
					//Deliver action is not needed even to the default sms application
					if (SMS_HANDLE_RESULT_BLOCK == hisStub.handleSmsDeliverAction(smsInfo)) {
						if (DBG){
							log("Sms is intercepted by harassment interception service");
							}
						//delete the sms from raw table to avoid re-delivering	after rebooting
						String deleteWhere = resultReceiver.mDeleteWhere;
						String [] deleteWhereArgs = resultReceiver.mDeleteWhereArgs;
						deleteFromRawTable(deleteWhere, deleteWhereArgs);
						//change state mathine to the right state
						sendMessage(EVENT_BROADCAST_COMPLETE);
						return ;
					}
				} catch (Exception e) {
					// TODO: handle exception
					loge("Get exception while communicate with harassment interception service: " + e.getMessage());
				}
				/* DTS2013121207896 lixue/00214442 20131212 end > */
			 /*  DTS2013120203869	lixue/00214442 20131202 end > */
