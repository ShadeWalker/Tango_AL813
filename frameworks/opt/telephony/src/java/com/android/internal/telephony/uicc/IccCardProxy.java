...
public class IccCardProxy extends Handler implements IccCard {
    ...
    private UiccCardApplication mSvlteUiccApp = null;
    ///M: SVLTE SIM PIN/SIM-ME LOCK 2015/01/20 @}
...
*/
        ...
        synchronized (mLock) {
            if (mUiccApplication != null) {
                ...
                //Exception e = new RuntimeException("ICC card is absent.");
					Exception e;
                if (hasIccCard()) {
                    e = CommandException.fromRilErrno(RILConstants.RADIO_NOT_AVAILABLE);
                } else {
                    e = new RuntimeException("ICC card is absent.");
                }
    ...
    public boolean hasIccCard() {
        synchronized (mLock) {
			...
			///HQ_xionghaifeng 20151101 add for HQ01463874 start @{
			if (mPhoneId == 0)
			{
				SystemProperties.set("gsm.slot1.insert", String.valueOf(isSimInsert));
			}
			else if (mPhoneId == 1)
			{
				SystemProperties.set("gsm.slot2.insert", String.valueOf(isSimInsert));
			}
			///@}
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        pw.println(" mSvlteUiccApp=" + mSvlteUiccApp);
    ...
    ///M: SVLTE SIM PIN/SIM-ME LOCK 2015/01/20 @{
    /**
     * Get Normal/SVLTE UiccCardApplication.
     *
     * @hide
     */
    private UiccCardApplication getUiccCardApplication() {
        return CdmaFeatureOptionUtils.isCdmaLteDcSupport()
                ? mSvlteUiccApp : mUiccApplication;
    }
    ///M: SVLTE SIM PIN/SIM-ME LOCK 2015/01/20 @}
