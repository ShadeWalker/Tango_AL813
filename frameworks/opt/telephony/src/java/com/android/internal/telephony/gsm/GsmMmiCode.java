...
*/
    ...
    newFromDialString(String dialString, GSMPhone phone, UiccCardApplication app) {
        ...
        /*HQ_yulifeng add for voicemail long click "1",HQ01383014,20151019,b */
        if (Build.TYPE.equals("eng")) {
            Log.d(LOG_TAG,"ylf[newFromDialString]_dialString:"+dialString);
        }
        String pMccMnc = PhoneNumberUtils.getSimMccMnc(0);
        String pMccMnc2 = PhoneNumberUtils.getSimMccMnc(1);
        Log.d(LOG_TAG,"ylf[newFromDialString]_pMccMnc:"+pMccMnc +";pMccMnc2:"+pMccMnc2);
        if(dialString!=null && dialString.equals("*9") && ("73406".equals(pMccMnc)||"73406".equals(pMccMnc2))){
            return null ;
        }
        if(dialString!=null && dialString.equals("*2") && ("73404".equals(pMccMnc)||"73404".equals(pMccMnc2))){
            return null ;
        }
	 //add by huangshuo for HQ02064028 at 2016/12/26
	if(dialString!=null && dialString.equals("*57*128#") && ("61906".equals(pMccMnc)||"61906".equals(pMccMnc2))){
            return null ;
        }
	 //end by huangshuo for HQ02064028 at 2016/12/26
	 
        /*HQ_yulifeng add for voicemail long click "1",HQ01383014,20151019,e */
		
     //add by huangshuo for HQ01311122 at 2015/10/30
      if(SystemProperties.get("ro.hq.phone.latin.ussd").equals("1")){
        if(pMccMnc!=null && pMccMnc2!=null &&  (pMccMnc.equals("73406") ||pMccMnc2.equals("73406") )){
		if(dialString!=null &&(dialString.equals("*9") ||dialString.equals("*5") )){
			return null;
		}
         }
	  if(pMccMnc!=null && pMccMnc2!=null &&  (pMccMnc.equals("73404") ||pMccMnc2.equals("73404") )){
	      if(dialString!=null ){
	  	Pattern keywordPattern1 = Pattern.compile("^\\*[0-9]{1,3}$");
                Pattern keywordPattern2 = Pattern.compile("^\\*[0-9]{1,2}\\#$");
                Pattern keywordPattern3 = Pattern.compile("^[0-9]{1,3}$");
		 Matcher matcher1 = keywordPattern1.matcher(dialString); 
                 boolean macherRes1=matcher1.matches();
                 Matcher matcher2 = keywordPattern2.matcher(dialString); 
                 boolean macherRes2=matcher2.matches();
                 Matcher matcher3 = keywordPattern3.matcher(dialString); 
                 boolean macherRes3=matcher3.matches();
		Log.d(LOG_TAG,"huangshuo[newFromDialString]_macherRes1:"+macherRes1+",macherRes2"+macherRes2+",macherRes3"+macherRes3);
		if( macherRes1==true  || macherRes2==true || macherRes3==true){
			return null;
		}
            }
	  }
	//if(dialString!=null &&(dialString.equals("*9") ||dialString.equals("*5") )){
	 if(dialString!=null && (dialString.equals("#9"))){ //modify by yulifeng for HQ01523060 20151121
	     if(pMccMnc!=null && pMccMnc2!=null ){
		       for(int i=0; i<specialUssdMncmcc.length;i++){
			 if(pMccMnc.equals(specialUssdMncmcc[i]) || pMccMnc2.equals(specialUssdMncmcc[i])){
			return null;
		       }
                    }  

			   // / Added by guofeiyao 2015/12/11
			   // For AT&T need
			   if ( SystemProperties.get("ro.hq.att.hash.nine").equals("1") ) {
                  return null;
			   }
			   // / End
            }
	}
	  if(pMccMnc!=null && pMccMnc2!=null &&  (pMccMnc.equals("70401") ||pMccMnc2.equals("70401") )){
		if(dialString!=null &&(dialString.equals("*9") ||dialString.equals("*5") ||dialString.equals("*1"))){
			return null;
		}
         }
	    if(pMccMnc!=null && pMccMnc2!=null &&  (pMccMnc.equals("708001") ||pMccMnc2.equals("708001") )){
		if(dialString!=null &&(dialString.equals("*5") ||dialString.equals("*1"))){
			return null;
		}
           }
     	}
       //end  by huangshuo for HQ01311122 at 2015/10/30

        // / Added by guofeiyao 2015/12/15
		// For movistar to not dial these number as USSD
		if ( SystemProperties.get("ro.hq.reverse.ussd").equals("1") ) {
             Pattern notUSSDPattern1 = Pattern.compile("^[0-9]$"); // 0-9
             Pattern notUSSDPattern2 = Pattern.compile("^[0-9]{2}$"); // 00-99
			 Pattern notUSSDPattern3 = Pattern.compile("^\\*[0-9]\\#$"); // *0#-*9#
			 Pattern notUSSDPattern4 = Pattern.compile("^\\*[0-9]{2}\\#$"); // *00#-*99#
             Log.e("duanze","dialString:"+dialString);
             if ( notUSSDPattern1.matcher(dialString).matches()
			 	  || notUSSDPattern2.matcher(dialString).matches()
			 	  || notUSSDPattern3.matcher(dialString).matches()
			 	  || notUSSDPattern4.matcher(dialString).matches()) {
    			  return null;
             }
		}
        // / End
      
		// [add by lizhao for HQ01527098 at&t ussd begain
		if (SystemProperties.get("ro.hq.latin.atandt.ussd").equals("1")) {
			if (dialString != null
					&& ("#9".equals(dialString) || "*228".equals(dialString))) {
				if (pMccMnc != null
						&& pMccMnc2 != null
						&& (pMccMnc.equals("334050")
								|| pMccMnc2.equals("334050")
								|| pMccMnc.equals("334090")
								|| pMccMnc2.equals("334090")
								|| pMccMnc.equals("33450") || pMccMnc2
									.equals("33450"))) {
					return null;
				}
			}
		}
		// add by lizhao for HQ01527098 at&t ussd end]
    ...
    private static String[] specialUssdMncmcc ={"334050","22201","222010","222011","222012",
                                                                                     "222013","222014","222015","222016","222017","222018","222019"};
...
*/
    ...
    processCode () {
        try {
	   String simMccMnc = mPhone.getOperatorNumeric();
	    if(SystemProperties.get("ro.hq.phone.movistar.ussd").equals("1")){
	   	if(mPoundString!=null){
	   	       Pattern keywordPattern = Pattern.compile("^\\*[0-9]{3}\\#$");
                       Matcher matcher = keywordPattern.matcher(mPoundString); 
                       boolean macherRes3=matcher.matches();
	               if (macherRes3==true) {
      ...
      //add by huangshuo for HQ01311122 at 2015/10/30
	   if(SystemProperties.get("ro.hq.phone.latin.ussd").equals("1")){
	   	if(mPoundString!=null){
		   if(simMccMnc!=null  && simMccMnc.equals("73404")){
	   	       Pattern keywordPattern = Pattern.compile("^\\*[0-9]{3}\\#$");
                       Matcher matcher = keywordPattern.matcher(mPoundString); 
                       boolean macherRes3=matcher.matches();
	               if (macherRes3==true) {
	   ...
	   // / Added by guofeiyao 2016/1/25
	   // For Russia customized USSD
       if ( SystemProperties.get("ro.hq.russia.spussd").equals("1") ) {
            if (null!=mPoundString) {
//				if ( true ) {
				if ( simMccMnc!=null  && simMccMnc.startsWith("250")){
                    Pattern ptn = Pattern.compile("^0[1-4]$"); // 01-04
                    if ( ptn.matcher(mPoundString).matches() ) {
                         sendUssd(mPoundString);
                         return;
					}
				}
	   ...
	   // / End
          ...
          /*HQ_yulifeng add for ussd "*333#",HQ01266357,20150925,b*/
            if(SystemProperties.get("ro.hq.phone.ussd.mena").equals("1")){
                boolean isSpecialSim = false;
                boolean isSpecialUssd = false;
                if(simMccMnc != null){
                    if(simMccMnc.equals("60203"))
                        isSpecialSim = true;
                }
                Rlog.d(LOG_TAG, "isSpecialSim"+isSpecialSim);
                if(!(isEmptyOrNull(mSc)) && "*".equals(mAction)){
                    if(mSc.equals("333"))
                        isSpecialUssd = true;
                }
                Rlog.d(LOG_TAG, "isSpecialUssd"+isSpecialUssd);
                if(isSpecialSim && isSpecialUssd){
                    Rlog.d(LOG_TAG, "SpecialUssd"+mPoundString);
                    if (mPoundString != null) {
                        sendUssd(mPoundString);
                        return;
                    } 
            ...
            /*HQ_yulifeng add for ussd "*333#",HQ01266357,20150925,e*/
            if (isShortCode()) {                     
