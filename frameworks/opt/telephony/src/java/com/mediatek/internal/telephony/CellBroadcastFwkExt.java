...
public class CellBroadcastFwkExt {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            Rlog.d(TAG, "receive message " + idToString(msg.what));
            AsyncResult ar;
            EtwsNotification noti;
            switch(msg.what) {
                case EVENT_QUERY_CB_CONFIG:
                    Rlog.d(TAG, "handle EVENT_QUERY_CB_CONFIG");
                    ar = (AsyncResult) msg.obj;
                    if (ar.exception != null) {
                        Rlog.d(TAG, "fail to query cb config");
                        return;
                    } else {
                ...
                case EVENT_OPEN_ETWS_CHANNEL_DONE:
                    Rlog.d(TAG, "handle EVENT_OPEN_ETWS_CHANNEL_DONE");
                    ar = (AsyncResult) msg.obj;
                    noti = (EtwsNotification) ar.userObj;
                    if (ar.exception == null) {
                        Rlog.d(TAG, "success to open cb channel " + noti.messageId);
                        int nextAction = msg.arg1;
                        if (nextAction == NEXT_ACTION_ONLY_ADD) {
                            ...
                            Rlog.d(TAG, "invalid next action " + nextAction);
                        }
                        updateDatabase(true);
                    } else {
                        Rlog.d(TAG, "fail to open cb channel");
                    }
                    break;

                case EVENT_CLOSE_ETWS_CHANNEL_DONE:
                    Rlog.d(TAG, "handle EVENT_CLOSE_ETWS_CHANNEL_DONE");
                    ar = (AsyncResult) msg.obj;
                    noti = (EtwsNotification) ar.userObj;
                    if (ar.exception == null) {
                        Rlog.d(TAG, "success to close cb channel " + noti.messageId);
                        int nextAction = msg.arg1;
                        ...
                        Rlog.d(TAG, "fail to close cb channel");
                    }
                    break;
                default:
                    Rlog.d(TAG, "unknown CB event " + msg.what);
                    break;
            }
        }
    };
    ...
    private void handleQueriedConfig(String config, int nextAction, EtwsNotification noti) {
        ...
        if (nextAction == EVENT_OPEN_ETWS_CHANNEL_DONE) {
            ...
            mCi.setCellBroadcastChannelConfigInfo(finalConfig, CB_SET_TYPE_OPEN_ETWS_CHANNEL,
                    response);
            ...
            finalConfig = parseSortedSetToString(sortedConfig);
            mCi.setCellBroadcastChannelConfigInfo(finalConfig, CB_SET_TYPE_CLOSE_ETWS_CHANNEL,
                    response);
        } else {
    ...
    private void updateDatabase(boolean open) {
        ...
        Uri uri = CHANNEL_URI;
        // FIXME: should not use the SIM_ID_2
        if (mPhoneId == PhoneConstants.SIM_ID_2) {
            uri = CHANNEL_URI1;
        }
        int channel = -1;
        String name = "";
