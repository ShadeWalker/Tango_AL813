        ...
        for (int i = 0; i < mTestPhoneCount; i++) {
            ...
            if (mExistingSimCount == 0) {
                ...
                for (int j = 0; j < mPhoneCount; j++) {
                    boolean result = (j == capabilityPhoneId) ? mCi[j].getRadioState().isOn() : !mCi[j].getRadioState().isOn();
                    assertTrue("CTA case: phone " + j + "'s radio result:", result);
                }
            } else {
                assertFalse("testSimHotPlug(): Hot plug out SIM fail", mTestCi[i].getRadioState().isOn());
            }                
        }
