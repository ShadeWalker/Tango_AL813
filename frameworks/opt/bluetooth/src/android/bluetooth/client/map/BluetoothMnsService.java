        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_EVENT:
                    ...
                    break;
                case BluetoothMasClient.MNS_SOCKET_ERROR:
                    synchronized (mCallbacks) {
                        Handler cb = mCallbacks.get(0);

                        if (cb != null) {
                            cb.obtainMessage(BluetoothMasClient.MNS_SOCKET_ERROR).sendToTarget();
                        } else {
                            Log.w(TAG, "Got event for instance which is not registered: 0");
                        }
                    }
                    break;
            }
