...
void SurfaceFlinger::checkEnableBootAnim() {
    ...
    if (strcmp(propVal, "")) {
        ...
        /** [HQ_FACTORYKIT_TEST] accelerate the first factory boot modify start*/
        	int fd = open("/proc/boot_status", O_RDONLY);
        	if (fd  >= 0 ){
                    char boot_mode = '0';
        	    size_t s_ext = read(fd, (void *)&boot_mode, sizeof(boot_mode));
        	    close(fd);

        	    if (s_ext <= 0)
        		XLOGI("[%s] can't read the boot_mode_ext = '%s'", __func__, propVal);
                    else if(boot_mode == 1)
                        mBootAnimationEnabled = false;
                } else {
        	    XLOGI("open /proc/boo_status error!");
                }
        	/** [HQ_FACTORYKIT_TEST] modify end*/
    } else {
        /*
        ...
        if (fd < 0) {
            ...
            if (s <= 0)
                ...
                boot_reason = '1';
            /** [HQ_FACTORYKIT_TEST] accelerate the first factory boot modify start*/
            		int fd_ext = open("/proc/boot_status", O_RDONLY);
            		if (fd_ext  >= 0 ){
            		    char boot_mode_ext = '0';
            		    size_t s_ext = read(fd_ext, (void *)&boot_mode_ext, sizeof(boot_mode_ext));
            		    close(fd_ext);

            		    if (s_ext <= 0)
            			XLOGI("[%s] can't read the boot_mode_ext = '%s'", __func__, propVal);
            		        else if(boot_mode_ext == 1)
            		             boot_reason = '1';
            		} else {
            		    XLOGI("open /proc/boo_status error!");
            		}
...
status_t SurfaceFlinger::getDisplayInfoEx(const sp<IBinder>& display, DisplayInfoEx* info) {
    ...
    XLOGD("getDisplayInfoEx subtype(%d)", (int)info->subtype);
