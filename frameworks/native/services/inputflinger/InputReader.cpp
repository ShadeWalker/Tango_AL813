...
void CursorButtonAccumulator::reset(InputDevice* device) {
    ...
    //mBtnRight = device->isKeyPressed(BTN_RIGHT);//delete by lihaizhou for HQ01354072 at 20150901
    } else {
        mBtnRight = device->isKeyPressed(BTN_LEFT);
        mBtnLeft = device->isKeyPressed(BTN_RIGHT);
    }
    mBtnMiddle = device->isKeyPressed(BTN_MIDDLE);
    mBtnBack = device->isKeyPressed(BTN_BACK) | device->isKeyPressed(BTN_RIGHT);//modified by lihaizhou for HQ01354072 at 20150901
...
void CursorButtonAccumulator::process(const RawEvent* rawEvent) {
    if (rawEvent->type == EV_KEY) {
		...
		mBtnLeft = rawEvent->value;
            break;*/ ////delete by lihaizhou for HQ01354072 at 20150901
        case BTN_MIDDLE:
            mBtnMiddle = rawEvent->value;
            break;
        case BTN_RIGHT://add by lihaizhou for HQ01354072 at 20150901
        case BTN_BACK:
...
void TouchInputMapper::dispatchMotion(nsecs_t when, uint32_t policyFlags, uint32_t source,
    ...
    if (action == AMOTION_EVENT_ACTION_DOWN) {
        POWERPUSH(125, "DOWN", "", ""); /*touch down*/
    } else if(action == AMOTION_EVENT_ACTION_UP){
        POWERPUSH(126, "UP", "", "");/*touch up*/
    }
#endif
