...
#include <errno.h>
#include <limits.h>
/*BEGIN PN:DTS2013052206422 Added by w81003630 2013/06/03*/
#include <cutils/properties.h>
/*END PN:DTS2013052206422 Added by w81003630 2013/06/03*/
#include <time.h>
...
/*BEGIN PN:DTS2013052206422 Added by w81003630 2013/06/03*/
/* read the property,then wheather we open this function */
bool initPropertyForKeyEvents(){
    char strProp[PROPERTY_VALUE_MAX];
    /* name of the property is "ro.config.KeyPassToUser"
     * and its value is expected to be "true" or "false",
     * we also handled the case that it's value is "1" or "0"
     */
    property_get("ro.config.keypasstouser", strProp, "false");
    if (0 == strcmp(strProp, "true") || 0 == strcmp(strProp, "1")){
        return true;
    }
    return true;
}
...
/*BEGIN PN:DTS2013052206422 Added by w81003630 2013/06/03*/
    mPowerEndcallPassToUser = initPropertyForKeyEvents();
/*END PN:DTS2013052206422 Added by w81003630 2013/06/03*/
    /// M:PerfBoost init @{
...
/*BEGIN PN:DTS2013052206422 Added by w81003630 2013/06/03*/
bool isWinNeedPowerEndcallEvents(const InputWindowInfo* win){
    if (NULL == win){
        return false;
    }
    char strProp[PROPERTY_VALUE_MAX];
    /* where not mmitest,we will do not pass key to user */
    property_get("sys.config.mmitest", strProp, "false");
    if (0 == strcmp(strProp, "false")){
        return false;
    }
    int32_t flags = win->layoutParamsFlags;
    return ((flags & InputWindowInfo::FLAG_KEYEVENT_PASS_TO_USER) != 0);
}
/*END PN:DTS2013052206422 Added by w81003630 2013/06/03*/
void InputDispatcher::notifyKey(const NotifyKeyArgs* args) {
    ...
    /*BEGIN PN:DTS2013052206422 Modified by w81003630 2013/06/03*/
    /* if user set the flag to use the power key,we will not intercept the power key */
    if (mPowerEndcallPassToUser && (mFocusedWindowHandle != NULL)) {
        if ((AKEYCODE_ENDCALL == args->keyCode || AKEYCODE_POWER == args->keyCode)
            && isWinNeedPowerEndcallEvents(mFocusedWindowHandle->getInfo())){
            // The focused window need POWER key and ENDCALL key, so we pass it to the window.
    ...
    /*END PN:DTS2013052206422 Modified by w81003630 2013/06/03*/
...
int32_t InputDispatcher::injectInputEvent(const InputEvent* event, int32_t displayId,
	...
	//HQ_zhangteng added for HQ01315380 at 2015-09-23 begin
	if (keyEvent->getKeyCode() == AKEYCODE_BACK)
	policyFlags &= ~POLICY_FLAG_DISABLE_KEY_REPEAT;
	//HQ_zhangteng added for HQ01315380 at 2015-09-23 end
