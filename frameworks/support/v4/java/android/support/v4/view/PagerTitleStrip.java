...
public class PagerTitleStrip extends ViewGroup implements ViewPager.Decor {
    ...
    void updateText(int currentItem, PagerAdapter adapter) {
        ...
        /// M: Long string feature
        mCurrText.setSelected(false);
        mCurrText.setSelected(true);
    ...
    */
    private void initLongStringSetting() {
        if (mPrevText != null && mCurrText != null && mNextText != null) {
            /// M: Set single line
            mPrevText.setSingleLine();
            mCurrText.setSingleLine();
            mNextText.setSingleLine();
            /// M: Set marquee when string over-long
            mPrevText.setEllipsize(TruncateAt.MARQUEE);
            mCurrText.setEllipsize(TruncateAt.MARQUEE);
            mNextText.setEllipsize(TruncateAt.MARQUEE);
            /// M: Set repeat limit when starting marquee
            mPrevText.setMarqueeRepeatLimit(2);
            mCurrText.setMarqueeRepeatLimit(2);
            mNextText.setMarqueeRepeatLimit(2);
            /// M: Set single line
            mPrevText.setHorizontalFadingEdgeEnabled(true);
            mCurrText.setHorizontalFadingEdgeEnabled(true);
            mNextText.setHorizontalFadingEdgeEnabled(true);
        }
   }
}
