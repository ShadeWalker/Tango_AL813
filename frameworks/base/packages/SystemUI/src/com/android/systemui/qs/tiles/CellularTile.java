...
public class CellularTile extends QSTile<QSTile.SignalState> {
    ...
    protected void handleClick() {
        ...
        if (mDataController.isMobileDataSupported() && isDefaultDataExist()) {
            showDetail(true);
        } else {
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        if (DBG) {
            Xlog.d(TAG, "handleUpdateState() cb.noSim: " + cb.noSim + " cb.enabled: " + cb.enabled + "cb.enabledDesc: "
                    + cb.enabledDesc);
        }
        ...
        if (TelephonyManager.from(mContext).getNetworkOperator() != null
                && !cb.noSim && !isDefaultDataExist()) {
            Xlog.d(TAG, "Default data sim not exist");
    ...
    // /M: Change the label when default SIM isn't setted @{
    private boolean isDefaultDataExist() {
        ...
        Xlog.d(TAG, "Default data sub id : " + defaultDataSubId);
        if (subinfoList != null && defaultDataSubId != SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
            for (SubscriptionInfo subInfo : subinfoList) {
                Xlog.d(TAG, "subInfo.getSubscriptionId : " + subInfo.getSubscriptionId());
                if (defaultDataSubId == subInfo.getSubscriptionId()) {
                    return true;
                }
            }
        }
