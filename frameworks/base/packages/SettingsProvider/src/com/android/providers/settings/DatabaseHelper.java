...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        //add by fudan for set exchange mail isn't first start
        //db.execSQL("INSERT INTO system values(null,'exchange_position_custom',1);");
        //add by fudan end
    ...
    private void loadVibrateWhenRingingSetting(SQLiteDatabase db) {
        ...
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO system(name,value)"
                    + " VALUES(?,?);");
          //hanchao change HQ01432249 20151203 begin
            if (SystemProperties.get("ro.hq.vibrate.ringing.support").equals("1")){
            	loadSetting(stmt, Settings.System.VIBRATE_WHEN_RINGING, 1);         
            }else{
            	loadSetting(stmt, Settings.System.VIBRATE_WHEN_RINGING, 0);
            }
          //hanchao change HQ01432249 20151203 end
        } finally {
            if (stmt != null) stmt.close();
        }
    }
    ...
    private void loadSystemSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            if (SystemProperties.get("ro.hq.tigo.screen.timeout").equals("1")) {//HQ_hushunli 2015-11-05 modify for HQ01453041
                loadIntegerSetting(stmt, Settings.System.SCREEN_OFF_TIMEOUT,
                        R.integer.def_tigo_screen_off_timeout);
            } else {
                loadIntegerSetting(stmt, Settings.System.SCREEN_OFF_TIMEOUT,
                        R.integer.def_screen_off_timeout);
            }
            if (SystemProperties.get("ro.hq.date.format").equals("1")) {//HQ_hushunli 2015-11-17 add for HQ01508447
                loadSetting(stmt, Settings.System.DATE_FORMAT, "dd/MM/yyyy");
            }

            //maheling HQ01311621 2015.11.4 start
            if (SystemProperties.get("ro.hq.default.setting.latin").equals("1")) {
                loadSetting(stmt, Settings.System.DTMF_TONE_WHEN_DIALING, 0);
            }
            //maheling HQ01311621 2015.11.22 end
            loadSetting(stmt, Settings.System.DATA_COST_TIP_SWITCH, 1);//HQ_hushunli 2015-11-30 add for HQ01454910
            ...
            loadBooleanSetting(stmt, Settings.System.SCREEN_BRIGHTNESS_MODE,
                    R.bool.def_screen_brightness_automatic_mode);
            //HQ_jiangchao modified for HQ01481958 at 20151106 start
            if (SystemProperties.get("ro.tigo.default.brightvalue").equals("1")) {
                loadSetting(stmt, Settings.System.SCREEN_AUTO_BRIGHTNESS_ADJ, 0.2f);
            }
            //HQ_jiangchao modified end
            loadDefaultAnimationSettings(stmt);
            ...
            //wangmeihua add for call reflection
            loadBooleanSetting(stmt, Settings.System.FLIP_PHONE_SWITCH, R.bool.def_flip_phone_switch);
            loadIntegerSetting(stmt, Settings.System.FLIP_PHONE_VALUE, R.integer.def_flip_phone_value);

            //yanqing add for data always on
            loadIntegerSetting(stmt, Settings.System.MOBILE_DATA_ALWAYS_ON, R.integer.def_cellular_data_always_on);

            //add by wuhuihui to hide font style
            loadStringSetting(stmt, "hw_hide_font_style", R.string.def_hide_font_style);
            loadStringSetting(stmt, "hw_hide_online_font", R.string.def_hide_online_font);
   
            //add by wangwenjia for HQ01491133
            loadIntegerSetting(stmt, Settings.System.TEXT_SHOW_PASSWORD, R.integer.def_text_show_pwd);

	    //add by caoxuhao for HQ01550282
            loadIntegerSetting(stmt, SettingsEx.System.SHOW_HWLOCK_FIRST, R.integer.def_show_hwlock_first);
			loadStringSetting(stmt, Settings.System.CALENDAR_PATH, R.string.default_same_calendar_value);

            ///M: load MTK new add System providers.
            mUtils.loadCustomSystemSettings(stmt);
            //add by wuhuihui 			
            loadIntegerSetting(stmt, Settings.System.KEYLOCK_GESTURES_SWITCH, R.integer.keylock_gestures_switch);
            loadIntegerSetting(stmt, Settings.System.KEYLOCK_GESTURES_C, R.integer.keylock_gestures_c);
            loadIntegerSetting(stmt, Settings.System.KEYLOCK_GESTURES_E, R.integer.keylock_gestures_e);
            loadIntegerSetting(stmt, Settings.System.KEYLOCK_GESTURES_W, R.integer.keylock_gestures_w);
            loadIntegerSetting(stmt, Settings.System.KEYLOCK_GESTURES_M, R.integer.keylock_gestures_m);
            loadIntegerSetting(stmt, Settings.System.KEYLOCK_GESTURES_DOUBLECLICK, R.integer.keylock_gestures_doubleclick);
            //add by wuhuihui end
            /* HQ_xuqian4 at 2015-09-25 modified for CL signal networktype begin*/
            String productName = SystemProperties.get("ro.product.name", "");
            if (productName.equalsIgnoreCase("TAG-CL00")) {
               loadIntegerSetting(stmt, Settings.System.ISSHOW_NETWORKTYPE, R.integer.isShow_NetworkType);
            }
            /* HQ_xuqian4 at 2015-09-25 modified end*/

            //add by lihaizhou for disable 3rdBrowser in ChildMode by begin
           loadStringSetting(stmt, Settings.System.BROWSER_PACKAGES, R.string.browser_packages);    
	  //add by lihaizhou for disable 3rdBrowser in ChildMode by end
	   //added by zhangteng for Smart Earphone Control at 2015-09-07 begin
	  loadIntegerSetting(stmt, Settings.System.SMART_EARPHONE_CONTROL, R.integer.smart_earphone_control);
	  //added by zhangteng for Smart Earphone Control at 2015-09-07 end
	   //added by zhangteng for HQ01544181 at 2015-12-07 begin
	  /* if need to customize different search engine, just create a new system property
	   * to control the value of HW_CUST_DISPLAY_SEARCH_ENGINE.
	   */
	  if("1".equals(SystemProperties.get("ro.hq.cust.search.engine.google"))){
	  loadStringSetting(stmt, Settings.System.HW_CUST_DISPLAY_SEARCH_ENGINE, R.string.hw_cust_display_search_engine);
	  }
	  //added by zhangteng for HQ01544181 at 2015-12-07 end
			//add by majian for gesture package string			
            loadStringSetting(stmt, Settings.System.KEYLOCK_GESTURES_C_PKG, R.string.keylock_gestures_c_pkg);
            loadStringSetting(stmt, Settings.System.KEYLOCK_GESTURES_E_PKG, R.string.keylock_gestures_e_pkg);
			loadStringSetting(stmt, Settings.System.KEYLOCK_GESTURES_W_PKG, R.string.keylock_gestures_w_pkg);
	        loadStringSetting(stmt, Settings.System.KEYLOCK_GESTURES_M_PKG, R.string.keylock_gestures_m_pkg);
			//add end
			
	 //add by xuweijie
	   loadIntegerSetting(stmt, Settings.System.FINGERPRINT_PHONE_SWITCH, R.integer.fingerprint_phone_switch);
            loadIntegerSetting(stmt, Settings.System.FINGERPRINT_CAMERA_SWITCH, R.integer.fingerprint_camera_switch);
	   loadIntegerSetting(stmt, Settings.System.FINGERPRINT_ALARMCLOCK_SWITCH, R.integer.fingerprint_alarmclock_switch);
            loadIntegerSetting(stmt, Settings.System.FINGERPRINT_KEYGUARD_SWITCH, R.integer.fingerprint_keyguard_switch);
            loadIntegerSetting(stmt, Settings.System.FINGERPRINT_SECURITY_SWITCH, R.integer.fingerprint_security_switch);
            loadIntegerSetting(stmt, Settings.System.FINGERPRINT_APPLOCK_SWITCH, R.integer.fingerprint_applock_switch);
	 //add by xuweijie end
	 
	 		/*add by liugang 20150727 HQ01265065*/
            /*HQ_xuqian4 20151117 modified for display battary percentage default  HQ01511649 begin*/
            if("1".equals(SystemProperties.get("ro.hq.battary.percentage"))){
            loadSetting(stmt, "battery_percent_switch", 1);
            }else{
            loadSetting(stmt, "battery_percent_switch", 0);//hanchao change HQ01432148 2015.10.08
            }
            /*HQ_xuqian4 modified end*/
	 		/*add end*/	 		
            loadSetting(stmt, "wifi_hotspot_auto_disable", 0);                   //hanchao change HQ01432185 2015.10.08
            loadSetting(stmt, "volume_alarm_speaker", 6);                        //hanchao change HQ01432248 2015.10.08
	 		
		       //add by zhouguo at 20151126 begin
                       //if(!"1".equals(SystemProperties.get("ro.sys.camera.pixel.flag"))){
                           loadSetting(stmt, "photo_backsize_cust", new String("4160*3120"));//modify by zhouguo for HQ01612515 at 21060104
                       //}
                       //add by zhouguo at 20151126 end			
			
			/*add by wangjing3  20151124 HQ01516077 begin*/
			/*if (productName.equalsIgnoreCase("TAG-L21")) {
				loadSetting(stmt, "photo_backsize_cust", new String("4160x2336"));
            		}*/
			/*add by wangjing3 20151124 for HQ01516077 end*/
			/*add by wangjing3  20151004 HQHQ01393239*/
	 		//loadSetting(stmt, "photo_backsize_cust", new String("4160x2336"));//modify by HQ_zhouguo at 20151117 for HQ01512417
	 		/*add end*/


			/*HQ_liugang add for set default theme 20150821*/
			loadStringSetting(stmt, Settings.System.HW_DEFAULT_THEME, R.string.def_hw_theme);
			/*add end*/

        loadSetting(stmt, "dual_clocks", 1); /*add by jiangchao 20151008 HQ01432181*/
        if (SystemProperties.get("ro.hq.latin.time.format").equals("1") || SystemProperties.get("ro.hq.africa.time.format").equals("1")) {//HQ_hushunli 2015-11-13 modify for HQ01501888
            loadSetting(stmt, "time_12_24", new String("12"));
        } else {
            loadSetting(stmt, "time_12_24", new String("24"));/*add by jiangchao 20151008 HQ01432180*/
        }
          if (SystemProperties.get("ro.config.movistar_shortcut").equals("1")) {
        	Log.d("huawei_showcut","start");
        	
            loadSetting(stmt, "hw_map_customized_app", new String("com.huawei.android.MiMovistar:cl.movistar.android;com.huawei.android.MovistarPlay:cl.movistarplay;com.huawei.android.modofutbol:com.smartboxtv.copamovistarchile;com.huawei.android.ClubMovistar:cl.jbravo.ams;"));
//            Log.d("huawei_showcut_hw_map_customized_app",Settings.System.getInt());
        } 
        if (SystemProperties.get("ro.hq.atandt.vibrate_on_email").equals("1")) {//add by zhouyoukun for atandt vibrate_on_email
            loadSetting(stmt, "vibrate_on_email", new String("1"));
        }
         if (SystemProperties.get("hw.telcel.email_itelcel").equals("1")) {
            loadSetting(stmt, "hw_email_hit_domain", new String("@itelcel.com"));
        }
         if (SystemProperties.get("ro.hW.email.smtp_nologin_telect").equals("1")) {
            loadSetting(stmt, "hw_email_smtp_nologin", new String("@itelcel.com"));
        }
         if (SystemProperties.get("ro.hq.telcel.email_exchange").equals("1")) {
            loadSetting(stmt, "exchange_position_custom", new String("1"));
        }
        if (SystemProperties.get("ro.hq.claro_email_hit").equals("1")) {
            loadSetting(stmt, "hw_email_hit_domain", new String("@iclaro.com.co"));
        }

	//*/HQ_daiwenqiang 20160108 modify for HQ01588720 start
		/*add by jingguangyao 20150727 HQ01265065*/
	if(1 == getFactoryKitMode()){
           loadIntegerSetting(stmt, Settings.System.TOUCH_DISABLE_MODE, R.integer.touch_enable_mode);
	} else {
	   loadIntegerSetting(stmt, Settings.System.TOUCH_DISABLE_MODE, R.integer.touch_disable_mode);
	}
		/*add end*/
        //*/HQ_daiwenqiang 20160108 modify for HQ01588720 end

        /*HQ_yulifeng for roaming default,HQ01308722,20151028,b*/
        loadBooleanSetting(stmt, Settings.System.FIRST_INTERNATIONAL_ROAMING_SIM1,R.bool.def_first_international_roaming_sim1);
        loadBooleanSetting(stmt, Settings.System.FIRST_INTERNATIONAL_ROAMING_SIM2,R.bool.def_first_international_roaming_sim2);
        loadBooleanSetting(stmt, Settings.System.FIRST_INTERNATIONAL_ROAM_STATE_SIM1,R.bool.def_first_international_roam_state_sim1);
        loadBooleanSetting(stmt, Settings.System.FIRST_INTERNATIONAL_ROAM_STATE_SIM2,R.bool.def_first_international_roam_state_sim2);
        /*HQ_yulifeng for roaming default,HQ01308722,20151028,b*/

        /*HQ_yulifeng for systemui  HQ01453039,20151105*/
        if("1".equals(SystemProperties.get("ro.conf.not_show_type_dis_data"))){
            loadIntegerSetting(stmt, Settings.System.NO_SHOW_NETWORKTYPE_DATA_DISABLE, R.integer.no_show_type_out_data);
        }
        /*HQ_xuqian4 for claro 4G LTE country 20151113 HQ01455082*/
        if("1".equals(SystemProperties.get("ro.hq.status.showlte4g.claro"))) {
	    loadStringSetting(stmt, Settings.System.SHOW_4G_ICON_CLARO,R.string.icon_4g_only_for_claro);
	    loadStringSetting(stmt, Settings.System.SHOW_LTE_ICON_CLARO,R.string.icon_lte_only_for_claro);
        }
        else {
		if("1".equals(SystemProperties.get("ro.hq.status.showlte4g.latin"))){
		    loadStringSetting(stmt, Settings.System.SHOW_4G_ICON_CLARO,R.string.icon_4g_claro);
		    loadStringSetting(stmt, Settings.System.SHOW_LTE_ICON_CLARO,R.string.icon_lte_claro);
		}
        }

        /*HQ_xuqian4 for claro 4G LTE country 20151113 HQ01455082*/
        if("1".equals(SystemProperties.get("ro.hq.showlte4g.movistar"))){
            loadStringSetting(stmt, Settings.System.SHOW_4G_ICON_CLARO,R.string.icon_4g_movistar);
            loadStringSetting(stmt, Settings.System.SHOW_LTE_ICON_CLARO,R.string.icon_lte_movistar);
        }
			//add by gaoyuhao for email's signature
			//loadIntegerSetting(stmt, Settings.System.EMAIL_SIGNATURE_WITH_PRODUCTNAME, R.integer.email_signature_with_productname);
            //end gaoyuhao
            //add by gaoyuhao for browser homepage&bookmarks
			if("1".equals(SystemProperties.get("ro.hq.custom.bookmarks"))){
			loadIntegerSetting(stmt, Settings.System.CUSTOM_BOOKMARK_HOMEPAGE, R.integer.custom_bookmark_homepage);
				}
			//add by wangmingyue for browser homepage&bookmarks
			if("1".equals(SystemProperties.get("ro.hq.change.bookmark.mcc"))){
			loadIntegerSetting(stmt, Settings.System.CHANGE_BOOKMARK_BY_MCC, R.integer.change_bookmark_by_mcc);
				}
			//add by wangmingyue for browser homepage&bookmarks
            //add by gaoyuhao for clock hide jerusalem
            if("1".equals(SystemProperties.get("ro.hq.is_hide_jerusalem"))){
            loadIntegerSetting(stmt, Settings.System.IS_HIDE_JERUSALEM, R.integer.is_hide_jerusalem);
                }
            //add by gaoyuhao for clock hide jerusalem
			//add by gaoyuhao for clock hide jerusalem
            if("1".equals(SystemProperties.get("ro.hq.att.first_day"))){
                  loadIntegerSetting(stmt, Settings.System.FIRST_DAY_OF_WEEK, R.integer.first_day_of_week);
            }
            if("yes".equals(SystemProperties.get("ro.hq.is_show_gps_error_toast"))){
			loadBooleanSetting(stmt, Settings.System.IS_SHOW_GPS_ERROR_TOAST,R.bool.is_show_gps_error_toast);
                }
			//end by gaoyuhao for wettherclock toast 
			//add by gaoyuhao for calendar vibrate
            if("yes".equals(SystemProperties.get("ro.hq.vibrate_on_calendar"))){
			loadIntegerSetting(stmt, Settings.System.VIBRATE_ON_CALENDAR,R.integer.vibrate_on_calendar);
                }
			//end by gaoyuhao for calendar vibrate
			//add by gaoyuhao for claroEmail HQ01453223
            if("yes".equals(SystemProperties.get("ro.hw_claroEmailConfig"))){
			loadStringSetting(stmt, Settings.System.USE_EXTRA_EMAILLIST,R.string.use_extra_emaillist);
			loadIntegerSetting(stmt, Settings.System.EXCHANGE_POSITION_CUSTOM,R.integer.exchange_position_custom);
			loadStringSetting(stmt, Settings.System.DOMAIN_FOR_30MIN_CHECK,R.string.domain_for_30min_check);
			loadStringSetting(stmt, Settings.System.HW_EMAIL_HIT_DOMAIN,R.string.hw_email_hit_domain);
                }
			//end by gaoyuhao for claroEmail HQ01453223
        } finally {
            if (stmt != null) stmt.close();
        }
    }
    ...
    private void loadUISoundEffectsSettings(SQLiteStatement stmt) {
        ...
        loadSetting(stmt, Settings.System.HAPTIC_FEEDBACK_ENABLED,0);           //hanchao change  HQ01432132
    ...
    private void loadDefaultHapticSettings(SQLiteStatement stmt) {
    	//hanchao change for HQ01558522 begin
        if (SystemProperties.get("ro.haptic.feedback.enable").equals("1")) {
            loadSetting(stmt, Settings.System.HAPTIC_FEEDBACK_ENABLED,1);
        }else{
    	...
    	//hanchao change for HQ01558522 end
    }
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            String defLocationProvidersAllowed = SystemProperties.get("ro.loc.providers.allowed");
            if (!TextUtils.isEmpty(defLocationProvidersAllowed)) {
                loadSetting(stmt, Settings.Secure.LOCATION_PROVIDERS_ALLOWED, defLocationProvidersAllowed);
            } else {
            ...
            loadSetting(stmt, "spell_checker_enabled",0);             // hanchao  add  HQ01432202  2015.10.8
            /* HQ_jiazaizheng 2015-10-22 modified for no disturb default value HQ01456836 begin */
            //loadSetting(stmt, "HW_PREVENT_MODE",0);                  // hanchao  add  HQ01432202  2015.10.12
            //loadSetting(stmt, "HW_PREVENT_DFT_VALUE_INIT_FLAG",1);    // hanchao  add  HQ01432202  2015.10.12
            /* HQ_jiazaizheng 2015-10-22 modified end */
    ...
    private void loadGlobalSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            /*HQ_yulifeng for data roaming state HQ01311843, 20151103*/
            loadSetting(stmt, Settings.Global.DATA_ROAMING_STATE,0);
            ...
            ///HQ_xionghaifeng 20151110 add for default data open start @{
            ///MOBILE_DATA_DEFAULT  ====> 1
            ...
            loadSetting(stmt, Settings.Global.CONVERT_BRIGHTNESS_FACTOR,Float.toString(3.2f));
            //fengyaling 20150814 add for default brightbess end

			//add by yanzewen to forbidden the Keyguard
		/*	if(1 == getFactoryKitMode()){
				loadSetting(stmt, Settings.Global.TOUCH_DISABLE_MODE,0);
			}*/
            
            //hanchao HQ01311621 2015.12.4 start
            if (SystemProperties.get("ro.hq.vibrate.silent.support").equals("1")) {
                loadSetting(stmt, "vibirate_when_silent", 1);
            }
            //hanchao HQ01527066 2015.12.4 end
    ...
    {
        //read data from /proc/boot_status file, if data is 1, should start factory kit.
        ...
        try {
            ...
            Log.w(TAG, "getFactoryKitMode:Failure reading /proc/boot_status:", e);
        }
