...
import android.content.Intent;//add by lihaizhou for ChildMode at 2015-07-26
import android.os.UserHandle;//add by lihaizhou for ChildMode at 2015-07-26
import android.util.Log;
import android.util.SparseArray;
import android.provider.ChildMode;//add by lihaizhou for ChildMode at 2015-07-26
...
public class WifiManager {
    ...
    public boolean setWifiEnabled(boolean enabled) {
	    ...
	    String networkTimeLimit = ChildMode.OFF;
           networkTimeLimit = ChildMode.getString(mContext.getContentResolver(),"internet_time_restriction_limit");
           if((ChildMode.ON).equals(networkTimeLimit)&& isChildModeOn()){
                  Intent sendintent = new Intent("com.childmode.network_time_limit_disable_alert");
                  mContext.sendBroadcastAsUser(sendintent, UserHandle.ALL);
                  return false;
           }
	/*lihaizhou add for forbid checked when child mode times up at 2015-07-26 by begin*/
	 if(isChildModeOn() && (isProhibitWlan() ||isWlanTimeUp()))
	 {
	 	return false;
	 }
	/* lihaizhou add for forbid checked when child mode times up at 2015-07-26 by end*/
	...
	/* add by lihaizhou for forbid checked when child mode times up by begin*/
        /** {@hide} */ 
	...
	/* add by lihaizhou for forbid checked when child mode times up by end*/
	/*add by lihaizhou for childmode at 2015-07-26 by end */
    /**
   ...
   /**
    ...
    public boolean setWifiDisabledByEpdg(int flag) {
        Log.d(TAG, "setWifiDisabledByEpdg, flag = " + flag);

        if (null == mService) {
            Log.d(TAG, "setWifiDisabledByEpdg, fail, null == mService");
            return false;
        }

        try {
            return mService.setWifiDisabledByEpdg(flag);
        } catch (RemoteException e) {
            return false;
        }
    }
    ...
    private final ArrayMap<WifiOffListener, IWifiOffListener>
        mWifiOffListeners
            = new ArrayMap<WifiOffListener, IWifiOffListener>();
    /**
     * Regist WifiOffListener callback function
     * @param listener is the listener to receive onWifiOff event
     *
     * @hide
     */
    public void addWifiOffListener(final WifiOffListener listener) {
         IWifiOffListener rl = new IWifiOffListener.Stub() {
             @Override
            public void onWifiOff(int reason) throws RemoteException {
                listener.onWifiOff(reason);
            }
         };
         try {
             mService.registWifiOffListener(rl);
            mWifiOffListeners.put(listener, rl);
         } catch (RemoteException e){
         }
    }
    ...
    public void removeWifiOffListener(WifiOffListener listener) {
         IWifiOffListener rl = mWifiOffListeners.get(listener);
         if (rl == null) {
             throw new IllegalArgumentException("Listener not registered: " + listener);
         }
         try {
             mService.unregistWifiOffListener(rl);
         } catch (RemoteException e){
         }
    }
