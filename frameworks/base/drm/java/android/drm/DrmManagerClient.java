...
//add for huawei DRM at 20151113 by lihaizhou by begin
import android.content.Intent;
import android.os.Binder;
import android.os.Bundle;
//add for huawei DRM at 20151113 by lihaizhou by end
...
public class DrmManagerClient {
    ...
    public ContentValues getConstraints(String path, int action) {
        ...
        /// M: Add for huawei, convert MTK define key-value to huawei type
        ContentValues values = _getConstraints(mUniqueId, path, action);
        if (values == null) {
            Log.d(TAG, "getConstraints with null!");
            return null;
        }
        ContentValues metadata = _getMetadata(mUniqueId, path);
        if (metadata == null) {
            Log.d(TAG, "getConstraints with null metadata!");
            return values;//do not return null but values, for gts not NullPointerException
        }
        /// 1. is valid
        int status = checkRightsStatus(path, action);
        values.put("valid", status == DrmStore.RightsStatus.RIGHTS_VALID);
        /// 2. is un-limited and auto use 
        int drmMethod = metadata.getAsInteger(DrmStore.MetadataKey.META_KEY_METHOD);
        values.put("unlimited", drmMethod == DrmStore.DrmMethod.METHOD_FL);
        values.put("is_auto_use", drmMethod == DrmStore.DrmMethod.METHOD_FL ? "true" : "false");//add by lihaizhou for only support set FL file as ring,wallpaper
        /// 3. max repeat count
        String maxRepeatCount = values.getAsString(DrmStore.ConstraintsColumns.MAX_REPEAT_COUNT);
        if (maxRepeatCount != null) {
            values.put("count", maxRepeatCount);
        }
        /// 4. remain repeat count
        String remainRepeatCount = values.getAsString(DrmStore.ConstraintsColumns.REMAINING_REPEAT_COUNT);
        if (remainRepeatCount != null) {
            values.put("rights_count", remainRepeatCount);
        }
        /// 5. license start time
        String startTime = values.getAsString(DrmStore.ConstraintsColumns.LICENSE_START_TIME);
	startTime = (Long.parseLong(startTime)*1000)+"";
        if (startTime != null) {
            values.put("license_start_time", startTime);
            values.put("startDate", startTime);
            values.put("startTime", startTime);
        }
        /// 6. license expire time
        String expireTime = values.getAsString(DrmStore.ConstraintsColumns.LICENSE_EXPIRY_TIME);
	expireTime = (Long.parseLong(expireTime)*1000)+"";
        if (expireTime != null) {
	    values.put("license_expiry_time", expireTime);
            values.put("endDateTime", expireTime);
            values.put("endDate", expireTime);
            values.put("endTime", expireTime);
        }
        /// 7. license available time
	String availableTime = ((Long.parseLong(expireTime)-Long.parseLong(startTime))/1000)+"";
        if (availableTime != null) {
	    values.put("license_available_time", availableTime);
            values.put("interval", availableTime);
            values.put("intervalDate", availableTime);
            values.put("intervalTime", availableTime);
        }
        Log.d(TAG, "getConstraints: path = " + path + ", action = " + action + ", values = " + values);
        /// @}
        return values;
    }
    ...
    */
    public ContentValues getMetadata(String path) {
        ...
        /// M: Add for huawei, convert MTK define key-value to huawei type
        ContentValues values = _getMetadata(mUniqueId, path);
        if (values == null) {
            Log.d(TAG, "getMetadata with null!");
            return null;
        }
        /// 1. drm method
        int drmType = DrmStore.DrmType.UNKOWN;
        int drmMethod = values.getAsInteger(DrmStore.MetadataKey.META_KEY_METHOD);
        if (drmMethod == DrmStore.DrmMethod.METHOD_FL) {
            drmType = DrmStore.DrmType.FORWARD_LOCK;
        } else if (drmMethod == DrmStore.DrmMethod.METHOD_CD) {
            drmType = DrmStore.DrmType.COMBINED_DELIVERY;
        } else if (drmMethod == DrmStore.DrmMethod.METHOD_SD) {
            drmType = DrmStore.DrmType.SEPARATE_DELIVERY;
        } else if (drmMethod == DrmStore.DrmMethod.METHOD_FLDCF) {
            drmType = DrmStore.DrmType.SEPARATE_DELIVERY_FL;
        }
        values.put("DRM-TYPE", drmType);
        /// 2. rights issuer
        String rightsIssuer = values.getAsString(DrmStore.MetadataKey.META_KEY_RIGHTS_ISSUER);
        if (rightsIssuer != null) {
            values.put("Rights-Issuer", rightsIssuer);
        }
        /// @}
        Log.d(TAG, "getMetadata: path = " + path + ", values = " + values);
        return values;
    }
    ...
    */
	...
	// M: Add for huawei, return drm method instead object type. it will cause some risk
	/// that other app use this android sdk api could not get wanted value. {@
	ContentValues metadata = getMetadata(path);
	if (metadata != null) {
	int drmType = metadata.getAsInteger("DRM-TYPE");
	Log.d(TAG, "getDrmObjectType = " + drmType);
	return drmType;
	}
	/// @}
	...
	/*add by lihaizhou for show dialog when click SD file in Gallery by begin*/
	if(action == 263)
	{
	   action = 0x101;
	}
	/*add by lihaizhou for show dialog when click SD file in Gallery by end*/
        if (null == path || path.equals("") || !DrmStore.Action.isValid(action)) {
            throw new IllegalArgumentException("Given path or action is not valid");
        }
                /// M: Add for huawei, if action is DRM_ACTION_SHOW_DIALOG, check and show renew or expire
        /// dialog if necessary. {@
        int status = DrmStore.RightsStatus.RIGHTS_INVALID;
        if (action == DrmStore.Action.DRM_ACTION_SHOW_DIALOG) {
            ContentValues metadata = getMetadata(path);
            if (metadata == null) {
                Log.w(TAG, "checkRightsStatus with null metadata, return invalid");
                return DrmStore.RightsStatus.RIGHTS_INVALID;
            }
            int drmMethod = metadata.getAsInteger(DrmStore.MetadataKey.META_KEY_METHOD);
            String rightsIssuer = metadata.getAsString(DrmStore.MetadataKey.META_KEY_RIGHTS_ISSUER);
            String drmMimetype = metadata.getAsString(DrmStore.MetadataKey.META_KEY_MIME);
            /// image use display action, audio/video use play action 
            if (drmMimetype == null) {
                Log.w(TAG, "checkRightsStatus with null mimetype, return invalid");
                return DrmStore.RightsStatus.RIGHTS_INVALID;
            }
            if (drmMimetype.startsWith("image/")) {
                status = _checkRightsStatus(mUniqueId, path, DrmStore.Action.DISPLAY);
            } else {
                status = _checkRightsStatus(mUniqueId, path, DrmStore.Action.PLAY);
            }
            /// TODO MTK check secure timer whether valid and will show toast to user, if huawei
            /// need, please add by yourself
            if (status == DrmStore.RightsStatus.SECURE_TIMER_INVALID) {
                Log.w(TAG, "checkRightsStatus with secure timer invalid");
                return DrmStore.RightsStatus.RIGHTS_INVALID;
            }
            /// valid rights and FL file needn't show dialog, CD show expire dialog,
            /// SD show renew dialog
            if (status != DrmStore.RightsStatus.RIGHTS_VALID
                    && drmMethod != DrmStore.DrmMethod.METHOD_FL) {
                Log.w(TAG, "checkRightsStatus with invlid right, show renew/expire dialog");
                Intent intent = new Intent("android.intent.action.SHOW_DRM_DIALOG");
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                Bundle extras = new Bundle();
                extras.putString("path", path);
                extras.putString(DrmStore.MetadataKey.META_KEY_RIGHTS_ISSUER, rightsIssuer);
                extras.putInt(DrmStore.MetadataKey.META_KEY_METHOD, drmMethod);
                intent.putExtras(extras);
                mContext.startActivity(intent);
            }
        } else {
            status = _checkRightsStatus(mUniqueId, path, action);
        }
        Log.d(TAG, "checkRightsStatus: path = " + path + ", action = " + action
                + ", status = " + status);
        /// @}
        return status;
    }
