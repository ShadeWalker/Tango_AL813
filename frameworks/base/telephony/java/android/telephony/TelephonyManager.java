...
/*zhangjing add for show reconfirmDialog when open data connection begin*/
...
/*zhangjing add for show reconfirmDialog when open data connection end*/

///xionghaifeng 20151012 add for HQ01382804 @{
...
import android.os.SystemProperties;//end by lipeng
/**
...
public class TelephonyManager {
	...
	/*zhangjing add for show reconfirmDialog when open data connection begin*/
    private static final String TAG_RECONFIRM = "TelephonyManager reconfirm: zhangjing";
	private int mPreSlotId;
	private int mSlotId;
	private int mGeminiSlotId = -1;
	private CheckBox next_remind_check = null;
	private AlertDialog mConfirmDialog = null;
	private ShowDialogHandler mHandler;
	private static final int EVENT_SHOW_RECONFIRM_DIALOG = 20;
	private String RECOVERY_DATA_UI = "data_connection_and_recovery_UI";
    private boolean not_use_sim_ready_condition = false;
	private String isFirstBoot;
	
	private static boolean HQ_SHOW_RECONFIRM_DIALOG_OPEN_DATA_CONNECTION = false;
			//	= SystemProperties.get("ro.hq_reconfirm_dlg").equals("1");
	/*zhangjing add for show reconfirmDialog when open data connection end*/
   ...
   /** Current network is IWLAN {@hide} */
    public static final int NETWORK_TYPE_IWLAN = 17;
   ...
   /**
    ...
    public String getSimOperatorNumericForPhone(int phoneId) {
        String curPlmn =  getTelephonyProperty(phoneId,
    ...
    */
    ...
    public int getTetherApnRequired(int subId) {
        try {
            return getITelephony().getTetherApnRequired(subId);
        } catch (RemoteException ex) {
    ...
    public void setDataEnabled(boolean enable) {
    	...
    	{
    		Log.d("xionghaifeng", "setDataEnabled has no sim");
    		return;
    	}
		...
		{
			setDataEnabled(subId, enable);
		}
		//has no default SIM or has no default value
		else
		{
			Log.d("xionghaifeng", "setDataEnabled has no default SIM");

			if (hasIccCard(0))
			{
				mSubscriptionManager.setDefaultDataSubId(getSubIdBySlot(0));
				setDataEnabled(getSubIdBySlot(0), enable);
			}
			else
			{
				mSubscriptionManager.setDefaultDataSubId(getSubIdBySlot(1));
				setDataEnabled(getSubIdBySlot(1), enable);
			}
		}
		/*HQ_xionghaifeng 20150825 modify for set data end*/
    ...
    public void setDataEnabled(int subId, boolean enable) {
        ...
        Log.d(TAG, "setDataEnabled (subid,enable)" + enable + " by " + pck);
		/* modify by zhangjinqiang for al812
        try {
            Log.d(TAG, "setDataEnabled: enabled=" + enable);
        ...
        */
        try {
            Log.d(TAG, "setDataEnabled: enabled=" + enable);
			if (HQ_SHOW_RECONFIRM_DIALOG_OPEN_DATA_CONNECTION) {
					mHandler = new ShowDialogHandler();
					if (!simStateReady(subId)){
						Log.d(TAG_RECONFIRM,"setMobileDataEnabled:sim not ready" );
						if (enable){
							Log.d(TAG_RECONFIRM,"setMobileDataEnabled:sim not ready,change request	from true to false" );
							enable = false;
						}
					}
					//dialog already show
					if (mConfirmDialog != null){
						Log.d(TAG_RECONFIRM,"dialog already show:do nothing");
						return ;
					}
					
					if (enable && shouldShowConfirmDlg()){
						Log.d(TAG_RECONFIRM,"send message to show reconfirm dialog");
						mHandler.sendMessage(mHandler.obtainMessage(EVENT_SHOW_RECONFIRM_DIALOG));
					} else {
						getITelephony().setDataEnabled(subId, enable);
					}
				}else{
    ...
    public boolean getDataEnabled(int subId) {
        //modified by maolikui at 2016-01-05
        String pck = mContext != null ? mContext.getPackageName() : "<unknown>";
		int mobile_data = -1;
		boolean retVal = false;
		Log.d(TAG, "getDataEnabled (subid)" + subId + " by " + pck);
        Log.i("maolikui","getDataEnabled (subid)" + subId + " by " + pck);
	  if (pck.contains("systemui")) {
            try {
             mobile_data  = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.MOBILE_DATA);
			 Log.i("maolikui","getDataEnabled (subid) mobile_data:" + mobile_data);
			} catch (Exception e) {
                Log.v(TAG, "Settings.Global.getInt Settings.Global.MOBILE_DATA Exception!");
            }
            
       }
      if(getITelephony() == null) return false; //add by majian for fatal error
        try {
        ...
        if(mobile_data == 1 && retVal == false){
			Log.i(TAG,"getDataEnabled mobile_data:" + mobile_data +",retVal:" +retVal);
			retVal = true;
        }
		Log.i("maolikui","getDataEnabled: retVal=" + retVal);
    ...
    */
    ...
    /**
     * Judge 4G Switch state xuqian4 2015/07/22
     * return true:4G Switch is opened
     * return false:4G Switch is closed
     */
    ...
    /**
     * set 4G Switch state xuqian4 2015/07/22
     *
     */
    public void setLTESwitchstate(boolean enabled) {

    }

	/*zhangjing add for show reconfirmDialog when open data connection begin*/
	private void setDataEnabledReally(boolean enabled){
			try {
				Log.d(TAG, "ITelephony#setDataEnabled"+enabled);
	            getITelephony().setDataEnabled(SubscriptionManager.getDefaultDataSubId(), enabled);
	        } catch (RemoteException e) {
	            Log.e(TAG, "Error calling ITelephony#setDataEnabled", e);
	        } catch (NullPointerException e) {
	            Log.e(TAG, "Error calling ITelephony#setDataEnabled", e);
	...
	private void saveNextRemind( boolean next_not_remind){
			Settings.Global.putInt(mContext.getContentResolver(),"next_not_remind", next_not_remind?1:0);
	}
	
	private void showOpenDataConnConfirmDlg( ){
		Log.d(TAG_RECONFIRM,"showOpenDataConnConfirmDlg");
		View v = View.inflate( mContext,com.mediatek.internal.R.layout.open_data_conn_confirm,null);
		next_remind_check = (CheckBox)v.findViewById( com.mediatek.internal.R.id.next_remind_checkbox );
		if( next_remind_check != null ){
			next_remind_check.setChecked( true );
		}
		
		AlertDialog.Builder b = new AlertDialog.Builder(mContext);
			b.setCancelable(true);
				b.setTitle(com.mediatek.internal.R.string.open_data_confirm_title);
				b.setView(v);
				b.setPositiveButton(com.mediatek.internal.R.string.open,mRecofirmDiaogClickListener);
				b.setNegativeButton(com.mediatek.internal.R.string.close_dialog,mRecofirmDiaogClickListener);

		AlertDialog confirDialog = b.create();
		confirDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
		confirDialog.show();
		confirDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
				public void onDismiss(DialogInterface dialog) {
					disMissDialog();
				}
			});
		mConfirmDialog = confirDialog;
		
	}
	
    OnClickListener mRecofirmDiaogClickListener = new OnClickListener(){
        public void onClick(DialogInterface a0, int which) {
            switch (which) {
                case DialogInterface.BUTTON_POSITIVE:
					Log.d(TAG_RECONFIRM,"positive button is clicked");
					setDataEnabledReally(true);
					if( next_remind_check != null ){
					 	saveNextRemind( next_remind_check.isChecked());
					}
					sendRecoveryDataUIBroadcast("1");//fengyaling modified for setup wizard 20150318
                    break;
                case DialogInterface.BUTTON_NEGATIVE:
					//if negative is clicked ,do nothing and notify he UI 
					Log.d(TAG_RECONFIRM,"negative button is clicked and do nothing");
					sendRecoveryDataUIBroadcast("0");//fengyaling modified for setup wizard 20150318
                    break;
            }
        }
    };  
	private void disMissDialog(){
		Log.d(TAG_RECONFIRM,"dismiss dialog");
		if( mConfirmDialog != null ){
			mConfirmDialog.dismiss();
			mConfirmDialog = null ;
		}
		sendRecoveryDataUIBroadcast("");//fengyaling modified for setup wizard 20150318
	}

	private void sendRecoveryDataUIBroadcast(String flag){
        Intent intent = new Intent( RECOVERY_DATA_UI );
        intent.putExtra("data_conn_form",flag); //fengyaling modified for setup wizard 20150318
		mContext.sendBroadcast(intent );
	}
	private boolean simStateReady(int subId){
			//Log.d(TAG_RECONFIRM, "getSimState() " + getSimState());
			//Log.d(TAG_RECONFIRM, "getDataSimState() " + getSimState(SubscriptionManager.getDefaultDataSubId()));
            int slotId = SubscriptionManager.getSlotId(subId);
            if (getSimState(slotId) == TelephonyManager.SIM_STATE_READY)
            {
                return true;
			}
			...
			{
				not_use_sim_ready_condition = Settings.Global.getInt(mContext.getContentResolver(), "not_use_sim_ready_condition", 0) == 1;
			}
			
			if( not_use_sim_ready_condition){
				Log.d(TAG_RECONFIRM,"not_use_sim_ready_condition = " + not_use_sim_ready_condition);
				return true;
			}
			else
            {
		...
		private boolean shouldShowConfirmDlg(){
			boolean next_not_tips = Settings.Global.getInt(mContext.getContentResolver(), "next_not_remind", 0) == 1;
			boolean shouldShowDiallg = !next_not_tips 
										&& ( Settings.Secure.getInt(mContext.getContentResolver(), Settings.Global.MOBILE_DATA, 0) == 0 );
			Log.d(TAG_RECONFIRM,"next_not_tips = " + next_not_tips +  " shouldShowDiallg = " + shouldShowDiallg);
			return shouldShowDiallg;
		}
		...
		private class ShowDialogHandler extends Handler {
			...
			public void handleMessage(Message msg) {
				...
				case EVENT_SHOW_RECONFIRM_DIALOG:
					Log.d(TAG_RECONFIRM,"handleMessage:" + EVENT_SHOW_RECONFIRM_DIALOG);
					if( HQ_SHOW_RECONFIRM_DIALOG_OPEN_DATA_CONNECTION ){
						showOpenDataConnConfirmDlg();
					}
					break;
				default:
					break;
...
/*zhangjing add for show reconfirmDialog when open data connection end*/
	...
	public static int getPreferredDataSubscription(){
		return (int)SubscriptionManager.getPreferredDataSubscription();
	}
	...
	private boolean isReadPhoneNumberBlocked()
	{
		try 
		{
			...
			Method staticMethod_checkPrecondition = null;
			Method staticMethod_holdForGetPermissionSelection = null;
			
			Class ctrlrClazz = Class.forName("com.huawei.hsm.permission.StubController");
			staticMethod_checkPrecondition = ctrlrClazz.getDeclaredMethod("checkPrecondition", int.class);
			staticMethod_holdForGetPermissionSelection = ctrlrClazz.getDeclaredMethod("holdForGetPermissionSelection", 
				int.class, int.class, int.class, String.class);

			if (!(boolean)staticMethod_checkPrecondition.invoke(ctrlrClazz, uid))
			{
				Log.d("xionghaifeng", "isReadPhoneNumberBlocked checkPrecondition return false");
				return false; 
			}

			int ret = (int) staticMethod_holdForGetPermissionSelection.invoke(ctrlrClazz, 16, uid, pid, null);
			Log.d("xionghaifeng", "isReadPhoneNumberBlocked ret = " + ret);
          ...
          Log.w("xionghaifeng", "isReadPhoneNumberBlocked", ex);
		} catch (RuntimeException ex) {
		  Log.w("xionghaifeng", "isReadPhoneNumberBlocked", ex);
		} catch (InvocationTargetException e) {
		  Log.w("xionghaifeng", "isReadPhoneNumberBlocked", e);
