...
import com.android.internal.telephony.EccOverride;
...
import android.os.Build;
import com.android.internal.telephony.ITelephony;
import com.android.internal.telephony.PhoneMatchOverride;//add by zhaizhanfeng for number match at 150914

/*add by yulifeng for Roaminglist,special spn show,b*/
import java.io.File;
...
/*add by yulifeng for Roaminglist,special spn show,e*/

//	 hq_hxb modified by  for  HQ01308688
...
public class PhoneNumberUtils
{
    ...
    private final static String mccmncSim = null;
    private final static String mccmncNoSimInserted = "999999";
    private static String mccmncSim_1 = mccmncNoSimInserted;
    private static String mccmncSim_2 = mccmncNoSimInserted;
    private static String virtualKey = null;
    private final static int VIRTUAL_PATTERN_IMSI = 1;
    private final static int VIRTUAL_PATTERN_GID1 = 2;
    private final static int VIRTUAL_PATTERN_SPN = 4;
    private final static int VIRTUAL_PATTERN_EFFILE = 8;	
    private final static int PATTERN_MCCMNC = 16;
    private final static String imsiSim = null;
    private static int virtualPattern = -1;
    private static int virtualPattern_1 = -1;
    private static int virtualPattern_2 = -1;
    private static String virtualValue_1  = null;
    private static String virtualValue_2  = null;
    private final static String defaultGID1 = "FF";
    private final static String defaultSPN = "FFFFF";
    private final static String defaultSST = "FF";
    /* HQ_guomiao add for HQ01453081 begin*/
    private static final String[] defaultwithoutSIMEccList ={"911","112","000","08","110","118","119","999"};
    private static final String[] defaultwithSIMEccList={"112","911"};
    /* HQ_guomiao add for HQ01453081 end*/
    /*
    ...
    /*add by yulifeng for Roaminglist,special spn show,b*/
    private static final String PARTNER_CUTOM_ROAMING_ENTITY_CONF_PATH ="system/etc/Cutom-Roaming_entity.xml";
    private static final String PARTNER_CUTOM_ROAMING_IMSI_CONF_PATH ="system/etc/Cutom-Roaming_imsi.xml";
    private static final String PARTNER_SPEC_SPN_CONF_PATH ="system/etc/Spec-spn-show-conf.xml";
    /*add by yulifeng for Roaminglist,special spn show,e*/
    ...
    public static String extractNetworkPortionAlt(String phoneNumber) {
        ...
        if(Build.TYPE.equals("eng")){Rlog.d(LOG_TAG, "[extractNetworkPortionAlt] phoneNumber: " + ret.toString());}
    ...
    compareLoosely(String a, String b) {
        ...
        if (sIsCtaSupport) {
        	...
        	minMatchLen = getMatchCountByLanguage();//modify by lipeng for mms number match
        }

        if(Build.TYPE.equals("eng")){Rlog.d(LOG_TAG, "[compareLoosely] a: " + a + ", b: " + b + ", minMatchLen:" + minMatchLen);}
...
/*add by zhaizhanfeng for number match update sqlite at 150924 start
...
add by zhaizhanfeng for number match update sqlite at 150924 end */

        int minMatchLen = MIN_MATCH;
        if (sIsCtaSupport) {
            minMatchLen = MIN_MATCH_CTA;
        } else if (sPhoneNumberExt != null) {
            //minMatchLen = sPhoneNumberExt.getMinMatch();//add by zhaizhanfeng for number match at 150914
            minMatchLen = getMatchCountByLanguage();//add by zhaizhanfeng for number match at 150914
        }
        /* HQ_guomiao 2015-10-09 modified for HQ01393571 begin */
        if (Build.TYPE.equals("eng")) {
            Log.d("zhai", "[toCallerIDMinMatch] phoneNumber: " + phoneNumber +
                    ", minMatchLen: " + minMatchLen +
                    ", result:" + internalGetStrippedReversed(np, minMatchLen));
        }
        /* HQ_guomiao 2015-10-09 modified for HQ01393571 end */
        String strStrippedReversed = internalGetStrippedReversed(np, minMatchLen);

       if(Build.TYPE.equals("eng")){ Rlog.d(LOG_TAG, "[toCallerIDMinMatch] phoneNumber: " + phoneNumber +
                 ", minMatchLen: " + minMatchLen + ", result:" + strStrippedReversed);
       	}
        return strStrippedReversed;
    }
    //add by zhaizhanfeng for number match at 150914 start
     public static int getMatchCountByLanguage() {
			...
			if((TextUtils.isEmpty(iccid1) || iccid1.equals("N/A")) && (TextUtils.isEmpty(iccid2) || iccid2.equals("N/A"))){
				return 7;
			}else if(!TextUtils.isEmpty(iccid1) && !iccid1.equals("N/A") && !TextUtils.isEmpty(iccid2) && !iccid2.equals("N/A")){
				int matchCountsim0 = getPhoneNumberMatchWithMcc(0);
				int matchCountsim1 = getPhoneNumberMatchWithMcc(1);
				Log.d("zhai","double card,matchCountsim0=" + matchCountsim0 + " matchCountsim1=" + matchCountsim1);
				return matchCountsim0 <= matchCountsim1 ? matchCountsim0:matchCountsim1;
			}else{
				if(!TextUtils.isEmpty(iccid1) && !iccid1.equals("N/A") && (TextUtils.isEmpty(iccid2) || iccid2.equals("N/A"))){
                	int matchCount = getPhoneNumberMatchWithMcc(0);
            		Log.d("zhai","single card one come in,matchCount =" + matchCount);
            		return matchCount;
				}
				if((TextUtils.isEmpty(iccid1) || iccid1.equals("N/A")) && !TextUtils.isEmpty(iccid2) && !iccid2.equals("N/A")){
                	int matchCount = getPhoneNumberMatchWithMcc(1);
            		Log.d("zhai","single card two come in,matchCount =" + matchCount);
            		return matchCount;
				}
				return 7;
			}
    ...
    private static int getPhoneNumberMatchWithMcc(int mSlotId) {
			String sim_imsi = getDefaultImsi(mSlotId);
	        String sim_gid1 = getDefaultGID1(mSlotId);
			String sim_spn = getDefaultEfspn(mSlotId);
			String sim_sst = getDefaultSST(mSlotId);
			String simNumeric = getSimMccMnc(mSlotId);
            /* HQ_guomiao 2015-10-09 modified for HQ01393571 begin */
            String sim_imsi_str = simNumeric + sim_imsi;
            String sim_gid1_str = simNumeric + "0x" + sim_gid1;
            String sim_spn_str = simNumeric + sim_spn;
            String sim_sst_str = simNumeric + "0x" + sim_sst;
            if (Build.TYPE.equals("eng")) {
                Log.d("zhai", "mSlotId="+mSlotId+";sim_imsi="+sim_imsi+";sim_gid1="+sim_gid1+
                        ";sim_spn="+sim_spn+"sim_sst="+sim_sst+"simNumeric="+simNumeric);
                Log.d("zhai","getPhoneNumberMatchWithMcc:sim_imsi_str=" + sim_imsi_str);
                Log.d("zhai","getPhoneNumberMatchWithMcc:sim_gid1_str=" + sim_gid1_str);
                Log.d("zhai","getPhoneNumberMatchWithMcc:sim_spn_str=" + sim_spn_str);
                Log.d("zhai","getPhoneNumberMatchWithMcc:sim_sst_str=" + sim_sst_str);
            }
            /* HQ_guomiao 2015-10-09 modified for HQ01393571 end */
            PhoneMatchOverride phoneMatchOverride = PhoneMatchOverride.getInstance();
			ArrayList<String> CarrierVmMapgetVirtualType = new ArrayList<String>(); 
			CarrierVmMapgetVirtualType = getVirtualType(mSlotId);
			if(CarrierVmMapgetVirtualType.get(0).equals("16")){
				Log.d("zhai","phy come in");
	            if(phoneMatchOverride.containsMccForPhoneMatch(simNumeric)) {
					Log.d("zhai","phy simNumeric come in");
	                String phoneMatch = phoneMatchOverride.getSphoneMatchWithMcc(simNumeric);
					Log.d("zhai","phy simNumeric phoneMatch=" + phoneMatch);
	                int minMatch = 7;
	                try {
						if(phoneMatch.indexOf(",")!= -1)
							phoneMatch = phoneMatch.substring(0,phoneMatch.indexOf(",")); 
	                    minMatch = Integer.parseInt(phoneMatch);
	                    Log.d(LOG_TAG, "getPhoneNumberMatchWithMcc || containsMccForPhoneMatch, matchCount = " + minMatch + ", simNumeric: "+simNumeric);
	                } catch(Exception e) {
	                    Log.d(LOG_TAG, "Exception, matchCount = " + MIN_MATCH + ", simNumeric: "+simNumeric);
	                    //return MIN_MATCH;
	                }
	                return minMatch;
	            }
			        ...
			        String phoneMatch="7";
					Log.d("zhai","vir come in");
				if(phoneMatchOverride.containsMccForVirPhoneMatch(sim_imsi_str))
	                {phoneMatch = phoneMatchOverride.getSVirphoneMatchWithMcc(sim_imsi_str);
					 Log.d("zhai","virimsi comein,phoneMatch=" + phoneMatch);}
				else if(phoneMatchOverride.containsMccForVirPhoneMatch(sim_gid1_str))
					{phoneMatch = phoneMatchOverride.getSVirphoneMatchWithMcc(sim_gid1_str);
					 Log.d("zhai","virgid1 comein,phoneMatch=" + phoneMatch);}
				else if(phoneMatchOverride.containsMccForVirPhoneMatch(sim_spn_str))
					{phoneMatch = phoneMatchOverride.getSVirphoneMatchWithMcc(sim_spn_str);
					 Log.d("zhai","virspn comein,phoneMatch=" + phoneMatch);}
				else if(phoneMatchOverride.containsMccForVirPhoneMatch(sim_sst_str))
					{phoneMatch = phoneMatchOverride.getSVirphoneMatchWithMcc(sim_sst_str);
					 Log.d("zhai","virsst comein,phoneMatch=" + phoneMatch);}
	                ...
	                try {
						if(phoneMatch.indexOf(",")!= -1)
							phoneMatch = phoneMatch.substring(0,phoneMatch.indexOf(",")); 
	                    minMatch = Integer.parseInt(phoneMatch);
	                    Log.d(LOG_TAG, "getPhoneNumberMatchWithMcc || containsMccForVirPhoneMatch, matchCount = " + minMatch + ", simNumeric: "+simNumeric);
	                } catch(Exception e) {
	                    Log.d(LOG_TAG, "Exception, matchCount = " + MIN_MATCH + ", simNumeric: "+simNumeric);
	                    //return MIN_MATCH;
	                }
            ...
            Log.d(LOG_TAG, "getPhoneNumberMatchWithMcc || !containsMccForPhoneMatch&VirPhoneMatch, matchCount = " + MIN_MATCH + ", simNumeric: "+simNumeric);
            return MIN_MATCH;
        }
	//add by zhaizhanfeng for number match at 150914 end
    /**
    ...
    public static String normalizeNumber(String phoneNumber) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (digit != -1) {
                sb.append(digit);
            } else if (sb.length() == 0 && c == '+' || (i == 0 && c == '*') || (i == 0 && c == '#') ) {
                sb.append(c);
     ...
     */
    public static boolean isEmergencyNumberInternal(String number,
                                                     String defaultCountryIso,
                                                     boolean useExactMatch) {
    ...
    private static boolean isEmergencyNumberInternal(int subId, String number,
        ...
        return isEmergencyNumberExt(number, PHONE_TYPE_GSM)
                  || isEmergencyNumberExt(number, PHONE_TYPE_CDMA);
    }
...
//add by huangshuo for HQ01308612 @20151015
    private static String[][] spnSepcialDisplay={  {"515039","51505","SUN"},
	                                                                           {"515034","51505","SUN"},
	                                                                           {"51505","51503","Sun"}};
     private static String[][] isCutomRoaming={  {"515039","51505"},
	                                                                            {"515034","51505"},
	                                                                            {"51505","51503"}};
    /*HQ_yulifeng Sepcial spn show for entity card
    Parameters:mSimOperatorNumeric : sim card mccmnc
                     strNumPlmn :Operator mccmnc
    return: networktitle     
    */
    public static String spnSepcialDisplay(String mSimOperatorNumeric,String strNumPlmn){
        Log.d(LOG_TAG,"ylf[spnSepcialDisplay]:mSimOperatorNumeric"+mSimOperatorNumeric+",strNumPlmn"+strNumPlmn);
        if(mSimOperatorNumeric ==null ||strNumPlmn==null ){
            return null;
        }
        return loadSpcialSpnConf(mSimOperatorNumeric,strNumPlmn);
    }
    //Modify by yulieng for HQ01308612 @20151021
    
    public static boolean  isCutomRoaming(String mSimOperatorNumeric,String strNumPlmn,boolean isRoaming,String sim_type){
        Log.d(LOG_TAG,"ylf[isCutomRoaming]:mSimOperatorNumeric:"+mSimOperatorNumeric+";strNumPlmn:"+strNumPlmn+";isRoaming:"+isRoaming+";sim_type;"+sim_type);
        if(mSimOperatorNumeric ==null ||strNumPlmn==null ){
            return isRoaming;
        }
        if("16".equals(sim_type)){//entity            
            return loadCutomRoamConfEntity(mSimOperatorNumeric,strNumPlmn,isRoaming);
        }else if("1".equals(sim_type)){//virtual imsi          
            return loadCutomRoamConfByImsi(mSimOperatorNumeric,strNumPlmn,isRoaming);              
        }else{
            return isRoaming;
        }
    }
    ...
    /*HQ_yulifeng for loadSpcialSpnConf,save to arraylist<string>
    mSimNumeric:simmccmnc
    mOperatorNumeric:operatormccmnc
    return:if mSimNumeric  mOperatorNumeric Match in PARTNER_SPEC_SPN_CONF_PATH,get the networktitle
    */
    public static String loadSpcialSpnConf( String mSimNumeric ,String mOperatorNumeric) {
        Log.d(LOG_TAG,"ylf[loadSpcialSpnConf]coming;mSimNumeric:"+mSimNumeric+";mOperatorNumeric:"+mOperatorNumeric);
        FileReader spnConfReader;
        final File spnConfFile = new File(PARTNER_SPEC_SPN_CONF_PATH);
        try {
            spnConfReader = new FileReader(spnConfFile);
        } catch (FileNotFoundException e) {
            Log.w(LOG_TAG, "Can't open " + PARTNER_SPEC_SPN_CONF_PATH);
            return null;
        }
        try {
            XmlPullParser parser = Xml.newPullParser();
            parser.setInput(spnConfReader);
            ...
            while (true) {
                ...
                if("spnshow".equals(name)){
                    String simNumeric = parser.getAttributeValue(null, "simNumeric");
                    String operatorNumeric = parser.getAttributeValue(null, "operatorNumeric");
                    Log.d(LOG_TAG,"ylf[loadSpcialSpnConf]simNumeric:"+simNumeric+";operatorNumeric:"+operatorNumeric);
                    if(simNumeric.equals(mSimNumeric) && operatorNumeric.equals(mOperatorNumeric)){
                        String networktitle = parser.getAttributeValue(null, "networktitle");
                        Log.d(LOG_TAG,"ylf[loadSpcialSpnConf]networktitle:"+networktitle);
                        return networktitle;
                    }else if(simNumeric.equals(mSimNumeric) && operatorNumeric.equals("")){
                        String networktitle = parser.getAttributeValue(null, "networktitle");
                        Log.d(LOG_TAG,"ylf[loadSpcialSpnConf]networktitle:"+networktitle);
                        return networktitle;
                    }
            ...
            Log.w(LOG_TAG, "Exception in XmlPullParserException " + e);
        } catch (IOException e) {
            Log.w(LOG_TAG, "Exception in XmlPullParserException " + e);
        }finally {
            try {
                if (spnConfReader != null) {
                    spnConfReader.close();
                }
            } catch (IOException e) {}
        }
	...
	/*HQ_yulifeng for loadCutomRoamConfEntity
    mSimNumeric:simcard mccmnc
    mOperatorNumeric:operator mccmnc
    return:if mSimNumeric  mOperatorNumeric Match in PARTNER_CUTOM_ROAMING_ENTITY_CONF_PATH
             true:roaming
             false:roaming white list
    */
    public static boolean loadCutomRoamConfEntity( String mSimNumeric ,String mOperatorNumeric,boolean isRoaming) {
        Log.d(LOG_TAG,"ylf[loadCutomRoamConfEntity]coming;mSimNumeric:"+mSimNumeric+";mOperatorNumeric:"+mOperatorNumeric+";isRoaming:"+isRoaming);
        FileReader roamConfReader;
        final File roamConfFile = new File(PARTNER_CUTOM_ROAMING_ENTITY_CONF_PATH);
        try {
            roamConfReader = new FileReader(roamConfFile);
        } catch (FileNotFoundException e) {
            Log.w(LOG_TAG, "Can't open " + PARTNER_CUTOM_ROAMING_ENTITY_CONF_PATH);
            return isRoaming;
        }
        try {
            XmlPullParser parser = Xml.newPullParser();
            parser.setInput(roamConfReader);
            XmlUtils.beginDocument(parser, "entity");
            while (true) {
                XmlUtils.nextElement(parser);
                String name = parser.getName();
                ...
                if("cutomroaming".equals(name)){
                    String simNumeric = parser.getAttributeValue(null, "simNumeric");
                    String operatorNumeric = parser.getAttributeValue(null, "operatorNumeric");
                    Log.d(LOG_TAG,"ylf[loadCutomRoamConfEntity]simNumeric:"+simNumeric+";operatorNumeric:"+operatorNumeric);
                    if(simNumeric.equals(mSimNumeric) && operatorNumeric.equals(mOperatorNumeric)){
                        return false;
                    }
                }
            ...
            try {
                if (roamConfReader != null) {
                    roamConfReader.close();
                }
            } catch (IOException e) {}
        }
        return isRoaming;
    ...
    /*HQ_yulifeng for loadCutomRoamConfByImsi  virtual
    mSimNumeric:simcard mccmnc sub 0-8
    mOperatorNumeric:operator mccmnc
    return:if mSimNumeric  mOperatorNumeric Match in PARTNER_CUTOM_ROAMING_IMSI_CONF_PATH
             true:roaming
             false:roaming white list
    */
    public static boolean loadCutomRoamConfByImsi( String mSimNumeric ,String mOperatorNumeric,boolean isRoaming) {
        Log.d(LOG_TAG,"ylf[loadCutomRoamConfByImsi]coming;mSimNumeric:"+mSimNumeric+";mOperatorNumeric:"+mOperatorNumeric+";isRoaming:"+isRoaming);
        FileReader roamConfReader;
        final File roamConfFile = new File(PARTNER_CUTOM_ROAMING_IMSI_CONF_PATH);
        try {
            roamConfReader = new FileReader(roamConfFile);
        } catch (FileNotFoundException e) {
            Log.w(LOG_TAG, "Can't open " + PARTNER_CUTOM_ROAMING_IMSI_CONF_PATH);
            return isRoaming;
        }
        try {
            XmlPullParser parser = Xml.newPullParser();
            ...
            while (true) {
                ...
                if("cutomroaming".equals(name)){
                    String simNumeric = parser.getAttributeValue(null, "simNumeric");
                    String operatorNumeric = parser.getAttributeValue(null, "operatorNumeric");
                    Log.d(LOG_TAG,"ylf[loadCutomRoamConfByImsi]simNumeric:"+simNumeric+";operatorNumeric:"+operatorNumeric);
                    if(mSimNumeric!=null && simNumeric!=null && operatorNumeric!=null&& mSimNumeric.startsWith(simNumeric) && operatorNumeric.equals(mOperatorNumeric)){
                        return false;
                    }
                }
            }
    ...
    public static boolean isSpecialEmergencyNumber(String dialString) {
      if(SystemProperties.get("ro.hq.custom.ecc").equals("1")){
   	 int slotId=getSlotId();
         ...
         e.fillInStackTrace();
         Log.i(LOG_TAG, "[isSpecialEmergencyNumber]" , e);
	String mccmnc = getSimMccMnc(slotId);
	if(mccmnc.equals(mccmncNoSimInserted)){
		 return false;
	 }
	  Log.d(LOG_TAG, "[isSpecialEccInner] dialString: " + dialString );
	  boolean isEcc = false;
	  String eccList = "";
	  String []emergencyNumList=null;
	   if(EccOverride.getInstance().hasVirtualKey(virtualKey)){	
			eccList = getSpecialVirtualEcc(virtualKey);
			Log.d(LOG_TAG, "[hasVirtualKey]:eccList ="  + eccList);
	   }else{														
			eccList = getSpecialEccWithSim(mccmnc);
			Log.d(LOG_TAG, "[hasNOVirtualKey]:eccList ="  + eccList);
	   }
	    if(eccList==null || eccList.length()==0){
		 emergencyNumList=defaultwithSIMEccList;
	    }else{	
		emergencyNumList = eccList.split(",");
	    }
     ...
     private static String getSpecialVirtualEcc(String virtualkey) {
		return EccOverride.getInstance().getSpecialEccWithSim(virtualkey);
    }
     private static String getSpecialEccWithSim(String mccmnc) {
		return EccOverride.getInstance().getSpecialEccWithSim(mccmnc);
    }
    /**
     * Checks a given number against the list of
     * emergency numbers provided by the RIL and SIM card.
    ...
    public static boolean isEmergencyNumberExt(String number, int phoneType) {
       ...
       //RuntimeException e = new RuntimeException("leon is here");
       //e.fillInStackTrace();
       //Log.i(LOG_TAG, "[isEmergencyNumberExt]", e);
       if (Build.TYPE.equals("eng")) {
           Log.i(LOG_TAG,"[isEmergencyNumberExt], number:" + number + ", phoneType:" + phoneType+"slotId"+slotId );
       }
        if (number == null) {
            return false;
        }
        ...
        if (PHONE_TYPE_CDMA == phoneType) {
        	Log.i(LOG_TAG, "sIsC2kSupport " + sIsC2kSupport);
            if (!sIsC2kSupport) {
                return false;
            }
            ...
            Log.i(LOG_TAG, "[isEmergencyNumberExt] cdma.ril.ecclist: " + numbers);
            if (!TextUtils.isEmpty(numbers)) {
                ...
                for (String emergencyNum : numbers.split(",")) {
                    ...
                    if (emergencyNum.equals(number) || numberPlus.equals(number)) {
                        Log.i(LOG_TAG, "[isEmergencyNumberExt] match cdma.ril.ecclist");
                        return true;
                    }
                }
                bSIMInserted = true;
            ...
            Log.i(LOG_TAG,"[isEmergencyNumberExt] cdma.ril.ecclist1: " + numbers);
            if (!TextUtils.isEmpty(numbers)) {
                // searches through the comma-separated list for a match,
                // return true if one is found.
                for (String emergencyNum : numbers.split(",")) {
                    numberPlus = emergencyNum + "+";
                    if (emergencyNum.equals(number) || numberPlus.equals(number)) {
                        Log.i(LOG_TAG, "[isEmergencyNumberExt] match cdma.ril.ecclist1");
                        return true;
                    }
                }
                bSIMInserted = true;
            ...
            if (!TextUtils.isEmpty(numbers)) {
                ...
                for (String emergencyNum : numbers.split(",")) {
                    numberPlus = emergencyNum + "+";
                    if (emergencyNum.equals(number) || numberPlus.equals(number)) {
                       Log.i(LOG_TAG, "[isEmergencyNumberExt] match cdma.ril.ecclist2");
                        return true;
                    }
                }
                bSIMInserted = true;
            ...
            if (!TextUtils.isEmpty(numbers)) {
                ...
                for (String emergencyNum : numbers.split(",")) {
                    numberPlus = emergencyNum + "+";
                    if (emergencyNum.equals(number) || numberPlus.equals(number)) {
                       Log.i(LOG_TAG, "[isEmergencyNumberExt] match cdma.ril.ecclist3");
                        return true;
                    }
                }
                bSIMInserted = true;
            }
			Log.i(LOG_TAG, "bSIMInserted=" + bSIMInserted);

            // 2. Check ECCs customized by user
            // Now CDMA doesn't care if the SIM is inserted or not
            // It's for future usage
            if(SystemProperties.get("ro.hq.custom.ecc").equals("1")){
                 Log.d(LOG_TAG, "[isEmergencyNumber] slotId: " + slotId);
		 String mccmnc = getSimMccMnc(slotId);
		  Log.d(LOG_TAG, "[isEmergencyNumber] Operator Mccmnc_cdma: " + mccmnc);
		 if(mccmnc.equals(mccmncNoSimInserted)){
		       mccmnc = getOperatorMccmnc();
		        Log.d(LOG_TAG, "[isEmergencyNumber] Operator Mccmnc: " + mccmnc);
		       String tmp = getEccWithoutSim(mccmnc);
		       String []emergencyNumList = defaultwithoutSIMEccList;

			   // / Modified by guofeiyao 2015/12/28
			   // For no-card ecc number
			   
		       // if(tmp!=null){
                   /* HQ_guomiao add for HQ01453081 begin*/
			   /*
                   String[] tmpNumList = tmp.split(",");
                   String[] newemergencyNumList = new String[emergencyNumList.length + tmpNumList.length];
                   System.arraycopy(tmpNumList, 0, newemergencyNumList, 0, tmpNumList.length);
                   System.arraycopy(emergencyNumList, 0, newemergencyNumList, tmpNumList.length, emergencyNumList.length);
                   emergencyNumList = newemergencyNumList;
                   */
                   /* HQ_guomiao add for HQ01453081 end*/
			 //}

			   if ( null==tmp || 0==tmp.length() ) {
                               emergencyNumList = defaultwithoutSIMEccList;
			   } else {
                                String[] tmpNumList = tmp.split(",");
			       // emergencyNumList = tmpNumList;
			      //add by huangshuo for HQ01747094 on 2016/02/16
                               emergencyNumList = new String[tmpNumList.length + defaultwithSIMEccList.length];
                               System.arraycopy(tmpNumList,0,emergencyNumList,0,tmpNumList.length);
                               System.arraycopy(defaultwithSIMEccList,0,emergencyNumList,tmpNumList.length,defaultwithSIMEccList.length);
			     //  End  by huangshuo for HQ01747094 on 2016/02/16
			   }
			   // / End
					
			for (int i = 0; i < emergencyNumList.length; i++) {
		     ...
		     initEccPatternAndKey(slotId);
		     Log.i(LOG_TAG,"virtualKey_CDMA"+EccOverride.getInstance().hasVirtualKey(virtualKey));
		  if(EccOverride.getInstance().hasVirtualKey(virtualKey)){	// virtual Ecc
		            plusNumber = getEccWithSim(virtualKey);
			    Log.i(LOG_TAG,"[hasVirtualKey_CDMA]:plusNumber"+plusNumber);
			    return isCustomizedEmergencyNumberExt(number,plusNumber,numberPlus);
		   }else{														
			   plusNumber = getEccWithSim(mccmnc);// Normal Ecc
			    return isCustomizedEmergencyNumberExt(number,plusNumber,numberPlus);
		   } 
	      }
        }else{
                    ...
                    for (EccEntry eccEntry : sCdmaCustomizedEccList) {
                        if (!eccEntry.getCondition().equals(EccEntry.ECC_NO_SIM)) {
                            ...
                            if (ecc.equals(number) || numberPlus.equals(number)) {
                              Log.i(LOG_TAG, "[isEmergencyNumberExt] match"
                                        + " customized ecc list");
                                return true;
                            }
                        }
           ...
           Log.i(LOG_TAG, "[isEmergencyNumberExt] no match");
            return false;
         }
        } else {
            ...
            // Strip the separators from the number before comparing it
            // to the list.
            /*HQ_guomiao add for HQ01496702 begin*/
            number = extractNetworkPortionAlt(number).replaceAll(" ", "");
            if (Build.TYPE.equals("eng")) Log.d(LOG_TAG, "replaceAll number:" + number);
            /*HQ_guomiao add for HQ01496702 end*/
            ...
            for (String emergencyNum : mHashMapForNetworkEccCategory.keySet()) {
                ...
                if (emergencyNum.equals(number)
                     || numberPlus.equals(number)) {
                    Log.i(LOG_TAG, "[isEmergencyNumberExt] match network ecc list");
                    return true;
                }
            }
            ...
            Log.i(LOG_TAG, "[isEmergencyNumberExt] ril.ecclist: " + numbers);
            if (!TextUtils.isEmpty(numbers)) {
                // searches through the comma-separated list for a match,
                // return true if one is found.
                for (String emergencyNum : numbers.split(",")) {
                    ...
                    if (emergencyNum.equals(number)
                         || numberPlus.equals(number)) {
                        Log.i(LOG_TAG, "[isEmergencyNumberExt] match ril.ecclist");
                        return true;
                    }
                }
                bSIMInserted = true;
           ...
           Log.i(LOG_TAG, "[isEmergencyNumberExt] ril.ecclist1: " + numbers);
            if (!TextUtils.isEmpty(numbers)) {
                // searches through the comma-separated list for a match,
                // return true if one is found.
                for (String emergencyNum : numbers.split(",")) {
                    ...
                    if (emergencyNum.equals(number)
                         || numberPlus.equals(number)) {
                        Log.i(LOG_TAG, "[isEmergencyNumberExt] match ril.ecclist1");
                        return true;
                    }
                }
                bSIMInserted = true;
            ...
            if (!TextUtils.isEmpty(numbers)) {
                ...
                for (String emergencyNum : numbers.split(",")) {
                    ...
                    if (emergencyNum.equals(number)
                         || numberPlus.equals(number)) {
                       Log.i(LOG_TAG, "[isEmergencyNumberExt] match ril.ecclist2");
                        return true;
                    }
                }
                bSIMInserted = true;
            ...
            if (!TextUtils.isEmpty(numbers)) {
                ...
                for (String emergencyNum : numbers.split(",")) {
                    ...
                    if (emergencyNum.equals(number)
                         || numberPlus.equals(number)) {
                        Log.i(LOG_TAG, "[isEmergencyNumberExt] match ril.ecclist3");
                        return true;
                    }
                }
                bSIMInserted = true;
          ...
          if(SystemProperties.get("ro.hq.custom.ecc").equals("1")){
		String mccmnc = getSimMccMnc(slotId);
		 Log.i(LOG_TAG,"mccmnc_gsm"+mccmnc);
		 if(mccmnc.equals(mccmncNoSimInserted)){
		       mccmnc = getOperatorMccmnc();
		       Log.d(LOG_TAG, "[isEmergencyNumber_gsm] Operator Mccmnc: " + mccmnc);
		       String tmp = getEccWithoutSim(mccmnc);
		       String []emergencyNumList = defaultwithoutSIMEccList;

			   // / Modified by guofeiyao 2016/1/4
			   // For no-card ecc number
			 ...
			 //}
			   ...
			   if ( null==tmp || 0==tmp.length() ) {
			   	//Log.e("duanze", "null==tmp || 0==tmp.length()");
                                 emergencyNumList = defaultwithoutSIMEccList;
			   } else {
			   //Log.e("duanze", "else");
                                 String[] tmpNumList = tmp.split(",");
		  ...
		  if(EccOverride.getInstance().hasVirtualKey(virtualKey)){	// virtual Ecc
		            plusNumber = getEccWithSim(virtualKey);
			   Log.i(LOG_TAG,"[hasVirtualKey_GSM]:plusNumber"+plusNumber);
			    return isCustomizedEmergencyNumberExt(number,plusNumber,numberPlus);
		   }else{														
			   plusNumber = getEccWithSim(mccmnc);// Normal Ecc
			   Log.i("huangshuo","huangshuo"+isCustomizedEmergencyNumberExt(number,plusNumber,numberPlus));
			   return isCustomizedEmergencyNumberExt(number,plusNumber,numberPlus);
		   } 
	      }
        	}else{
                    ...
                    for (EccEntry eccEntry : mCustomizedEccList) {
                        if (!eccEntry.getCondition().equals(EccEntry.ECC_NO_SIM)) {
                            ...
                            if (ecc.equals(number)
                                 || numberPlus.equals(number)) {
                                Log.i(LOG_TAG, "[isEmergencyNumberExt] match" + " customized ecc list");
                                return true;
                            }
                        }
                    }
    ...
    public static String getOperatorMccmnc() {
		String mccmnc = mccmncNoSimInserted;
		ITelephonyEx iTel =ITelephonyEx.Stub.asInterface(ServiceManager.getService(Context.TELEPHONY_SERVICE_EX));
		if(iTel == null){
		   Log.i("huangshuo","TELEPHONY_SERVICE_EX is not ready or is null");
			  return null;}
		try{
                   mccmnc = iTel.getDefaultLocatedPlmn(); 
		}catch (RemoteException e){
		          return null;
		      
		}

		if(mccmnc!=null && mccmnc.length()>0 && mccmnc.length()<7){
			return mccmnc;
		}
		    return mccmncNoSimInserted;
	       }
	//add by huangshuo on 2015/9/14
     public  static int getSlotId(){
          if (!hasIccCard(0) && !hasIccCard(1)){
    		Log.d("huangshuo", "has no sim on slot");
    		return -1;
    	  }
	  int slotId=-1;
	  ...
	  if (SubscriptionManager.isValidSubscriptionId(subId)){
		  ...
		  if(SubscriptionManager.isValidSlotId(slotId)){
		     ...
		     return 1	;	
	          ...
	          if (hasIccCard(0)){
	  	     Log.i("huangshuo","[getSlotId]:slotId(0)");
		    return 0;
	         }else{
		     return 1	;	
	         }	
	...
	//end by huangshuo on 2015/9/14
	
	 /*HQ_huangshuo 20150911 modify for emergency call start*/
       private static int getSubIdBySlot(int slotId) {
            ...
            Log.d("huangshuo", "getSubIdBySlot: sub id = " + subid   + "sim Slot = " + slotId);
            return subid;
           }
       /*HQ_huangshuo 20150911 modify for emergency call end  */
             private static String getEccWithoutSim(String mccmnc) {
		return EccOverride.getInstance().getEccWithoutSim(mccmnc);
            }
     ...
     private static ITelephony getITelephony() {
	   ...
	   public static boolean isCustomizedEmergencyNumberExt(String number, String plusNumber, String numberPlus) {
                String []emergencyNumList = null;
        /* HQ_guomiao 2015-10-09 modified for HQ01393571 begin */
        if (Build.TYPE.equals("eng")) {
            Log.i("huangshuo", "[isCustomizedEmergencyNumberExt]:number" + number + ", plusNumber="
                    + plusNumber + ",numberPlus=" + numberPlus);
        }
        /* HQ_guomiao 2015-10-09 modified for HQ01393571 end */
	 	if(plusNumber==null||plusNumber.length()<=0){
			emergencyNumList = defaultwithSIMEccList;
		}else{
			emergencyNumList = plusNumber.split(",");
		}
		...
		for (int i=0;i<emergencyNumList.length;i++) {
			...
			|| numberPlus.equals(number)) {
             ...
             private static String getEccWithSim(String mccmnc) {
		return EccOverride.getInstance().getEccWithSim(mccmnc);
            }
	    private static String getVirtualEcc(String virtualkey) {
		return EccOverride.getInstance().getEccWithSim(virtualkey);
            }
	

    /**
	 * get Virtual Type & value 
	 * @author lipeng
	 * @param slotId	
	 * @return ArrayList<String> 
	 * 			<0> : Virtual_Type
	 * 			<1>	: value	(mccmnc+value)
	 * 			<1>	: mccmnc
	 * 
	 *  Virtual_Type & value
	 *   error = -1
	 * 	VIRTUAL_PATTERN_IMSI = 1; 	value = imsipattern 
	 *  VIRTUAL_PATTERN_GID1 = 2;   value = mccmnc+gid1
	 *  VIRTUAL_PATTERN_SPN = 4;	value = mccmnc+efspn
	 *  VIRTUAL_PATTERN_EFFILE = 8;	value = mccmnc+efsst
	 *  PATTERN_MCCMNC = 16;		value = mccmnc
	 */
	public static ArrayList<String> getVirtualType(int slotId){
		
		ArrayList<String> tmpList = new ArrayList<String>(); 
		initEccPatternAndKey(slotId);
		Log.d("al813_slotId", "[slotId] = " + slotId);
		Log.d("al813_virtualPattern", "[virtualPattern_1] = " + virtualPattern_1);
		Log.d("al813_virtualValue", "[virtualValue_1] = " + virtualValue_1);
		Log.d("al813_virtualPattern", "[virtualPattern_2] = " + virtualPattern_2);
		Log.d("al813_virtualValue", "[virtualValue_2] = " + virtualValue_2);
		if(slotId==0){
			tmpList.add(""+virtualPattern_1);
			tmpList.add(virtualValue_1);
			tmpList.add(mccmncSim_1);
		}else if(slotId==1){
			tmpList.add(""+virtualPattern_2);
			tmpList.add(virtualValue_2);
			tmpList.add(mccmncSim_2);
		}else{
			tmpList.add("-1");
			tmpList.add("");
			tmpList.add("999999");
	...
	//add by huangshuo for init sim virtual mccmnc on 2015/09/14
	private static void initEccPatternAndKey(int slotId) {
		if(slotId==0){
		   if(virtualValue_1!=null && virtualValue_1.length()>0  && !virtualValue_1.equals("999999")
			&& (virtualPattern_1==VIRTUAL_PATTERN_IMSI || virtualPattern_1==VIRTUAL_PATTERN_GID1
			|| virtualPattern_1==VIRTUAL_PATTERN_SPN || virtualPattern_1==VIRTUAL_PATTERN_EFFILE
			|| virtualPattern_1==PATTERN_MCCMNC)){
			return;
		   }
		}else if(slotId==1){
		       if(virtualValue_2!=null && virtualValue_2.length()>0  && !virtualValue_2.equals("999999")
			&& (virtualPattern_2==VIRTUAL_PATTERN_IMSI || virtualPattern_2==VIRTUAL_PATTERN_GID1
			|| virtualPattern_2==VIRTUAL_PATTERN_SPN || virtualPattern_2==VIRTUAL_PATTERN_EFFILE
			|| virtualPattern_2==PATTERN_MCCMNC)){
			return;
		   }
		}
		 int tmpVirtualPattern = PATTERN_MCCMNC;
		String tmVirtualKey	  = null;
		String mccmnc = getSimMccMnc(slotId);
		if(!EccOverride.getInstance().hasVirtualMccmnc(mccmnc)){
			setVirtualPatternAndValue(slotId,PATTERN_MCCMNC,mccmnc);
			return; 
		}

	       if(SystemProperties.get("ro.hq.virtual.ecc").equals("1")){
			String imsi = getDefaultImsi(slotId);
			String imsipattern = EccOverride.getInstance().getVirtualKeyByImsi(imsi);
			if(imsipattern!=null){
			       setVirtualPatternAndValue(slotId,VIRTUAL_PATTERN_IMSI,imsipattern);
				return;
			}
		}
	          if(SystemProperties.get("ro.hq.virtual.ecc").equals("1")){
			String gid1 = getDefaultGID1(slotId);
			if(EccOverride.getInstance().hasVirtualKey(mccmnc+gid1)){
				setVirtualPatternAndValue(slotId,VIRTUAL_PATTERN_GID1,mccmnc+gid1);
				return;
			}	
		}
                if(SystemProperties.get("ro.hq.virtual.ecc").equals("1")){
			String efspn = getDefaultEfspn(slotId);
			if(EccOverride.getInstance().hasVirtualKey(mccmnc+efspn)){
				setVirtualPatternAndValue(slotId,VIRTUAL_PATTERN_SPN,mccmnc+efspn);
				return;
			}
	       }
            if(SystemProperties.get("ro.hq.virtual.ecc").equals("1")){
			String efsst = getDefaultSST(slotId);
			if(EccOverride.getInstance().hasVirtualKey(mccmnc+efsst)){
				setVirtualPatternAndValue(slotId,VIRTUAL_PATTERN_EFFILE,mccmnc+efsst);
				return;
			}
		}
		virtualPattern = PATTERN_MCCMNC;
		virtualKey = mccmnc;
		setVirtualPatternAndValue(slotId,PATTERN_MCCMNC,mccmnc);
            }
	private static void setVirtualPatternAndValue(int slotId, int pattern, String value) {
		if(slotId==0){
			virtualPattern_1 = pattern;
			virtualValue_1 = value;
			virtualPattern = pattern;
			virtualKey = value;
		}else if(slotId==1){
		        virtualPattern_2 = pattern;
			virtualValue_2 = value;
			virtualPattern = pattern;
			virtualKey = value;
		}
      }
	// end by huangshuo for  init sim virtual mccmnc on 2015/09/14
	
	 public static String getSimMccMnc(int slotId) {
	 	   Log.i("huangshuo","[getSimMccMnc]:slotId"+slotId);
		   String codeMNCMCC = mccmncNoSimInserted;
			String mccmnc_temp = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC);
			if ((mccmnc_temp != null) && (mccmnc_temp.length() > 0)) {
                          String values[] = mccmnc_temp.split(",");
			  for(int i=0;i<values.length;i++){
                           if ((slotId >= 0) && (slotId < values.length) && (values[slotId] != null)) {
                               codeMNCMCC = values[slotId];
                           }
                      }
                    }
	           Log.i("huangshuo","[getSimMccMnc]:codeMNCMCC1"+codeMNCMCC);
		if(codeMNCMCC==null || codeMNCMCC.length()<5){
			codeMNCMCC = mccmncNoSimInserted;
		}
		return codeMNCMCC;
	      }
	...
	public static String getDefaultImsi(int slotId) {
		String tmpImsi = "";
	        String imsiTmp = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_IMSI);
		if ((imsiTmp != null) && (imsiTmp.length() > 0)) {
			String values[] = imsiTmp.split(",");
			for(int i=0;i<values.length;i++){
                           if ((slotId >= 0) && (slotId < values.length) && (values[slotId] != null)) {
                               tmpImsi = values[slotId];
                           }
			}
		}
        /* HQ_guomiao 2015-10-12 modified for HQ01393571 begin */
        if (Build.TYPE.equals("eng")) {
            Log.d(LOG_TAG,"[getDefaultImsi] slotId = " + slotId + ",    tmpImsi = "+ tmpImsi);
        }
        /* HQ_guomiao 2015-10-12 modified for HQ01393571 begin */
		return tmpImsi;
	}

	public static String getDefaultGID1(int slotId) {
		String tmpGID1 = "";
		 if (slotId >= 0 && slotId < SIM_RECORDS_PROPERTY_GID1_LIST.length)  {
		  	   tmpGID1 = SystemProperties.get(SIM_RECORDS_PROPERTY_GID1_LIST[slotId]);
			    if(tmpGID1 !=null){
					return tmpGID1;
			    }
		  }
                Log.d("huangshuo","[getDefaultGID1] slotId = " + slotId + "tmpGID1 = "+ tmpGID1);
		return tmpGID1;
	}
	
        private static String[] SIM_RECORDS_PROPERTY_GID1_LIST = {
		"ril.gid1.sim1",
		"ril.gid1.sim2",
		"ril.gid1.sim3",
		"ril.gid1.sim4",
	};
	public static String getDefaultEfspn(int slotId) {
		String tmpSpn = "";
		String  SpnTmp = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_ALPHA);
		if ((SpnTmp != null) && (SpnTmp.length() > 0)) {
			String values[] = SpnTmp.split(",");
			for(int i=0;i<values.length;i++){
                             if ((slotId >= 0) && (slotId < values.length) && (values[slotId] != null)) {
                                  tmpSpn = values[slotId];
                             }
			}
		}
		Log.d("huangshuo","[getDefaultEfspn] slotId = " + slotId + ",	tmpSpn = "+ tmpSpn);
		
		return tmpSpn;
	}
	public static String getDefaultSST(int slotId) {
		String tmpSST = "";
		if(slotId==0){
			tmpSST = SystemProperties.get("ril.ef.sst.sim1");
			Log.d("zl","MMS tmpSST end: "+ tmpSST);
		}else if(slotId==1){
			tmpSST = SystemProperties.get("ril.ef.sst.sim2");
		}else{
			tmpSST = "";
		}
		return tmpSST;
    ...
    private static void parseEccList() {
        ...
        Rlog.d(LOG_TAG, "parseEccList, mCustomizedEccList:" + mCustomizedEccList);
	...
	public static String getSimStatusForSlot(int slotId){
                    String strAllSimState = "";
                    String strCurSimState = "UNKNOW";
		...
		return strCurSimState;
	}
	

	public static String getOperatorMccMncForSlot(int slotId,Context context){
		String mccMnc=null;
		TelephonyManager telephonyManager = (TelephonyManager)context.getSystemService(Context.TELEPHONY_SERVICE);
		if(slotId==0){
			mccMnc = telephonyManager.getNetworkOperatorForSubscription(getSubIdBySlot(0));
		}else if(slotId==1){
			mccMnc = telephonyManager.getNetworkOperatorForSubscription(getSubIdBySlot(1));			
		}
		return mccMnc;
	}

	public static boolean getStatesAirplaneMode(Context context) {
