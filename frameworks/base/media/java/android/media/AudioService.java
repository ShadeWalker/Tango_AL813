...
public class AudioService extends IAudioService.Stub {
    ...
    private static final boolean VOLUME_SETS_RINGER_MODE_SILENT = true; //wuhuihui modified for HQ01176554

    /** In silent mode, are volume adjustments (raises) prevented? */
    private static final boolean PREVENT_VOLUME_ADJUSTMENT_IF_SILENT = false;//wuhuihui modified for HQ01176554
   ...
   /// M: Modify the max stream volume @{
    ...
    /// M:add for identify mhl link smartbook or TV
    private static final String Params_SmartBook = "mhl2smartbook=1";

    /// M: Add for feature option @{
    private static final boolean IS_SMARTBOOK_SUPPORT = SystemProperties.getBoolean("ro.mtk_smartbook_support", false);
    private static final boolean IS_DOLBY_DAP_SUPPORT = SystemProperties.getBoolean("ro.mtk_dolby_dap_support", false);
    /// @}
    ...
    private void adjustStreamVolume(int streamType, int direction, int flags,
        ...
        if (adjustVolume && (direction != AudioManager.ADJUST_SAME)) {
            ...
            //HQ_pangxuhui modify 20151123 for bug HQ01507360 
            if ((direction == AudioManager.ADJUST_RAISE) &&
                    !checkSafeMediaVolume(streamTypeAlias, aliasIndex + step, device) && !SystemProperties.getBoolean("ro.hq.ce.safevloume.not.support", false)) {
            //HQ_pangxuhui modify end 
                Log.e(TAG, "adjustStreamVolume() safe volume index = "+oldIndex);
              //HQ01309015 zhouyoukun start
            
              if(1 != Settings.System.getInt(mContext.getContentResolver(),"sys.factorytest.status",0)) {                 
    ...
    private void sendVolumeUpdate(int streamType, int oldIndex, int index, int flags) {
        ...
        if (mStreamVolumeAlias[streamType] == AudioSystem.STREAM_RING) {
            ...
            android.os.SystemProperties.set("persist.sys.bv","0."+index);// HQ_gepengfei added to adjust bootaudio's volume
        }
        }
    }
    ...
    public void setStreamMute(int streamType, boolean state, IBinder cb) {
        ...
        if (isStreamAffectedByMute(streamAlias)) {
            ...
            for (int stream = 0; stream < mStreamStates.length; stream++) {
                /*add by lihaizhou for SoundRecorder HQ01268613 by begin */
                if (streamAlias == mStreamVolumeAlias[stream]) {
                    if (AudioSystem.STREAM_RING == stream && 
                            AudioSystem.STREAM_RING != streamType) { 
                        Log.i(TAG, "Do not mute ring stream for " + streamType); 
                        continue; 
                    } 
                /*add by lihaizhou for SoundRecorder HQ01268613 by end */
                    mStreamStates[stream].mute(cb, state);
        ...
        try {
            synchronized (mSettingsLock) {
                ...
                if (external) {
                    ...
                    //hanchao change HQ01409005 begin 2015.9.28
                    
                    /*if (mRingerModeDelegate != null) {
                        ringerMode = mRingerModeDelegate.onSetRingerModeExternal(ringerModeExternal,
                                ringerMode, caller, ringerModeInternal);
                        Log.d(TAG, "-------------mRingerModeDelegate != null-----------------" + ringerMode);
                    }*/
                    //hanchao change HQ01409005 end  2015.9.28
    ...
    private void setRingerModeExt(int ringerMode) {
        ...
        //xuqian HQ01268255 RINGER_MODE Match AUDIOPROFILE start
...
//        mProfileManager = (AudioProfileManager) mContext.getSystemService(Context.AUDIO_PROFILE_SERVICE);
        /*HQ_yuankangbo 2015-08-14 modify for switch to outdoor mode*/
//        mProfileManager.setActiveProfile(mProfileManager.getActiveProfileKey());
        /* HQ_xuqian4 2015-9-16 modified for HQ01386988 Silent mode appears crash start*/
//        if(ringerMode==0){
//	        mProfileManager.setActiveProfile("mtk_audioprofile_silent");
//        }
        /* HQ_xuqian4 2015-9-16 modified for HQ01386988 Silent mode appears crash end*/
        //xuqian HQ01268255 RINGER_MODE Match AUDIOPROFILE end
    }
    ...
    private int getDeviceForStream(int stream) {
		...
		int isReceiverTest = Settings.System.getInt(mContext.getContentResolver(),"sys.factorytest.receiver",0);
		...
		Log.d(TAG, "isReceiverTest = " + isReceiverTest);
		if(isReceiverTest == 1){
            ...
            if (isConnected && !connected) {
             ...
             Log.d(TAG, "handleDeviceConnection remove:" + "connected:" + connected + ",device:" + device);
                ...
                AudioSystem.setDeviceConnectionState(device, AudioSystem.DEVICE_STATE_AVAILABLE,
                        whetherSmartBookConnected(device) ? Params_SmartBook : params);
    ...
    private void onSetWiredDeviceConnectionState(int device, int state, String name)
    {
        ...
        Log.d(TAG, "onSetWiredDeviceConnectionState:" + "device:" + device + ",state:" + state);
            ...
            if (state != 0) {
                ...
                /// M: When connect smartbook,set the HDMI to not fixed volume device @{
                if (whetherSmartBookConnected(device)) {
                    Log.d(TAG, "connect smartbook");
                    mFixedVolumeDevices &= ~AudioSystem.DEVICE_OUT_HDMI;
                }
                /// @}
            } else {
                ...
                /// M: When disconnect smartbook,set the HDMI to fixed volume device @{
                if ((device == AudioSystem.DEVICE_OUT_HDMI) && !whetherSmartBookConnected(device)) {
                    Log.d(TAG, "disconnect smartbook");
                    mFixedVolumeDevices |= AudioSystem.DEVICE_OUT_HDMI;
                }
                /// @}
            }
				//HQ_zhangteng modified for Smart Earphone Control at 2015-09-09
           // if (!isUsb && (device != AudioSystem.DEVICE_IN_WIRED_HEADSET)) {
		if(!isUsb){
                sendDeviceConnectionIntent(device, state, name, shouldNotify);
        /*< DTS2014091300536 tiandazhang/00107790 20140913 begin */
          sendDeviceConnectionIntentForImcs(device, state, name);
		Log.i(TAG,"if(!isUsb)->sendDeviceConnectionIntentForImcs(device, state, name);");
            /* DTS2014091300536 tiandazhang/00107790 20140913 end >*/
            }
            /**
             * @}
             */
      ...
      protected void sendDeviceConnectionIntentForImcs(int device, int state, String name) {
      			Log.i(TAG,"hook sendDeviceConnectionIntentForImcs");
        //hook
      }
  /* DTS2014091300536 tiandazhang/00107790 20140913 end >*/
    ...
    */
    ...
    /// M: Add for indentify mhl is connecting to smartbook and TV @{
    private boolean whetherSmartBookConnected(int device) {
        boolean isSmartBookConnected = false;
        if (IS_SMARTBOOK_SUPPORT && device == AudioSystem.DEVICE_OUT_HDMI) {
            IMtkHdmiManager hdmiManager = IMtkHdmiManager.Stub.asInterface(ServiceManager
                    .getService(Context.HDMI_SERVICE));
            if (null == hdmiManager) {
                Log.d(TAG, "hdmiManager is null");
                return false;
            }
            try {
                Log.d(TAG, "displaytype:" + hdmiManager.getDisplayType());
                if (hdmiManager.getDisplayType() == HdmiDef.DISPLAY_TYPE_SMB) {
                    isSmartBookConnected = true;
                } else {
                    isSmartBookConnected = false;
                }
            } catch (RemoteException e) {
                Log.e(TAG, "getDisplayType RemoteException");
                return false;
            }
        }
