...
*/
    ...
    private static Ringtone getRingtone(final Context context, Uri ringtoneUri, int streamType) {
        try {
            ...
            if (ringtoneType != -1) {
                ...
                /*if (ringtoneUri != null && validRingtoneUri(context, ringtoneUri) == -1) {
                    Log.d(TAG, "cannot found ringtone uri from MediaProvider, so return the Default uri.");
    ...
    public static int validRingtoneUri(Context context, Uri ringtoneUri, int type) {
        final RingtoneManager rm = new RingtoneManager(context);
        rm.setType(type);
    ...
    public static Uri getActualDefaultRingtoneUri(Context context, int type) {
        ...
        if(uriString == null){
            Log.i(TAG, "in getActualDefaultRingtoneUri type=" + type + " setting=" + setting + " uriString=" + uriString);
            return null;
        }

        Cursor c = null;
        try {
            c = context.getContentResolver().query(Uri.parse(uriString), null, null,
                    null, null);
            if (c != null && c.getCount() > 0) {
                Log.i(TAG, "Get actual default ringtone uri= " + (uriString!=null ? uriString : null));
            } else {
            	Uri defaultRingtoneUri = getDefaultRingtoneUri(context, type);
				//add by wangjing3 for HQ01710929 2016-02-23 begin
				StorageManager mStorageManager = null;
				mStorageManager = (StorageManager) context.getSystemService(Context.STORAGE_SERVICE);
				String mState = mStorageManager.getVolumeState("/storage/sdcard1");
				if(!mState.equals(Environment.MEDIA_SHARED)){
					setActualDefaultRingtoneUri(context, type, defaultRingtoneUri);
				}
				//add by wangjing3 for HQ01710929 2016-02-23 end
    ...
    public static Uri getDefaultRingtoneUri(Context context, int type) {
        ...
        switch (type) {
			...
			case TYPE_RINGTONE_SIM2://add by wuhuihui
                uriString = Settings.System.getString(resolver, KEY_DEFAULT_RINGTONE_SIM2);//add by wuhuihui
                break;//add by wuhuihui
