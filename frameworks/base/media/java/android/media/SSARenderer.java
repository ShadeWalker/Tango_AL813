...
/** @hide */
public class SSARenderer extends Renderer {
    ...
    public SSARenderer(Context context) {
        this(context, null);
    }

    SSARenderer(Context mContext, Handler mEventHandler) {
    ...
    public boolean supports(MediaFormat format) {
        if (format.containsKey(MediaFormat.KEY_MIME)) {
            if (!format.getString(MediaFormat.KEY_MIME)
                    .equals(MediaPlayer.MEDIA_MIMETYPE_TEXT_SUBSSA)) {
                return false;
            };
    ...
    public SubtitleTrack createTrack(MediaFormat format) {
        ...
        if (mRender) {
            return new SSATrack(mRenderingWidget, format);
        } else {
            return new SSATrack(mEventHandler, format);
        }
    }
}

class SSATrack extends WebVttTrack {
    ...
    private static final String TAG = "SSATrack";
    private static final boolean DEBUG = Log.isLoggable(TAG, Log.DEBUG);
    private final Handler mEventHandler;

    SSATrack(WebVttRenderingWidget renderingWidget, MediaFormat format) {
        super(renderingWidget, format);
        mEventHandler = null;
    }

    SSATrack(Handler eventHandler, MediaFormat format) {
        super(null, format);
    ...
    private static long parseMs(String in) {
        if (DEBUG) Log.d(TAG, "parseMs in String is "+in);
        long hours = Long.parseLong(in.split(":")[0].trim());
        long minutes = Long.parseLong(in.split(":")[1].trim());
        long seconds = Long.parseLong(in.split(":")[2].split("\\.")[0].trim());
        long millies = Long.parseLong(in.split(":")[2].split("\\.")[1].trim());
        if (DEBUG) Log.d(TAG, "hours: "+hours+" minutes: "+minutes+" seconds: "+seconds+" millies: "+millies);
