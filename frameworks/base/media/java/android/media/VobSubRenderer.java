    ...
    public boolean supports(MediaFormat format) {
        if (format.containsKey(MediaFormat.KEY_MIME)) {
            if (!format.getString(MediaFormat.KEY_MIME)
                    .equals(MediaPlayer.MEDIA_MIMETYPE_TEXT_IDX)) {
                return false;
            };
...
class VobSubTrack extends WebVttTrack {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.startsWith("#")) {
                    ...
                    for(int i=0; i<paletteArray.length; i++) {
                        ...
                        if (DEBUG) Log.d(TAG, "mPalette["+i+"]: "+Integer.toHexString(mPalette[i]));
                    }
                    ...
                    if (DEBUG) Log.d(TAG, "firstTimestamp: "+firstTimestamp+", firstFilePos: "+firstFilePos);
                   
                    String timestampLine;
                    while ((timestampLine = br.readLine()) != null && !timestampLine.trim().equals("")) {
                        if (timestampLine.startsWith("timestamp:")) {
                            ...
                            if (DEBUG) Log.d(TAG, "timestamp: "+timestamp+" filePos: "+filePos);
                            ...
                            lastFilePos = filePos;
                            if (DEBUG) Log.d(TAG, "lastTimestamp: "+lastTimestamp+", lastFilePos: "+lastFilePos);
    ...
    public void updateView(Vector<Cue> activeCues) {
        ...
        for (Cue cue : activeCues) {
            ...
            if (DEBUG) Log.d(TAG, "index: "+result[0]+", width: "+result[1]+", height: "+result[2]);
