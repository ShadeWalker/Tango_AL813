...
/* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
bool parseOptA(const char* name, ResTable_config* out) {
    if (strcmp(name, kWildcardName) == 0) {
        if (out) (out->mHwConfig).opta = 0;
        return true;
    }
    const char* c = name;
    if (tolower(*c) != 'o') return false;
    c++;
    if (tolower(*c) != 'p') return false;
    c++;
    if (tolower(*c) != 't') return false;
    c++;
    if (tolower(*c) != 'a') return false;
    c++;
...
bool parseOptB(const char* name, ResTable_config* out) {
    if (strcmp(name, kWildcardName) == 0) {
        if (out) (out->mHwConfig).optb = 0;
        return true;
    }
    const char* c = name;
    if (tolower(*c) != 'o') return false;
