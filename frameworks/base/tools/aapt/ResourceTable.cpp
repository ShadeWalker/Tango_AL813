                             ...
                             ResourceTable* outTable)
{
    ...
    // fix google ID
    const String16 android16("android");
    ...
    while ((code=block.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
        if (code == ResXMLTree::START_TAG) {
            ...
            /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
            // fix google ID
            } else if (strcmp16(block.getElementName(&len), android16.string()) == 0) {
                //android res
                SourcePos srcPos(in->getPrintableSource(), block.getLineNumber());
                ...
                ssize_t typeIdx = block.indexOfAttribute(NULL, "type");
                if (typeIdx < 0) {
                    srcPos.error("A 'type' attribute is required for <android>\n");
                    hasErrors = localHasErrors = true;
                }
                type = String16(block.getAttributeStringValue(typeIdx, &len));
                ...
                ssize_t nameIdx = block.indexOfAttribute(NULL, "name");
                if (nameIdx < 0) {
                    srcPos.error("A 'name' attribute is required for <android>\n");
                    hasErrors = localHasErrors = true;
                }
                name = String16(block.getAttributeStringValue(nameIdx, &len));
                ...
                if (!localHasErrors) {
                    err = outTable->addAndroid(srcPos, myPackage, type, name, ident);
                    if (err < NO_ERROR) {
                        hasErrors = localHasErrors = true;
                    }
                }
                ...
                while ((code=block.next()) != ResXMLTree::END_DOCUMENT && code != ResXMLTree::BAD_DOCUMENT) {
                    if (code == ResXMLTree::END_TAG) {
                        if (strcmp16(block.getElementName(&len), android16.string()) == 0) {
                            break;
                        }
                    }
                }
    ...
    , mBundle(bundle)
{
    ...
    switch (mPackageType) {
        ...
        case System:
            ...
            if (strcmp16(String16("androidhwext"), assetsPackage.string()) == 0) {
                packageId = 0x02;
            }
    ...
    for (pi=0; pi<N; pi++) {
        ...
        for (ti=0; ti<N; ti++) {
            ...
            for (size_t ci=0; ci<N; ci++) {
                ...
                /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
                /** generate android.xml data */
                // printf("<android type=\"%s\" name=\"%s\" id=\"0x%08x\" />\n", String8(t->getName()).string(), String8(c->getName()).string(), rid);
                /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
                                                       ...
                                                       bool autoAddOverlay)
{
    ...
    if (c == NULL) {
        ...
        /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
        // fix google ID
        const int32_t N = mOrderedConfigs.size();
        if (N == 0) {
            const String16 fake_entry("fake_entry");
            for (int32_t i = 0; i < mAndroid.size(); i++) {
                sp<ConfigList> cl = new ConfigList(fake_entry, sourcePos);
                mOrderedConfigs.add(cl);
            }
        }

        if (mAndroid.indexOfKey(entry) >= 0) {
            c = new ConfigList(entry, sourcePos);
            mConfigs.add(entry, c);
            const Android& p = mAndroid.valueFor(entry);
...
/* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
// fix google ID
status_t ResourceTable::addAndroid(const SourcePos& sourcePos,
                                  const String16& package,
                                  const String16& type,
                                  const String16& name,
                                  const uint32_t ident)
{
    uint32_t rid = mAssets->getIncludedResources()
        .identifierForName(name.string(), name.size(),
                           ...
                           package.string(), package.size());
    if (rid != 0) {
        sourcePos.error("Error declaring android resource %s/%s for included package %s\n",
                String8(type).string(), String8(name).string(),
                String8(package).string());
        return UNKNOWN_ERROR;
    }
    ...
    return t->addAndroid(sourcePos, name, ident);
}

status_t ResourceTable::Type::addAndroid(const SourcePos& sourcePos,
                                        const String16& name,
                                        const uint32_t ident)
{
    if (mAndroid.indexOfKey(name) < 0) {
        mAndroid.add(name, Android(sourcePos, String16(), ident));
        //printf("add android res -- type: %s  name:%s id:0x%08x \n", String8(getName()).string(), String8(name).string(), ident);
    } else {
        Android& p = mAndroid.editValueFor(name);
        if (p.ident != ident) {
            sourcePos.error("android resource %s/%s has conflicting android identifiers"
                    " (0x%08x vs 0x%08x).\n"
                    "%s:%d: Originally defined here.\n",
                    String8(mName).string(), String8(name).string(), p.ident, ident,
                    p.sourcePos.file.string(), p.sourcePos.line);
