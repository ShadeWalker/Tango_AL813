...
public class BitmapFactory {
	...
	/**
         * M: Add by MTK to support Huawei OMA DRM, mIsDrm mark decode image as drm, and if in
         * inThumbnailMode, don't need consume right.
         * @hide
         */
    ...
    public static Bitmap decodeFile(String pathName, Options opts) {
        /// M: Add for huawei, If decode DRM file, use DcfDecoder to decode bitmap @{
        if (pathName != null && pathName.endsWith(".dcf")) {
            // Decode thumbnail needn't consume right, otherwise consume right
            boolean consumeRights = opts != null ? !opts.inThumbnailMode : true;
            Log.d("BitmapFactory", "decodeFile: pathName = " + pathName + ", consumeRights = " + consumeRights);
            DcfDecoder dcfDecoder = new DcfDecoder();
            return dcfDecoder.forceDecodeFile(pathName, opts, consumeRights);
        }
        /// @}
        Bitmap bm = null;
    ...
    public static Bitmap decodeResource(Resources res, int id, Options opts) {
        ...
        try {
            /* < DTS2015011403501 chengli/cwx213999 20140119 begin */
            bm = HwThemeManager.getThemeBitmap(res, id);
            if (bm != null) {
            return bm;
            }
            /*DTS2015011403501 chengli/cwx213999 20150119 end > */
    ...
    public static Bitmap decodeFileDescriptor(FileDescriptor fd, Rect outPadding, Options opts) {
        /// M: Add for huawei, use below method to convert fd to path. {@
        ...
        if (application != null && (context = application.getApplicationContext()) != null) {
            ...
            DrmInfoRequest request = new DrmInfoRequest(DrmInfoRequest.TYPE_GET_OMA_DRM_INFO,
                    DrmStore.DrmObjectMimeType.MIME_DRM_CONTENT);
            request.put("action", "getDrmInfoWithFd");
            request.put("FileDescriptorKey", fd.toString());
            try {
                DrmInfo drmInfo = client.acquireDrmInfo(request);
                if (drmInfo != null) {
                    String path = String.valueOf(drmInfo.get("path"));
                    Log.d("BitmapFactory", "decodeFileDescriptor with: path = " + path + ", context = " + context);
                    if (path != null && path.endsWith(".dcf")) {
			/*add by lhz for no sound while set FL media as ring by begin at 20151228*/
			if (path.startsWith("/data/media/0")) {
			path = path.replace("/data/media/0", "/storage/emulated/0");
			}
			/*add by lhz for no sound while set FL media as ring by end at 20151228*/
