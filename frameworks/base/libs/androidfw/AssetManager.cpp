...
static const char* kSystemAssets_hwext = "framework/framework-res-hwext/framework-res-hwext.apk";
/* DTS2014102105499 yinwenshuai 00211458 20141107 end > */
static const char* kSystemAssets_huaqin = "framework/hq-res/hq-res.apk";
...
bool AssetManager::addDefaultAssets()
{
    ...
    if (!isOK1)
    {
	   ...
	   isOK1 =addAssetPath(path2, NULL);
       if(!isOK1){
	      ALOGW("AssetManager-->addDefaultAssets isok2 is false");
	   }

	   String8 path3(root);	   
	   path3.appendPath(kSystemAssets_hwext);
	   
	   isOK1 =addAssetPath(path3, NULL);
       if(!isOK1){
	      ALOGW("AssetManager-->addDefaultAssets isok3 is false");
	   }

           String8 path4(root);
           path4.appendPath(kSystemAssets_huaqin);
           isOK1 =addAssetPath(path4, NULL);
       if(!isOK1){
              ALOGW("AssetManager-->addDefaultAssets isok4 is false");
           }
       return isOK1;
		/* DTS2014102105499 yinwenshuai 00211458 20141107 end > */
       ///@}       
...
Asset* AssetManager::openNonAsset(const char* fileName, AccessMode mode, int32_t* outCookie)
{
    ...
    /* < DTS2014102105499 yinwenshuai 00211458 20141107 begin */
    if(i == 2 && (strcmp(fileName,"AndroidManifest.xml") == 0)) {
        ALOGV("AssetManager openNonAsset '%s'  i:%d\n", fileName, i);
        Asset* pAsset = openNonAssetInPathLocked(
            fileName, mode, mAssetPaths.itemAt(0));
        if (pAsset != NULL) {
            return pAsset != kExcludedAsset ? pAsset : NULL;
        }
    } else {
...
{
    {
        ...
        if (mResourceTableAsset == NULL) {
            /* < DTS2014121906332 zhangxinming/00181848 20141230 begin */
            // Sync from DTS2014080403541 jiaxinglong
            // getBuffer is a time-consuming operation for inflate zip file
            // mResourceTableAsset = asset;
            // This is not thread safe the first time it is called, so
            // do it here with the global lock held.
            asset->getBuffer(true);
            mResourceTableAsset = asset;
            /*   DTS2014121906332 zhangxinming/00181848 20141230 end > */
            return asset;
        }
    }
    delete asset;
