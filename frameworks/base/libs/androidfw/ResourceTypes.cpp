...
void ResTable_config::copyFromDtoH(const ResTable_config& o) {
    ...
    // opt
    mHwConfig.copyFromDtoH(o.mHwConfig);
...
int ResTable_config::compare(const ResTable_config& o) const {
    ...
    // opt
    if (diff != 0) return diff;
    diff = mHwConfig.compare(o.mHwConfig);
    /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
    return (int)diff;
}
...
int ResTable_config::compareLogical(const ResTable_config& o) const {
    ...
    /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
    // opt
    return mHwConfig.compareLogical(o.mHwConfig);
    /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
}
...
bool ResTable_config::isMoreSpecificThan(const ResTable_config& o) const {
    ...
    /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
    // opt
    return mHwConfig.isMoreSpecificThan(o.mHwConfig);
    /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
}
...
bool ResTable_config::isBetterThan(const ResTable_config& o,
    ...
    if (requested) {
        ...
        /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
        // opt
        return mHwConfig.isBetterThan(o.mHwConfig);
        /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
    }
...
bool ResTable_config::match(const ResTable_config& settings) const {
    ...
    /* < DTS2014102105499 yinwenshuai/00211458 20141105 begin */
    // opt
    return mHwConfig.match(settings.mHwConfig);
    /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
}
...
String8 ResTable_config::toString() const {
    ...
    if (mHwConfig.opta != 0) {
        if (res.size() > 0) res.append("-");
        res.appendFormat("opta%d", dtohs(mHwConfig.opta));
    }
    if (mHwConfig.optb != 0) {
        if (res.size() > 0) res.append("-");
        res.appendFormat("optb%d", dtohs(mHwConfig.optb));
    }
    /* DTS2014102105499 yinwenshuai/00211458 20141105 end > */
...
/* < DTS2014112008959 yinwenshuai/00211458 20141120 begin */
// share lib of Android L begin with packageId(2), but framework-res-hwext
// as another system lib is fixed packageId(2). So here we modify the init
// of mNextPackageId from 2 to 3. To make share lib's ID in Huawei start
// with 0x03.
ResTable::ResTable()
    : mError(NO_INIT), mNextPackageId(3)
{
...
/* DTS2014112008959 yinwenshuai/00211458 20141120 end > */
                             ...
                             bool enforcePrivate) const
{
    ...
    if (*s == '?') {
        ...
        /* the number 2 and 1 represent bytes */
        if (len > 2 && s[1] == '*') {
            enforcePrivate = false;
            resourceRefName = s + 2;
            resourceNameLen = len - 2;
...
//modified by WengRongjian for auto language config from ProjectConfig.mk 
    size_t I = configs.size();
#if 0
//wangyi add
//#ifdef HQ_COUNTRYCODE_CUST
//    char propcc[PROPERTY_VALUE_MAX];
//    property_get("ro.product.countrycode", propcc, "US");
//#endif
//wangyi add end
    ...
    for (size_t i=0; i<I; i++) {
        ...
        if (j == J) {
#ifdef HQ_COUNTRYCODE_CUST //added by wangyi.2015.7.27
		if(
		0 == strcmp("en-XA",locale)||
		0 == strcmp("en-GB",locale)||
		0 == strcmp("ar-XB",locale)||
		0 == strcmp("ka-GE",locale)||
		0 == strcmp("km-KH",locale)||
		0 == strcmp("si-LK",locale)||
		0 == strcmp("my-MM",locale)||
		0 == strcmp("bo-CN",locale)||
		0 == strcmp("gl-ES",locale)||
		0 == strcmp("eu-ES",locale)||
		0 == strcmp("es-US",locale)||
		0 == strcmp("de-AT",locale)||
		0 == strcmp("pt-PT",locale)||
		0 == strcmp("zz-ZX",locale)||
		0 == strcmp("az-AZ",locale)||
		0 == strcmp("kk-KZ",locale)||
		0 == strcmp("uz-UZ",locale)||
		0 == strcmp("fr-CA",locale)
			){
			continue;
			}
		ALOGW("this is : %s\n",locale);
    ...
    char locale[RESTABLE_MAX_LOCALE_LEN];
    I = sizeof(countryCode) / sizeof(countryCode[0]);
    
    for (size_t i=0; i<I; i++)
    {
	    if(0 == strlen(countryCode[i]))
        {
            continue;
        }
        memset(locale, 0 ,RESTABLE_MAX_LOCALE_LEN);
        strcpy(locale, countryCode[i]);
        const size_t J = locales->size();
        size_t j;
        for (j=0; j<J; j++)
        {
    ...
    : mAssignedPackageId(packageId)
{
    ...
    /* < DTS2014102105499 yinwenshuai 00211458 20141107 begin */
    mLookupTable[SYS_HW_PACKAGE_ID] = SYS_HW_PACKAGE_ID;
    /* DTS2014102105499 yinwenshuai 00211458 20141107 end > */
}
