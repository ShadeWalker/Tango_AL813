...
#if defined(MTK_USER_BUILD) // keep default behavior in user build
...
#else // eng/userdebug build define all options as 1
...
// Turn on to insert detailed event markers
#define DEBUG_DETAILED_EVENTS 1
...
#if DEBUG_INIT
    #define INIT_LOGD(...) \
        if (g_HWUI_debug_init) ALOGD(__VA_ARGS__)
#else
    #define INIT_LOGD(...)
#endif
