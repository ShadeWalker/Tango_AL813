...
import android.os.SystemProperties;/*add by lihaizhou for forbid uninstall or install package at PC while ChildMode is on*/
public final class Pm {
    private static final String TAG = "Pm";
    /*add by lihaizhou for forbid uninstall or install package at PC while ChildMode is on by begin*/
    boolean Prohibit = SystemProperties.getBoolean("persist.sys.isProhibit",false);
    boolean ProhibitInstall = SystemProperties.getBoolean("persist.sys.isProhibitInstall",false);
    /*add by lihaizhou for forbid uninstall or install package at PC while ChildMode is on by end*/
    IPackageManager mPm;
    ...
    private int runInstall() {
        ...
        String abi = null;
		Log.d(TAG, "[MTK] runInstall() ProhibitInstall = " + ProhibitInstall);
        /*add by lihaizhou for forbid install package at PC while ChildMode is on by begin*/
	if(ProhibitInstall)
	{
	System.err.println("Failure:forbid install package while ChildMode is on");
        return 1;
        }
	 /*add by lihaizhou for forbid install package at PC while ChildMode is on by end*/
        while ((opt=nextOption()) != null) {
            if (opt.equals("-l")) {
		...
		Log.d(TAG, "[MTK] runInstall() userId = " + userId + ", installFlags = " + installFlags);
        ...
        try {
            ...
            synchronized (obs) {
                ...
                if (obs.result == PackageManager.INSTALL_SUCCEEDED) {
					...
					Log.d(TAG, "[MTK] runInstall() Success");
                    return 0;
                } else {
                    System.err.println("Failure ["
                            + installFailureToString(obs)
                            + "]");
					Log.d(TAG, "[MTK] runInstall() Failure = " + installFailureToString(obs));
                    return 1;
                }
            }
        } catch (RemoteException e) {
    ...
    private int runUninstall() throws RemoteException {
        int flags = 0;
        int userId = UserHandle.USER_ALL;
		Log.d(TAG, "[MTK] runUninstall() Prohibit = " + Prohibit);
        /*add by lihaizhou for forbid uninstall package at PC while ChildMode is on by begin*/
        if(Prohibit)
	{
	System.err.println("Failure:forbid uninstall package while ChildMode is on");
        return 1;
        }
	/*add by lihaizhou for forbid uninstall package at PC while ChildMode is on by end*/
        String opt;
        while ((opt=nextOption()) != null) {
            if (opt.equals("-k")) {
        ...
        try {
			...
			Log.d(TAG, "[MTK] runUninstall() info2 = " + info2);
        } catch (RemoteException e) {
            System.err.println(e.toString());
            System.err.println(PM_NOT_RUNNING_ERR);
			Log.e(TAG, "[MTK] runUninstall() RemoteException", e);
            return 1;
        }
        if (info2 == null) {
            System.err.println("Failure - not installed for " + userId);
