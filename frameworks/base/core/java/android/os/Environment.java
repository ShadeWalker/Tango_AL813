...
/* HQ_ChenWenshuai 2015-07-26 modified for HQ01264483*/
import com.mediatek.storage.StorageManagerEx;
import android.os.SystemProperties;//chenwenshuai add for HQ01705512
...
public class Environment {
    ...
    public static class UserEnvironment {
        ...
        public UserEnvironment(int userId) {
			...
			/* HQ_ChenWenshuai 2015-07-26 modified for HQ01264483*/
            String rawSDCardStorage = "/storage/sdcard1";
            ...
            if (!TextUtils.isEmpty(rawEmulatedTarget)) {
				...
				/* HQ_ChenWenshuai 2015-07-26 modified for HQ01264483*/
				externalForApp.add(new File(rawSDCardStorage));
				/* HQ_ChenWenshuai 2015-09-24 modified for CTS pass */
				externalForVold.add(new File(rawSDCardStorage));
    ...
    public static File getExternalStorageDirectory() {
        throwIfUserRequired();
        /*add by lihaizhou for HQ01377154 by begin */
        if(SystemProperties.get("persist.sys.sd.defaultpath")
			.equals("/storage/sdcard1")){//chenwenshuai add for HQ01705512
			return sCurrentUser.getExternalDirsForApp()[1];
		}
		else{
        	return sCurrentUser.getExternalDirsForApp()[0];
		}
        /*add by lihaizhou for HQ01377154 by end */
        //return sCurrentUser.getExternalDirsForApp()[0];/*delete by lihaizhou for HQ01377154*/
    }
    /*add by lihaizhou for by begin */
   /** {@hide} */
   public static File getInternalStorageDirectory() {
        throwIfUserRequired();        
        return sCurrentUser.getExternalDirsForApp()[0];
    }
  /*add by lihaizhou  by end */
     /** {@hide} */
    public static File getExternalStorageDirectoryEx() {
        throwIfUserRequired();
        return sCurrentUser.getExternalDirsForApp()[1];
    }
    //added ends
    ...
    public static File getExternalStoragePublicDirectory(String type) {
        throwIfUserRequired();		
		/* HQ_ChenWenshuai 2015-07-26 modified for HQ01264483 begin */
		if(StorageManagerEx.getDefaultPath().equals("/storage/sdcard1")){
			return sCurrentUser.buildExternalStoragePublicDirs(type)[1];
		}
		else{
        	return sCurrentUser.buildExternalStoragePublicDirs(type)[0];
		}
		/*HQ_ChenWenshuai 2015-07-26 modified end */
    }
	
	/* HQ_ChenWenshuai 2015-09-25 modified for HQ01383238 begin */
	public static File getInternalStoragePublicDirectory(String type) {
        throwIfUserRequired();
		return sCurrentUser.buildExternalStoragePublicDirs(type)[0];
	}
	/*HQ_ChenWenshuai 2015-09-25 modified end */
	...
	//added for install app sdcard. chenwenshuai.
    /** {@hide} */
    public static String getExternalStorageStateEx(){
	...
	//added for install app to extern sd card.chenwenshuai
    /** {@hide} */
    public static boolean isExternalStorageEmulatedEx(){ //check whether second sdcard is emulated or not. false by default.
        return false;
    }
	...
	/* added for install app to sd card.chenwenshuai begin */    
  /** {@hide} */
   public static String getExternalStorageStateExtraSd() {
		...
		Log.d("chao", "getExternalStorageStateExtraSd volume: " +volume.getPath());
                if ((volume.getPath()).equals(System.getenv(ENV_SECONDARY_STORAGE))) {
                    return mountService.getVolumeState(volume.getPath());
                }
            }
        } catch (RemoteException e) {
    ...
    /** {@hide} */
    public static String getExternalSdcardState() {
        boolean hasSdcard = SystemProperties.getBoolean("vold.sdcard.insert", false);
        if (!hasSdcard){
            return Environment.MEDIA_UNKNOWN;
        }
        ...
        try {
            final IMountService mountService = IMountService.Stub.asInterface(
                    ServiceManager.getService("mount"));
            return mountService.getVolumeState(getExternalSdcardPath());
        } catch (RemoteException e) {
            Log.w(TAG, "Failed to find external sdcard state: " + e);
        }
        return Environment.MEDIA_UNKNOWN;
    ...
    public static String getInternalSdcardState() {
        try {
            final IMountService mountService = IMountService.Stub.asInterface(
                    ServiceManager.getService("mount"));
            return mountService.getVolumeState(getInternalSdcardPath());
        } catch (RemoteException e) {
            Log.w(TAG, "Failed to find internal sdcard state: " + e);
        }
        return Environment.MEDIA_UNKNOWN;

    }
   /** {@hide} */
    public static File getExternalStorageDirectoryExtraSd() {
        return  (new File(System.getenv(ENV_SECONDARY_STORAGE)));
    }	
    /* added for install app to sd card.end */

    /* added for install app to sd card.chenwenshuai begin */
   /** {@hide} */
    public static String getExternalSdcardPath() {
        boolean hasSdcard = SystemProperties.getBoolean("vold.sdcard.insert", false);
        boolean isSwitch = SystemProperties.getBoolean("persist.sys.sdcard.switch", false);
  ...
  public static String getInternalSdcardPath() {
        ...
        if (hasSdcard){
            if(isSwitch){
                ...
                path = "/storage/emulated/0";  //for sdcard fuse switch 
            }
        }else{
