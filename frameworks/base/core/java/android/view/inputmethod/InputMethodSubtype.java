...
public final class InputMethodSubtype implements Parcelable {
    ...
    public CharSequence getDisplayName(
            Context context, String packageName, ApplicationInfo appInfo) {//HQ_hushunli 2015-11-16 modify for HQ01501727
        final Locale locale = constructLocaleFromString(mSubtypeLocale);
        final String localeStr = locale != null ? locale.getDisplayName() : mSubtypeLocale;
        if (mSubtypeNameResId == 0) {
            if (SystemProperties.get("ro.hq.aosp.nocountryinfo").equals("1") && !TextUtils.isEmpty(localeStr)) {
                return getNoCountryInfo(localeStr);
            } else {
                return localeStr;
            }
        }
        ...
        if (!TextUtils.isEmpty(subtypeName)) {
            final String replacementString =
                    containsExtraValueKey(EXTRA_KEY_UNTRANSLATABLE_STRING_IN_SUBTYPE_NAME)
                            ...
                            : localeStr;
            try {
                String returnName = String.format(
                        subtypeName.toString(), replacementString != null ? replacementString : "");
                if (SystemProperties.get("ro.hq.aosp.nocountryinfo").equals("1") && !TextUtils.isEmpty(returnName)) {
                    return getNoCountryInfo(returnName);
                } else {
                    return returnName;
                }
    ...
    private String getNoCountryInfo(String displayName) {//HQ_hushunli 2015-11-16 add for HQ01501727
        String name = "";
        int startIndex = displayName.indexOf("(");
        if (startIndex != -1) {
            name = displayName.substring(0, startIndex);
        } else {
            name = displayName;
        }
        return name;
