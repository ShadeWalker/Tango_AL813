...
public final class BatteryStatsImpl extends BatteryStats {
    ...
    public BatteryStatsImpl(File systemDir, Handler handler) {
        if (systemDir != null) {
            //add by liuchaochao for HQ01398861 begin
            boolean ret = (new File(systemDir, "batterystats.bin")).delete();
            if (!ret) {
                Slog.i(TAG, "Delete file data/system/batterystats.bin fail");
            } else {
                Slog.i(TAG, "File data/system/batterystats.bin has been removed");
            }
            //add by liuchaochao for HQ01398861 end
