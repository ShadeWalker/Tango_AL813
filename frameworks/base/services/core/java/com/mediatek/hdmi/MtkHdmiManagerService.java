...
public final class MtkHdmiManagerService extends IMtkHdmiManager.Stub {
    ...
    private void handleMultiChannel() {
        if (isSupportMultiChannel()) {
            mAudioOutputMode = Settings.System.getIntForUser(mContentResolver, Settings.System.HDMI_AUDIO_OUTPUT_MODE, 0,
                    UserHandle.USER_CURRENT);
            log(TAG, "current mode from setting provider : " + mAudioOutputMode);
            if (mAudioOutputDialog == null) {
                String title = mContext.getResources().getString(R.string.hdmi_audio_output);
                String stereo = mContext.getResources().getString(R.string.hdmi_audio_output_stereo);
                String multiChannel = mContext.getResources().getString(R.string.hdmi_audio_output_multi_channel);
                mAudioOutputDialog = new AlertDialog.Builder(mContext).setTitle(title).setSingleChoiceItems(
                        new String[] { stereo, multiChannel }, mAudioOutputMode, new DialogInterface.OnClickListener() {

                            public void onClick(DialogInterface dialog, int which) {
                                mAudioOutputMode = which;
                                log(TAG, "mAudioOutputDialog clicked.. which: " + which);
    ...
    private void setAudioParameters(boolean isStereoChecked) {
        int maxChannel = getAudioParameter(HdmiDef.HDMI_MAX_CHANNEL, HdmiDef.HDMI_MAX_CHANNEL_OFFSETS);
        if (isStereoChecked) { // stereo is checked.
            maxChannel = HdmiDef.AUDIO_OUTPUT_STEREO;
        }
        int maxSampleate = getAudioParameter(HdmiDef.HDMI_MAX_SAMPLERATE, HdmiDef.HDMI_MAX_SAMPLERATE_OFFSETS);
        int maxBitwidth = getAudioParameter(HdmiDef.HDMI_MAX_BITWIDTH, HdmiDef.HDMI_MAX_BITWIDTH_OFFSETS);
        ...
        Settings.System.putIntForUser(mContentResolver, Settings.System.HDMI_AUDIO_OUTPUT_MODE, mAudioOutputMode,
                UserHandle.USER_CURRENT);
        log(TAG, "setAudioParameters mAudioOutputMode: " + mAudioOutputMode + " ,maxChannel: " + maxChannel
                + " ,maxSampleate: " + maxSampleate + " ,maxBitwidth: " + maxBitwidth);
    }

    @Override
    public int getAudioParameter(int mask, int offsets) {
        int param = (mCapabilities & mask) >> offsets;
        log(TAG, "getAudioParameter() mask: " + mask + " ,offsets: " + offsets + " ,param: " + param + " ,mCapabilities: 0x" + Integer.toHexString(mCapabilities));  
        return param;
    }
