...
import android.provider.ChildMode;//add by lihaizhou for ChildMode 
...
import android.text.TextUtils;//add by lihaizhou for ChildMode at 2015-07-22
import android.util.Log;//add by lihaizhou for ChildMode at 2015-07-22
...
public final class ActivityStackSupervisor implements DisplayListener {
	...
	if(component != null && component.getClassName() != null && component.getClassName().contains("com.mediatek.mtklogger.issuesubmitter")){
		Slog.v(TAG,"startActivityMayWait return because component className = " + component.getClassName());
	...
	// mtk add end
        ...
        if (isSuperPowerSavingMode)
        {
            ...
            if (null != categories && categories.contains(Intent.CATEGORY_HOME)){
                intent.removeCategory(Intent.CATEGORY_HOME);
                intent.addFlags(Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT);
                intent.setClassName("com.huawei.android.launcher", "com.huawei.android.launcher.limitpowermode.LimitPowerModeLauncher");
            }
        }
        /* DTS2013122400480 f00209624 20140122 end > */
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err == ActivityManager.START_SUCCESS) {
            ...
            if ((mCurActivityPkName == null) || !mCurActivityPkName.equals(strPkg)) {
                LogPower.push(LogPower.APP_START_SPEEDUP, strPkg);
            }
            /*<DTS2014112601808 huangwen 00181596 20141126 end */
        ...
        /* Modified By lihaizhou kill app after add to black list at 2015-7-22 begin */
            if(mService.isEnable){
                String packageName = null ;
                ComponentName cn = intent.getComponent();
                if(cn!=null){
                    packageName = cn.getPackageName();
                }
                if (ActivityManagerService.mTimesOfStartLauncher  > 0 
                       && !TextUtils.isEmpty(packageName) 
                       && mService.mAppLimitList.contains(packageName)) {
                    final Intent i = new Intent("com.settings.childmode.appdisable");
                    i.putExtra("package_name",packageName);
                    Log.d("ass", "send broadcast");
                    mService.mContext.sendBroadcast(i);
                    ActivityOptions.abort(options);
                    return ActivityManager.START_SUCCESS;
        ...
        if (!mLeanbackOnlyDevice &&
            ...
            if (MultiWindowProxy.isFeatureSupport() && MultiWindowProxy.getInstance() != null) {
                if ((r.intent.getFlags()&Intent.FLAG_ACTIVITY_FLOATING) != 0
                            ...
                            && !MultiWindowProxy.getInstance().matchDisableFloatPkgList(r.packageName)) {
        ...
        if (((launchFlags & Intent.FLAG_ACTIVITY_NEW_TASK) != 0 &&
            ...
            if (inTask == null && r.resultTo == null) {
                ...
                if (intentActivity != null) {
                    ...
                    /// M: BMW, [ALPS02056018]. If find activity in floating stack, add its task to mRecentTasks again. 
                    /// Because task may be removed from mRecentTasks by starting other task that has same affinity. @{
                    if (MultiWindowProxy.isFeatureSupport() && targetStack.isFloatingStack()) {
                        if (DEBUG_TASKS) Slog.i(TAG,"[BMW] find task in floating , add to Recent again.");
                        mService.addRecentTaskLocked(intentActivity.task);
                    }
                    /// @}
    ...
    private boolean restoreRecentTaskLocked(TaskRecord task) {
        ...
        if (mLeanbackOnlyDevice) {
            ...
            for (int stackNdx = homeDisplayStacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                /// M: BMW, [ALPS02039490]. Only find App stack, and we will configure floating stack for floating task later. @{
                if (MultiWindowProxy.isFeatureSupport() && tmpStack.isFloatingStack()) {
                    continue;
                }
                /// @}
       ...
       /// M: BMW, [ALPS02039490]. If task is floating, restore the task to its old stack.  @{
       if (MultiWindowProxy.isFeatureSupport() && (task.intent.getFlags() & Intent.FLAG_ACTIVITY_FLOATING) != 0) {
           stack = task.stack;
       }
       /// @}
    ...
    boolean moveNextFloatingStackToTopLocked(int returnTaskType, ActivityStack curTop) {
        ...
        if (next != null) {
            ...
            for (int i = 0; i < stackSize; i++) {
                ...
                /// M: ALPS01992302. SKIP the current top stack.@{
                if (floatingStack == curTop)
                    continue;
                /// @}
    ...
    ActivityStack moveFloatingStackToAppStackLocked(ActivityStack floatingStack, boolean doResumeTop) {
        ...
        if (appStack != null) {
            ...
            if (targetActivity != null) {
                ...
                // [ALPS02046870] avoid null Exception.
                if (targetActivity.app != null) {
                    targetActivity.app.inMaxOrRestore = true;
                } else {
                    // This should not occur
                    Slog.e(TAG,"[BMW] moveActivityTaskToFloatingStackLocked: app is null");
                }
