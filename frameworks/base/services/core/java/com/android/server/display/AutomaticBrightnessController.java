...
class AutomaticBrightnessController {
    ...
    // Mediatek AAL support
    private static final int MSG_UPDATE_RUNTIME_CONFIG = 2;
    ...
    // Mediatek AAL modified, range from dark to bright
    private Spline mScreenAutoBrightnessSpline;

    //HQ_wuhuihui_20150714 add for auto-brightness spline adjustment
    //Range from bright to dark
    private Spline mScreenAutoBrightnessSpline_dark;
    ...
    public void setDarkAutoBrightnessSpline(Spline spline) {
        mScreenAutoBrightnessSpline_dark = spline;
    }
    ...
    private void updateAutoBrightness(boolean sendUpdate) {
        ...
        float value_dark = -1.0f;
        float gamma = 1.0f;
        if (null != mScreenAutoBrightnessSpline_dark) {
            value_dark = mScreenAutoBrightnessSpline_dark.interpolate(mAmbientLux);
        }
		...
		//HQ_jiangchao modified for HQ01386969 at 20150922
            gamma *= adjGamma; //HQ_wuhuihui modified for HQ01250545
            if (DEBUG) {
        ...
        if ((gamma != 1.0f) && (value_dark != -1.0f)) {
            final float in = value_dark;
            value_dark = MathUtils.pow(value_dark, gamma);
            if (DEBUG) {
                Slog.d(TAG, "updateAutoBrightness: gamma=" + gamma
                        + ", in=" + in + ", out=" + value_dark);
            }
        }

        int newScreenAutoBrightness =
            clampScreenBrightness(Math.round(value * PowerManager.BRIGHTNESS_ON));
        int newScreenAutoBrightness_dark =
            clampScreenBrightness(Math.round(value_dark * PowerManager.BRIGHTNESS_ON));
        if (mScreenAutoBrightness != newScreenAutoBrightness) {
            if (DEBUG) {
                Slog.d(TAG, "updateAutoBrightness: mScreenAutoBrightness="
                        + mScreenAutoBrightness + ", newScreenAutoBrightness="
                        + newScreenAutoBrightness + ", ambientLux="
                        + mAmbientLux + ", newScreenAutoBrightness_dark=" + newScreenAutoBrightness_dark);
            }
            if (mScreenAutoBrightness <= newScreenAutoBrightness) {
                Slog.d(TAG, "wuhuihui Use rise brightness");
                mScreenAutoBrightness = newScreenAutoBrightness;
            } else {
                Slog.d(TAG, "wuhuihui Use dark brightness");
                mScreenAutoBrightness = newScreenAutoBrightness_dark;
            }
    ...
    private final SensorEventListener mLightSensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (mLightSensorEnabled) {
                ...
                // Mediatek AAL support
                if (time - mPrevLogTime >= 500L || // long period
                    mPrevLogLux * 1.2f <= lux || lux * 1.2f <= mPrevLogLux) // significant lux changed
                {
