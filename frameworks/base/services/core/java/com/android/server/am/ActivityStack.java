...
final class ActivityStack {
    ...
    Boolean mRuningBackground  = false;
    /* DTS2014112601808 huangwen 00181596 20141126 end>*/
    ...
    void minimalResumeActivityLocked(ActivityRecord r) {
		...
		Slog.v(TAG, "zhaofanfan "+ " LogPower.APP_RUN_FRONT ==="+LogPower.APP_RUN_FRONT);
        /*<DTS2014112601808 huangwen 00181596 20141126 begin */
        if(!mStackSupervisor.mCurActivityPkName.equals(r.packageName)){
            LogPower.push(LogPower.APP_RUN_FRONT,r.packageName);
			Slog.v(TAG, "zhaofanfan "+ " mRuningBackground ==="+mRuningBackground);
            if (mRuningBackground) {
				Slog.v(TAG, "zhaofanfan "+ " LogPower.APP_RUN_BG ==="+LogPower.APP_RUN_BG);
                LogPower.push(LogPower.APP_RUN_BG,mStackSupervisor.mCurActivityPkName);
                mRuningBackground = false;
            }
            mStackSupervisor.mCurActivityPkName = r.packageName;
			Slog.w(TAG+"huawei", "r.packageName = " + r.packageName);
        }
        /*<DTS2014112601808 huangwen 00181596 20141126 end */
    ...
    public final Bitmap screenshotActivities(ActivityRecord who) {
        ...
        /* HQ_xuqian4 20151030 modified for HQ01449489 begin */
        if (mService != null && mWindowManager != null) {
            Configuration newConfig = mService.mConfiguration;
            if(newConfig != null && Configuration.ORIENTATION_PORTRAIT == newConfig.orientation) {
               /* HQ_xuqian4 20151116 modified for HQ01507311 begin*/
               int w = mService.mThumbnailWidth + 440;
               int h = mService.mThumbnailHeight + 706;
               /* HQ_xuqian4 20151116 modified */
               if (w > 0) {
               ...
               Slog.e(TAG, "Invalid thumbnail dimensions: " + w + "x" + h);
                ...
                /* HQ_xuqian4 20151105 modified for HQ01449489*/
                int w = mService.mThumbnailHeight+80;
                int h = mService.mThumbnailWidth;
                Bitmap bitmapLandscape = mWindowManager.screenshotApplications(who.appToken, Display.DEFAULT_DISPLAY,
                        w, h, SCREENSHOT_FORCE_565);
                if (bitmapLandscape == null)
                    return null;
                Matrix m = new Matrix();
                m.setRotate(90, (float) bitmapLandscape.getWidth() / 2, (float) bitmapLandscape.getHeight() / 2);
                Bitmap newBitmap = null;
                try {
                    newBitmap = Bitmap.createBitmap(bitmapLandscape, 0, 0, bitmapLandscape.getWidth(), bitmapLandscape.getHeight(), m, true);
                    return newBitmap;
        ...
        /* HQ_xuqian4 20151030 modified end */
        return null;
    }
    ...
    private void completePauseLocked(boolean resumeNext) {
        ...
        /*<DTS2014112601808 huangwen 00181596 20141126 begin */
        mRuningBackground = true;
		Slog.v(TAG+"huawei", "mRuningBackground= " + mRuningBackground);
        /* DTS2014112601808 huangwen 00181596 20141126 end>*/
        if (prev != null) {
            ...
            if (prev.finishing) {
                ...
                /*<DTS2014112601808 huangwen 00181596 20141126 begin */
                mRuningBackground = false;
				Slog.v(TAG+"huawei", "if(prev.finishing) mRuningBackground= " + mRuningBackground);
                /* DTS2014112601808 huangwen 00181596 20141126 end>*/
                ...
                if (prev.configDestroy) {
                    ...
                    /*<DTS2014112601808 huangwen 00181596 20141126 begin */
                    mRuningBackground = false;
					Slog.v(TAG+"huawei", "if(prev.configDestroy) mRuningBackground= " + mRuningBackground);
                    /* DTS2014112601808 huangwen 00181596 20141126 end>*/
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        /// M:BWM, [ALPS02019499], allow nested call . 
        /// If there is no activity to resume in App/Floating stack , we will resuem home stack.    
        if (!MultiWindowProxy.isFeatureSupport() && mStackSupervisor.inResumeTopActivity) {
...
*/
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            /*<DTS2014112601808 huangwen 00181596 20141126 begin */
            if(!mStackSupervisor.mCurActivityPkName.equals(next.packageName)){
                LogPower.push(LogPower.APP_RUN_FRONT, next.packageName);
                if (mRuningBackground) {
                    LogPower.push(LogPower.APP_RUN_BG, mStackSupervisor.mCurActivityPkName);
                    mRuningBackground = false;
                }
                mStackSupervisor.mCurActivityPkName = next.packageName;
				Slog.v(TAG+"huawei", "next.packageName= " + next.packageName);
            }
            /* DTS2014112601808 huangwen 00181596 20141126 end>*/
            ...
            try {
                ...
                if (MultiWindowProxy.isFeatureSupport() 
                    ...
                    if(!toMax && (MultiWindowProxy.getInstance().matchDisableFloatActivityList(next.shortComponentName)
                        || MultiWindowProxy.getInstance().matchDisableFloatPkgList(next.packageName))){ 
    ...
    private void insertTaskAtTop(TaskRecord task) {
        ...
        if (isOnHomeDisplay()) {
            ...
            /// Or, if normal Ap is launched from Home, keep fromHome is true.@{
            if (MultiWindowProxy.isFeatureSupport()) {
                ...
                // M: BMW, [ALPS02069449].
                // If lastStack is floating, and appStack is not in resumed, set mFloatingBackIsHome true.
                if (!isHomeStack() && lastStack.isFloatingStack()) {
                    ActivityStack appStack = getAppStackLocked();
                    if (appStack != null) {
                        ActivityRecord next = appStack.topRunningNonDelayedActivityLocked(null);
                        if (next != null && next.state == ActivityState.RESUMED) {
                        } else {
                            task.mFloatingBackIsHome = true;
                        }
                    }
            ...
            if (!isHomeStack() && (fromHome || topTask() != task)) {
                ...
                /// M: BMW, [ALPS02069449]. If activity is launched from home, set TaskToReturnTo home.@{
                if(MultiWindowProxy.isFeatureSupport()){
                    if (task.mFloatingBackIsHome) {
    ...
    final void moveTaskToFrontLocked(TaskRecord tr, ActivityRecord source, Bundle options,
        ...
        if (MultiWindowProxy.isFeatureSupport()) {
            /// [ALPS01996294], avoid multi-user null pointer exception.
            /// [ALPS02008213], after reboot, recent task has no running activity.
            if (tr.topRunningActivityLocked(null) != null) {
    ...
    final boolean moveTaskToBackLocked(int taskId) {
        ...
        if (!isFloatingStack() && !isHomeStack() && numValidTasks == 1) {
