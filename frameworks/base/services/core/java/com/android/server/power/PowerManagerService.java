...
import android.bluetooth.BluetoothDevice;//add by chenwenshuai on 20151028 for HQ01471981
...
public final class PowerManagerService extends SystemService
	...
	private boolean mIsInCallFlag = false;
    private boolean mIsDTMFFlag = false;
	private static final int CUSTOM_DTMF_DELAY_TIME = 30000;
    ...
    public void systemReady(IAppOpsService appOps) {
        synchronized (mLock) {
			...
			//add by HQ_chenwenshuai at 2015-10-28 for HQ01471981
            filter = new IntentFilter();
            filter.addAction(BluetoothDevice.ACTION_PAIRING_REQUEST);
            mContext.registerReceiver(new PairrequestReceiver(), filter, null, mHandler);
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY
            ...
            if (mSmartBookPlugState) {
                if (!mSmartBookSleeped) {
					Slog.d("guo", "updateUserActivitySummaryLocked smartbook!!!");
                    final int sleepTimeout = getSleepTimeoutLocked();
					//add by guojianhui for DTMF screen timeout delay 30s feature start
					final int screenOffTimeout;
					if(SystemProperties.get("persist.dtmf_scnoff_timeout").equals("1")) {
						screenOffTimeout = getIncallDTMFState() ? (getScreenOffTimeoutLocked(sleepTimeout) + CUSTOM_DTMF_DELAY_TIME) : getScreenOffTimeoutLocked(sleepTimeout);
            ...
            if (mWakefulness == WAKEFULNESS_AWAKE
                    ...
                    || mWakefulness == WAKEFULNESS_DOZING) {
                Slog.d("guo", "updateUserActivitySummaryLocked wakefulness: "+mWakefulness);
                final int sleepTimeout = getSleepTimeoutLocked();
				//add by guojianhui for DTMF screen timeout delay 30s feature start
				final int screenOffTimeout;
				if(SystemProperties.get("persist.dtmf_scnoff_timeout").equals("1")) {
	...
	/*2015-10-28 add by chenwenshuai for HQ01471981 begin*/
    private final class PairrequestReceiver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
                if (DEBUG) {
    ...
    /*2015-10-28 add by chenwenshuai for HQ01471981 end*/
...
/*<DTS2014112601808 huangwen 00181596 20141126 begin*/
        public void setLcdRatio(int ratio,boolean autoAdjust) {
            Light lcdLight = mLightsManager.getLight(LightsManager.LIGHT_ID_BACKLIGHT);
            lcdLight.setLcdRatio(ratio, autoAdjust);
        }
/*<DTS2014112601808 huangwen 00181596 20141126 begin*/
		...
		//add by guojianhui for DTMF screen timeout delay 30s feature start
		 //@Override
		public void setInCallDTMFFlag(boolean isincall, boolean isDTMF) {

        	mIsInCallFlag = isincall;
            mIsDTMFFlag  = isDTMF;
            Slog.d("guo", " mIsInCallFlag = "+mIsInCallFlag + ",   mIsDTMFFlag = "+mIsDTMFFlag); 
		}
		//add by guojianhui for DTMF screen timeout delay 30s feature end
    }
	...
	//add by guojianhui for DTMF screen timeout delay 30s feature start
	private boolean getIncallDTMFState() {
		return mIsDTMFFlag && mIsInCallFlag;
	}
	//add by guojianhui for DTMF screen timeout delay 30s feature end
}
