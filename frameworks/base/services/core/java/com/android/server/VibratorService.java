...
public class VibratorService extends IVibratorService.Stub
    ...
    //add by wuhuihui for HQ_VIBRATOR_MODE  -----start@{
    public void setVibrateMode(int mode) {
        mContext.enforceCallingOrSelfPermission(
                android.Manifest.permission.VIBRATE,
                "cancelVibrate");
        //android.util.Log.e("gongpc", "setVibrateMode11:  "+mode);
        // so wakelock calls will succeed
        long identity = Binder.clearCallingIdentity();
        try {
            synchronized (mVibrations) {
	...
	public long getVibratorOnTimes(long millis){
		if (mCurMode < 0 || mCurMode > 5)
		{
			mCurMode = 0;
		}
        //android.util.Log.e("gongpc", "getVibratorOnTimes111 millis:  "+millis + " mCurMode:" + mCurMode);
		millis = (millis << 2) + mCurMode;
    ...
    private void startVibrationLocked(final Vibration vib) {
        ...
        if (vib.mTimeout != 0) {
            ...
            /* DTS2017011711259 chengjing/00378874 20160118 begin>*/
            if (vib.mPattern != null && vib.mPattern.length != 0) {
				Slog.d(TAG, "pattern is not null");
				mThread = new VibrateThread(vib);
                mThread.start();
            } else {
                Slog.d(TAG, "pattern is null or pattern's length is 0");
            }
            /* DTS2017011711259 chengjing/00378874 20160118 end>*/
    ...
    private void doVibratorOn(long millis, int uid, int usageHint) {
        synchronized (mInputDeviceVibrators) {
            ...
            //add by wuhuihui for HQ_VIBRATOR_MODE  -----start@{
                millis = getVibratorOnTimes(millis);
